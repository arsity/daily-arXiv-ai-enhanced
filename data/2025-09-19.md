...
Motivation: Deep models perform poorly under distribution shifts. Most domain generalization methods require multi-domain training or computationally intensive test-time adaptation, which motivates the need for a lightweight complementary approach.
...
Result: Extensive experiments on PACS and Office-Home datasets show consistent performance gains across different domain generalization algorithms and backbones, demonstrating effectiveness and generality.

Conclusion: CI-TTA provides an effective and lightweight test-time augmentation strategy that complements existing domain generalization methods and improves model robustness to distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: Existing tokenizers specialize in either reconstruction or understanding for single modalities, lacking a unified approach for diverse visual inputs. The goal is to create a single framework that can handle multiple visual modalities and tasks.

Method: Uses a pure transformer architecture with 4D rotary position embeddings to process arbitrary resolutions and temporal durations. Employs adversarial-free training with perceptual and Gram matrix losses, and progressive training curriculum from single images to videos and 3D.
...
Conclusion: AToken demonstrates successful unification of visual tokenization across multiple modalities, enabling next-generation multimodal AI systems with competitive performance across all benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Address the stability-plasticity dilemma in incremental multi-view clustering where models need to adapt to new data while maintaining long-term knowledge and preventing catastrophic forgetting.

Method: Three modules: 1) Hippocampus-inspired view alignment to capture new view information, 2) Cognitive forgetting mechanism simulating human memory decay, 3) Prefrontal cortex-inspired knowledge consolidation using temporal tensor stability.

Result: Extensive experiments show MemEvo achieves strong knowledge retention and outperforms state-of-the-art methods in scenarios with growing number of views.

Conclusion: The neuroscience-inspired memory mechanism approach effectively balances stability and plasticity in incremental multi-view clustering, demonstrating superior performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing edge-aware SISR methods often introduce redundancy and unstable optimization through ad hoc fusion approaches, while struggling to balance structural fidelity and perceptual realism.

Method: Proposes an edge-guided attention mechanism that creates adaptive modulation maps from jointly encoded edge features and intermediate activations, integrated into a lightweight residual design with composite loss (pixel-wise, perceptual, adversarial).
...
Conclusion: Principled edge-conditioned modulation provides a parameter-efficient approach for perceptual super-resolution, demonstrating effectiveness without requiring deeper or heavily overparameterized architectures.
...
Method: Uses score-based diffusion model with noise variation estimation to determine adaptive denoising schedule, two-stage sampling strategy for feature and gradient fusion, and iterative network invocation following adaptive step sizes.

Result: Achieves clean and smooth denoised point clouds with better shape boundary and detail preservation, outperforming state-of-the-art methods both qualitatively and quantitatively on synthetic and real-scanned datasets.

Conclusion: The adaptive iterative approach based on diffusion models provides superior point cloud denoising performance across various noise patterns and levels while maintaining structural details.
...
Method: DiffVL reformulates visual localization as GPS denoising using diffusion models. It learns to reverse GPS noise perturbations by jointly modeling GPS trajectories, SD maps, and visual BEV features through iterative diffusion refinement, recovering the true pose distribution.

Result: Experiments show DiffVL achieves state-of-the-art sub-meter accuracy on multiple datasets, outperforming BEV-matching baselines like OrienterNet and transformer-based registration approaches.

Conclusion: The work demonstrates that diffusion models can enable scalable localization by treating noisy GPS as a generative prior, representing a paradigm shift from traditional matching-based methods and eliminating the need for costly HD maps.
...
Result: Significantly outperforms state-of-the-art baselines in reconstructing high-quality CT images under both uniform and non-uniform sparse-view settings (15, 30, 60 views out of 180 total views).
...
<details>
  <summary>Details</summary>
Motivation: Existing UC severity estimation methods suffer from domain shifts across hospitals due to different imaging devices and clinical settings. Current domain adaptation methods struggle with lack of supervision in target domains or high annotation costs.
...
Conclusion: The proposed weakly supervised approach effectively addresses domain shift challenges in UC severity estimation by utilizing routinely recorded patient-level diagnostic results as supervision, achieving superior performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current dynamic point cloud compression methods rely on explicit motion estimation that struggles with intricate dynamics and fails to fully exploit temporal correlations due to irregular point cloud structure and local variations.

Method: Feature-aligned Motion Transformation (FMT) framework uses spatiotemporal alignment to implicitly model continuous temporal variations, employing aligned features as temporal context within latent-space conditional encoding. Includes random access reference strategy for bidirectional motion referencing and layered encoding.
...
<details>
  <summary>Details</summary>
Motivation: Mamba models show superior performance in 3D biomedical image segmentation by addressing CNN's long-range dependency limitations and Transformer's computational overhead, but they may compromise critical local structural information leading to boundary ambiguity and regional distortion.

Method: Proposed HybridMamba architecture with: 1) feature scanning strategy integrating axial-traversal and local-adaptive pathways to harmonize local-global representations, and 2) gated module combining spatial-frequency analysis for comprehensive contextual modeling. Evaluated on multi-center CT dataset for lung cancer and other MRI/CT datasets.

Result: Experiments demonstrate that HybridMamba significantly outperforms state-of-the-art methods in 3D medical image segmentation on both MRI and CT datasets.

Conclusion: HybridMamba successfully addresses the limitations of existing approaches by balancing local and global context modeling through dual complementary mechanisms, achieving superior performance in 3D medical image segmentation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Address two key challenges in skeleton-based action recognition: scarcity of labeled training data and difficulty modeling both short- and long-range temporal dependencies effectively.

Method: Proposes Long-Short Term Temporal Convolution (LSTC) module with parallel branches for short/long-term features, adaptively fused using learned similarity weights. Extends Joint Mixing Data Augmentation with Additive Mixup at input level, restricting mixup to same camera view to avoid distribution shifts.

Result: Achieves state-of-the-art performance: 94.1% (NTU 60 X-Sub), 97.5% (NTU 60 X-View), 90.4% (NTU 120 X-Sub), 92.0% (NTU 120 X-Set), 97.2% (NW-UCLA).

Conclusion: LSTC-MDA effectively addresses temporal modeling limitations and data scarcity through innovative parallel temporal processing and view-constrained data augmentation, demonstrating significant performance improvements across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current instruction-based image editing methods struggle with challenging tasks due to limited dataset diversity and noisy image-caption pairs that introduce biases and restrict model capabilities in complex editing scenarios.

Method: A novel dataset construction pipeline utilizing two multi-modal large language models (MLLMs) to generate visual-adaptive editing instructions and produce high-fidelity edited images, creating over 107K high-quality samples.
...
Conclusion: MultiEdit provides a valuable resource for advancing research into more diverse and challenging instruction-based image editing capabilities, addressing limitations of existing datasets through high-quality, comprehensive coverage of complex editing scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing explanation methods lack adaptability to complex models and often require manual layer selection, limiting their practical application in visual foundation models.
...
Result: Outperformed baseline methods on CUB-200-2011 and ImageNet-S datasets, achieving significant improvements in mean IoU (53.2-point improvement), insertion score, deletion score, and insertion-deletion score.

Conclusion: The proposed method successfully addresses adaptability issues in visual explanation generation and significantly enhances interpretability while maintaining competitive performance on benchmark datasets.
...
Method: Proposes DACoN framework that uses foundation models to capture part-level semantics in line drawings, fuses low-resolution semantic features with high-resolution spatial features from CNNs, and removes the Multiplex Transformer constraint to support any number of reference images.
...
Conclusion: DACoN effectively addresses challenges in anime colorization by leveraging foundation models for robust semantic understanding and enabling flexible use of multiple reference images, achieving state-of-the-art performance.
...
<details>
  <summary>Details</summary>
Motivation: Reconstructing high-quality human avatars from monocular videos is challenging due to insufficient geometric information from single-view observations and limitations of existing 3D Gaussian splatting methods in preserving surface details.

Method: Proposes two key innovations: 1) Mesh-Guided 2D Gaussian Splatting where 2D Gaussian primitives are attached to template mesh faces with constrained parameters for better surface alignment, and 2) Leveraging foundation models (like Sapiens) with coordinated training strategy using selective gradient isolation to handle conflicting optimization objectives from multi-modal prior knowledge.

Result: Experimental evaluation shows superior reconstruction quality compared to existing methods, with significant improvements in geometric accuracy, appearance fidelity, and semantic information. The method enables spatially and temporally consistent rendering under novel views and poses.

Conclusion: The combination of enhanced representation through mesh-guided 2D Gaussian splatting and coordinated information distillation from foundation models significantly advances 3D monocular human avatar reconstruction, providing high-fidelity results with rich semantic information.
...
Method: Uses listwise ranking prompts to enable MLLMs to directly re-rank candidate images, with query deconstruction prompts for fine-grained analysis and image evaluation prompts to assess alignment with user queries.
...
Conclusion: CoTRR effectively leverages MLLMs' reasoning capabilities for direct ranking, demonstrating superior performance and providing interpretable decision-making for image retrieval tasks.
...
Result: Incorporating synthetic data improves detection performance, with the approach being effective even with small amounts of data and showing potential for further enhancements when scaled up.
...
Result: Confirmed suspected bias and revealed both data-specific and task-specific behaviors of SVLMs under different frame-sampling techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing parametric representations for point clouds face trade-offs between computational efficiency and accuracy - either computationally expensive (spline surfaces) or low degrees of freedom with limited accuracy (Gaussian mixture models, planes).
...
Conclusion: The proposed multi-model approach successfully balances real-time performance with high accuracy for parametric point cloud representation, making it suitable for memory-efficient mapping and multi-robot collaboration applications.
...
<details>
  <summary>Details</summary>
Motivation: Current data distillation methods for SR rely heavily on pre-trained networks and class-specific information, limiting generalizability. Training deep neural networks requires large datasets and computational resources, making data efficiency crucial.

Method: Extract high-gradient patches, categorize images using CLIP features, then fine-tune a diffusion model on selected patches to learn distribution and synthesize distilled training images.

Result: Achieves state-of-the-art performance with only 0.68% of original dataset (performance drop just 0.3 dB). Diffusion fine-tuning takes 4 hours, SR training completes in 1 hour vs 11 hours with full dataset.

Conclusion: The proposed method provides an effective data distillation approach that doesn't require class labels or pre-trained SR models, significantly improving data efficiency and reducing computational requirements while maintaining high performance.
...
Method: Proposed algorithm combines intensity and gradient norm in a novel edge-detector function, incorporates distance information as adaptive step size for contour evolution, and allows fracture prompt embedding to interact with bone fractures.
...
<details>
  <summary>Details</summary>
Motivation: Recent learning-based cortical surface reconstruction methods are fast but struggle with ensuring optimal deformation energy and consistency across training runs.

Method: Design a MED loss as a regularizer on deformation trajectories, complementing Chamfer distance, and incorporate it into the V2C-Flow model.
...
Method: Multi-modal prototype-based architecture that uses both DEXA scans of lumbar spine and patient records, designed to be explainable by default rather than relying on post-hoc analysis.

Result: Achieved 87.58% accuracy in vision-only tasks and 89.8% in multi-modal variant using a dataset of 4,160 NHS patients, surpassing existing published methods.

Conclusion: ProtoMedX demonstrates that high accuracy and explainability can be achieved simultaneously in medical AI applications, providing clinically interpretable explanations alongside state-of-the-art performance.
...
Method: Uses Metric Depth Estimation models to calculate geocoordinates based on object distance from camera, geometric principles, and camera specifications. Validated against LiDAR point clouds in urban environments.

Result: The module effectively estimates distances and geocoordinates, with performance analyzed across different distance intervals and semantic areas (roads, vegetation). Demonstrated practical effectiveness with traffic signs and road damage use cases.

Conclusion: MapAnything provides an automated solution for urban object and incident mapping, offering recommendations for automating database maintenance while maintaining accuracy comparable to LiDAR measurements.
...
<details>
  <summary>Details</summary>
Motivation: Previous approaches assumed models overfit to all degradation types equally, but this paper discovers models predominantly overfit to noise due to its distinct degradation pattern compared to other types like blur or JPEG.

Method: Proposes a targeted feature denoising framework with noise detection and denoising modules that can be integrated with existing super-resolution models without architectural modifications.

Result: Superior performance compared to previous regularization-based methods across five traditional benchmarks and datasets, including both synthetic and real-world scenarios.

Conclusion: The framework provides a general solution that effectively addresses the specific problem of noise overfitting in super-resolution models, demonstrating better generalization capabilities under unknown degradations.
...
<details>
  <summary>Details</summary>
Motivation: To verify claims from arXiv:2311.17983 that Diffusion Denoised Smoothing (DDS) enhances interpretability robustness of Vision Transformers against attacks, and to extend the investigation to additional interpretability methods.

Method: Reproduced FViTs with DDS, tested robustness against attacks in segmentation and classification tasks, extended analysis to baseline methods and Attribution Rollout method, and measured computational costs and environmental impact.

Result: Results broadly agree with original study - DDS improves interpretability robustness to attacks, though minor discrepancies were identified and discussed.

Conclusion: DDS effectively enhances robustness of Vision Transformer interpretability methods against attacks, but comes with computational costs that should be considered.
...
Method: MARIC uses an Outliner Agent to analyze global themes and generate prompts, three Aspect Agents to extract fine-grained descriptions from different visual dimensions, and a Reasoning Agent to synthesize outputs through reflective integration.

Result: Experiments on 4 diverse benchmark datasets show MARIC significantly outperforms baseline methods, demonstrating improved performance and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Growing need to protect personal identities in portrait images while keeping anonymized images useful for downstream computer vision tasks.
...
Result: Outperforms state-of-the-art approaches on CelebA-HQ and FFHQ datasets, requires no additional model training.

Conclusion: Proposed framework provides effective portrait anonymization with complete control over the process while maintaining utility for computer vision applications.
...
Method: Multi-task model that learns representations from longitudinal MRI data, forming trajectories in latent space to predict treatment response. The model represents appearance, fosters temporal continuity, and accounts for heterogeneity in non-responder cohort.

Result: On ISPY-2 dataset, linear classifier in latent trajectory space achieved balanced accuracy of 0.761 (pre-treatment only), 0.811 (early response), and 0.861 (four imaging time points).

Conclusion: The proposed method effectively captures early treatment response dynamics from imaging data and shows improved prediction accuracy with more time points, demonstrating potential for personalized therapy decisions.
...
TL;DR: A method to visualize 3D visual cues used by spacecraft pose estimation networks by training a NeRF-based image generator with backpropagated gradients, helping understand the decision process of data-driven pose estimators.
...
Conclusion: The proposed visualization method effectively reveals the 3D visual cues used by pose estimation networks, improving interpretability and potentially facilitating adoption of data-driven methods in real spacecraft missions.
...
Method: Pseudo-labeling strategy: trained SAM2 generates labels for MOSE test set, combined with existing data. Inference uses SAM2Long for primary segmentation and SeC model for complementary predictions with cascaded decision mechanism.
...
<details>
  <summary>Details</summary>
Motivation: To systematically quantify the trade-offs between task-specific performance and cross-domain generalization when fine-tuning foundation models like CLIP for specialized biometric tasks including face recognition, morphing attack detection, and presentation attack detection.

Method: Evaluated three instances of CLIP fine-tuned for FR, MAD, and PAD on 14 general vision datasets using zero-shot and linear-probe protocols, alongside standard biometric benchmarks. Compared performance against original CLIP baseline and analyzed impact of task complexity and classification head design.

Result: Fine-tuned models showed over-specialization, especially for complex FR tasks. FR model achieved 58.52% improvement on IJB-C benchmark but suffered 18.21% drop on ImageNetV2 compared to baseline. Larger CLIP architectures preserved more generalization ability than smaller variants.

Conclusion: Task complexity and classification head design correlate with catastrophic forgetting. Increased model capacity helps mitigate over-specialization, but fine-tuning foundation models for specialized tasks comes at the cost of reduced cross-domain generalization capabilities.
...
Method: Developed an interactive system with intuitive interface that integrates multiple AI capabilities as modular, interchangeable services (garment generation, makeup transfer, background synthesis, hair editing) that can be deployed on local machines or cloud.

Result: The system enables efficient generation of high-quality virtual KOL images and allows dynamic composition of promotional visuals, significantly streamlining branded content production.

Conclusion: GenKOL can lower costs and accelerate marketing workflows through scalable virtual KOL creation, providing an adaptable solution across diverse use cases and computational environments.
...
Method: Extracts latent knowledge from MLLMs and injects it into training via prompts to unlock MLLMs' intrinsic discrimination potential.

Result: Achieves outstanding detection accuracy exceeding expert models while maintaining MLLM interpretability.

Conclusion: DF-LLaVA provides a simple yet effective framework that bridges the gap between accuracy and interpretability in synthetic image detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D class-incremental learning methods struggle with extreme data scarcity, geometric misalignment, and texture bias, leading to semantic blurring and catastrophic forgetting when integrating 2D foundation models like CLIP.
...
Result: Extensive experiments show significant improvement in 3D few-shot class-incremental learning, achieving superior geometric coherence and robustness to texture bias in both cross-domain and within-domain settings.

Conclusion: CMGR effectively addresses geometric misalignment and texture bias in 3D class-incremental learning, providing a robust framework for open-world scenarios with data scarcity through cross-modal geometric rectification and stable prototype learning.
...
<details>
  <summary>Details</summary>
Motivation: Standard Euclidean GNNs struggle to represent the hierarchical topology of brain networks from fMRI data due to spatial constraints and high distortion, limiting clinical performance in psychiatric disorder classification.

Method: Proposed Brain-HGCN framework based on hyperbolic geometry (Lorentz model) with hyperbolic graph attention layer featuring signed aggregation for excitatory/inhibitory connections, and geometrically sound Fréchet mean for graph readout.
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of product categories in e-commerce makes object identification challenging due to intra-class variability, rare items, visual similarities, diverse packaging, occlusions, and viewpoint changes that cause performance drops in 2D-only methods.

Method: A two-stage framework: 1) Train a large vision model for 2D feature extraction and candidate ranking, 2) Use a lightweight 3D-feature-awareness module to determine if 3D re-ranking is needed, and if so, employ a robot 3D retrieval transformer with geometry-aware dense features and keypoint-based matching instead of cosine similarity.

Result: RoboEye improves Recall@1 by 7.1% over the previous state-of-the-art (RoboLLM) and operates using only RGB images, eliminating the need for explicit 3D inputs and reducing deployment costs.

Conclusion: The proposed RoboEye framework effectively bridges the training-deployment gap in warehouse automation by dynamically combining 2D and 3D reasoning, achieving superior performance while maintaining practical deployment feasibility with RGB-only inputs.
...
<details>
  <summary>Details</summary>
Motivation: Existing rotation-invariant point cloud masked autoencoders rely on random masking that overlooks geometric structure and semantic coherence, treating patches independently and failing to capture spatial relationships consistent across orientations.
...
Conclusion: The proposed dual-stream masking approach effectively addresses limitations of random masking in rotation-invariant point cloud MAE, providing plug-and-play components that integrate into existing frameworks without architectural changes while ensuring broad compatibility and performance improvements.
...
Method: Proposed EchoVLM model with Mixture of Experts (MoE) architecture trained on data spanning seven anatomical regions. The model performs ultrasound report generation, diagnosis, and visual question-answering tasks.

Result: EchoVLM achieved significant improvements: 10.15 and 4.77 points higher in BLEU-1 and ROUGE-1 scores respectively compared to Qwen2-VL on ultrasound report generation task.

Conclusion: EchoVLM shows substantial potential to enhance diagnostic accuracy in ultrasound imaging and provides a viable technical solution for future clinical applications. Source code and model weights are publicly available.
...
Method: Created a large-scale synthetic dataset with 12,328 annotated scenes and 4.7M renderings, then developed SpatialGen - a multi-view multi-modal diffusion model that generates appearance, geometry, and semantic maps from 3D layouts and reference images.

Result: SpatialGen consistently generates superior results compared to previous methods, producing realistic and semantically consistent 3D indoor scenes across arbitrary viewpoints.

Conclusion: The open-sourced dataset and SpatialGen model advance indoor scene understanding and generation by addressing the data bottleneck and providing high-quality, consistent scene synthesis capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Accurate wire detection is crucial for autonomous drone navigation due to wires' slender profile posing unique collision risks that need to be addressed for safe operation.
...
<details>
  <summary>Details</summary>
Motivation: Existing underwater image models focus on discoloration but overlook distance-dependent visibility loss in turbid environments, missing the forward scattering term and nonuniform medium considerations.

Method: Proposed improved synthetic data generation pipeline incorporating forward scattering term and nonuniform medium modeling. Collected BUCKET dataset under controlled turbidity conditions with real turbid footage and corresponding reference images.

Result: Demonstrates qualitative improvements over reference model, especially under increasing turbidity. Achieved 82.5% selection rate by survey participants preferring the proposed approach.
...
<details>
  <summary>Details</summary>
Motivation: Missing MRI modalities are common in clinical practice but limit the effectiveness of existing deep learning segmentation methods that require complete multi-modal inputs.
...
Result: Outperforms existing methods on BraTS 2018 and 2024 datasets, showing superior segmentation accuracy and robustness, especially in single-modality and weak-modality configurations.

Conclusion: AdaMM provides an effective solution for missing-modality brain tumor segmentation, with knowledge distillation proving superior among six evaluated strategies, offering practical guidance for clinical applications.
...
Method: Proposes a reinforcement learning framework that treats hyperparameter optimization as a sequential decision-making task using Markov Decision Process. Uses proximal policy optimization to dynamically adjust hyperparameters across denoising steps with editing objectives integrated into the reward function.
...
Conclusion: The method advances practical deployment of diffusion-based image editing by making hyperparameter optimization more efficient and accessible for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Address the synthetic-to-real domain gap in object detection by training models exclusively on synthetic data to detect specific objects (soup cans) without real-world training data.

Method: Extensive experimentation with YOLOv11 using synthetic data, domain randomization, data augmentation variations, dataset composition strategies, and model scaling. Evaluation through synthetic metrics, qualitative visual inspection, and quantitative testing on manually labeled real-world data.

Result: Best configuration (YOLOv11l with expanded diverse dataset) achieved 0.910 mAP@50 on Kaggle competition's hidden test set. Synthetic validation metrics were poor predictors of real performance.

Conclusion: Increasing synthetic dataset diversity with varied perspectives and complex backgrounds, combined with tuned data augmentation, is crucial for bridging domain gaps, though challenges remain in fully capturing real-world variability.
...
Method: Retrospective study of 32 patients with chest CT scans (3,645 slices) using three deep learning models evaluated with quantitative metrics (volumetric similarity, Dice coefficient, Hausdorff distance) and qualitative clinical acceptability scoring.
...
Conclusion: Unet-R231 provides the most accurate automated lung segmentation, but specialized model fine-tuning is needed for severe pathology cases as performance declines significantly in moderate-to-severe disease.
...
Conclusion: The proposed approach enables accurate and efficient camera parameter optimization for dynamic scenes without relying on motion priors or additional supervision beyond a single RGB video.
...
TL;DR: Integrating geometric visual illusions into image classification training improves model generalization and structural sensitivity, showing perceptual psychology can enhance deep learning.


<details>
  <summary>Details</summary>
Motivation: Deep learning models rely on statistical patterns but lack perceptual insights from psychology. The paper explores how incorporating visual illusions can provide beneficial inductive biases.

Method: Created synthetic geometric-illusion dataset and tested three multi-source learning strategies combining illusion recognition with ImageNet classification using CNN and transformer architectures.

Result: Incorporating geometric illusions as auxiliary supervision systematically improved generalization, especially for challenging visual cases with intricate contours and fine textures.

Conclusion: Perceptually driven inductive biases from synthetic stimuli can enhance vision models, demonstrating novel integration of perceptual science and machine learning for improved model design.
...
TL;DR: A novel attack called Adversarial Instructional Prompt (AIP) that manipulates RAG systems by subtly altering instructional prompts to degrade retrieval behavior while maintaining stealth and utility.


<details>
  <summary>Details</summary>
Motivation: Existing RAG attacks rely on manipulating user queries, which is often impractical. Instructional prompts are widely reused, publicly shared, rarely audited, and implicitly trusted, making them a more realistic attack vector.
...
TL;DR: A model-agnostic framework called M&N that transfers knowledge from 2D pretrained vision models to improve 3D medical image segmentation in semi-supervised settings with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of limited labeled 3D medical images by leveraging knowledge from general 2D pretrained vision models to improve 3D segmentation performance in semi-supervised learning scenarios.

Method: Progressive knowledge distillation from 2D pretrained model to 3D segmentation model through iterative co-training using pseudo-masks, with learning rate guided sampling that adaptively adjusts labeled/unlabeled data proportion based on prediction accuracy and stability.

Result: Achieves state-of-the-art performance, outperforming thirteen existing semi-supervised segmentation approaches across multiple publicly available datasets under all different settings.

Conclusion: M&N provides an effective model-agnostic framework for knowledge transfer from 2D to 3D medical imaging, ensuring adaptability to different architectures and future advanced models while minimizing adverse effects from inaccurate pseudo-masks.
...
Result: RA-GAN outperformed SAM-GAN by 13.14% on average across age groups and CUSP-GAN by 9.1% in 60+ group for racial accuracy. Improved kinship verification accuracy across all relationships (up to 5.22% on KinFaceW-I and 2.9% on KinFaceW-II) while better preserving subject identities.

Conclusion: The proposed racially unbiased face aging approach significantly enhances kinship verification performance by addressing racial biases and age gaps, demonstrating the importance of unbiased age transformation for accurate family relationship analysis.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of MLLMs in spatio-temporal video grounding, particularly their tendency to suffer from suboptimal grounding due to inability to fully integrate text query cues like attributes and actions for inference.

Method: Proposes decomposed spatio-temporal highlighting (DSTH) strategy that decouples queries into attribute and action sub-queries, uses logit-guided re-attention module to learn spatial and temporal prompts, and temporal-augmented assembling (TAS) strategy to improve temporal consistency.

Result: Outperforms state-of-the-art methods on three common STVG benchmarks across various MLLMs.

Conclusion: The proposed framework successfully unleashes the reasoning ability of MLLMs for zero-shot spatio-temporal video grounding by effectively handling attribute and action cues through novel decomposition and assembly strategies.
...
TL;DR: MSDD is a new high-quality aerial image dataset for maize seedling detection and stand counting, showing YOLO models achieve high accuracy for single plants but struggle with multi-plant detection due to class imbalance and irregular appearances.
...
Method: The MSDD dataset contains three classes (single, double, and triple plants) captured under diverse conditions including different growth stages, planting setups, soil types, lighting, camera angles, and densities. Benchmarking was performed using YOLO models to evaluate detection performance.

Result: Detection is most reliable during V4-V6 growth stages and under nadir views. YOLO11 is the fastest model, while YOLOv9 achieves highest accuracy for single plants (precision up to 0.984, recall up to 0.873). Multi-plant detection remains challenging due to rarity and irregular appearance from planting errors, with class imbalance reducing accuracy.

Conclusion: MSDD establishes a strong foundation for developing models that enhance stand counting and support real-time decision-making in agriculture. Despite challenges with multi-plant detection, the dataset represents a step toward automating agricultural monitoring and advancing precision agriculture.
...
TL;DR: ST-AR framework introduces self-supervised objectives to address limitations of autoregressive models in visual tasks, improving both image understanding and generation quality without pre-trained models.
...
Method: Proposed Self-guided Training for AutoRegressive models (ST-AR) that introduces self-supervised objectives during training to address the identified three key limitations of next-token prediction in visual domains.
...
Conclusion: Self-supervised objectives effectively address the fundamental limitations of autoregressive models in visual tasks, enabling improved visual representation learning and generation without relying on pre-trained models.
...
<details>
  <summary>Details</summary>
Motivation: Existing watermarking methods are vulnerable to geometric transformations (crop, rotation) that desynchronize the watermark detection. There's a need for robust synchronization to estimate and invert these transformations.
...
Conclusion: SyncSeal provides an effective solution for robust image synchronization that can be integrated with existing watermarking methods to enhance their geometric transformation robustness.
...
TL;DR: RynnVLA-001 is a vision-language-action model that uses a novel two-stage pretraining approach with ego-centric video generation and human-centric trajectory modeling, achieving state-of-the-art performance on robotics tasks.
...
Method: Two-stage pretraining: 1) Ego-Centric Video Generative Pretraining on 12M videos to predict future frames, 2) Human-Centric Trajectory-Aware Modeling that jointly predicts future keypoint trajectories. Also uses ActionVAE to compress action sequences into latent embeddings.
...
Conclusion: The proposed pretraining strategy provides a more effective initialization for VLA models, demonstrating the value of bridging visual prediction with action prediction through trajectory-aware modeling.
...
TL;DR: Novel Out-of-Sight Trajectory (OST) prediction method that handles noisy sensor data for occluded objects using vision-positioning denoising, achieving state-of-the-art performance on Vi-Fi and JRDB datasets.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory prediction methods assume complete, noise-free data, but real-world scenarios involve out-of-sight objects and noisy sensor data due to limited camera coverage and obstructions, creating safety risks.

Method: Enhanced Vision-Positioning Denoising Module that uses camera calibration to create vision-positioning mapping, denoising noisy sensor data in unsupervised manner. Extends to pedestrians and vehicles.

Result: Achieves state-of-the-art performance in both trajectory denoising and prediction, significantly surpassing previous baselines and traditional methods like Kalman filtering on Vi-Fi and JRDB datasets.
...
TL;DR: A novel multi-view stereo framework that introduces diffusion models for depth refinement, achieving state-of-the-art performance with improved efficiency through conditional diffusion process and confidence-based sampling.


<details>
  <summary>Details</summary>
Motivation: To improve computational efficiency in 3D reconstruction from calibrated images by leveraging diffusion models' iterative refinement capabilities, addressing the limitations of traditional multi-view stereo methods that rely on coarse-to-fine depth estimation.

Method: Formulates depth refinement as a conditional diffusion process using a condition encoder, combines lightweight 2D U-Net with convolutional GRU for efficiency, and implements confidence-based sampling strategy to adaptively sample depth hypotheses.

Result: DiffMVS achieves competitive performance with state-of-the-art efficiency in runtime and GPU memory, while CasDiffMVS achieves state-of-the-art performance on DTU, Tanks & Temples, and ETH3D benchmarks.

Conclusion: The proposed diffusion-based MVS framework successfully integrates diffusion models into multi-view stereo, demonstrating superior performance and efficiency compared to existing methods, with potential for broader applications in 3D reconstruction tasks.
...
Method: Built a large-scale dataset spanning 6 operating systems and 3 task domains using a closed-loop pipeline combining automated agents with human experts, then trained the ScaleCUA model on this scaled-up data.
...
Method: Propose cross-modal distillation paradigm leveraging Vision Foundation Models (VFMs) to generate dense proxy labels. Use spatially aligned event streams and RGB frames. Adapt VFMs like Depth Anything v2 and develop novel recurrent architecture for event-based depth inference.

Result: Evaluation on synthetic and real-world datasets shows competitive performance compared to fully supervised methods without expensive depth annotations, achieving state-of-the-art performance with VFM-based models.
...
Method: Uses student-teacher paradigm with vocabulary alignment strategy, Low-Rank Adaptation (LoRA) for efficient fine-tuning, and Top-K class selection mechanism to reduce memory requirements.
...
Conclusion: Sets a new standard for source-free adaptation in open-vocabulary semantic segmentation with efficient and effective performance gains.
...
TL;DR: CalibPrompt is a novel framework that calibrates Medical Vision-Language Models during prompt tuning to improve confidence reliability without significantly affecting accuracy, using regularization and angular separation loss techniques.
...
Conclusion: The framework successfully addresses confidence calibration in Med-VLMs through prompt tuning with specialized objectives, enhancing reliability for clinical applications while maintaining performance.
