...
Motivation: Traditional segmentation models struggle with generalization across diverse datasets, requiring improved algorithms for accurate preoperative planning and 3D anatomical reconstruction beyond breast cancer applications.

Method: Processed 120 breast MRIs through anonymization, manual segmentation, co-registration, and 3D visualization using ITK-SNAP. Employed human-in-the-loop approach with U-Mamba model for segmentation refinement across imaging scenarios.

Result: U-Mamba achieved DSC values of 0.97 (±0.013) for whole organs, 0.96 (±0.024) for fibroglandular tissue, and 0.82 (±0.12) for tumors. Generated accurate 3D reconstructions and received positive clinician feedback for improved planning and navigation.

Conclusion: The human-in-the-loop ML approach successfully generalizes algorithms for 3D reconstruction and anatomical segmentation, offering enhanced visualization for clinicians, improved preoperative planning, and more effective patient education across medical applications.
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of TRISO particle fuel cross-sections is time-consuming, subjective, and cumbersome due to the thousands of particles in each fuel compact. There's a need for automated, objective methods to analyze irradiation-induced morphological changes.

Method: Generated a dataset of 2,000+ microscopic images of irradiated TRISO particles with annotations. Tested several CNN architectures including RU-Net (newly developed), U-Net, ResNet, and Attention U-Net for automatic segmentation of TRISO layers.

Result: The RU-Net model performed best in terms of Intersection over Union (IoU) metric. CNN models successfully automated the segmentation process.

Conclusion: CNN-based approaches can significantly accelerate TRISO particle analysis, reduce manual labor, and improve the objectivity of segmentation results for nuclear fuel performance assessment.
...
<details>
  <summary>Details</summary>
Motivation: Current nutrient management approaches require lengthy analyses preventing real-time optimization, and imaging-based methods are computationally intensive for resource-constrained environments.

Method: Hierarchical pipeline using autoencoder for early anomaly detection, comparing vegetation index features with Random Forest vs. raw whole-image Vision Transformer for status estimation of fresh weight, dry mass, and tissue nutrients.

Result: 73% net detection of severely nutrient-depleted samples 9 days after transplanting at lower energy cost than wasted nitrogen; ViT outperformed RF on phosphorus and calcium estimation but with higher energy consumption.

Conclusion: The modular pipeline enables edge diagnostics and practical agricultural sustainability by providing flexible trade-offs between efficiency and accuracy for real-time nutrient management.
...
Method: Created PixelHumor benchmark with 2,800 annotated multi-panel comics and conducted experiments with state-of-the-art LMMs to evaluate their humor interpretation and narrative sequencing capabilities.

Result: Top LMMs achieved only 61% accuracy in panel sequencing (far below human performance), revealing substantial gaps in multimodal integration for coherent narrative and humor understanding.

Conclusion: The benchmark provides a rigorous framework for evaluating multimodal contextual reasoning and aims to drive development of LMMs with better social awareness and natural interaction capabilities.
...
Conclusion: The proposed online approach effectively addresses the limitations of offline methods in real-time HOI scenarios, demonstrating superior performance in both generation and perception tasks.
...
<details>
  <summary>Details</summary>
Motivation: To analyze the dual nature of deep learning technology, highlighting both its revolutionary benefits and the specific dangers posed by deepfake applications in social, political, and security contexts.

Method: Analysis of deepfake technology's capabilities and applications, examining how it produces counterfeit photographs and films that can manipulate facial recognition systems and influence public opinion through deceptive imagery.

Result: Deepfake technology has advanced to create highly realistic fake media that can significantly impact privacy, damage reputations of public figures, threaten national security, and potentially sabotage election campaigns through manipulation of public perception.

Conclusion: While deep learning offers revolutionary benefits, the improper application of deepfake technology presents serious societal risks including privacy violations, reputational damage, security threats, and political manipulation that require careful consideration and mitigation strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing studies on neural network simplicity bias focus on small models or synthetic tasks, leaving a gap in understanding SB in large models like CLIP and its relevance to real-world image classification tasks with varying complexity requirements.

Method: The authors theoretically analyze limitations of existing complexity measures, propose a frequency-aware measure for finer-grained SB assessment, validate it on CLIP models using SB-modulation methods, and examine SB-performance relationships across zero-shot and fine-tuning settings on diverse image classification tasks.

Result: The proposed frequency-aware measure proves more informative and consistent than previous measures. Experiments reveal that stronger simplicity bias correlates with better out-of-distribution generalization but may not benefit adversarial robustness, showing task-dependent effects.

Conclusion: The study demonstrates the importance of aligning a model's inductive biases with target task characteristics, highlighting that simplicity bias has varying relevance across different image classification scenarios and should be considered in model design and evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Current AI systems for dermoscopy focus only on images and ignore important patient metadata (age, sex, site) and physical scale information needed for geometric analysis of lesions.
...
Result: Achieves AUC of 0.9812 (vs 0.9440 baseline), with ruler segmentation Dice 0.904, lesion segmentation Dice 0.908, and scale regression MAE of 1.5 pixels. Sparse graphs using 25% of edges maintain near-optimal performance.

Conclusion: Integrating calibrated scale, lesion geometry, and metadata in population graphs provides substantial improvements over image-only approaches, with sparser graphs enabling efficient deployment while maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Current TIMT methods only translate all text in images without providing bounding boxes and cover limited scenarios, limiting practical applications that require precise localization and layout preservation.

Method: Extends TIMT to PATIMT with two sub-tasks: region-specific translation and full-image translation with grounding. Constructs PATIMTBench benchmark with 10 diverse scenarios using adaptive OCR refinement pipeline and manually annotated test set of 1,200 instances.
...
Conclusion: PATIMT addresses practical limitations of traditional TIMT by enabling fine-grained, layout-preserving translations with precise localization, supported by a comprehensive benchmark that facilitates fair evaluation and model improvement.
...
<details>
  <summary>Details</summary>
Motivation: SAR images provide all-weather wake detection but have abstract/noisy features, while optical images offer clearer visual cues but suffer from domain shift when applied to SAR data.

Method: Uses WakeGAN for style transfer to create pseudo-SAR images, instance-level feature similarity filtering to prioritize target-like samples, and Feature-Confidence Memory Bank with KNN fusion to calibrate pseudo-labels, enhanced by region-mixed training.

Result: Experimental results show improved accuracy and robustness in cross-modal ship wake detection tasks.

Conclusion: The proposed SimMemDA method effectively addresses the cross-modal domain adaptation challenge for ship wake detection between optical and SAR imagery.
...
<details>
  <summary>Details</summary>
Motivation: No single data source provides seamless spatiotemporal air temperature data - weather stations offer continuous monitoring but limited spatial coverage, while satellites provide broad coverage but lack continuous temporal data.

Method: Physics-guided deep learning with two neural networks: first reconstructs GOES-16 surface temperature data through neural network encoded with annual temperature cycle, then transforms surface temperature to air temperature using latent relationships with Earth surface properties. Enhanced with deep ensemble learning for uncertainty estimation.

Result: Achieved hourly air temperature mapping accuracy of 1.93°C in station-based validation, built and tested on 77.7 billion surface temperature pixels and 155 million air temperature records from 2018-2024 across contiguous US.

Conclusion: The approach successfully provides seamless high-resolution air temperature monitoring and can be extended to other satellite sources for broader applications.
...
<details>
  <summary>Details</summary>
Motivation: To investigate how the effectiveness of post-hoc metric learning depends on the initial quality and domain-specificity of backbone embeddings for animal re-identification tasks.

Method: Compared general-purpose DINOv2 with domain-specific MegaDescriptor as backbones, used K-NN classifier with thresholding for identification, and applied triplet-learning projection head for metric learning refinement.

Result: Triplet-learning improved specialized MegaDescriptor by 0.13 points but only gained 0.03 for general-purpose DINOv2 on BAKS/BAUS metrics. General-purpose manifold proved difficult to reshape for fine-grained tasks.

Conclusion: General-purpose features have critical limitations for specialized re-ID tasks, highlighting the importance of domain-specific pre-training over post-hoc refinement of general features.
...
<details>
  <summary>Details</summary>
Motivation: Orthomosaic imagery from UAVs suffers from stitching artifacts and heavy preprocessing, limiting field deployment suitability for individual tree mapping in ecological monitoring and forest management.

Method: Used state-of-the-art detectors and keypoint models to compare detection performance between orthomosaic and raw UAV imagery, including within-domain and cross-domain transfer analysis, and evaluated crown-center annotations versus bounding-box centroids for localization accuracy.

Result: Raw imagery yields superior performance in deployment-relevant scenarios, while orthomosaics provide better cross-domain generalization. Crown-center annotations significantly improve localization accuracy beyond bounding-box centroids.

Conclusion: Raw UAV imagery with crown-center annotations offers practical advantages for accurate tree detection and precise localization, providing valuable guidance for UAV-based biodiversity and conservation monitoring applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for articulated objects assume simplified kinematic structures or require joint annotations, but mechanical assemblies like gears exhibit motion through geometric coupling (meshing teeth, aligned axes) which is difficult to reason about from geometry alone.

Method: Introduced MechBench dataset with 693 synthetic gear assemblies and ground-truth motion trajectories. Proposed DYNAMO, a dependency-aware neural model that predicts per-part SE(3) motion trajectories directly from segmented CAD point clouds.
...
Conclusion: MechBench and DYNAMO establish a novel systematic framework for data-driven learning of coupled mechanical motion in CAD assemblies, addressing the challenge of predicting motion from geometric coupling rather than predefined joints.
...
<details>
  <summary>Details</summary>
Motivation: Cotton harvesting is limited by labor-intensive manual picking, low efficiency, and yield losses from missing optimal harvest windows. Accurate recognition of cotton bolls and their maturity is essential for automation, yield estimation, and breeding research.
...
Result: Achieves 91.5% Precision, 89.8% Recall, 93.3% mAP50, 71.3% mAP, and 90.6% F1-Score with only 7.5 GFLOPs. Maintains stable performance under multi-scale and rotational variations. Dataset of 4,966 images curated and external validation set of 1,216 images released.

Conclusion: Cott-ADNet provides an accurate and efficient solution for in-field deployment, offering a reliable basis for automated cotton harvesting and high-throughput phenotypic analysis. Code and dataset are publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Existing surveys primarily focus on updating network architectures for unordered point clouds but ignore practical implementation challenges like large data volumes, diverse scenes, varying point density, and multi-modal data needs in real-world applications.

Method: The authors conduct a meta-review covering deep learning approaches and datasets for critical point cloud processing tasks including scene completion, registration, semantic segmentation, and modeling across urban and environmental applications.

Result: The review identifies gaps that need to be addressed as deep learning methods transition into practical applications, examining both algorithmic and practical aspects of the surveyed methods.

Conclusion: The paper concludes by highlighting the practical implementation challenges and gaps that must be closed for deep learning point cloud processing methods to be successfully deployed in real-world practices across various geomatics and computer vision applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing glaucoma prognosis methods are constrained by fixed-length inputs and struggle with limited dataset sizes, requiring a more flexible and data-efficient approach.
...
Conclusion: The proposed framework enhances glaucoma prognosis performance while maintaining compact parameter size, addressing key limitations of existing methods.
...
Method: Proposes a two-phase training: 1) Main-training with latent perturbation strategy to simulate sampling noises and improve tokenizer robustness, 2) Post-training that optimizes the tokenizer decoder with a well-trained generative model to mitigate distribution differences. Also introduces pFID evaluation metric.
...
TL;DR: GRT is a foundational model for 4D single-chip mmWave radar that achieves high-quality 3D occupancy and semantic segmentation using the largest available raw radar dataset (1M samples), demonstrating strong generalization and logarithmic data scaling.


<details>
  <summary>Details</summary>
Motivation: mmWave radars have poor angular resolution despite being robust and inexpensive, and there's a lack of standardized foundational models or large datasets for radar sensing applications.

Method: Collected 1M raw radar samples (29 hours) and trained a Generalizable Radar Transformer (GRT) foundation model that processes raw radar data rather than lossy representations.

Result: GRT achieves 3D occupancy and semantic segmentation quality comparable to higher-resolution sensors, shows 20% improvement per 10x data scaling, and generalizes across diverse settings.

Conclusion: Raw radar data significantly outperforms conventional representations, and approximately 100M samples (3000 hours) would fully exploit GRT's potential for mmWave radar applications.
...
Method: Used comprehensive evaluation framework with lexical metrics (BLEU, METEOR, ROUGE, CIDEr) and neural-based similarity measures with sentence embeddings to assess semantic alignment across diverse datasets under controlled noise conditions.
...
Conclusion: The study highlights nuanced trade-offs between model size, dataset characteristics, and noise resilience, providing a standardized benchmark for future robust multimodal learning research.
...
<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised semantic segmentation methods struggle with precise object boundary localization and often focus only on the most discriminative regions, requiring expensive pixel-level annotations.
...
Result: Achieves 82.3% mIoU before post-processing and 86.6% after CRF refinement on PASCAL VOC 2012, significantly outperforming previous methods with superior localization accuracy and complete object coverage.

Conclusion: IG-CAM establishes a new benchmark for weakly supervised semantic segmentation, offering a practical solution when pixel-level annotations are unavailable or too expensive, with demonstrated robustness and generalization capability.
...
<details>
  <summary>Details</summary>
Motivation: Artist-created meshes are crucial for commercial graphics but existing methods assume clean inputs or use complex pipelines, limiting real-world applicability

Method: Reformulates 3D point cloud refinement as 2D inpainting task to leverage powerful generative models for end-to-end mesh generation
...
Method: Geometry-aware computer vision pipeline combining deep learning instance segmentation, 3D point cloud reconstruction, and PCA-based axis-aligned slicing to estimate stalk diameter while handling curvature, occlusion, and noise.

Result: The approach provides robust diameter estimation by mitigating common challenges in plant imaging, offering a scalable solution for crop phenotyping.

Conclusion: This computer vision pipeline enables reliable, high-throughput stalk diameter measurement to support breeding and agronomic research programs.
...
Method: Proposes feature distribution alignment using Neural Collapse theory, introduces feature disentanglement module for multi-label settings, uses predefined shared NC structure to guide clustering, and designs regularization losses for compact feature clustering.
...
<details>
  <summary>Details</summary>
Motivation: Address the ecological invalidity gap between offline benchmarks and real-world face forgery detection efficacy by capturing diverse human forgery intents and complex text-image interactions in social media contexts.

Method: Multi-agent framework with LLM-powered agents equipped with profile and memory modules that simulate forgery creation in a social environment, using Adaptive Rejection Sampling (ARS) to ensure data quality and diversity.
...
Conclusion: The simulation-driven approach successfully bridges the gap between synthetic benchmarks and real-world detection needs by generating ecologically valid training data with nuanced labels beyond binary classification.
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based methods don't explicitly model relational structures in HOI detection, while GNNs are inherently better suited for modeling relationships between human-object pairs.

Method: Multimodal Graph Network Modeling (MGNM) with four-stage graph structure and multi-level feature interaction mechanism that leverages both vision and language features for enhanced information propagation.
...
Conclusion: GNN-based relational modeling with multimodal features is more effective than Transformer approaches for HOI detection, providing better relationship recognition and balanced performance across interaction categories.
...
<details>
  <summary>Details</summary>
Motivation: Existing lighting estimation methods struggle with either restoring detailed illumination textures or maintaining fast running speeds while ensuring texture fidelity.

Method: Uses VQVAE to extract discrete features of illumination maps (avoiding posterior collapse) and ViT to capture global context and dependencies for predicting illumination outside field of view. Formulates lighting estimation as a multiclass classification task.
...
<details>
  <summary>Details</summary>
Motivation: Existing consistency distillation methods rely on fixed timestep selection strategies that limit flexibility and require custom schedulers for different distillation processes, restricting practical application potential.

Method: Introduces three strategies: (1) dynamic target timestep selection based on computed importance, (2) optimized alternating sampling along solution trajectory using time step importance, and (3) smoothing clipping and color balancing for stable high-quality generation.

Result: Experimental validation across various consistency distillation methods consistently demonstrated significant improvements in generative performance and strong adaptability.

Conclusion: The adaptive sampling scheduler effectively overcomes limitations of existing methods, providing flexibility and enhanced generation quality across different consistency distillation frameworks.
...
Result: Removing Top-K keypoint regions significantly degrades registration accuracy. Hiding few critical regions causes noticeable localization drift. Physical-world attacks successfully replicated the effects observed in KITTI data.

Conclusion: The attack demonstrates veracity and generality, moving closer to realistic physical-world attacks on LiDAR-based localization systems, highlighting serious security vulnerabilities in self-driving car localization.
...
<details>
  <summary>Details</summary>
Motivation: Existing reflection removal methods work only in image domain and ignore spectral property variations of reflected light, limiting their ability to effectively discern and remove reflections from captured images.

Method: Proposes Spectral Codebook to reconstruct optical spectrum of reflection image, uses spectral prior refinement modules for spatial pixel redistribution and spectral difference enhancement, and employs Spectrum-Aware Transformer for joint recovery in spectral and pixel domains.

Result: Experimental results on three different reflection benchmarks demonstrate superior performance and generalization ability compared to state-of-the-art models.

Conclusion: The spectral learning perspective and proposed spectral-based approach effectively address reflection removal by leveraging wavelength differences and spectral properties, outperforming existing image-domain methods.
...
<details>
  <summary>Details</summary>
Motivation: Maps are essential for autonomous driving, and their representation and production processes have evolved significantly, requiring comprehensive review and analysis of these developments.

Method: Categorizes map evolution into three stages, provides comprehensive review of production workflows, highlights technical challenges, summarizes academic solutions, and discusses cutting-edge research in map representations.

Result: Systematic classification and analysis of map evolution stages, identification of technical challenges, compilation of academic solutions, and exploration of integration possibilities with end-to-end autonomous driving frameworks.

Conclusion: The evolution from HD to Lite to Implicit maps represents significant advancements in autonomous driving technology, with ongoing research focusing on integrating innovative map representations into end-to-end frameworks for improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing ARD methods suffer from degraded performance on clean examples due to divergent optimization objectives in dual-teacher frameworks and performance deterioration of robust teacher models from iterative adversarial examples.

Method: Proposes Cyclic Iterative ARD (CIARD) with: 1) multi-teacher framework with contrastive push-loss alignment to resolve optimization conflicts, and 2) continuous adversarial retraining to maintain teacher robustness against varying adversarial examples.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks suffer from misalignment artifacts that don't reflect real-world manipulation patterns, where practical attacks maintain semantic consistency across modalities rather than creating easily detectable anomalies.

Method: Constructed SAMM dataset through two-stage pipeline: 1) applying state-of-the-art image manipulations, 2) generating contextually-plausible textual narratives. Proposed RamDG framework that retrieves external contextual evidence and processes through image forgery grounding and deep manipulation detection modules.

Result: Extensive experiments show the framework significantly outperforms existing methods, achieving 2.06% higher detection accuracy on SAMM compared to state-of-the-art approaches.

Conclusion: The work pioneers detection of semantically-coordinated manipulations and provides a more realistic benchmark (SAMM dataset) with effective detection framework (RamDG) that handles real-world manipulation patterns.
...
<details>
  <summary>Details</summary>
Motivation: Existing cross-view geo-localization methods neglect spatial and semantic information while extracting features through segmentation, failing to address significant appearance variations across different viewpoints and difficulty in extracting discriminative features.

Method: MFAF method with Multi-Frequency Branch-wise Block (MFB) to capture low-frequency structural features and high-frequency edge details across multiple scales, and Frequency-aware Spatial Attention (FSA) module to focus on key regions of frequency features while reducing background noise interference.

Result: Extensive experiments on University-1652, SUES-200, and Dense-UAV benchmarks show competitive performance in both drone localization and drone navigation tasks.

Conclusion: The proposed MFAF method effectively addresses viewpoint variability and background noise issues in cross-view geo-localization by leveraging multi-scale frequency analysis and attention mechanisms, achieving state-of-the-art performance.
...
Method: The authors curated two underwater datasets (Coral Disease: 4,480 images/18 classes; Fish Species: 7,500 images/20 classes) with four training regimes (25%, 50%, 75%, 100% of images). They trained YOLOv8-s, YOLOv9-s, YOLOv10-s, and YOLOv11-s with identical hyperparameters and evaluated precision, recall, mAP50, mAP50-95, inference time, and FPS. Grad-CAM visualizations were used to analyze feature utilization.

Result: Accuracy saturated after YOLOv9 across both datasets, indicating that architectural innovations primarily target efficiency rather than accuracy. However, inference speed improved markedly. YOLOv10-s demonstrated the best speed-accuracy trade-off for embedded AUV deployment.

Conclusion: The study provides the first comprehensive benchmark of recent YOLO variants on underwater imagery, establishes YOLOv10-s as optimal for AUV applications, and delivers an open, reproducible codebase to accelerate future marine-vision research.
...
<details>
  <summary>Details</summary>
Motivation: Address the limited diversity in existing stereo training data and poor generalization of learning-based stereo matching models for autonomous driving applications.

Method: Built on CARLA simulator with diverse camera configurations (baselines, viewpoints, placements) and varied environmental conditions (lighting, weather, road geometries) to create a comprehensive synthetic dataset.

Result: Models trained on StereoCarla achieve superior generalization accuracy across four standard evaluation datasets (KITTI2012, KITTI2015, Middlebury, ETH3D) compared to models trained on 11 existing stereo datasets, and significantly improve multi-dataset training performance.

Conclusion: StereoCarla provides a valuable benchmark for developing robust stereo algorithms under realistic and diverse conditions, facilitating better depth perception systems for autonomous vehicles.
...
<details>
  <summary>Details</summary>
Motivation: Early-stage fire scenes (0-15 minutes) have reduced visibility due to smoke, impairing emergency response. Lack of large-scale real-world datasets limits smoke removal algorithm development.

Method: Created SmokeBench dataset containing precisely aligned image pairs captured under diverse scenes and smoke concentrations, enabling supervised learning and evaluation.

Result: Comprehensive experiments benchmarking various desmoking methods on the dataset. The dataset provides foundation for robust and practical image desmoking in real fire scenes.

Conclusion: SmokeBench addresses the critical need for real-world paired datasets in smoke removal research and has been publicly released to advance emergency response capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-driven infrared and visible image fusion methods lack proper supervision and evaluation for how effectively input text contributes to fusion outcomes. The observation that referring image segmentation and text-driven fusion share the common objective of highlighting text-referred objects motivated this unified approach.

Method: Proposed RIS-FUSION framework with LangGatedFusion module that injects textual features into the fusion backbone to enhance semantic alignment. Created MM-RIS benchmark with 12.5k training and 3.5k testing triplets (infrared-visible image pairs, segmentation masks, and referring expressions) to support multimodal referring image segmentation.
...
Conclusion: The unified framework successfully bridges text-driven image fusion and referring image segmentation, demonstrating superior performance through joint optimization and semantic alignment with textual guidance.
...
Conclusion: DeFA effectively addresses CZSL challenges by leveraging neuroscientific inspiration to create debiased feature augmentation that supports compositional generalization.
...
<details>
  <summary>Details</summary>
Motivation: Existing MoE approaches struggle with asymmetry between visual (spatial completeness) and linguistic (sequential context) processing, causing language experts to lose contextual grounding and rely on parametric knowledge rather than utilizing provided multimodal information.

Method: Proposes AsyMoE with three specialized expert groups: intra-modality experts for modality-specific processing, hyperbolic inter-modality experts for hierarchical cross-modal interactions, and evidence-priority language experts to suppress parametric biases and maintain contextual grounding.
...
Conclusion: AsyMoE effectively addresses the asymmetry challenge in vision-language MoE models through specialized expert design, achieving significant performance improvements with reduced parameter activation.
...
Method: Introduces two dynamic spatial benchmarks with local perception, environment feedback, and global objectives. Proposes a subjective experience-based memory mechanism for cross-task experience transfer and validation.

Result: Experiments show benchmarks reveal key limitations of mainstream models in dynamic spatial reasoning and long-term memory.

Conclusion: Provides a comprehensive platform for future methodological advances in dynamic spatial reasoning under partial observability and environmental changes.
...
<details>
  <summary>Details</summary>
Motivation: Existing single-view 3D generative models suffer from inter-view inconsistencies and cannot faithfully represent complex internal structures or nontrivial topologies due to their reliance on multiview diffusion priors.
...
Result: SPGen significantly outperforms existing baselines in both geometric quality and computational efficiency, demonstrating superior performance in generating consistent 3D structures.

Conclusion: The spherical projection approach provides a unified solution for single-view 3D generation that simultaneously achieves consistency, flexibility for complex structures, and computational efficiency through direct inheritance of 2D diffusion priors.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models are vulnerable to jailbreak attacks, and existing methods lack both effectiveness and efficiency. The paper reveals that incorporating weak defense into attacks can significantly improve jailbreak performance.

Method: Three-component approach: (1) visual optimizer embedding adversarial perturbations with affirmative semantics, (2) textual optimizer using defense-styled prompts, and (3) red-team suffix generator with reinforcement fine-tuning.

Result: Achieves superior jailbreak performance on four VLMs and four safety benchmarks, outperforming state-of-the-art methods that require multiple attempts.

Conclusion: The work provides a new perspective on jailbreaking VLMs by demonstrating that leveraging defensive patterns can significantly enhance attack effectiveness and efficiency.
...
TL;DR: A novel Gaussian management approach for 3D object reconstruction that dynamically activates spherical harmonics or normals, adapts SH orders based on gradients, and performs task-decoupled pruning to achieve superior reconstruction quality with fewer parameters.
...
Method: Introduces a densification strategy that dynamically activates spherical harmonics or normals under surface reconstruction supervision. Develops lightweight Gaussian representation with adaptive SH order adjustment based on gradient magnitudes and task-decoupled pruning to remove unnecessary Gaussians.
...
Conclusion: The proposed Gaussian management approach effectively mitigates gradient conflicts, improves representation efficiency, and is model-agnostic - it can be seamlessly integrated into other frameworks to enhance performance while reducing model size.
...
Method: Applied clustering to extract 8 master key filters, computed spatial spread measures, modeled filters as difference operators applied to Gaussian smoothing, and performed model fitting using spatial variance matching and norm minimization.

Result: The idealized models show good qualitative similarity to learned filters and demonstrate good predictive properties when replacing learned filters with idealized ones in networks.

Conclusion: Learned filters in depthwise-separable deep networks can be well approximated by discrete scale-space filters, supporting the separability hypothesis and spatial offset properties.
...
Method: The researchers curated a dataset of human preferences using synthetically generated image pairs, analyzed inter-task correlation between image quality attributes for both humans and LLMs, and created synthetic datasets with controlled variations for each specific quality attribute.

Result: Humans show strong relationships between different image quality attributes when making judgments, while LLMs exhibit much weaker correlations. Humans can easily judge all specific quality attributes, but LLMs struggle particularly with attributes like anatomical accuracy.

Conclusion: There are significant differences in how humans and multimodal LLMs perceive and evaluate image quality, with LLMs showing weaker understanding of the relationships between quality attributes and specific difficulties in judging certain aspects like anatomical accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing CVOGL approaches treat the task as one-shot detection, making them vulnerable to feature noise and lacking error correction mechanisms.

Method: Reformulates CVOGL as recurrent localization using learnable tokens that iteratively attend to reference features. Incorporates SAM-based knowledge distillation for semantic guidance and Reference Feature Enhancement Module with hierarchical attention.
...
Conclusion: Recurrent localization with iterative refinement and knowledge distillation provides more accurate and efficient cross-view object geo-localization.
...
TL;DR: A-TDOM is a near real-time True Digital Orthophoto Map generation method using On-the-Fly 3D Gaussian Splatting optimization that processes images as they're acquired, enabling active TDOM rendering within seconds per new image while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Traditional TDOM generation relies on complex offline photogrammetric pipelines causing delays that hinder real-time applications, and quality degrades due to inaccurate camera poses, DSM errors, and scene occlusions.

Method: Uses On-the-Fly SfM to compute pose and sparse point cloud for each new image, integrates new Gaussians into previously unseen regions, and employs orthogonal splatting to render TDOM immediately after each 3DGS field update.
...
Conclusion: A-TDOM successfully addresses the limitations of traditional TDOM generation by providing a near real-time solution that maintains quality and accuracy through on-the-fly 3DGS optimization.
...
TL;DR: A novel Mamba-based framework for automatic pancreatic tumor subtyping using multi-phase contrast-enhanced CT scans, achieving 97.4% accuracy in distinguishing PDAC from PNETs.


<details>
  <summary>Details</summary>
Motivation: Current methods fail to effectively utilize contextual information across multiple CECT phases used in radiologists' diagnostic workflows, limiting performance in pancreatic tumor subtyping due to high heterogeneity and variability.

Method: Proposes dual hierarchical contrast-enhanced-aware Mamba module with spatial and temporal sampling sequences to explore intra/inter-phase contrast variations. Includes similarity-guided refinement for local tumor regions, space complementary integrator, and multi-granularity fusion for cross-scale semantics.
...
Conclusion: The proposed framework demonstrates potential as a more accurate and efficient tool for pancreatic tumor subtyping by effectively modeling temporal and spatial information from multi-phase CECT data.
...
TL;DR: A Contextualized Representation Learning Network for HOI detection that integrates affordance-guided reasoning and contextual prompts to better capture complex interactions by modeling multivariate relationships including auxiliary entities like tools.


<details>
  <summary>Details</summary>
Motivation: Existing two-stage HOI detection approaches face challenges due to incomplete context modeling, particularly with complex interactions involving auxiliary objects.

Method: Expands HOI detection beyond simple human-object pairs to include multivariate relationships using triplet structures <human, tool, object>. Uses affordance-guided reasoning and learnable prompts enriched with instance categories, integrated with visual features via attention mechanism for language-image alignment.

Result: Demonstrates superior performance on both HICO-Det and V-COCO datasets in most scenarios.

Conclusion: The proposed contextualized representation learning approach effectively captures complex, context-dependent interactions by modeling functional roles of auxiliary objects and aligning language with visual content at multiple levels.
...
<details>
  <summary>Details</summary>
Motivation: Visual anomaly inspection in manufacturing suffers from scarcity of real anomaly samples for training robust detectors. Current synthetic data generation methods have limitations: structurally inconsistent anomalies and feature entanglement between images and masks that undermine perceptual realism.

Method: Double Helix Diffusion (DH-Diff) uses a unique double helix architecture with modules for feature separation, connection, and merging. It employs domain-decoupled attention to mitigate feature entanglement and semantic score map alignment to ensure structural authenticity. Offers flexible control via text prompts and optional graphical guidance.

Result: Extensive experiments show DH-Diff significantly outperforms state-of-the-art methods in diversity and authenticity, leading to substantial improvements in downstream anomaly detection performance.

Conclusion: DH-Diff successfully addresses the key challenges in synthetic anomaly generation by providing structurally consistent anomalies without feature entanglement, enabling better training of anomaly detection models in manufacturing applications.
...
TL;DR: SPAM (SuperPixel Anything Model) is a deep learning framework that generates accurate and regular superpixels by combining image feature extraction with large-scale pretrained semantic-agnostic segmentation models.
...
Method: Train a model to extract image features for superpixel generation, then leverage large-scale pretrained semantic-agnostic segmentation at inference to align superpixels with object masks. Can handle prior segmentation and interactive object focusing.

Result: SPAM outperforms state-of-the-art methods both qualitatively and quantitatively on segmentation tasks, providing accurate yet regular superpixels.

Conclusion: SPAM is a versatile and robust framework that successfully balances accuracy and regularity in superpixel generation, making it valuable for various computer vision applications.
...
TL;DR: Hunyuan3D Studio is an AI-powered platform that automates 3D asset creation from concept images or text descriptions, producing game-ready models with optimized geometry and PBR textures.
...
Method: The platform integrates advanced neural modules including Part-level 3D Generation, Polygon Generation, and Semantic UV into a unified end-to-end system that transforms concept images or text into production-quality 3D assets.

Result: Generated assets are visually compelling and meet technical requirements of contemporary game engines, significantly reducing production time while maintaining high quality standards.

Conclusion: Hunyuan3D Studio represents a significant advancement in AI-assisted workflows for game development, providing a seamless bridge from creative intent to technical asset creation.
...
TL;DR: SAGA introduces selective adaptive gating to improve linear attention by addressing feature redundancy and low-rank constraints, achieving better computational efficiency and performance compared to traditional linear attention methods.


<details>
  <summary>Details</summary>
Motivation: Transformer architectures face quadratic complexity bottlenecks with softmax attention, especially for high-resolution images. Linear attention reduces complexity but suffers from uniform compression of KV information leading to feature redundancy and performance gaps.

Method: Proposes SAGA with input-adaptive learnable gates that selectively modulate information aggregation into KV feature maps, enhancing semantic diversity while reducing low-rank constraints. Uses efficient Hadamard-product decomposition for gate computation without additional memory overhead.

Result: Achieves 1.76× throughput improvement and 2.69× reduction in peak GPU memory compared to PVT-T at 1280×1280 resolution. Improves top-1 accuracy by up to 4.4% on ImageNet.

Conclusion: SAGA effectively addresses limitations of conventional linear attention through selective gating, demonstrating both computational efficiency and improved model performance for vision tasks.
...
TL;DR: Systematic study of continual pretraining for medical vision encoders (CLIP-style MI2 and DINOv2-style RAD-DINO) on 3.5M chest x-rays shows task-specific scaling patterns and that 30k in-domain samples can surpass open-weights foundation models.


<details>
  <summary>Details</summary>
Motivation: Medical imaging foundation models are limited by smaller datasets compared to general vision models like CLIP/DINOv2. Need to understand how data scale and pretraining paradigms affect performance in medical settings.

Method: Continual pretraining of MI2 (CLIP-style) and RAD-DINO (DINOv2-style) encoders on up to 3.5M chest x-rays from single institution. Evaluated on classification (findings, lines/tubes), segmentation (lines/tubes), and report generation tasks.

Result: MI2 scales better for finding-related tasks, RAD-DINO stronger on tube-related tasks. Continual pretraining with both reports and structured labels improves performance. 30k in-domain samples sufficient to surpass open-weights foundation models for some tasks.

Conclusion: Center-specific continual pretraining enables medical institutions to achieve significant performance gains using in-domain data, with different encoder paradigms excelling at different medical imaging tasks.
...
TL;DR: Combining DISTS and VMAF metrics improves NeRF quality assessment by overcoming individual metric limitations and achieving better correlation with subjective scores.


<details>
  <summary>Details</summary>
Motivation: NeRF-generated outputs exhibit unique artifacts that make evaluation challenging, with no single metric performing well across all datasets.

Method: Proposed combining DISTS and VMAF metrics using two normalization strategies and two fusion strategies, tested on Synthetic and Outdoor datasets across three configurations.

Result: The fusion metrics demonstrated improved correlation with subjective quality scores compared to individual metrics, showing robustness and generalizability.

Conclusion: Combining perceptual metrics based on different methods (DISTS and VMAF) provides a more effective approach for evaluating NeRF-generated content quality.
...
<details>
  <summary>Details</summary>
Motivation: Data collection for AI models is challenging in medical domains due to rare occurrences and high costs, particularly for canine musculoskeletal condition documentation where abnormalities are visually marked.

Method: Developed mapping to segment visual documentations into 200+ labeled regions, used LLMs with guided decoding, chain-of-thought reasoning, and few-shot prompting to generate synthetic visual documentations for patellar luxation and other diagnoses.

Result: Generated 1,000 synthetic documentations sensitive to location/severity but independent of dog's sex. Model trained on synthetic data alone achieved 88% F1 score on 70 real-world cases.

Conclusion: LLM-generated synthetic data shows strong potential for addressing data scarcity in medical applications and rare diseases, with methodology adaptable to other domains.
...
TL;DR: CCS is a label-free metric that evaluates object detector reliability using test-time data augmentation and spatial consistency of predictions across augmented views, achieving over 90% congruence with traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Ground-truth annotations are rarely available in real-world deployment settings, making it challenging to continuously monitor and compare object detection models.

Method: Applies test-time data augmentation to each image, collects predicted bounding boxes across augmented views, computes overlaps using Intersection over Union, normalizes maximum overlaps, and averages across augmentation pairs to measure spatial consistency.
...
TL;DR: PENet introduces a dual-stream prototype expansion framework using diffusion model features and conventional supervised learning to address intra-class diversity and inter-set inconsistency in few-shot 3D point cloud segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing prototype-based methods for few-shot 3D point cloud segmentation suffer from limited representational capacity (intra-class diversity) and misalignment between support and query feature spaces (inter-set inconsistency).

Method: PENet uses a dual-stream architecture with an Intrinsic Learner for conventional features and a Diffusion Learner using pre-trained diffusion model encoder for generalizable features. It includes Prototype Assimilation Module with push-pull cross-guidance attention and Prototype Calibration Mechanism to prevent semantic drift.
...
Conclusion: The framework successfully addresses prototype limitations by leveraging diffusion model features and dual-stream learning, achieving superior performance in few-shot 3D point cloud semantic segmentation.
...
TL;DR: Lego-Edit is a novel instruction-based image editing framework that uses MLLMs to organize model-level editing tools, achieving state-of-the-art performance on benchmarks with strong generalization to open-domain instructions.
...
Method: Proposes Lego-Edit with: (1) a model-level toolkit of diverse editing models and manipulation functions for fine-grained composition, and (2) a three-stage progressive reinforcement learning approach using unannotated open-domain instruction feedback to train MLLMs.
...
Conclusion: Lego-Edit effectively addresses the generalization challenge in instruction-based image editing by leveraging MLLMs to organize editing tools and reinforcement learning for real-world instruction handling.
...
TL;DR: Proposes high-order inversion method using Runge-Kutta solver for better image reconstruction and DDTA mechanism for disentangled attention in diffusion transformers, achieving SOTA performance in fidelity and editability.
...
Method: High-order inversion method based on Runge-Kutta solver for differential equations + Decoupled Diffusion Transformer Attention (DDTA) to disentangle text and image attention.

Result: Extensive experiments show state-of-the-art performance in image reconstruction and text-guided editing tasks with improved fidelity and editability.

Conclusion: The proposed methods effectively address inversion accuracy and attention entanglement challenges in rectified flow models, enabling better consistency and control.
...
TL;DR: MEJO framework addresses surgical triplet recognition challenges through shared-specific disentangled learning and coordinated gradient learning, achieving state-of-the-art performance on surgical datasets.


<details>
  <summary>Details</summary>
Motivation: Surgical triplet recognition suffers from long-tailed data distribution and optimization conflicts between tasks (inter-task) and within tasks (intra-task due to class imbalance).

Method: Proposes MLLM-Engaged Joint Optimization (MEJO) with: 1) S²D learning scheme to disentangle task-shared and task-specific representations, 2) MLLM-powered probabilistic prompt pool for semantic augmentation, 3) Coordinated Gradient Learning to rebalance gradients from head/tail classes.
...
Conclusion: MEJO framework successfully addresses both inter-task and intra-task optimization conflicts in surgical triplet recognition through disentangled representation learning and coordinated gradient optimization.
...
TL;DR: DialNav introduces a collaborative embodied dialog task where a Navigator and Guide communicate to reach a goal location, with the novel requirement that the Guide must infer the Navigator's position. The RAIN dataset and benchmark are released to support this research.


<details>
  <summary>Details</summary>
Motivation: Prior embodied dialog tasks don't holistically evaluate navigation and communication, particularly lacking the requirement for the Guide to infer the Navigator's location, which makes communication essential for task success.

Method: Collected and released the RAIN dataset containing human-human dialog paired with navigation trajectories in photorealistic environments. Designed a comprehensive benchmark to evaluate both navigation and dialog performance through extensive experiments with different Navigator and Guide models.

Result: The paper presents the DialNav task framework, RAIN dataset, and evaluation benchmark. Experimental results analyze the impact of different model configurations and highlight key challenges in embodied collaborative navigation.

Conclusion: DialNav represents a significant advancement in embodied dialog tasks by requiring holistic evaluation of navigation and communication. The released dataset, code, and evaluation framework will foster future research in collaborative embodied navigation systems.
...
Method: Propose CLVS with vision memory that smooths attention across layers. Initialize with position-unbiased attention, update iteratively, and use uncertainty to terminate smoothing when visual understanding is complete.

Result: Experiments on 4 benchmarks across 3 LVLMs show effectiveness. Achieves SOTA performance with significant improvements in relation and attribute understanding.

Conclusion: CLVS is effective and generalizable, demonstrating that sustained attention on key objects through cross-layer smoothing enhances LVLM visual capabilities.
...
TL;DR: MSGFusion is a novel infrared and visible image fusion framework that uses structured scene graphs to explicitly model entities, attributes, and spatial relationships, achieving superior fusion performance through hierarchical semantic guidance.


<details>
  <summary>Details</summary>
Motivation: Current deep learning fusion methods rely too much on low-level visual cues like texture and contrast, failing to capture high-level semantic information. Existing text-guided approaches use unstructured descriptions without explicit modeling of entities, attributes, and relationships, limiting fine-grained fusion performance.

Method: MSGFusion deeply couples structured scene graphs from text and vision to explicitly represent entities, attributes, and spatial relations. It uses successive modules for scene graph representation, hierarchical aggregation, and graph-driven fusion to synchronously refine high-level semantics and low-level details.

Result: Extensive experiments on multiple public benchmarks show MSGFusion significantly outperforms state-of-the-art approaches, particularly in detail preservation and structural clarity. It delivers superior semantic consistency and generalizability in downstream tasks including low-light object detection, semantic segmentation, and medical image fusion.

Conclusion: The structured scene graph guidance approach effectively bridges the gap between low-level visual features and high-level semantic understanding, providing a robust framework for infrared and visible image fusion with strong performance across various applications.
...
TL;DR: A novel generative anomaly detection method using image-to-image translation for anomaly-free reconstruction and patch similarity scoring, achieving improved segmentation performance in chest CT and brain MRI.


<details>
  <summary>Details</summary>
Motivation: Early disease detection and medical anomaly segmentation are crucial but challenged by normal tissue variability. Existing generative methods struggle with fine-grained anatomical variations.

Method: Image-to-image translation for anomaly-free reconstruction followed by patch similarity scoring between observed and generated image pairs for precise anomaly localization.

Result: Improved pixel-level anomaly segmentation with relative DICE score improvements of +1.9% on chest CTs and +4.4% on brain MRIs compared to state-of-the-art reconstruction-based methods.

Conclusion: The proposed approach effectively addresses normal tissue variability challenges in medical anomaly detection and shows strong generalizability across different imaging modalities and medical conditions.
...
<details>
  <summary>Details</summary>
Motivation: Existing aerial trackers focus on spatial cues but neglect temporal dependencies, leading to poor robustness in long-term tracking and occlusion scenarios. Siamese trackers are limited by linear correlation operations against complex non-linear appearance changes.

Method: Extends SiamTPN architecture with temporal feature fusion and attention-based interactions for explicit temporal modeling, strengthening temporal consistency and enabling richer feature representations.

Result: Significant improvements over baseline: 13.7% higher success rate and 14.7% better precision. Runs at 7.1 FPS on Jetson Nano, demonstrating real-time capability without runtime overhead.

Conclusion: Temporal modeling is crucial for Siamese tracking frameworks. T-SiamTPN provides a strong, efficient solution for aerial object tracking suitable for real-world embedded applications.
...
TL;DR: A three-stage compression pipeline for YOLOv8 that combines sparsity-aware training, structured channel pruning, and knowledge distillation, achieving 73.51% parameter reduction with minimal accuracy loss and significant speed improvements for aerial object detection on edge devices.
...
Method: A novel three-stage compression pipeline: 1) Sparsity-aware training with dynamic sparsity during optimization, 2) Structured channel pruning using batch normalization scaling factors to eliminate redundant channels, 3) Channel-Wise Knowledge Distillation (CWD) with adjustable temperature and loss weighting scheme to mitigate accuracy drop.

Result: For YOLOv8m: Parameters reduced from 25.85M to 6.85M (73.51% reduction), FLOPs from 49.6G to 13.3G, MACs from 101G to 34.5G, with only 2.7% AP50 drop. Inference speed improved from 26 FPS to 45 FPS. With TensorRT optimization: 68 FPS with AP50 47.6.

Conclusion: The proposed compression pipeline effectively enables real-time deployment of aerial object detection models on resource-constrained edge devices while maintaining high detection accuracy, demonstrating practicality for high-throughput scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods treat point cloud registration validation as a classification task with limited quality classes, which lacks fine-grained quantification. The authors aim to provide more precise registration error estimation through regression and improve robustness for diverse datasets.

Method: The method uses regression instead of classification for PCR validation, extends misalignment-related features with multiscale extraction, and employs attention-based aggregation to handle heterogeneous spatial densities in point clouds.

Result: The approach achieves accurate and robust registration error estimation across diverse datasets, particularly excelling with point clouds having heterogeneous spatial densities. When applied to mapping tasks, it significantly improves mapping quality compared to state-of-the-art classification-based methods.

Conclusion: Regression-based PCR validation with multiscale feature extraction and attention mechanisms provides superior fine-grained quality quantification and practical benefits for downstream applications like SLAM and object tracking compared to traditional classification approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing self-supervised methods struggle with accurate reconstruction of dynamic objects due to imprecise motion estimation and weak temporal consistency, leading to incomplete or distorted representations in driving scenes.

Method: Proposes a 4D Radar-assisted Gaussian initialization scheme for dynamic object segmentation and monocular depth scale recovery, plus a Velocity-guided PointTrack model for fine-grained dynamic trajectory tracking with scene flow supervision.

Result: Achieves state-of-the-art performance in dynamic driving scene 3D reconstruction on the OmniHD-Scenes dataset.

Conclusion: 4DRadar-GS effectively addresses dynamic object reconstruction challenges by leveraging 4D Radar data for accurate motion estimation and temporal consistency, advancing self-supervised 3D reconstruction for autonomous driving applications.
...
TL;DR: A new method for 3D scene understanding using object-level Gaussian representations with CLIP feature aggregation, enabling accurate open-vocabulary object retrieval and seamless 2D/3D segmentation without differentiable rendering for semantics.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting enables real-time photorealistic rendering but lacks semantic understanding capabilities due to alpha blending averaging semantics across objects, limiting applications in AR/VR and robotics.
...
Conclusion: The proposed paradigm-shifting approach enables accurate object-level semantic understanding in 3DGS scenes, facilitating broader applications in AR/VR and robotics without the limitations of traditional alpha blending methods.
...
TL;DR: TMKT is a novel fine-grained mixing strategy that interpolates RGB and DVS inputs at different time-steps to enable effective knowledge transfer from RGB to event camera data for spiking neural networks.
...
Method: Time-step Mixup knowledge transfer (TMKT) exploits the asynchronous nature of SNNs by interpolating RGB and DVS inputs at various time-steps. Introduces modality-aware auxiliary learning objectives to support cross-modal label mixing and enhance modality discrimination.
...
Conclusion: TMKT provides an effective solution for cross-modal knowledge transfer between RGB and event camera data, demonstrating strong performance in spiking neural network applications for visual processing.
...
TL;DR: A method for interactive multi-surface segmentation using user clicks and multi-modal inputs, with a network architecture that integrates RGB images, non-RGB modalities, erroneous masks, and encoded clicks to predict improved segmentation masks.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of segmenting multiple entangled surfaces in the same image through interactive user clicks, while leveraging multi-modal inputs to improve segmentation accuracy and efficiency.

Method: A network architecture that takes RGB images, non-RGB modalities, erroneous masks, and encoded clicks as input, designed with constraints that the RGB backbone is black-box and interaction-specific information is integrated after feature extraction to reduce response time.

Result: The method reduces NoC@90 by up to 1.28 clicks per surface on DeLiVER and up to 1.19 on MFNet datasets. The RGB-only baseline also shows competitive performance in classical single-mask interactive segmentation scenarios.
...
TL;DR: FEST Competition challenges participants to develop few-shot text line segmentation systems for ancient handwritten documents using only 3 annotated images per manuscript for training on the U-DIADS-TL dataset.


<details>
  <summary>Details</summary>
Motivation: Historical handwritten documents present unique segmentation challenges due to irregular handwriting, faded ink, complex layouts, and scarcity of large annotated datasets, making fully supervised learning impractical.

Method: The competition uses few-shot learning approach where participants develop systems that can segment text lines using only three annotated images per manuscript for training on the diverse U-DIADS-TL dataset.

Result: The competition aims to produce robust and adaptable text line segmentation methods that work with minimal annotation effort.

Conclusion: FEST competition promotes development of practical automated document analysis tools that can be easily adopted by humanities scholars for historical research with minimal manual annotation requirements.
...
TL;DR: SHREC 2025 track evaluated 15 protein surface shape retrieval methods on 11,555 protein surfaces with electrostatic potential. Best performance came from methods combining electrostatic potential with surface shape.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare different protein surface shape retrieval methods, particularly focusing on the role of electrostatic potential as an additional molecular surface descriptor.

Method: Evaluation of 15 proposed methods from 9 teams on a dataset of 11,555 protein surfaces with calculated electrostatic potential. Performance measured using Accuracy, Balanced accuracy, F1 score, Precision, and Recall.

Result: Best retrieval performance achieved by methods that used electrostatic potential complementary to molecular surface shape. This was also valid for classes with limited data.

Conclusion: Electrostatic potential is a valuable complementary descriptor for protein surface shape retrieval, especially important for classes with limited training data.
...
TL;DR: WINNER addresses SIRENs' spectral bottleneck issue by using Gaussian noise-based weight initialization adapted to target signal's spectral centroid, achieving state-of-the-art performance without extra parameters.


<details>
  <summary>Details</summary>
Motivation: SIRENs struggle with signals outside their frequency support due to inappropriate initialization, causing spectral bottleneck where models fail to recover even representable frequencies.

Method: Proposes WINNER - Weight Initialization with Noise for Neural Representations. Perturbs uniformly initialized SIREN weights with Gaussian noise, with noise scales adaptively determined by the target signal's spectral centroid.
...
Conclusion: WINNER overcomes SIRENs' spectral limitations and suggests new adaptive, target-aware initialization strategies for optimizing deep neural network training.
...
TL;DR: Omnidirectional vision is gaining importance in embodied AI, offering complete 360-degree environmental awareness for applications like robotics and industrial inspection. This paper presents recent breakthroughs in omnidirectional generation, perception, understanding, and datasets, proposing a PANORAMA system architecture and discussing future challenges.


<details>
  <summary>Details</summary>
Motivation: Omnidirectional vision provides holistic environmental awareness that significantly enhances scene perception completeness and decision-making reliability compared to traditional pinhole vision, but foundational research has historically lagged behind.

Method: The paper presents an overview of emerging trends in omnidirectional vision, highlighting recent breakthroughs across generation, perception, understanding, and datasets. It proposes PANORAMA - an ideal panoramic system architecture with four key subsystems for the embodied AI era.

Result: The paper synthesizes state-of-the-art advancements in omnidirectional vision and provides insights from both academia and industry, outlining a comprehensive system architecture and identifying emerging trends at the intersection of panoramic vision and embodied AI.

Conclusion: Omnidirectional vision is rapidly developing due to industrial demand and academic interest, with significant potential for building robust, general-purpose AI systems in the embodied AI era, though challenges remain that require future research.
...
TL;DR: A dual-stage framework for detecting subtle user mistakes from egocentric video using feature fusion and multiple classifier objectives to handle class imbalance and improve generalization.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of identifying infrequent and subtle user mistakes in egocentric video data, which requires handling class imbalance and improving detection of rare instances.

Method: Dual-Stage Reweighted Mixture-of-Experts (DR-MoE) with frozen ViViT and LoRA-tuned ViViT feature extraction, combined with three specialized classifiers using reweighted cross-entropy, AUC loss, and label-aware loss with sharpness-aware minimization.

Result: The method achieves strong performance, particularly in identifying rare and ambiguous mistake instances from egocentric video data.

Conclusion: The proposed DR-MoE framework effectively handles class imbalance and improves mistake detection in egocentric videos through its dual-stage architecture and specialized loss functions.
...
TL;DR: Modern Vision Foundation Models (VFMs) outperform specialized AI-generated image detectors by over 20% in real-world scenarios due to their learned alignment with forgery-related concepts from pre-training data exposure.


<details>
  <summary>Details</summary>
Motivation: Specialized AI-generated image detectors perform well on curated benchmarks but fail catastrophically in real-world scenarios with high false-negative rates, highlighting the need for more robust detection methods.

Method: Using a simple linear classifier on top of modern Vision Foundation Models (VFMs) trained on identical data, and analyzing text-image similarities to understand how VLMs align synthetic images with forgery-related concepts.

Result: The VFM-based approach boosts in-the-wild accuracy by over 20% compared to specialized detectors. The performance advantage comes from VFMs' learned alignment with forgery concepts, which disappears when tested on data collected after the model's pre-training cutoff date.

Conclusion: Updated Vision Foundation Models provide superior real-world detection capability compared to static specialized detectors, and true generalization evaluation requires test data that is completely independent of the model's entire training history including pre-training.
...
<details>
  <summary>Details</summary>
Motivation: Small drones pose increasing threats to military and civilian infrastructure, requiring early automated detection solutions.

Method: Uses spiking neural networks with neuromorphic cameras (event cameras) deployed on neuromorphic chips to create a virtual tripwire detection system. Investigates synthetic data for training.

Result: System is several orders of magnitude more energy efficient than edge GPU solutions, capable of running over a year on battery. Model relies on drone shape rather than propeller motion.

Conclusion: The small size and low power consumption enables easy deployment in contested areas or locations lacking power infrastructure.
...
<details>
  <summary>Details</summary>
Motivation: Current single-image 3D avatar reconstruction faces challenges with occluded regions and lacks control over geometry and texture. The task remains ill-posed due to limited monocular input information.

Method: Two-stage approach: 1) Lightweight adapter-enhanced multi-view generation with Pose-Adapter (injects SMPL-X/skeletal info) and ID-Adapter-G (preserves facial identity), plus BLIP2 for text descriptions. 2) Feedforward Transformer with multi-view fusion and ID-Adapter-R for 3D Gaussian Splat reconstruction.
...
Conclusion: Dream3DAvatar provides an efficient and text-controllable solution for high-fidelity 3D avatar generation from single images, effectively addressing occluded region challenges through innovative adapter mechanisms and multi-view fusion.
...
TL;DR: A two-stage reinforcement learning framework that enhances both perceptual and reasoning capabilities of vision-language models, addressing the complexity of visual understanding before reasoning.


<details>
  <summary>Details</summary>
Motivation: Directly applying RL methods from LLMs to VLMs is suboptimal because VLMs require accurate visual perception before reasoning can occur, making their tasks inherently more complex.
...
Conclusion: The proposed two-stage RL approach effectively addresses the unique challenges of VLMs by jointly optimizing perception and reasoning, demonstrating significant performance improvements over traditional methods.
...
TL;DR: HERO is a training-free framework that reduces computational overhead in high-resolution LVLMs by selectively dropping less important visual tokens based on content importance and complementary token roles.


<details>
  <summary>Details</summary>
Motivation: High-resolution LVLMs generate excessive visual tokens when processing cropped image tiles, causing significant computational and memory overhead that needs to be addressed.

Method: HERO uses content-adaptive token budget allocation and function-aware token selection based on empirical findings about varying tile importance and CLIP encoder's two-stage attention patterns.
...
Conclusion: The framework provides both empirical insights and practical solutions for efficient inference in high-resolution vision-language models through intelligent token reduction.
...
<details>
  <summary>Details</summary>
Motivation: Existing RIS methods struggle with multimodal misalignment and language semantic loss, especially in complex scenes with multiple visually similar objects

Method: Three-stage framework: Knowledge Plus Stage (MLAM for multiscale bidirectional alignment), Knowledge Fusion Stage (CFSM for long-range cross-modal dependencies), Knowledge Intensification Stage (WFDM for semantic degradation compensation)

Result: Proposed system enhances multimodal alignment accuracy and addresses semantic loss in complex scenes

Conclusion: TFANet provides systematic hierarchical alignment that improves performance on challenging RIS tasks with multiple similar objects
...
TL;DR: LLFDisc is a U-shaped deep enhancement network that uses cross-attention and gating mechanisms for frequency-aware enhancement, with a novel distribution-aware loss using KL-Divergence to better align Fourier-domain information than traditional MSE losses.


<details>
  <summary>Details</summary>
Motivation: Traditional Fourier frequency fitting with pixel-wise loss functions focuses too much on local information and causes global information loss. The phase components in Fourier domain capture spatial structures while amplitude encodes luminance, requiring better alignment methods.

Method: Proposed LLFDisc - a U-shaped deep enhancement network with cross-attention and gating mechanisms. Introduces distribution-aware loss using closed-form KL-Divergence to directly fit Fourier-domain information. Also enhances VGG-based perceptual loss with KL-Divergence on deep features.
...
Conclusion: The proposed frequency-aware enhancement approach with distribution-aware loss and enhanced perceptual loss significantly improves Fourier-domain information alignment and structural fidelity compared to conventional methods.
...
Method: Proposes a dual-network architecture with Cross Consistency Enhancement module using cross pseudo and entropy-filtered supervision, dynamic weighting strategy with uncertainty-aware mechanism (KL divergence), and self-supervised contrastive learning to align uncertain voxel features with reliable class prototypes.

Result: Achieves superior performance across three 3D segmentation datasets (Left Atrial, NIH Pancreas, BraTS-2019), with 89.95% Dice score on Left Atrial using only 10% labeled data, outperforming state-of-the-art methods.

Conclusion: The proposed framework effectively addresses noisy pseudo-labels and insufficient supervision in semi-supervised medical image segmentation, demonstrating consistent performance improvements across multiple datasets and settings.
...
TL;DR: Synthetic data generation from CAD models enables efficient visual assembly control with high detection accuracy (up to 99.5% on synthetic data, 93% on real data), making computer vision quality control more accessible for SMEs.


<details>
  <summary>Details</summary>
Motivation: Traditional computer vision methods for assembly quality control require expensive data collection and annotation, creating barriers for small- and medium-sized enterprises (SMEs) that lack resources for extensive training and manual labeling.

Method: Leverages simulated scene generation based on CAD data and object detection algorithms to create synthetic training data, eliminating the need for manual image collection and annotation.

Result: Achieved mean Average Precision (mAP@0.5:0.95) up to 99.5% on synthetic planetary gear system components and up to 93% when transferred to real-world camera-captured testing data, demonstrating effective transfer learning.

Conclusion: Synthetic data generation provides a time-saving and resource-efficient pipeline for visual assembly control, making computer vision quality control solutions more accessible and practical for SMEs in manufacturing environments.
...
TL;DR: Hierarchical Deep Fusion Framework (HDFF) uses ensemble of four pre-trained models for facial forgery detection, achieving 0.96852 score and 20th place in competition.


<details>
  <summary>Details</summary>
Motivation: Deepfake technology proliferation creates digital security challenges, requiring robust detection models that work across diverse manipulation techniques.

Method: Integrates Swin-MLP, CoAtNet, EfficientNetV2, and DaViT models fine-tuned on MultiFFDI dataset, with feature concatenation and final classifier training.

Result: Achieved score of 0.96852 on private leaderboard, ranking 20th out of 184 teams in the competition.

Conclusion: Hierarchical fusion approach effectively leverages collective strengths of multiple models for complex image classification tasks like deepfake detection.
...
TL;DR: Weakly and self-supervised class-agnostic motion prediction from LiDAR point clouds using foreground/background or non-ground/ground masks as supervision, reducing annotation effort while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Understanding motion in dynamic environments is critical for autonomous driving, and class-agnostic motion prediction is essential but typically requires expensive motion annotations.

Method: Proposes weakly supervised paradigm using fully/partially annotated foreground/background masks (1%, 0.1%) instead of motion annotations. Uses non-ground/ground masks as alternative to further reduce annotation. Develops Robust Consistency-aware Chamfer Distance loss with multi-frame information and robust penalty functions.
...
TL;DR: This paper introduces CRPS-D, a large-scale parking slot detection dataset with diverse real-world conditions, and SS-PSD, the first semi-supervised method for parking slot detection that leverages unlabeled data to improve performance.


<details>
  <summary>Details</summary>
Motivation: Current parking slot detection datasets are limited in scale and lack real-world noise variations. Manual annotation is error-prone and expensive, creating a need for better datasets and methods that can utilize unlabeled data.

Method: Created CRPS-D dataset with various lighting, weather conditions, and challenging parking slot variants. Developed SS-PSD, a semi-supervised teacher-student model with confidence-guided mask consistency and adaptive feature perturbation.

Result: SS-PSD outperforms state-of-the-art methods on both the new dataset and existing datasets. Performance improvements are more significant with more unlabeled data available.

Conclusion: The proposed dataset and semi-supervised approach significantly advance parking slot detection by addressing dataset limitations and leveraging unlabeled data, with publicly available resources for community use.
...
Method: Two-stage distillation: 1) Reconstruction-guided feature distillation to align and densify features, 2) Diffusion-guided feature distillation with a lightweight diffusion network and noise adapter to refine features by treating stage-one features as noisy versions of teacher representations.
...
TL;DR: TexTAR is a multi-task Transformer model that uses 2D RoPE and context-aware data selection to accurately recognize text attributes (bold, italic, underline, strikeout) in multilingual documents, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for textual attribute recognition struggle with computational efficiency and adaptability in noisy, multilingual environments, despite these attributes being essential for understanding document semantics and structure.
...
Conclusion: Contextual awareness through the proposed architecture and data selection pipeline enables superior textual attribute recognition performance across diverse, real-world multilingual documents.
...
<details>
  <summary>Details</summary>
Motivation: Current video language models suffer from spatio-temporal incompleteness in individual videos, leading to hallucinations and inaccuracies. Multiple related videos could improve reasoning but direct feeding of video data is inefficient due to token redundancy.

Method: Proposes a framework with: 1) Video Structuring Module to represent video knowledge as spatio-temporal graphs, 2) Graph Fusion Module to fuse structured knowledge from related videos into augmented graph node tokens, 3) Multi-video structured prompt integrating graph, visual, and textual tokens for LLM input.

Result: Extensive experiments show the framework effectively enhances video language model performance.

Conclusion: The multi-video collaborative framework with structured graph representation provides a promising approach for advancing video language models by addressing spatio-temporal incompleteness issues.
...
TL;DR: WHU-STree is a comprehensive multi-modal dataset for urban street tree analysis, featuring 21,007 annotated trees across 50 species from two cities, supporting over 10 inventory tasks with synchronized point clouds and high-resolution images.


<details>
  <summary>Details</summary>
Motivation: Traditional field surveys for street tree inventories are time-consuming and labor-intensive, while existing Mobile Mapping System datasets are limited by small scale, poor annotation, or single modality, restricting comprehensive urban tree analysis.

Method: The authors created WHU-STree by collecting synchronized point clouds and high-resolution images across two distinct cities, annotating 21,007 tree instances with 50 species classifications and 2 morphological parameters to support multiple street tree inventory tasks.

Result: Extensive experiments demonstrated the significant potential of multi-modal data fusion for street tree analysis and highlighted cross-domain applicability as critical for practical algorithm deployment, with benchmarks established for tree species classification and individual tree segmentation tasks.

Conclusion: The WHU-STree dataset addresses limitations of existing urban tree datasets and enables research in multi-modal fusion, multi-task collaboration, cross-domain generalization, spatial pattern learning, and Multi-modal Large Language Models for comprehensive street tree asset management.
...
<details>
  <summary>Details</summary>
Motivation: Large Language Models present unprecedented opportunities to revolutionize medical contrastive vision-language pre-training by enabling automated, low-cost creation of large-scale supervised datasets from medical reports.

Method: Using LLMs to automatically extract diagnostic labels from radiology reports without complex prompt engineering, creating "silver-standard" datasets for supervised pre-training of vision encoders using 3D ResNet-18 with vanilla CLIP training.

Result: Achieved 83.8% AUC for zero-shot diagnosis on CT-RATE, 77.3% AUC on RAD-ChestCT, and substantial improvements in cross-modal retrieval (MAP@50=53.7% for image-image, Recall@100=52.2% for report-image). Vision encoder performance comparable to specialized BERT-based models.

Conclusion: LLMs can democratize access to large-scale supervised pre-training and fundamentally improve contrastive vision-language alignment, enabling more performant and scalable medical AI systems at minimal cost (~$3 for 50k CT image-report pairs).
...
TL;DR: This paper introduces temporal road-obstacle video segmentation, addressing the limitation of existing frame-by-frame methods that produce inconsistent predictions. The authors curate four benchmarks, evaluate 11 state-of-the-art methods, and propose two strong baseline methods using vision foundation models, achieving new SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing road-obstacle segmentation methods process individual frames independently, ignoring the temporal nature of the problem which leads to inconsistent predictions between consecutive frames. The task is inherently temporal since segmentation maps for consecutive frames are strongly correlated.

Method: The authors curated and adapted four evaluation benchmarks for road-obstacle video segmentation. They evaluated 11 state-of-the-art image- and video-based segmentation methods on these benchmarks. Additionally, they introduced two strong baseline methods based on vision foundation models.

Result: The proposed approach establishes a new state-of-the-art in road-obstacle video segmentation for long-range video sequences, demonstrating superior performance compared to existing methods.

Conclusion: The work provides valuable insights and direction for future research in temporal road-obstacle segmentation, highlighting the importance of considering temporal consistency in autonomous driving applications for safer navigation.
...
Method: Integrates optimized YOLOv8 (with GhostNetV3 backbone, EMA attention, and pruning) for human detection and TSN for binary violence classification. Models trained separately on pedestrian and violence datasets.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models have advanced image synthesis but raise concerns about malicious misuse. Existing approaches struggle to detect images generated by diffusion-based inpainting models, even when trained on similar inpainted images.

Method: End4 uses an end-to-end denoising diffusion approach with a denoising reconstruction model to align latent spaces, and a Scale-aware Pyramid-like Fusion Module (SPFM) that refines local image features using attention pyramid layers at different scales.

Result: Extensive experiments show that End4 effectively generalizes to unseen masking patterns and remains robust under various perturbations. A comprehensive benchmark with five distinct masked regions was established for evaluation.
...
TL;DR: A novel curriculum multi-task self-supervised learning framework for lightweight hyperspectral imaging analysis that combines masked image modeling with spatial/spectral jigsaw puzzles, achieving strong performance with models 16,000x lighter than state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging generates high-dimensional data with slow satellite transmission rates, requiring compact models for onboard processing to minimize redundant data transmission like cloud-covered areas.

Method: Curriculum multi-task self-supervised learning (CMTSSL) integrating masked image modeling with decoupled spatial and spectral jigsaw puzzle solving, guided by progressive curriculum learning that increases data complexity.

Result: Validated on four public benchmarks, showing consistent gains in downstream segmentation tasks using architectures over 16,000x lighter than state-of-the-art models.
...
TL;DR: Vision-based quality control system using k-NN algorithm to optimize vacuum thermoforming parameters with minimal data requirements
...
Method: Developed comprehensive dataset using visual data from vacuum-formed samples with various process parameters, applied image augmentation, and used k-Nearest Neighbour algorithm to map low-quality parts to high-quality counterparts
...
Conclusion: Vision-based system effectively predicts and optimizes process parameters for enhanced part quality in vacuum thermoforming
...
TL;DR: ResidualViT is a vision transformer architecture that reduces computational cost for temporally dense video tasks by leveraging temporal redundancy through residual connections and token reduction, achieving up to 60% cost reduction and 2.5x speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Temporally dense video tasks require high temporal resolution frame processing, which is computationally expensive. There's a need to reduce feature computation costs while maintaining performance.
...
Result: Achieves significant computational cost reduction (up to 60%) and inference speed improvement (up to 2.5x faster) while closely matching the accuracy of the original foundation model across four tasks and five datasets.

Conclusion: The ResidualViT architecture effectively reduces computational burden for temporally dense video tasks by exploiting temporal redundancy, making high-resolution video processing more efficient without sacrificing accuracy.
...
Method: Combines gamification with large public datasets and AI-driven feedback. RadGame Localize uses bounding box drawing compared to radiologist annotations with visual explanations from vision-language models. RadGame Report provides structured AI feedback on report composition using radiology report generation metrics compared to ground truth reports.

Result: Participants achieved 68% improvement in localization accuracy (vs 17% with traditional methods) and 31% improvement in report-writing accuracy (vs 4% with traditional methods) after exposure to the same cases.

Conclusion: RadGame demonstrates the potential of AI-driven gamification to deliver scalable, feedback-rich radiology training and reimagines medical AI applications in education.
...
<details>
  <summary>Details</summary>
Motivation: Need for reliable quantification of AI-generated image realness and identification of visually inconsistent regions to improve photorealism and enable practical use of AI-generated images.

Method: Uses vision-language models trained on large datasets to generate textual descriptions of visual inconsistencies, serving as substitutes for human annotations for both overall realness assessment and local inconsistency identification.

Result: The multimodal approach improves objective realness prediction performance and produces dense realness maps that effectively distinguish between realistic and unrealistic spatial regions.
...
<details>
  <summary>Details</summary>
Motivation: Creating 3D assets that match existing style patterns is crucial for applications like video gaming and VR, but current methods struggle with fine-grained style control over both texture and geometry.

Method: Proposes Style Disentangled Attention (SD-Attn) module with cross-3D attention mechanism and style-disentangled feature selection strategy, plus Style Guided Control (SGC) mechanism for exclusive geometry/texture stylization.

Result: Outperforms existing baseline methods in producing high-fidelity 3D assets with fine-grained style control, enabling texture-only, geometry-only, or combined stylization.

Conclusion: StyleSculptor provides an effective zero-shot solution for style-guided 3D generation with adjustable style intensity and disentangled control over texture and geometry styles.
...
TL;DR: SR-3D is a vision-language model that bridges 2D images and 3D data through shared visual tokens, enabling flexible region annotation and spatial reasoning across frames without multi-frame labeling.


<details>
  <summary>Details</summary>
Motivation: To connect single-view 2D images with multi-view 3D data and enable accurate spatial reasoning across frames, even when objects don't appear in the same view, without requiring exhaustive multi-frame labeling.

Method: Enrich 2D visual features with 3D positional embeddings, creating a shared visual token space that allows the 3D model to leverage strong 2D priors for spatial reasoning.

Result: Achieves state-of-the-art performance on both general 2D vision-language and specialized 3D spatial benchmarks, and works effectively on in-the-wild videos without 3D inputs or ground-truth annotations.

Conclusion: SR-3D effectively unifies 2D and 3D representation spaces for scene understanding, demonstrating accurate spatial relationship inference and metric measurements in diverse scenarios.
