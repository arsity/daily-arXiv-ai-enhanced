...
Motivation: Existing MAE approaches for skeleton action recognition use raw joint coordinate reconstruction, causing computational redundancy and limited semantic representation.

Method: General Feature Prediction framework with collaborative learning - lightweight target generation network produces diversified supervision signals across spatial-temporal hierarchies with constrained optimization.

Result: 6.2x faster training than standard methods, state-of-the-art performance on NTU RGB+D 60/120 and PKU-MMD datasets.

Conclusion: High-level feature prediction outperforms conventional low-level reconstruction in masked skeleton modeling, offering both efficiency and superior representation quality.
...
<details>
  <summary>Details</summary>
Motivation: Manual mitosis counting is time-consuming and subjective, while AI solutions suffer from domain shift and severe class imbalance between mitotic figures and normal nuclei.

Method: Pixel-level segmentation with UNet backbone integrating contrastive representation learning and domain-adversarial training. Teacher-student strategy generates pseudo-masks for mitoses, hard negatives, and normal nuclei. Multi-scale CNN classifier uses segmentation features in multi-task learning.
...
Conclusion: The unified framework combining segmentation-based detection and classification effectively addresses domain shift and data imbalance, demonstrating robust mitosis analysis performance.
...
Result: Validated on FB CMNIST, CelebA, and COCO datasets - boosted worst group accuracy, halved multi-attribute bias amplification, and set new low in SBA metric even with increased bias complexity and distribution shifts.
...
Method: Conducted ablation study to identify most effective nnU-Net components, then developed lightweight U-Net with simple affine augmentations and deep supervision while removing unnecessary complexity.

Result: Achieved statistically equivalent Dice scores (0.93/0.85/0.89 vs 0.93/0.86/0.89) with 2M vs 33M parameters, 16x smaller model, and 4x faster inference (1.35ms vs 5.40ms per frame).

Conclusion: Simple architectural choices and targeted optimizations can produce highly efficient cardiac segmentation models without sacrificing performance, enabling real-time clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for tree instance segmentation require large annotated datasets and substantial computational resources, creating a need for more resource-efficient alternatives.

Method: Revised unsupervised treeX algorithm combining clustering-based stem detection with region growing for crown delineation, with two parameter presets for different laser scanning systems (ground-based TLS/PLS and UAV-borne ULS).

Result: Significant improvements over original treeX: F1-score gains of +0.11 to +0.49 for ground-based data, achieves F1-score of 0.58 for ULS data (where original failed), and achieves accuracy comparable to recent open-source deep learning methods.

Conclusion: The method serves as a resource-efficient alternative to deep learning when data characteristics align (sufficient stem visibility and point density), and can be used for semi-automatic label generation for deep learning models. Open-source implementation provided.
...
Method: Proposes a dual-supervision paradigm with complementary object-level and frame-level reconstruction tasks within a dual-encoder architecture to enforce geometric consistency and develop spatial reasoning capabilities.
...
Result: Achieves comparable accuracy to full-precision systems while reducing system-level latency by 3.2x and improving mAP30 by +9.5 over full-precision baselines. Enables larger models within strict memory budgets.
...
Method: Evaluated three deep learning models (ResNet50, MobileNetV2, EfficientNetB0) on the Swedish Leaf Dataset containing 1,125 images from 15 plant species, using standard performance metrics during training and testing phases.

Result: ResNet50 achieved 94.11% training accuracy but overfitted (88.45% testing accuracy). MobileNetV2 showed better generalization (93.34% testing accuracy). EfficientNetB0 performed best with 94.67% testing accuracy and all metrics exceeding 94.6%.
...
Conclusion: The approach provides an efficient alternative to traditional graph matching networks for floor plan graph comparison, offering faster inference without sacrificing accuracy.
...
Method: Leverages Singular Value Decomposition (SVD) to modify CLIP's internal parameter space without adding extra modules. Only fine-tunes singular values of parameter matrices to rescale basis vectors while retaining pretrained weights.

Result: Achieves state-of-the-art classification on 11 natural and 10 biomedical datasets, outperforming previous methods in accuracy and generalization under few-shot settings using only 0.04% of total parameters.

Conclusion: CLIP-SVD provides an effective parameter-efficient adaptation technique that preserves pretrained knowledge while enabling superior domain adaptation performance and better generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing lightweight networks use generic attention mechanisms that poorly capture subtle pathological features like irregular lesion shapes and complex textures in plant disease diagnosis, making edge deployment challenging.

Method: Twofold approach: 1) Training-free neural architecture search (DeepMAD) for efficient backbone, 2) STAM with dual attention branches - deformable convolutions for shape awareness and Gabor filter bank for texture awareness.

Result: STA-Net achieved 89.00% accuracy and 88.96% F1 score on CCMT dataset with only 401K parameters and 51.1M FLOPs, significantly outperforming baseline and standard attention models.

Conclusion: Integrating domain knowledge through decoupled shape-texture attention provides an effective approach for edge-deployed precision agriculture AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Underwater camouflaged object detection is crucial for marine ecology but remains underexplored due to optical distortions, water turbidity, and complex marine organism traits that hinder accurate identification.

Method: Proposed SLENet framework with Gamma-Asymmetric Enhancement (GAE) module and Localization Guidance Branch (LGB) to enhance multi-scale features and generate semantic-rich location maps, guided by Multi-Scale Supervised Decoder (MSSD) for accurate predictions.
...
Conclusion: The introduced DeepCamo dataset and SLENet framework effectively address the challenges of underwater camouflaged object detection, providing a robust solution with strong performance and generalization capabilities for marine ecological applications.
...
<details>
  <summary>Details</summary>
Motivation: Standard diffusion models are trained on static images, which is information-deficient for capturing temporal dynamics, leading to slower convergence and limited generalization.

Method: A simple training strategy that incorporates temporal inductive bias from continuous video frames into standard diffusion training pipelines without architectural modifications.

Result: 2x faster convergence, lower FID scores on both training and validation distributions, improved generative diversity, and reduced gradient variance.

Conclusion: Leveraging temporal coherence in video data significantly improves diffusion model training efficiency and performance while maintaining simplicity and compatibility with existing pipelines.
...
Method: MedVista3D performs local and global image-text alignment for fine-grained representation learning within full-volume context. It uses language model rewrites and a Radiology Semantic Matching Bank to address report variability and enable semantics-aware alignment.
...
Conclusion: MedVista3D effectively addresses key challenges in 3D radiology analysis through multi-scale semantic-enriched vision-language pretraining, demonstrating strong performance across multiple diagnostic tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based prompt learning methods perform poorly on fine-grained datasets due to their inability to capture subtle class discrepancies.

Method: Uses attribute disentanglement module with Brownian Bridge Diffusion Model to separate shared and class-specific attributes, and granule learning module to construct visual granules for causal inference-based recognition.

Result: Significantly outperforms state-of-the-art prompt learning methods on 15 datasets, especially on fine-grained datasets.

Conclusion: Visual granulation through causal inference enables more discriminative text prompts for fine-grained recognition tasks.
...
Method: Proposes an EGTM framework that extracts pixel-level reliable turbulence-free guidance from event streams using the 'event-lucky insight' - revealing correlation between turbulence distortions and inverse spatiotemporal distribution of event streams. Uses event cameras' microsecond-level temporal resolution for temporal lucky fusion.

Result: Significantly surpasses existing SOTA TM method by 710 times in model size, 214 times in inference latency, and 224 times in model complexity, while achieving state-of-the-art restoration quality (+0.94 PSNR and +0.08 SSIM) on real-world EGTM dataset.

Conclusion: Event cameras provide great efficiency merit for turbulence mitigation tasks, with the proposed framework demonstrating superior performance and efficiency compared to traditional frame-based approaches.
...
Conclusion: FocusMamba achieves better balance between accuracy and efficiency through adaptive collaborative sparsification and effective fusion of multimodal features, outperforming current RGB-Event detection approaches.
...
Result: Achieves state-of-the-art results on proposed benchmarks CZSFood-90 and CZSFood-164, and performs well on general CZSL datasets.

Conclusion: The proposed method effectively addresses key challenges in zero-shot food recognition and demonstrates superior performance on both food-specific and general compositional zero-shot learning benchmarks.
...
Method: In-depth survey analyzing the field across three modalities (vision, text, audio) and detailing five key phases: input, motion planning, motion video generation, refinement, and output.

Result: Provides thorough overview of latest developments, technological trends, and milestone works in human motion video generation.
...
Conclusion: The proposed approach effectively establishes dense functional correspondence by combining vision-language pseudo-labeling with contrastive learning, demonstrating superior performance over existing methods.
...
Conclusion: Attn-Adapter provides an effective online few-shot learning solution that overcomes limitations of traditional prompt learning approaches, enabling dynamic adaptation without base model retraining.
...
<details>
  <summary>Details</summary>
Motivation: Existing metrics for long image captions have problems: n-gram metrics lack semantic understanding, RS metrics have low human correlation, and LLM-based metrics are too expensive for iterative development.
...
Method: Developed ChexGen using latent diffusion transformer architecture, pretrained on 960,000 chest X-ray and report pairs - the largest curated dataset to date. Supports text, mask, and bounding box-guided synthesis.

Result: Achieves accurate radiograph synthesis validated by expert evaluations and quantitative metrics. Enables data augmentation and supervised pretraining that improves disease classification, detection, and segmentation with minimal training data. Creates diverse patient cohorts to detect and mitigate demographic biases.

Conclusion: ChexGen demonstrates the transformative role of generative foundation models in building more accurate, data-efficient, and equitable medical AI systems.
...
Method: Proposes feature-based inter-view motion vector prediction using decoded independent-view motion features with inter-view motion entropy model. Also develops disparity-free inter-view context prediction from independent-view content features with inter-view contextual entropy model.
...
Conclusion: Establishes a strong baseline for future research in learned multiview video coding, demonstrating significant compression efficiency improvements over traditional standards.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D tubular structure reconstruction rely on voxel-wise overlap measures that fail to capture topological correctness and completeness. Patch-wise approaches with topology-aware losses cannot guarantee global preservation or correct geometric errors during inference.

Method: TopoSculpt adopts (i) holistic whole-region modeling to capture full spatial context, (ii) introduces Topological Integrity Betti (TIB) constraint that jointly enforces Betti number priors and global integrity, and (iii) employs curriculum refinement with persistent homology to progressively correct errors from coarse to fine scales.

Result: Extensive experiments on pulmonary airway and Circle of Willis datasets show substantial improvements: β₀ errors reduced from 69.00 to 3.40 on airway dataset and from 1.65 to 0.30 on CoW dataset, with Tree length detected and branch detected rates improving by nearly 10%.

Conclusion: TopoSculpt effectively corrects critical topological errors and advances high-fidelity modeling of complex 3D tubular anatomy, demonstrating significant improvements in both geometry and topology reconstruction for medical applications.
...
Method: U-Net architecture with EfficientNet-B4 encoder, trained on SIIM-ACR dataset with data augmentation and combined binary cross-entropy plus Dice loss function.

Result: Achieved IoU of 0.7008 and Dice score of 0.8241 on independent PTX-498 test dataset, demonstrating accurate localization of pneumothorax regions.

Conclusion: The proposed deep learning pipeline can accurately segment pneumothorax regions in chest X-rays and has potential to support radiologists in clinical diagnosis.
...
Method: Leverage MLLMs to identify OOD samples, generate expressive negative sentences for far-OOD detection, and create visually similar negative labels for near-OOD scenarios. Use adaptive weighted score to balance both approaches.

Result: On ImageNet benchmark, reduces FPR95 by 4.2%, establishing new state-of-the-art performance.

Conclusion: ANTS provides training-free, zero-shot solution that effectively handles both near and far-OOD detection with high adaptability and scalability.
...
<details>
  <summary>Details</summary>
Motivation: Current WSI classification methods face challenges with spatial relationship encoding, length extrapolation issues between training/testing sequences, and computational complexity of Transformer-based approaches.

Method: Uses positional encoding module to encode spatial coordinates instead of sequence indices, and SAC block (MLP-based) for full instance correlations with linear time complexity, avoiding custom CUDA kernels.

Result: Achieved state-of-the-art performance on CAMELYON-16, TCGA-LUNG, and TCGA-BRAC datasets.

Conclusion: SAC-MIL provides an efficient, easy-to-deploy solution for WSI classification that handles spatial relationships effectively while maintaining computational efficiency.
...
Result: Auxiliary data improves vessel segmentation accuracy even when not available during inference. Benefits are most significant when few annotations are available for training. Validation on brain tumor segmentation confirms cross-domain applicability.
...
<details>
  <summary>Details</summary>
Motivation: Prompt design for Large Multimodal Models in Multiple-Choice Question Answering remains poorly understood, and minor prompt variations cause significant performance fluctuations, challenging fair model evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D semantic occupancy methods overlook height-axis information when processing voxel features, and conventional channel attention assigns uniform weights across all height layers, limiting their ability to capture semantic variations along the vertical axis.

Method: Proposes SliceSemOcc with vertical slice-based multimodal framework: extracts voxel features using global and local vertical slices, employs global-local fusion module to reconcile spatial details with contextual information, and introduces SEAttention3D module that preserves height-wise resolution and assigns dynamic channel attention weights to each height layer.

Result: Extensive experiments on nuScenes-SurroundOcc and nuScenes-OpenOccupancy datasets show significant improvements in mean IoU, with especially pronounced gains on most small-object categories.

Conclusion: The proposed SliceSemOcc framework effectively addresses height-axis information processing limitations and significantly enhances 3D semantic occupancy prediction performance, particularly for small objects.
...
Result: Method successfully identifies spurious correlations; training methodology significantly impacts model's reliance on spurious signals; certain ImageNet classes contain easily detectable spurious signals; provides exhaustive list of problematic images.

Conclusion: Findings highlight importance of detecting spurious correlations for building reliable models, calls for caution in using identified problematic images, and demonstrates real-world relevance through breast mass classification case study.
...
<details>
  <summary>Details</summary>
Motivation: Addresses real-world applications where only majority class labels are available for bags of instances, such as pathology imaging, political voting, sentiment analysis, and environmental monitoring.

Method: Uses a Counting Network to estimate bag-level majority labels by counting instances per class, and a Majority Proportion Enhancement Module (MPEM) that removes minority instances to increase majority class proportion.

Result: Demonstrates superiority over conventional MIL methods on four datasets, with ablation studies confirming effectiveness of each proposed module.

Conclusion: LML is a valuable problem with practical applications, and the proposed Counting Network with MPEM effectively solves it by enhancing learning through majority proportion optimization.
...
Result: Achieves stable single-frame processing delay of 50-100 ms for 1920*1080 video streams with multi-scale target recognition accuracy over 98.5%.

Conclusion: Provides an innovative solution for UAV monitoring with domestic chips, offering both low latency and high precision performance.
...
<details>
  <summary>Details</summary>
Motivation: Previous re-ranking methods rely on single-view images which cause view bias issues like pose variation, viewpoint changes, and occlusions. Multi-view features can help reduce this bias.

Method: K-nearest Weighted Fusion (KWF) method that aggregates neighbors' features in an unsupervised manner to generate multi-view features, with exploration of weight selection strategies during feature aggregation.

Result: Significant improvements in Rank@1 and mAP: 9.8%/22.0% improvement on MSMT17 and Occluded-DukeMTMC datasets respectively, with substantial computational efficiency gains compared to other re-ranking methods.

Conclusion: The proposed unsupervised re-ranking approach effectively reduces view bias by leveraging multi-view features from neighbors, achieving state-of-the-art performance while being computationally efficient and applicable to large-scale datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing AVVP methods either treat noisy segment-level pseudo labels as reliable supervision or spread indiscriminate attention across all frames, leading to repeated error amplification during training.

Method: Proposes BiT module for semantic injection and dynamic calibration of audio/visual features to locate cleaner semantic cues, and CATS module for semantic propagation and connection across time.

Result: Achieves state-of-the-art performance on multiple key indicators across two benchmark datasets (LLP and UnAV-100).

Conclusion: The proposed method successfully integrates strengths of previous research directions to overcome error amplification issues in weakly supervised AVVP tasks.
...
<details>
  <summary>Details</summary>
Motivation: Advanced Driver Assistance Systems require real-time perception models that can process multiple tasks efficiently while maintaining safety and effectiveness in real-world driving environments.

Method: The study introduces TriLiteNet, a lightweight multi-task model designed to optimize performance while minimizing computational costs. It includes both base and tiny configurations with different parameter sizes.

Result: On BDD100k dataset, TriLiteNet_base achieved 85.6% recall for vehicle detection, 92.4% mIoU for drivable area segmentation, and 82.3% accuracy for lane line segmentation with only 2.35M parameters and 7.72 GFLOPs. The tiny version has just 0.14M parameters with low latency and reasonable power consumption on embedded devices.

Conclusion: TriLiteNet provides a practical and deployable solution for autonomous driving applications by balancing performance, computational efficiency, and scalability, making it suitable for real-world deployment on resource-constrained embedded systems.
...
<details>
  <summary>Details</summary>
Motivation: To address pedestrian safety by providing event-based data for detecting pedestrians and analyzing crossing intentions, particularly in adverse weather conditions where traditional cameras struggle.

Method: Created dataset with two sources: synthetic event streams from CARLA simulator with controlled scenes, and real-world JAAD dash-cam videos converted to event streams using v2e tool. Includes paired RGB frames, event frames, and frame-level labels.

Result: Provides comprehensive dataset with raw event files and metadata. Baseline SNNs demonstrate dataset usability and reveal sim-to-real gap, highlighting need for domain adaptation and multimodal fusion.

Conclusion: DVS-PedX aims to accelerate research in event-based pedestrian safety, intention prediction, and neuromorphic perception by providing a valuable dataset for the community.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with character consistency, leading to artifact generation and inaccurate dialogue rendering, which results in disjointed storytelling.

Method: Uses pre-trained LLM for per-frame descriptions, character details, and dialogues via in-context learning. Implements bounded attention-based per-box mask technique, identity-consistent self-attention for character consistency, region-aware cross-attention for object placement, and CLIPSeg for dialogue assignment.

Result: Experimental results demonstrate that TaleDiffusion outperforms existing methods in consistency, noise reduction, and dialogue rendering.

Conclusion: The proposed framework effectively addresses the challenges of multi-character story visualization by maintaining character consistency and accurate dialogue assignment through innovative attention mechanisms and postprocessing techniques.
...
Method: Two-component framework: 1) Position-Style-Aware module with fine-tuned LLM for prompt decomposition, 2) Multi-Expert Diffusion module with attention-based gating to route specialized models (realism, stylization experts) to different spatial regions.

Result: MEPG significantly outperforms baseline models with the same backbone in both image quality and style diversity.

Conclusion: The framework provides strong extensibility through lightweight expert integration and supports real-time spatial layout editing with interactive style selection from expert portfolios.
...
Conclusion: Simple vision backbone approaches with proper tuning can achieve commercial-level performance, offering practical deepfake detection solutions with good accuracy-computational cost tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional image annotation tools require extensive manual input, limiting scalability for large datasets. AI models need annotated data but the annotation process is labor-intensive.

Method: Integrates state-of-the-art foundation models (CLIP, Ultralytics detectors, Grounding DINO) with a filtering pipeline. Uses low-confidence thresholding for initial annotations, interactive refinement tools, and WebGPU-accelerated segmentation with Segment Anything.
...
Conclusion: VisioFirm provides an efficient hybrid approach to image annotation that significantly reduces human effort through AI automation while offering flexible refinement tools and export options.
...
<details>
  <summary>Details</summary>
Motivation: Existing UCIR methods struggle with domain gaps because object features are often entangled with domain-specific styles, making cross-domain retrieval challenging without annotations.
...
Conclusion: The proposed feature disentanglement approach effectively addresses domain gap challenges in unsupervised cross-domain image retrieval, demonstrating superior performance over existing methods.
...
Conclusion: Multi-scale shape features from DMP provide critical information that enhances semantic segmentation performance in remote sensing imagery, with hybrid fusion approaches showing superior results compared to direct input methods.
...
<details>
  <summary>Details</summary>
Motivation: Tau PET scans are crucial for Alzheimer's diagnosis but are expensive and limited in availability, while MRI and plasma biomarkers are more accessible alternatives.

Method: A 3D diffusion model that uses textual prompts from plasma p-tau217 measurements and anatomical constraints from structural MRI to synthesize tau PET images.

Result: The approach successfully generates realistic 3D tau PET images across various disease stages using ADNI clinical data.

Conclusion: This framework offers a non-invasive, cost-effective solution for tau PET data augmentation and disease progression simulation using widely available biomarkers.
...
<details>
  <summary>Details</summary>
Motivation: Existing semi-supervised medical image segmentation networks overlook effective methodological guidance for feature extraction and important prior information from datasets.
...
Conclusion: The method successfully learns articulated object representations from minimal supervision (4 views per articulation, no camera poses), demonstrating robustness and effectiveness under challenging input conditions.
...
Method: Proposes leveraging synthetic images as reliable reference points to identify and correct mislabeled samples, rather than using them directly for training.

Result: Achieves 30% improvement on CIFAR-10, 11% on CIFAR-100 under 70% semantic noise, and 24% on ImageNet-100 under real-world noise conditions.

Conclusion: The method effectively addresses semantic label noise and is orthogonal to existing noise-robust techniques, enabling superior performance when combined with state-of-the-art methods.
...
Result: Maintains competitive performance while achieving substantial efficiency gains. Experimental evaluation also reveals current limitations of Multimodal Large Language Models in structured visual reasoning tasks.

Conclusion: The proposed efficient DINO-based approach successfully addresses the computational challenges of odd-one-out anomaly detection while maintaining performance, and highlights areas for improvement in MLLMs for visual reasoning tasks.
...
Method: Developed GeoArena platform that allows users to upload in-the-wild images and uses pairwise human judgments to evaluate which model output better aligns with human expectations.

Result: Deployed online for two months, collected thousands of voting records, conducted detailed analysis, and established a leaderboard for different LVLMs on image geolocalization.

Conclusion: GeoArena provides a more accurate and human-centered benchmarking approach for evaluating large vision-language models on worldwide image geolocalization tasks.
...
TL;DR: FE2E framework adapts image editing models (not text-to-image generators) for dense geometry estimation, achieving superior performance through consistent velocity training and leveraging Diffusion Transformer architecture for joint depth and normal estimation.


<details>
  <summary>Details</summary>
Motivation: Dense prediction is inherently an image-to-image task, suggesting image editing models may be more suitable than text-to-image generative models as foundation for fine-tuning. Editing models possess inherent structural priors that enable more stable convergence.

Method: FE2E adapts advanced editing model based on Diffusion Transformer architecture. Uses consistent velocity training objective instead of flow matching loss, logarithmic quantization for precision, and leverages DiT's global attention for joint depth and normal estimation in single forward pass.

Result: Achieves over 35% performance gains on ETH3D dataset, outperforms DepthAnything series trained on 100x more data. Impressive improvements in zero-shot monocular depth and normal estimation across multiple datasets without scaling training data.

Conclusion: Image editing models are superior to generative models for dense geometry prediction tasks due to their inherent structural priors, and FE2E demonstrates this through state-of-the-art performance with efficient architecture adaptations.
...
Conclusion: MICACL effectively addresses key challenges in DFER by integrating spatio-temporal modeling and long-tailed optimization, demonstrating significant improvements over existing methods in real-world scenarios.
...
TL;DR: Computer vision pipeline transforms body-camera footage into panoramic images for rapid situational awareness by using SLAM for camera tracking, clustering key viewpoints, and stitching representative frames.


<details>
  <summary>Details</summary>
Motivation: First responders need quick visual summaries of incident scenes from body-camera footage since reviewing lengthy videos is impractical in time-critical situations.

Method: Uses monocular SLAM to estimate camera trajectories and reconstruct environment layout, clusters camera poses to identify key viewpoints, selects representative frames, and fuses them into panoramic images using multi-frame stitching.
...
Conclusion: The pipeline effectively transforms raw body-camera footage into concise visual summaries that support quick interpretation and incident review for first responders.
...
TL;DR: AnomalyLMM is the first framework using Large Multi-modal Models for text-based person anomaly search, achieving state-of-the-art performance on the PAB dataset with a training-free approach.


<details>
  <summary>Details</summary>
Motivation: Address the challenges of fine-grained cross-modal alignment between textual anomalies and visual behaviors, and anomaly recognition under sparse real-world samples, which are not adequately handled by conventional methods.
...
Conclusion: AnomalyLMM successfully bridges the domain gap between generative knowledge and discriminative retrieval, demonstrating LMMs' potential for fine-grained anomaly search tasks with effective zero-shot adaptation strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing AIC works using MLLMs don't specifically adapt models to focus on aesthetic content and rely on fine-tuning without explicit aesthetic saliency integration.

Method: Proposes ASE-MLLM with Image Aesthetic Saliency Module (IASM) to extract aesthetic saliency features and IAS-ViT encoder that fuses aesthetic features with original image features via cross-attention mechanism.
...
Conclusion: ASE-MLLM is the first framework to successfully integrate image aesthetic saliency into MLLMs specifically for AIC tasks, demonstrating the importance of explicit aesthetic feature extraction and fusion.
...
TL;DR: A novel 3D style transfer pipeline that uses 2D diffusion priors to generate stylized key views with cross-view attention and instance-level consistency, achieving superior results compared to state-of-the-art methods.
...
Conclusion: The proposed pipeline effectively integrates 2D diffusion priors with innovative cross-view style alignment and instance-level transfer techniques to overcome limitations of existing 3D style transfer methods.
...
<details>
  <summary>Details</summary>
Motivation: X-ray ptychography's nanoscale resolution is limited by the challenge of accurate image reconstruction with unknown illuminating probes, especially under low-signal conditions in low-dose and high-speed experiments.

Method: PtyINR parameterizes both object and probe as continuous neural representations, performing end-to-end reconstruction directly from raw diffraction patterns without probe pre-characterization.

Result: Extensive evaluations show superior reconstruction quality on both simulated and experimental data, with remarkable robustness under challenging low-signal conditions.

Conclusion: PtyINR provides a generalizable, physics-informed framework for probe-dependent inverse problems, making it applicable to a wide range of computational microscopy problems.
...
TL;DR: Proposes image-oriented self-adaptive dataset construction method for real-world multimodal safety scenarios, generating 35K image-text pairs with guidance responses, and introduces standardized safety evaluation metric using fine-tuned safety judge model.


<details>
  <summary>Details</summary>
Motivation: Current risk-oriented dataset construction methods fail to cover the growing complexity of real-world multimodal safety scenarios, and lack unified evaluation metrics to prove overall effectiveness.

Method: Image-oriented self-adaptive dataset construction starting with images and constructing paired text and guidance responses. Fine-tunes a safety judge model for standardized evaluation across safety datasets.

Result: Generated RMS dataset with 35k image-text pairs. Extensive experiments demonstrate effectiveness of the image-oriented pipeline, confirming scalability and effectiveness of the approach.
...
TL;DR: MDT-dist framework accelerates 3D flow generation by distilling pretrained models to 1-2 sampling steps while maintaining quality, achieving 6.5-9x speedup.
...
Method: Proposes MDT-dist framework with two optimizable objectives: Velocity Matching (VM) to match velocity fields between student and teacher, and Velocity Distillation (VD) to enhance optimization using learned velocity fields for probability density distillation.
...
TL;DR: Durian is a zero-shot method for portrait animation with facial attribute transfer using dual reference networks and diffusion models.


<details>
  <summary>Details</summary>
Motivation: To enable high-fidelity, spatially consistent facial attribute transfer from reference images to target portraits in videos without explicit supervision.
...
Result: Achieves state-of-the-art performance on portrait animation with attribute transfer and enables multi-attribute composition in single generation pass.
...
TL;DR: Deep learning-based segmentation with differentiable Hough transform for robust collimation shadow detection in X-ray imaging, achieving median Hausdorff distances of 4.3-5.0mm.


<details>
  <summary>Details</summary>
Motivation: Collimation reduces radiation dose by restricting exposure to ROI, but detecting collimator shadows is challenging when edges are obscured by scattered radiation. Prior knowledge of polygonal shadow shapes can be leveraged.

Method: Differentiable Hough transform-based network that detects collimation borders and extracts ROI center information. Combines both tasks during inference to generate line-constrained segmentation masks.
...
Conclusion: The method effectively leverages geometric constraints of collimation shadows and is not fundamentally limited by the number of edges, demonstrating strong performance in challenging X-ray imaging conditions.
...
TL;DR: The paper introduces UCF-UM, a cyclic evaluation framework that measures semantic drift when alternating between image-to-text and text-to-image generation in unified visual language models, revealing significant variations in cross-modal consistency that standard single-pass metrics miss.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for unified visual language models only assess image-to-text and text-to-image capabilities in isolation, failing to measure whether models preserve semantic meaning when cycling between modalities or whether understanding translates to accurate generation.

Method: The authors developed UCF-UM, a cyclic evaluation protocol that alternates I2T and T2I over multiple generations, with three metrics: Mean Cumulative Drift (embedding-based semantic loss), Semantic Drift Rate (decay rate), and Multi-Generation GenEval (object-level compliance). They created ND400 benchmark from NoCaps and DOCCI datasets to assess generalization beyond COCO.

Result: Evaluation of seven recent models revealed substantial variation in cross-modal stability - some models like BAGEL maintained semantics over many alternations, while others like Vila-u drifted quickly despite strong single-pass scores, showing that cyclic consistency differs significantly from isolated capability assessments.

Conclusion: Cyclic consistency evaluation is a necessary complement to standard I2T and T2I metrics, providing practical measures to assess unified models' cross-modal stability and the strength of their shared representations, with UCF-UM offering a comprehensive framework for this purpose.
...
<details>
  <summary>Details</summary>
Motivation: Growing demand for spatial intelligence and holistic scene perception in applications like VR, autonomous driving, and robotics, coupled with the unique challenges of adapting perspective image methods to omnidirectional images due to geometric differences.

Method: Systematic review of panoramic imaging pipeline and projection methods, analysis of three main domain adaptation challenges (geometric distortions, non-uniform sampling, boundary continuity), and cross-method/task analysis across four major categories of panoramic vision tasks.

Result: Comprehensive survey covering 20+ representative tasks from over 300 research papers, providing structured analysis of adaptation strategies and classification of panoramic vision into four categories: quality enhancement/assessment, understanding, multimodal understanding, and generation.
...
Method: Zero-shot framework that enables consistent story generation and provides fine-grained control over story visualizations at various levels of detail.

Result: Not specified in the abstract - this appears to be an introduction of a new framework rather than reporting experimental results.

Conclusion: Plot'n Polish addresses the challenges of maintaining consistency and providing editing control in story visualization applications of text-to-image diffusion models.
...
Method: Introduces TRUST-VL with a Question-Aware Visual Amplifier module for task-specific visual features, trained on TRUST-Instruct dataset (198K samples) with structured reasoning chains aligned with human fact-checking workflows.
...
<details>
  <summary>Details</summary>
Motivation: To enable generation of arbitrarily long virtual try-on videos without resource-intensive processes or extensive video data requirements.
...
Result: Successfully generates minute-scale virtual try-on videos with local smoothness and global temporal consistency under various motions.

Conclusion: VFR is a pioneering framework for long virtual try-on video generation that addresses key challenges of segment coherence and overall consistency.
