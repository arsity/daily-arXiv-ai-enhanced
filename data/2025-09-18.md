...
Motivation: To create a more reliable and interpretable uncertainty-aware decision-making system that avoids confidently incorrect outcomes and provides transparent, auditable decisions through explicit supporting evidence.

Method: Retrieves proximal exemplars in embedding space for each test instance, fuses their predictive distributions using Dempster-Shafer theory, and uses the resulting fused belief as a per-instance adaptive thresholding mechanism.
...
Conclusion: Evidence-conditioned tagging provides a more reliable and interpretable alternative to fixed prediction entropy thresholds for operational uncertainty-aware decision-making, requiring only a few evidences to achieve substantial gains.
...
<details>
  <summary>Details</summary>
Motivation: To systematically compare hybrid quantum-classical neural networks with purely classical models to evaluate performance, efficiency, and robustness differences.

Method: Integrated parameterized quantum circuits with classical deep learning architectures and compared against conventional CNNs across three benchmark datasets over 50 training epochs, evaluating validation accuracy, test accuracy, training time, computational resources, and adversarial robustness.

Result: Hybrid models achieved higher accuracy (99.38% vs 98.21% on MNIST, 41.69% vs 32.25% on CIFAR100, 74.05% vs 63.76% on STL10), trained 5-12x faster, used 6-32% fewer parameters, consumed less memory, and showed better adversarial robustness on simpler datasets.

Conclusion: Hybrid quantum-classical architectures offer significant advantages in accuracy, training efficiency, and parameter scalability, particularly for complex vision tasks.
...
Method: Optimized YOLOv11 to YOLOv11-DIoU with DIoU Loss, improved DeepSort with fused Mahalanobis and cosine distances, used Greenberg model for traffic flow analysis, and built GRU-Attention model for congestion warning.

Result: YOLOv11-DIoU achieved 95.7% mAP (6.5% higher than baseline), DeepSort reached 93.8% MOTA (11.3% higher than SORT), GRU-Attention achieved 99.7% test accuracy (7-9% higher than traditional GRU), with 95% warning accuracy and ≤1 minute time error.

Conclusion: The framework provides quantitative support for expressway congestion control with promising intelligent transportation applications, demonstrating high accuracy in both perception and prediction tasks.
...
<details>
  <summary>Details</summary>
Motivation: To optimize real-time cloud-based on-street parking services by automating ground truth testing processes and reducing human engineering workload in analyzing crowd-sourced ultrasonic sensor data.

Method: Applied machine learning methods, particularly convolutional neural networks for image pattern recognition, to automate the analysis process and enrich the database by substituting human engineering work.

Result: Achieved a 99.58% reduction in human resource time through the automated analysis system, with predefined metrics demonstrating high performance levels in parking spot classification.

Conclusion: The research successfully automated the parking service testing process using CNN-based image recognition, significantly reducing human intervention while maintaining service quality, with potential for future development and broader application of the automation tool.
...
<details>
  <summary>Details</summary>
Motivation: Despite VLMs like CLIP showing strong zero-shot OOD detection capabilities, there's limited understanding of why they work so well, their advantages over single-modal approaches, and their behavioral robustness.

Method: The authors conducted a systematic empirical analysis using in-distribution and OOD prompts to characterize VLM embedding space properties, compare against single-modal methods, and test robustness to image noise and prompt variations.

Result: VLMs outperform single-modal methods due to rich semantic novelty utilization. They show resilience to image noise but are highly sensitive to prompt phrasing, revealing an asymmetric robustness profile.

Conclusion: The study provides structured understanding of VLM strengths and vulnerabilities in OOD detection, offering empirical guidance for developing more robust and reliable AI systems.
...
<details>
  <summary>Details</summary>
Motivation: To develop a quantitative measure for assessing the effectiveness of data representations from dimensionality reduction techniques and to explore the large-scale geometry of empirical networks.

Method: Utilizes abstract notions of sectional curvature to capture metric relations between triples of points and other points, constructing a curvature-based geometric profile of discrete metric spaces.

Result: The curvature-based analysis can be employed to estimate the intrinsic dimensionality of datasets and evaluate the effectiveness of dimensionality reduction techniques.

Conclusion: The introduced curvature profile provides a valuable quantitative tool for analyzing discrete metric spaces, assessing data representations, and exploring network geometry through intrinsic dimensionality estimation.
...
Method: Used supervised machine learning with training datasets, unsupervised k-means clustering on Google Earth Engine, and convolutional neural networks for land cover classification of Landsat-8 satellite imagery.

Result: Created visualizations of change detection showing urban area changes over time, providing monitoring capabilities for land cover transformations.

Conclusion: The study successfully developed machine learning frameworks for monitoring urbanization patterns in Fiji, offering valuable technical support for land use change detection and modeling.
...
Method: Three-stage approach: 1) YOLOv7 segmentation for object localization, 2) ConvNeXt-based feature extractor with triplet loss for discriminative embeddings, 3) Feature-assisted IoU tracker for resilient multi-object tracking under occlusion and motion. Optimized with mixed-precision inference for edge deployment.

Result: High accuracy and robustness demonstrated across diverse FOI scenarios on real-world surveillance and drone video datasets. Hardware benchmarks confirm practical performance on NVIDIA Jetson devices.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation protocols for instruction-based image editing either depend on limited paired reference images or rely on imprecise zero-shot VLMs, creating a bottleneck for reliable and interpretable assessment.

Method: The framework decomposes images into semantic objects, synthesizes diverse editing instructions, and integrates VLMs with open-vocabulary object detectors, semantic feature extractors, and human preference models for comprehensive evaluation.

Result: Combining VLMs with object detectors yields stronger agreement with human judgments compared to using VLMs alone or CLIP-based metrics. The framework successfully evaluates 11 state-of-the-art editing models across 9 instruction types.

Conclusion: EdiVal-Agent provides a scalable, fine-grained evaluation approach that can identify failure modes in current editing models and inform the development of next-generation editing systems through its modular design.
...
<details>
  <summary>Details</summary>
Motivation: To create a universal 3D reconstruction backbone that can handle diverse 3D vision tasks with a single feed-forward model, eliminating the need for specialized architectures for different tasks.

Method: Uses a factored representation of multi-view scene geometry (depth maps, local ray maps, camera poses, metric scale factor) with transformer architecture. Standardizes supervision across datasets and employs flexible input augmentation to handle various input configurations.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting produces massive data requiring compression, but existing methods use simple uniform quantization that may not be optimal.

Method: Scene-adaptive lattice vector quantization (SALVQ) with optimized lattice basis per scene and dynamic density scaling for multiple bit rates.

Result: Improved rate-distortion performance with minimal computational overhead and the ability to handle multiple compression levels with a single model.

Conclusion: SALVQ enhances existing 3DGS compression methods by providing better quantization efficiency while maintaining low complexity and flexibility.
...
Conclusion: MINGLE provides an effective approach for detecting social group regions from images, addressing the challenge of interpreting subtle interpersonal relations and supporting urban planning applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing bias discovery methods focus on output-level demographic distributions but fail to reveal deeper concept-level representational biases and entanglements between demographics and semantics in text-to-image models.
...
Result: Shows that existing fairness interventions reduce output distributional gaps but fail to disentangle concept-level coupling, while BiasMap successfully mitigates concept entanglement in image generation and complements distributional bias mitigation.

Conclusion: BiasMap provides a deeper lens into hidden representational biases and offers effective mitigation that addresses both concept-level entanglement and distributional bias in text-to-image models.
...
<details>
  <summary>Details</summary>
Motivation: Existing image annotation tools require pre-collected datasets, which limits real-time data acquisition from laboratory instruments and introduces unnecessary steps in scientific workflows where immediate annotation is needed.

Method: Developed a Python-based graphical interface that integrates with various imaging systems, featuring Bezier splines, binary masks, non-destructive layers, and optimized performance using OpenCV and Numpy for real-time object detection operations.

Result: LivePyxel provides seamless real-time annotation capabilities with wide device compatibility, enabling precise area delimitation and high-performance editing tools similar to commercial graphics software.

Conclusion: The tool accelerates AI model development in experimental workflows by facilitating immediate data collection and labeling directly from live imaging sources, making it freely available for scientific use.
...
<details>
  <summary>Details</summary>
Motivation: Real-world virtual try-on applications require limited training/inference budgets, but current methods involve extensive end-to-end training of large pre-trained models.

Method: Freezes pre-trained model parameters and trains a small h-transform network (1.42% parameters) for conditional generation. Adds adaptive consistency loss combining consistency training and denoising score matching for efficient fine-tuning.
...
Conclusion: DEFT-VTON provides an efficient solution for virtual try-on that significantly reduces training parameters and inference time while maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: Synthetic data is crucial for autonomous driving scenarios but introduces domain gaps between synthetic and real domains, particularly for VRU (vulnerable road users) recognition.

Method: Deploy data augmentation to generate custom traffic scenarios with VRUs, provide pipeline for Cityscapes dataset augmentation with virtual pedestrians, and develop novel generative network architecture for adversarial learning of lighting conditions.
...
Conclusion: The proposed method effectively bridges the domain gap between synthetic and real data for VRU recognition in autonomous driving applications.
...
<details>
  <summary>Details</summary>
Motivation: Traditional deep learning approaches for medical image processing lack interpretability, and existing Kolmogorov-Arnold networks disrupt spatial structure by using flattened feature representations, limiting their effectiveness for image data.

Method: Proposed Functional Kolmogorov-Arnold Network (FunKAN) that generalizes the Kolmogorov-Arnold theorem to functional spaces, learning inner functions using Fourier decomposition over Hermite functions. Also developed U-FunKAN for medical segmentation.

Result: Outperformed other KAN-based methods in medical image enhancement (PSNR, TV metrics on IXI dataset for Gibbs ringing suppression) and segmentation (IoU, F1 scores on BUSI, GlaS, and CVC-ClinicDB datasets for breast cancer, glands, and polyps detection).

Conclusion: FunKAN bridges theoretical function approximation with medical image analysis, providing a robust and interpretable solution that preserves spatial structure and achieves state-of-the-art performance in clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal classification methods treat all video content uniformly instead of emphasizing hateful components, and cannot systematically capture structured information in videos, limiting detection effectiveness.
...
Result: Extensive experiments show state-of-the-art performance in hateful video classification with strong explainability on public datasets.

Conclusion: The proposed model effectively addresses limitations of existing methods by focusing on hateful components and systematically modeling structured relationships, achieving superior hateful video detection.
...
Method: Uses diffusion-based depth estimation trained on synthetic colonoscopy sequences to learn robust geometric priors, combined with a style transfer technique to adapt real clinical videos to the synthetic training domain.
...
Conclusion: While full trajectory 3D reconstruction remains challenging, ColonCrafter demonstrates clinically relevant applications for depth estimation in colonoscopy with improved temporal consistency.
...
Conclusion: The proposed methods effectively address the memory constraints of embedded platforms while enhancing reconstruction quality, making 3DGS more suitable for resource-constrained devices like MAVs.
...
<details>
  <summary>Details</summary>
Motivation: Trajectory prediction models face distribution shifts in real-world deployment, with rare scenarios causing OOD cases. Current OOD detection research focuses on computer vision tasks, leaving trajectory-level detection underexplored.

Method: Builds on quickest change detection formulation with adaptive mechanisms that explicitly model mode-dependent error distributions that evolve over time with dataset-specific dynamics.

Result: Substantial improvements in both detection delay and false alarm rates. Significantly outperforms prior UQ- and vision-based OOD approaches in accuracy and computational efficiency.

Conclusion: Provides a practical path toward reliable, driving-aware autonomy by offering robust OOD detection for trajectory prediction in complex driving environments.
...
<details>
  <summary>Details</summary>
Motivation: Deforestation in the Amazon has significant global impacts on climate and biodiversity, requiring effective monitoring tools.

Method: Uses deep learning to compare satellite image pairs from different dates, combined with visual semantic model that extracts annotations from scientific documents about the Amazon region.

Result: Effective deforestation detection and relevant annotation generation demonstrated on Amazon image pair dataset.

Conclusion: Provides a useful monitoring tool for studying deforestation impacts, with potential applications beyond environmental domains.
...
Method: Integrates Google Gemini 2.5 Flash with visual feature extraction and NLP for contextual image interpretation, using coordinate verification, probabilistic Gaussian modeling for anomaly distribution, multi-layered visualization techniques, and precise prompt engineering for structured clinical information extraction.

Result: High performance in anomaly detection across multiple modalities with 80 pixels average deviation in location measurement, featuring a user-friendly Gradio interface and demonstrating zero-shot learning capabilities to reduce dataset dependence.

Conclusion: The framework represents significant advancement in automated diagnostic support and workflow efficiency, though clinical validation and multi-center evaluation are needed before widespread adoption.
...
<details>
  <summary>Details</summary>
Motivation: To generalize the CLAP algorithm beyond 2D localization applications and demonstrate its broader applicability to handle noise and uncertainty in various fields.

Method: Extended the clustering-based CLAP framework from 2D to 3D localization and image stitching, while establishing theoretical relationships with RANSAC and Hough transform methods.

Result: Successfully generalized CLAP to handle 3D localization and image stitching problems, demonstrating its versatility beyond the original 2D application.

Conclusion: CLAP provides a robust alternative to traditional outlier rejection methods and has wide applicability across different domains for dealing with noise and uncertainty through clustering-based approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional weakly supervised registration methods require anatomical priors like segmentation masks or landmarks, which are often unavailable in practice. The authors aim to leverage the strong representation learning capabilities of visual foundation models to overcome this limitation.

Method: The framework uses SAM's pretrained image encoder to extract structure-aware feature embeddings instead of raw images. It includes a lightweight 3D head for feature refinement and a Hierarchical Feature Consistency Loss for coarse-to-fine feature matching and anatomical alignment.

Result: SAMIR significantly outperforms state-of-the-art methods, achieving performance improvements of 2.68% on ACDC cardiac dataset and 6.44% on abdomen CT dataset.

Conclusion: The proposed SAMIR framework demonstrates that leveraging visual foundation models like SAM can effectively enhance medical image registration without requiring additional anatomical labels, making it more practical for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Accurate deforestation identification from satellite images is essential for geographical understanding, but centralized training methods compromise data security by requiring data combination from different clients.

Method: Uses Federated Learning with FLOWER and RAY frameworks to enable distributed training across edge satellite centers. Employs YOLOS-small (Vision Transformer), Faster R-CNN with ResNet50, and Faster R-CNN with MobileNetV3 models on public datasets.

Result: The framework provides efficient client spawning and distributed learning workload execution while maintaining data privacy and security for satellite image processing centers.

Conclusion: This federated learning approach offers a novel perspective for image segmentation tasks on satellite imagery, enabling collaborative model training without compromising client data security.
...
<details>
  <summary>Details</summary>
Motivation: Conventional two-view pose estimation methods are not metric (scale ambiguity) and struggle with wide baselines, textureless surfaces, and reflective surfaces.

Method: Uses metric monocular depth estimation and Gaussian scene reconstruction to create metric 3D GMMs for each image, then optimizes a differentiable GMM alignment objective considering geometry, color, covariance, and semantic features.

Result: Outperforms both classical and state-of-the-art learning-based methods including MASt3R on Real-Estate10K dataset.

Conclusion: Bridging single-view perception with multi-view geometry enables robust metric relative pose estimation without requiring explicit 2D correspondences.
...
Result: The lookup networks achieve higher efficiency in terms of energy consumption and inference speed while maintaining competitive performance compared to vanilla convolutional networks. Extensive experiments show state-of-the-art performance on both classification and regression tasks across different data types.

Conclusion: Lookup operations provide an effective alternative to multiplication operations in neural networks, enabling efficient deployment on resource-constrained devices while maintaining performance across various computer vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional patch-wise visual projectors struggle to balance token reduction and semantic clarity, often keeping overly long token sequences to avoid performance drops in Referring Image Segmentation.

Method: Proposes semantic visual projector using SAM-generated superpixels as visual words, with semantic superpixel positional embedding for geometry awareness and semantic superpixel aggregator to preserve details and global context.
...
Conclusion: The semantic visual projector approach effectively addresses visual token redundancy in MLLM-SAM frameworks for RIS, providing efficient compression while preserving semantic information through superpixel-based visual words.
...
<details>
  <summary>Details</summary>
Motivation: Existing BEV segmentation methods work well with pinhole cameras but struggle with fisheye cameras due to severe geometric distortion, ambiguous multi-view correspondences, and unstable temporal dynamics, which degrade performance.

Method: Proposes three complementary innovations: 1) Distortion-Resilient Multi-scale Extraction backbone for robust features under distortion, 2) Uncertainty-aware Spatial Cross-Attention for reliable cross-view alignment, and 3) Distance-aware Temporal Self-Attention for balancing near/far field details and ensuring temporal coherence.
...
Conclusion: FishBEV provides an effective solution for fisheye camera BEV segmentation by specifically addressing the unique challenges of distortion, multi-view alignment, and temporal dynamics that degrade performance in fisheye camera systems.
...
<details>
  <summary>Details</summary>
Motivation: To develop accurate and interpretable medical image classification models suitable for resource-limited clinical settings with limited and diverse datasets.

Method: Three spline-based Kolmogorov-Arnold Networks: SBTAYLOR-KAN (B-splines + Taylor series), SBRBF-KAN (B-splines + Radial Basis Functions), and SBWAVELET-KAN (B-splines + Morlet wavelet transforms) that capture local and global nonlinearities without preprocessing.

Result: SBTAYLOR-KAN achieved up to 98.93% accuracy, maintained over 86% accuracy with only 30% training data, and outperformed other models with just 2,872 parameters vs. ResNet50's 24.18M parameters.

Conclusion: The framework provides a lightweight, interpretable, and generalizable solution for medical image classification in data-scarce clinical environments, addressing challenges of limited datasets and resource constraints.
...
<details>
  <summary>Details</summary>
Motivation: The rapid advancement of diffusion models enables malicious actors to easily replicate artists' unique styles, threatening their creative labor and intellectual property, creating an urgent need for style protection methods.

Method: The method identifies cross-attention layers sensitive to artistic styles through activation analysis, then introduces StyleProtect which selectively updates only these sensitive layers to defend against style mimicry while maintaining imperceptibility.

Result: Experiments on WikiArt-based dataset (30 artists) and Anita cartoon dataset show promising performance in safeguarding unique artistic styles from malicious diffusion customization with competitive imperceptibility.

Conclusion: StyleProtect provides an efficient and lightweight solution for protecting artworks against style mimicry by diffusion models, effectively defending distinctive artistic styles while maintaining visual quality.
...
<details>
  <summary>Details</summary>
Motivation: Self-supervised monocular depth estimation methods struggle with uncertainty in input data, particularly in low-texture regions and dynamic object boundaries, which reduces depth accuracy. Existing motion-aware approaches often require additional labels, auxiliary networks, or incur inference-time costs.

Method: Proposes UM-Depth framework with motion- and uncertainty-aware refinement using a teacher-student training strategy. Embeds uncertainty estimation into both training pipeline and network architecture. Uses optical flow exclusively in teacher network during training to avoid runtime costs and additional labeling requirements.
...
Conclusion: The framework successfully enhances depth accuracy in challenging regions without inference-time overhead or additional labeling demands, making it suitable for real-world applications like robotics and autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Existing query-based RVOS methods suffer from query selection bias where static textual queries are easily misled by distractors with similar appearance or motion, leading to inaccurate object segmentation.

Method: Proposes Triple Query Former (TQF) that factorizes queries into three components: appearance query, intra-frame interaction query, and inter-frame motion query. Uses dynamic query construction integrating linguistic and visual cues, plus two motion-aware aggregation modules for intra-frame and inter-frame processing.

Result: Extensive experiments on multiple RVOS benchmarks demonstrate advantages of TQF and effectiveness of the structured query design and motion-aware aggregation modules.

Conclusion: The proposed TQF framework with specialized query components and motion-aware aggregation effectively addresses query selection bias in RVOS, achieving improved performance through better cross-modal alignment and temporal coherence.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat GREC and GRES independently, missing the benefits of joint training for consistent predictions. Current GRES methods treat it as semantic segmentation without instance-awareness and lack consistency between instance-level boxes and masks.

Method: Proposes InstanceVG framework that uses instance queries with prior reference points to unify joint predictions of instance-level boxes and masks. Each query has a reference point that also aids target matching, enabling consistent point-box-mask predictions for the same instance.

Result: Extensive experiments on ten datasets across four tasks show state-of-the-art performance, significantly surpassing existing methods in various evaluation metrics.

Conclusion: InstanceVG is the first framework to simultaneously tackle both GREC and GRES with instance-aware capabilities, demonstrating superior performance through unified multi-granularity predictions.
...
Result: Achieves state-of-the-art performance on three public datasets among all global matching methods.

Conclusion: FMFA effectively addresses cross-modal alignment challenges through full-mode fine-grained alignment, improving both global matching and local feature verification.
...
Result: Experimental results show excellent performance in color continuity and controllability, enabling users to specify target RGB ranges for continuous color variations.

Conclusion: The proposed method successfully addresses color editing challenges by providing finer-grained, continuous, and controllable color manipulation while maintaining semantic consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing safety methods rely only on LLMs to refine text prompts, ignoring the actual generated images which can lead to unsafe outputs or unnecessary changes to safe prompts.

Method: Iterative prompt refinement algorithm that uses Vision Language Models (VLMs) to analyze both input prompts and generated images, leveraging visual feedback. Also introduces a new dataset with textual and visual safety labels using multi-modal LLM for supervised fine-tuning.
...
Conclusion: Offers a practical solution for generating safer T2I content by incorporating visual feedback into the prompt refinement process.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAW data processing methods face limitations: large-scale ISP networks have heavy computational overhead, while traditional ISP tuning methods have limited representational capacity. The goal is to leverage RAW data's rich information for visual perception tasks without excessive computational costs.

Method: Proposes Task-Aware Image Signal Processing (TA-ISP) that predicts a small set of lightweight, multi-scale modulation operators acting at global, regional, and pixel scales to reshape image statistics across different spatial extents, replacing heavy dense convolutional pipelines.

Result: TA-ISP consistently improves downstream accuracy on RAW-domain detection and segmentation benchmarks under both daytime and nighttime conditions while significantly reducing parameter count and inference time.

Conclusion: TA-ISP provides an efficient solution for processing RAW data for visual perception tasks, making it suitable for deployment on resource-constrained devices by expanding representational capacity while keeping computational requirements low.
...
<details>
  <summary>Details</summary>
Motivation: Existing deraining techniques perform poorly in nighttime conditions due to spatial heterogeneity of rain distribution and light-dependent stripe visibility, which hampers surveillance and autonomous navigation systems.

Method: Proposed NDLPNet with Position Perception Module (PPM) to capture spatial contextual information and recalibrate feature channel importance. Also created a new Night Scene Rainy (NSR) dataset with 900 real-world nighttime image pairs.
...
Conclusion: The proposed network effectively addresses nighttime deraining challenges by capturing spatial positional information and rain density distribution, providing superior performance while preserving crucial background information.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely primarily on visual cues for rtMRI segmentation, but synchronized acoustic and phonological signals provide complementary context that can enrich visual information and improve precision.

Method: Multimodal framework integrating video, audio, and phonological inputs through cross-attention fusion for dynamic feature alignment, with contrastive learning objective to enhance cross-modal representation even when audio is unavailable at inference.

Result: Achieves state-of-the-art performance on USC-75 rtMRI dataset with Dice score of 0.95 and 95th percentile Hausdorff Distance of 4.20 mm, outperforming both unimodal and multimodal baselines.

Conclusion: Integrative multimodal modeling with cross-attention and contrastive learning significantly improves segmentation precision and robustness for vocal tract analysis.
...
<details>
  <summary>Details</summary>
Motivation: Traditional codecs and learned methods struggle with perceptual quality at high compression ratios, while existing generative approaches face visual fidelity and generalization challenges with mixed natural/AI-generated content.

Method: Uses pre-optimized encoder for compressed-domain representations, integrated with pretrained diffusion models via lightweight adapter and attentive fusion module, plus distribution renormalization for enhanced fidelity.

Result: Outperforms existing methods in visual fidelity at low bitrates, achieves 79% compression improvement over H.266/VVC, and efficiently handles AI-generated content while adapting to broader content types.

Conclusion: The proposed diffusion-based generative coding framework provides an efficient, high-fidelity compression solution that leverages existing pretrained models with minimal retraining costs for diverse content types.
...
<details>
  <summary>Details</summary>
Motivation: Current CoT reasoning in VLA models introduces unnecessary computational overhead in simple scenarios without improving decision quality, requiring a more adaptive approach.

Method: Pretrained on AD scenarios with QA and trajectory data, then fine-tuned with two-mode dataset (fast answering w/o CoT, slow thinking with CoT). Uses Adaptive Think Reward strategy with GRPO to reward selective CoT application.

Result: Achieves PDMS of 90.3 on Navsim benchmark (1.7 points better than best vision-only baseline), reduces inference time by 14% compared to always-Think baseline, and outperforms both never-Think and always-Think baselines.

Conclusion: AdaThinkDrive effectively balances accuracy and efficiency through adaptive reasoning, demonstrating the value of selective CoT application in autonomous driving scenarios.
...
<details>
  <summary>Details</summary>
Motivation: While deepfake detection accuracy has improved, precise localization of manipulated regions remains challenging. Current approaches neglect the complementary nature of local details and global semantic context, and naive fusion strategies amplify noise and errors.

Method: Proposes independent prediction of manipulated regions from both local and global perspectives, followed by fusion using morphological operations to suppress noise while maintaining spatial coherence.
...
Conclusion: The proposed approach effectively addresses localization challenges by leveraging complementary local and global information with optimized fusion strategy, achieving improved performance in deepfake region localization.
...
<details>
  <summary>Details</summary>
Motivation: Existing event camera methods require predefined time windows (introducing latency) or suffer from high computational costs that prevent real-time efficiency. Current approaches convert event streams into intermediate representations like frames, voxel grids, or point clouds, which limits performance.

Method: Proposes a lightweight causal spatial neighborhood encoder to capture local geometric relations, followed by Mamba-based state space models for temporal modeling with linear complexity. Includes an adaptive controller that adjusts processing speed based on event rate during inference.

Result: The method achieves direct processing of raw event streams without intermediate representations, enabling scalable temporal modeling with optimal balance between window latency and inference latency.

Conclusion: The Variable-Rate Spatial Event Mamba architecture overcomes limitations of existing methods by eliminating intermediate representations and window latency while maintaining real-time efficiency through adaptive processing and linear complexity modeling.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers suffer from sequential denoising latency that limits real-world applicability. Existing acceleration methods either compromise quality or fail to properly reuse intermediate features.
...
Result: Extensive experiments show BWCache achieves up to 2.24x speedup with comparable visual quality across several video diffusion models.

Conclusion: BWCache effectively reduces computational redundancy in DiT-based video generation while preserving output quality, making it a practical acceleration solution without requiring retraining.
...
<details>
  <summary>Details</summary>
Motivation: Hybrid spacecraft pose estimation pipelines perform well on synthetic data but suffer from domain gap when applied to real imagery. Existing unsupervised domain adaptation methods underperform when limited labeled target data is available.

Method: Proposes a Supervised Domain Adaptation framework based on Learning Invariant Representation and Risk (LIRR) paradigm. Jointly optimizes domain-invariant representations and task-specific risk using both labeled synthetic and limited labeled real data.

Result: Outperforms source-only, fine-tuning, and oracle baselines on SPEED+ benchmark. With only 5% labeled target data, matches or surpasses oracle performance trained on larger fractions of labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of existing retrieval-based UAV localization methods including poor real-time performance, environmental sensitivity, and limited generalization in dynamic environments.

Method: Proposes SWA-PF method with semantic weighting mechanism and optimized particle filtering architecture, integrating semantic features from UAV images and satellite imagery. Uses a new Multi-Altitude Flight Segments dataset.
...
Conclusion: The SWA-PF method effectively addresses persistent challenges in UAV localization, providing efficient and accurate positioning in GNSS-denied environments with strong generalization capabilities.
...
Method: Proposes Masked Feature Modeling (MFM) that performs feature masking and reconstruction directly in feature space using a lightweight Rebuilder module, leveraging the segmentation decoder to classify reconstructed features.

Result: Extensive experiments across various architectures and UDA benchmarks demonstrate consistent enhancement of segmentation performance.

Conclusion: MFM offers a simple, efficient, and generalizable strategy for unsupervised domain-adaptive semantic segmentation that maintains compatibility with standard architectures without inference overhead.
...
<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art 1D-Justo-LiuNet model performance deteriorates with limited training data, creating a need for more robust spectral classification methods.

Method: Investigates MiniROCKET and HDC-MiniROCKET for spectral classification, which extract well-engineered features without trainable parameters in the feature extraction part.
...
Conclusion: MiniROCKET provides a more robust solution for spectral classification, particularly beneficial when training data is limited, while maintaining competitive performance across different data scenarios.
...
Method: Uses three expert networks: segmentation expert, signed distance field regression expert, and boundary prediction expert, with Multi-Gating Pseudo-labeling module and Adaptive Multi-Objective Loss for dynamic balancing.

Result: Outperforms state-of-the-art approaches on GlaS and CRAG benchmarks in low-label settings.

Conclusion: Demonstrates the potential of MoE-based architectures in advancing semi-supervised segmentation for histopathology images.
...
TL;DR: VHBench-10 benchmark reveals visual encoders have distinct hallucination patterns. VisionWeaver routing network dynamically combines multiple experts to significantly reduce object hallucinations in LVLMs.


<details>
  <summary>Details</summary>
Motivation: Object hallucination in Large Vision-Language Models limits real-world use. Different visual encoders have diverse training paradigms and inductive biases, leading to varied hallucination behaviors that existing benchmarks fail to capture.
...
Result: Evaluations confirm visual encoders exhibit unique hallucination characteristics. VisionWeaver significantly reduces hallucinations and improves overall model performance compared to simple feature fusion approaches.

Conclusion: Fine-grained analysis of encoder-specific hallucinations is crucial. VisionWeaver's dynamic expert routing effectively mitigates object hallucinations, demonstrating the importance of specialized feature aggregation in LVLMs.
...
TL;DR: This paper challenges the assumption that uncorrelated views automatically learn meaningful representations, proposing Consistent View Alignment to explicitly structure latent space alignment without false positives, achieving top results in MICCAI 2025 SSL3D challenge.


<details>
  <summary>Details</summary>
Motivation: The authors challenge the common assumption in representation learning that uncorrelated views of data naturally produce meaningful latent structures, arguing that explicit structural induction is necessary for effective representation learning.

Method: Proposed Consistent View Alignment method that aligns representations from different data views to capture complementary information while avoiding false positive alignments in the latent space.

Result: The method achieved first place with Primus vision transformer and second place with ResEnc CNN in MICCAI 2025 SSL3D challenge, demonstrating improved downstream task performance.

Conclusion: Structured view alignment is critical for learning effective representations, and explicit induction of meaningful structure in latent space significantly outperforms approaches relying on uncorrelated views alone.
...
TL;DR: SpecDiff is a training-free multi-level feature caching strategy that uses self-speculative future information to accelerate diffusion models, achieving 2.7-3.2× speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Existing feature caching methods rely solely on historical information, which constrains accuracy and speed performance. The paper aims to overcome this limitation by incorporating future information.

Method: Proposes SpecDiff with two algorithms: (1) feature selection using dynamic importance scores based on self-speculative and historical information, and (2) multi-level feature classification using importance score differences and multi-level calculation strategy.
...
Conclusion: SpecDiff overcomes the speedup-accuracy trade-off bottleneck by merging speculative and historical information, pushing the Pareto frontier of efficient diffusion model inference.
...
TL;DR: EDITS is a novel dataset distillation framework that leverages textual semantics from vision-language models to create more effective synthetic datasets, outperforming traditional methods that only capture low-level visual features.
...
Conclusion: The proposed EDITS framework successfully exploits implicit textual semantics within image data to achieve enhanced dataset distillation, providing a more comprehensive approach that captures both visual and semantic information.
...
TL;DR: LamiGauss is a novel X-ray laminography reconstruction method that uses Gaussian Splatting with specialized initialization to handle sparse-view data, achieving high-quality results with only 3% of full views.


<details>
  <summary>Details</summary>
Motivation: Traditional CT struggles with plate-like structures due to geometric constraints, and existing laminography methods face challenges with sparse-view acquisition and reconstruction artifacts.

Method: Combines Gaussian Splatting radiative rasterization with detector-to-world transformation incorporating laminographic tilt angle, plus artifact-filtering initialization strategy to prevent redundant Gaussians.

Result: Achieves superior performance over iterative methods using only 3% of full views, with effective reconstruction from sparse projections on both synthetic and real datasets.

Conclusion: LamiGauss provides an accurate and efficient solution for sparse-view laminography reconstruction, effectively handling geometric constraints and artifacts while optimizing directly from limited projection data.
...
<details>
  <summary>Details</summary>
Motivation: Memory-based video segmentation methods like SAM2 perform well but are not optimized for visual object tracking where distractors (visually similar objects) cause tracking drift and poor redetection after occlusion.
...
Result: Outperforms SAM2.1 on 13 benchmarks, sets new SOTA on 10. Integration with EfficientTAM gives 11% improvement, matches non-real-time SAM2.1-L quality. EdgeTAM integration delivers 4% boost.

Conclusion: The distractor-aware memory module effectively reduces tracking drift and improves redetection, demonstrating good generalization across different tracker architectures.
...
TL;DR: PelFANet is a dual-stream attention network that combines raw pelvic X-rays with segmented bone images to improve fracture detection, especially for subtle or invisible fractures on standard radiographs.
...
Method: A dual-stream attention network with Fused Attention Blocks (FABlocks) that fuses raw pelvic X-rays with segmented bone images, trained in a two-stage pipeline with segmentation-guided approach.

Result: Achieved 88.68% accuracy and 0.9334 AUC on visible fractures, and 82.29% accuracy and 0.8688 AUC on invisible fracture cases despite not being trained on them.
...
TL;DR: EvHand-FPV is a lightweight framework for egocentric 3D hand tracking using a single event camera, achieving 89% reduction in parameters and FLOPs while improving accuracy, making it suitable for resource-constrained XR devices.


<details>
  <summary>Details</summary>
Motivation: Frame-based hand tracking methods struggle with accuracy, latency, and energy efficiency in XR devices. Event cameras offer high temporal resolution with low power consumption, but lack egocentric benchmarks for hand tracking.

Method: Uses a wrist-based ROI for hand localization via geometric cues, end-to-end mapping with embedded ROI offsets to reduce computation, and multi-task learning with auxiliary geometric feature head. Constructed synthetic training data with 3D labels and real event data with 2D labels.
...
TL;DR: Proposes WARM, a novel prototype generation method using whitening and coloring transformations with cross-attention to address distribution gaps in few-shot 3D point cloud segmentation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing prototype generation methods for few-shot 3D point cloud segmentation suffer from randomness and underexplored processes, with vanilla attention modules experiencing distributional gaps between learnable tokens and support features.
...
Conclusion: WARM's simple yet effective design enables robust attention and generates representative prototypes by capturing semantic relationships among support features, overcoming distribution misalignment issues.
...
TL;DR: SRC framework improves LVLMs by iteratively calibrating rationale-answer alignment through rationale fine-tuning, candidate response generation with R-Scorer evaluation, and preference-based fine-tuning.
...
Method: Self-Rationale Calibration (SRC) framework: 1) Rationale fine-tuning to modify response format, 2) Generate diverse candidate responses, 3) Pairwise scoring with R-Scorer for rationale quality and factual consistency, 4) Confidence-weighted preference curation for alignment calibration.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models like Stable Diffusion enable powerful image customization but create security risks including deepfakes and copyright infringement. Existing protective perturbations can be removed by purification methods, exposing images to malicious use again.

Method: AntiPure uses two guidance mechanisms: 1) Patch-wise Frequency Guidance to reduce model influence over high-frequency components, and 2) Erroneous Timestep Guidance to disrupt the denoising strategy across different timesteps. This embeds imperceptible perturbations that persist through purification.

Result: AntiPure achieves minimal perceptual discrepancy and maximal distortion in post-customization images, outperforming other protective perturbation methods within the purification-customization workflow.
...
TL;DR: Noise Level Guidance (NLG) is a simple, efficient method to optimize initial noise in diffusion models without extra data, networks, or backpropagation, improving image quality and prompt adherence.
...
Method: Propose Noise Level Guidance (NLG) that refines initial noise by increasing its alignment with general guidance. No additional training data, auxiliary networks, or backpropagation needed. Works with both conditional and unconditional diffusion models.

Result: Extensive experiments on five standard benchmarks show NLG enhances output generation quality and input condition adherence. Maintains computational efficiency while integrating with existing guidance methods.

Conclusion: NLG establishes itself as a practical and scalable enhancement to diffusion models, providing a unified framework that works with various diffusion-level guidance methods without additional computational overhead.
...
Method: Created PairTally dataset with 681 images containing two object categories (both inter-category and intra-category settings), then benchmarked various state-of-the-art models including exemplar-based methods, language-prompted models, and large vision-language models.
...
Conclusion: PairTally provides a new foundation for diagnosing and improving fine-grained visual counting systems, highlighting the limitations of existing approaches and the need for better selective counting capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient transfer of multimodal semantics to lightweight object detectors without modifying the teacher model or requiring textual input during inference, making it suitable for real-world deployment.

Method: Uses a translation module to map student features into joint space, trained with dual-objective loss enforcing both local alignment and global relational consistency at object level rather than dense or global alignment.
...
Conclusion: MOCHA proves effective for transferring multimodal knowledge to compact vision-only detectors, enabling deployment of lightweight models with multimodal capabilities without text input requirements.
...
Method: Enhanced YOLO-FEDER FusionNet framework integrating generic object detection with camouflage object detection. Key improvements include: systematic training data composition with large-scale synthetic data and real-world samples, evaluation of intermediate multi-scale FEDER features, and benchmarking multiple YOLO-based backbone configurations.

Result: Integration of intermediate FEDER features with backbone upgrades led to significant performance improvements. The best configuration (YOLOv8l backbone with FEDER features from DWD module) achieved up to 39.1 percentage points FNR reduction and 62.8 percentage points mAP increase at IoU threshold of 0.5 compared to baseline.

Conclusion: The enhanced YOLO-FEDER FusionNet framework effectively addresses drone detection challenges in complex environments through systematic integration of camouflage detection techniques, improved training data strategy, and optimized feature fusion, demonstrating substantial performance gains over baseline approaches.
...
<details>
  <summary>Details</summary>
Motivation: To create an open-source vision-language foundation model that advances multimodal understanding and reasoning capabilities beyond previous models, serving as an efficient and extensible foundation for the open-source community.

Method: Three core innovations: 1) Large-scale data curation pipeline with scoring/filtering strategies, 2) Progressive training framework starting with SAIL-ViT encoder through multimodal pre-training to thinking-fusion SFT-RL hybrid paradigm, 3) Architectural advances including sparse Mixture-of-Experts designs.
...
Conclusion: SAIL-VL2 demonstrates comprehensive multimodal understanding and reasoning capabilities, serving as a competitive and efficient foundation model for the open-source community with strong performance across diverse benchmarks.
...
TL;DR: PROFUSEme is a multi-modal AI system that fuses clinical, radiology, and pathology data to predict biochemical recurrence in prostate cancer patients after radical prostatectomy, achieving superior performance over late fusion methods.


<details>
  <summary>Details</summary>
Motivation: Nearly 30% of prostate cancer patients experience biochemical recurrence after surgery, which increases mortality risk. Early accurate prediction at surgery time would enable better clinical decisions and improved patient outcomes.

Method: PROFUSEme uses intermediate fusion of multi-modal embeddings (clinical, radiology, pathology data) combined with Cox Proportional Hazard regressors to learn cross-modal interactions.

Result: The approach achieved mean C-index of 0.861 on internal 5-fold nested cross-validation and 0.7103 on CHIMERA 2025 challenge validation leaderboard holdout data, outperforming late fusion configurations.

Conclusion: The proposed intermediate fusion multi-modal approach effectively predicts biochemical recurrence in prostate cancer patients, demonstrating superior performance for clinical decision support.
...
TL;DR: Wan-Animate is a unified framework for character animation and replacement that can animate characters from reference videos or replace characters in videos while maintaining environmental consistency.


<details>
  <summary>Details</summary>
Motivation: To create a unified solution for character animation and replacement that can precisely replicate expressions, movements, and environmental lighting for seamless integration.

Method: Built on the Wan model with modified input paradigm using spatially-aligned skeleton signals for body motion and implicit facial features for expressions. Includes auxiliary Relighting LoRA for environmental integration.

Result: Achieves state-of-the-art performance in generating high-fidelity character videos with high controllability and expressiveness while maintaining appearance consistency.
...
TL;DR: VSE-MOT framework uses vision-language models to enhance multi-object tracking in low-quality videos, achieving 8-20% performance improvement over existing methods while maintaining robustness in conventional scenarios.


<details>
  <summary>Details</summary>
Motivation: Current MOT algorithms perform poorly on low-quality videos with real-world image deterioration, limiting their practical applications. There's a need to advance MOT algorithms for real-world low-quality video scenarios.

Method: Proposes a Visual Semantic Enhancement-guided MOT framework with tri-branch architecture using vision-language models. Includes MOT-Adapter to adapt visual semantic information for tracking tasks and Visual Semantic Fusion Module to improve feature fusion effectiveness.

Result: The method achieves significant performance improvements of 8-20% over existing methods in low-quality video scenarios while maintaining robust performance in conventional scenarios.

Conclusion: VSE-MOT framework effectively addresses MOT challenges in low-quality videos by leveraging visual semantic information from vision-language models, demonstrating superior performance in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Traditional ZSAD methods rely on CLIP, but DINOv3 offers superior transferable representations. However, adapting DINOv3 for ZSAD faces challenges with domain bias and global semantic focus that can miss subtle anomalies.

Method: Uses DINOv3 as visual backbone with CLIP text encoder. Introduces lightweight adapters to bridge domain gap and an Anomaly-Aware Calibration Module (AACM) to guide attention to anomalous regions rather than generic foreground semantics.
...
Conclusion: AD-DINOv3 demonstrates superiority as a general zero-shot anomaly detection framework, effectively overcoming domain bias and semantic focus issues in DINOv3 adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Previous methods focused on refining global predictions but lacked stable segment-level supervision and class-aware cross-modal alignment, leading to suboptimal performance in detecting audible, visible, and audio-visual events without temporal annotations.

Method: Two strategies: (1) EMA-guided pseudo supervision framework generating reliable segment-level masks via adaptive thresholds or top-k selection, and (2) class-aware cross-modal agreement (CMA) loss that aligns audio and visual embeddings at reliable segment-class pairs while preserving temporal structure.

Result: Achieves state-of-the-art performance across multiple metrics on LLP and UnAV-100 datasets.

Conclusion: The proposed EMA-guided pseudo supervision and class-aware cross-modal alignment effectively address the limitations of previous methods, providing stable temporal guidance and ensuring cross-modal consistency for improved weakly-supervised audio-visual video parsing.
...
TL;DR: Proposes CSMoE model integrating Soft MoE mechanism into RS foundation models to improve computational efficiency while maintaining performance across remote sensing tasks.
...
Method: Integrates Soft mixture-of-experts mechanism into Cross-Sensor Masked Autoencoder (CSMAE), creating CSMoE model with modality-specific expert specialization and cross-sensor representation learning. Uses thematic-climatic descriptor-driven sampling for diverse training set.

Result: Achieves more than twice the computational efficiency of existing RS FMs while maintaining competitive performance across scene classification, semantic segmentation, and image retrieval tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional histopathological diagnosis requires multiple chemical stains which is time-consuming, labor-intensive, and environmentally taxing. Existing virtual staining methods struggle with clinical applications due to reliance on well-aligned paired data, which is difficult to obtain because chemical staining processes distort tissue structures.
...
<details>
  <summary>Details</summary>
Motivation: Digital beautification through social media filters raises concerns about reliability of facial images and effectiveness of automated face analysis systems, particularly for detecting manipulated content like deepfakes and morphing attacks.
...
Result: Performance degradation observed in detectors, highlighting vulnerabilities introduced by facial enhancements.

Conclusion: There is a need for robust detection models that are resilient to facial alterations and beautification filters.
...
TL;DR: The MARS2 2025 Challenge introduces a multimodal reasoning benchmark with two datasets (Lens and AdsQA) across 12 daily scenarios and advertisement videos, featuring three competition tracks and evaluating 40+ baselines with 76 participating teams.


<details>
  <summary>Details</summary>
Motivation: To advance multimodal machine learning and LLMs by creating a comprehensive benchmark that addresses real-world and specialized scenarios, moving beyond general-purpose models to broaden multimodal reasoning applications.

Method: Released two tailored datasets (Lens for general reasoning in 12 daily scenarios, AdsQA for domain-specific reasoning in advertisement videos), evaluated 40+ baselines including generalist MLLMs and task-specific models, and organized three competition tracks: VG-RS, VQA-SA, and VR-Ads.

Result: 76 teams from academic and industrial institutions registered, with 40+ valid submissions out of 1200+ included in ranking lists. The datasets, code sets (40+ baselines and 15+ participants' methods), and rankings are publicly available.

Conclusion: The MARS2 2025 Challenge successfully established a large-scale multimodal reasoning benchmark that brings together diverse approaches and facilitates tracking state-of-the-art progress in this dynamic field, with ongoing updates and future events planned.
...
<details>
  <summary>Details</summary>
Motivation: To understand the performance gap between abstract primitive shape images and traditional raster images in conveying visual semantic information to deep learning models, and to determine how much high-level semantic content can be preserved at different abstraction levels.

Method: Introduces the Hierarchical Abstraction Image Dataset (HAID) containing abstract images generated from normal raster images at multiple abstraction levels. Trains and evaluates conventional vision systems on HAID across classification, segmentation, and object detection tasks.

Result: The study provides a comprehensive comparison between rasterized and abstract image representations, analyzing performance across multiple computer vision tasks at different abstraction levels.

Conclusion: The paper discusses whether abstract images can be considered an effective format for conveying visual semantic information and contributing to vision tasks, based on the systematic evaluation across various abstraction levels.
...
TL;DR: BEVUDA++ is a geometric-aware teacher-student framework that addresses domain adaptation challenges in multi-view 3D object detection for Bird's Eye View perception, achieving state-of-the-art performance in cross-domain scenarios.
...
Method: Proposes BEVUDA++ with Reliable Depth Teacher (blends target LiDAR with depth predictions using uncertainty estimation) and Geometric Consistent Student (maps multi-space features to unified geometric embedding). Includes Uncertainty-guided Exponential Moving Average to reduce error accumulation.

Result: Achieves state-of-the-art performance in four cross-domain scenarios, with 12.9% NDS and 9.5% mAP enhancement on Day-Night adaptation.

Conclusion: The proposed geometric-aware teacher-student framework effectively addresses domain shift accumulation across multiple geometric spaces in BEV perception, demonstrating significant improvements in cross-domain 3D object detection.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers achieve state-of-the-art performance in semantic segmentation but suffer from high computational and memory costs, limiting their practical deployment.

Method: Proposes STEP framework with dCTS (lightweight CNN-based policy network) for dynamic patch merging into superpatches, and integrates early-exits in encoder blocks to remove high-confident tokens before final processing.

Result: Token count reduced by 2.5x, computational cost reduced by 2.6x, throughput increased by 3.4x with ViT-Large backbone. Full STEP achieves 4x computational complexity reduction and 1.7x inference speed gain with <2.0% accuracy drop. 40% of tokens can be halted early.

Conclusion: STEP effectively addresses ViT efficiency issues through hybrid token reduction, enabling high-performance semantic segmentation with significantly reduced computational requirements.
...
TL;DR: GRT enables efficient high-FPS video understanding through motion-compensated tokenization and semantic merging, reducing computational overhead while maintaining dense temporal information.


<details>
  <summary>Details</summary>
Motivation: Current VLLMs use low-frame-rate sampling that discards dense temporal information, failing for tasks requiring precise temporal alignment like lecture comprehension where information appears in nearly every frame.

Method: Gated Residual Tokenization (GRT) with two stages: (1) Motion-Compensated Inter-Gated Tokenization uses pixel-level motion estimation to skip static regions, achieving sub-linear token growth; (2) Semantic-Scene Intra-Tokenization Merging fuses tokens across static regions to reduce redundancy while preserving dynamic semantics.

Result: GRT outperforms larger VLLM baselines on the DIVE benchmark and scales positively with FPS, demonstrating efficient high-FPS video understanding capabilities.

Conclusion: Dense temporal information is crucial for video understanding, and GRT provides an efficient, scalable solution for high-FPS video comprehension while reducing computational costs.
...
TL;DR: Cineaste benchmark introduces 3,119 multiple-choice questions from 200 movies to test fine-grained narrative understanding, revealing current MLLMs struggle with long-range temporal reasoning (top model: 63.15% accuracy).
...
Method: Created dataset from 1,805 scenes across 200 movies using GPT-4o to generate diverse questions integrating visual descriptions, captions, scene titles, and summaries. Implemented two-stage filtering: Context-Independence filtering to ensure video context requirement, and Contextual Veracity filtering to validate factual consistency against movie content.

Result: Existing MLLMs perform poorly on Cineaste, with the top open-source model achieving only 63.15% accuracy. Long-range temporal reasoning is identified as the primary bottleneck.
...
TL;DR: GenExam is the first multidisciplinary text-to-image exam benchmark with 1,000 samples across 10 subjects, designed to rigorously evaluate AI models' ability to integrate knowledge, reasoning, and generation through exam-style prompts.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on understanding/reasoning or world knowledge illustration, but neglect rigorous evaluation of drawing exams that require integrated intelligence.

Method: Created 1,000 exam-style prompts across 10 subjects organized under a four-level taxonomy, each with ground-truth images and fine-grained scoring points for precise evaluation of semantic correctness and visual plausibility.

Result: State-of-the-art models like GPT-Image-1 and Gemini-2.5-Flash-Image achieved less than 15% strict scores, with most models scoring almost 0%, demonstrating the benchmark's significant challenge.

Conclusion: GenExam provides a rigorous assessment framework for evaluating models' integrated knowledge, reasoning, and generation capabilities, offering valuable insights for advancing toward general AGI.
