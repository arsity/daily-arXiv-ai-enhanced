...
Motivation: Despite advances in autonomous driving, entirely safe self-driving cars remain elusive due to novel scenarios and out-of-distribution hazards that current systems cannot adequately handle.

Method: The 2COOOL workshop provides a forum for researchers and industry experts to collaborate on novelty handling through approaches like out-of-distribution detection, vision-language models, new benchmarking methodologies, and safe driving practices.

Result: Building on the success of its inaugural edition at WACV 2025, the workshop aims to inspire development of new algorithms for hazard avoidance using techniques from anomaly detection, open-set recognition, open-vocabulary modeling, and domain adaptation.

Conclusion: The workshop serves as a dedicated platform to advance the state of the art in handling novel scenarios and out-of-label hazards, which are critical barriers to achieving safe autonomous driving deployment.
...
<details>
  <summary>Details</summary>
Motivation: To develop automated deep learning methods for classifying dental conditions in panoramic X-ray images to support diagnostic processes and improve efficiency.

Method: Evaluated three approaches: custom CNN, hybrid models (CNN feature extraction + traditional classifiers), and fine-tuned pre-trained architectures (VGG16, Xception, ResNet50) on 1,512 radiographs with 11,137 expert annotations across four conditions. Used 5-fold cross validation with accuracy, precision, recall, and F1 score metrics.

Result: Hybrid CNN-Random Forest model achieved highest performance with 85.4% accuracy, significantly surpassing custom CNN baseline (74.3%). VGG16 performed best among pre-trained models at 82.3% accuracy. Hybrid models improved discrimination of morphologically similar conditions.

Conclusion: Combining CNN-based feature extraction with ensemble classifiers provides efficient and reliable performance for automated dental diagnostic support, though larger datasets and further clinical validation are needed.
...
<details>
  <summary>Details</summary>
Motivation: Zero-shot appearance transfer with large-scale image generation models faces significant attention leakage issues due to Query-Key alignment capturing semantic mapping between images.
...
Conclusion: Q-Align effectively mitigates attention leakage and improves semantic alignment in zero-shot appearance transfer through its novel Query-Query alignment approach.
...
Conclusion: ERTACache effectively maintains baseline fidelity while significantly improving efficiency through principled error rectification in diffusion model caching.
...
Method: Proposes Temporal Visual Screening (TVS) that: (1) retains focus-critical video segments, (2) synchronously reconstructs queries to direct form while preserving answer consistency, (3) maintains invariance for any possible answer. TVS serves as a modular front-end adapter for both training and inference pipelines.

Result: TVS achieves relative gains of 7.33% during training and 34.6% during inference. The ReSimplifyIt baseline outperforms prior approaches by 0.47 in F-1 score on video trimming while maintaining competitive query rewriting performance.

Conclusion: Temporal information screening through TVS effectively improves video-language understanding by optimizing reasoning burden distribution and aligning queries with focus-critical visual information.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for surgical tool localization rely on segmentation annotations, but performance is limited by the availability of diverse annotated data. The authors propose that skeletal pose annotations strike a better balance between semantic information richness and annotation efficiency.

Method: The authors created ROBUST-MIPS dataset by enriching the existing ROBUST-MIS dataset with tool pose annotations and instance segmentation. They developed custom annotation software and benchmarked popular pose estimation methods to demonstrate the adequacy of pose annotations.

Result: The study shows that pose annotations yield high-quality results for surgical tool localization, providing an efficient alternative to segmentation-based approaches while maintaining sufficient semantic information.

Conclusion: Skeletal pose annotations represent a more efficient annotation paradigm for surgical tools that can accelerate the growth of annotated datasets. The released dataset, benchmark models, and annotation software aim to encourage adoption of this annotation style in computer-assisted interventional technologies.
...
<details>
  <summary>Details</summary>
Motivation: Existing safety mechanisms for Text-to-Image models are either susceptible to evasion under distribution shifts or require extensive model-specific adjustments, raising serious safety concerns about potential misuse.

Method: Safe-Control uses data-driven strategies and safety-aware conditions to inject safety control signals into locked T2I models in a patch-like manner. It allows developers to construct various safety patches that can be flexibly merged into a unified patch.

Result: Safe-Control reduces unsafe content generation to 7% compared to approximately 20% for most baseline methods, while maintaining quality and text alignment of benign images across six diverse T2I models.

Conclusion: Safe-Control provides an effective, adaptable plug-and-play solution that significantly outperforms existing safety mechanisms in mitigating unsafe content generation in T2I models with similar denoising architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods underperform in handling stuff-type target regions with ambiguous boundaries, as well as cases with absent or multiple targets in navigation tasks involving natural language instructions and visual inputs.

Method: Proposed GENNAV system that predicts target existence and generates segmentation masks for multiple stuff-type target regions. Also created GRiN-Drive benchmark with no-target, single-target, and multi-target samples for evaluation.

Result: GENNAV achieved superior performance over baseline methods on standard evaluation metrics. Real-world experiments with four automobiles in five distinct urban areas demonstrated strong zero-shot transfer performance and robustness across diverse environments.

Conclusion: GENNAV effectively addresses the challenges of existence prediction and segmentation for stuff-type target regions with ambiguous boundaries, showing robust performance in both benchmark evaluations and real-world deployment scenarios.
...
Method: Uses bi-mode annealing and Bi-mode Policy Optimization (BPO) to train the model with both thinking and non-thinking capabilities, followed by improved GRPO framework training.
...
<details>
  <summary>Details</summary>
Motivation: Traditional RGB-based detection methods struggle with occlusion, camouflage, and lighting variations, creating a need for more robust, modality-agnostic approaches that can handle adverse conditions.

Method: The framework uses a Mamba-based fusion mechanism to capture complementary signals across RGB, thermal, and depth modalities, identifying modality-specific features and fusing them into a unified representation.

Result: State-of-the-art or competitive performance across multiple benchmark datasets, demonstrating enhanced detection of obscured or camouflaged targets compared to existing unimodal and naive fusion strategies.

Conclusion: Mamba-based fusion architectures significantly advance multimodal object detection, particularly under visually degraded or complex conditions, highlighting the efficacy of the proposed fusion design.
...
<details>
  <summary>Details</summary>
Motivation: Traditional CT/X-ray registration methods suffer from spatial information loss and domain gap issues, while existing 3D reconstruction approaches require dense views and struggle with noisy X-rays, creating a need for more robust registration solutions.

Method: Joint 3D Radiative Gaussians (RadGS) reconstruction with 3D/3D registration, featuring Counterfactual Attention Learning mechanism for vertebral region focus in noisy X-rays, and patient-specific pre-training strategy for simulation-to-real adaptation.

Result: State-of-the-art performance on both reconstruction and registration tasks, surpassing existing methods on in-house datasets.

Conclusion: RadGS-Reg effectively addresses limitations of traditional methods by providing accurate vertebral-level CT/X-ray registration through joint reconstruction and registration with attention mechanisms and adaptive pre-training.
...
Method: Created a synthetic dataset with over 6.2 million residential buildings at LoD 4, featuring three modalities: semantic 3D wireframe graphs, floor plan images, and roof point clouds with derived semantic annotations.

Result: A publicly available multi-modal dataset that enables development of generative AI algorithms for automated 3D building model creation with semantic-geometric consistency.

Conclusion: SYNBUILD-3D provides a valuable resource for advancing 3D building generation research and applications, bridging the gap in annotated 3D building data availability.
...
Method: Leverages contrastive learning to align concept representations across layers and employs an attention-based fusion mechanism to construct globally integrated CAVs. Also introduces TGCAV for testing with the unified representations.
...
Method: Visual In-Context Prompting (VICP) framework that synergizes LLMs and vision foundation models. LLMs infer semantic identity rules from few-shot positive/negative pairs through task-specific prompting, which then guides a VFM (e.g., DINO) to extract ID-discriminative features via dynamic visual prompts.

Result: VICP outperforms baselines by a clear margin on unseen categories across ShopID10K dataset (10K e-commerce object instances) and diverse ReID benchmarks, demonstrating effective generalization without retraining.

Conclusion: VICP enables generalization to novel object categories using only in-context examples, eliminating dataset-specific retraining by aligning LLM-derived semantic concepts with VFM's pre-trained prior knowledge.
...
Method: SegResNet models within Auto3DSeg architecture were trained using 5-fold cross-validation with STAPLE ensembling on region-of-interest focused data. Task 1 used 91 T1-weighted arterial contrast-enhanced MRI cases, while Task 2 used 50 T2-weighted MR-Linac cases.
...
Conclusion: Results demonstrate challenges in MRI-based pancreatic tumor segmentation with small datasets and highlight variability from different MRI sequences. Despite modest performance, there is potential for automated delineation, emphasizing need for larger standardized datasets.
...
<details>
  <summary>Details</summary>
Motivation: Pretrained segmentation models struggle with generalization across different MRI sequences due to contrast variations from different imaging protocols, despite being governed by the same fundamental spin properties.

Method: The method reversely infers underlying spin properties (proton density, T1, T2) from cardiac MRI images by solving ill-posed nonlinear inverse problems regularized by a learned spin prior distribution from mSASHA dataset using a generative diffusion model.

Result: The approach enables highly accurate segmentation across vastly different image contrasts and imaging protocols, achieving wide-spectrum generalization of cardiac MRI segmentation.

Conclusion: Reverse Imaging provides an interpretable physics-driven solution for domain adaptation in cardiac MRI by leveraging fundamental spin properties to enable flexible image synthesis and overcome generalization challenges.
...
<details>
  <summary>Details</summary>
Motivation: Traditional HMR methods are user-agnostic and optimized for generalization, but they compromise 3D accuracy by failing to account for person-specific body shapes and 3D pose plausibility when refining poses using 2D constraints.
...
Conclusion: PHD provides an effective personalized approach to 3D human mesh recovery that leverages user-specific shape information to achieve superior 3D pose estimation accuracy compared to traditional user-agnostic methods.
...
Conclusion: The proposed HTP framework successfully addresses computational inefficiency in diffusion-based 3D human pose estimation through hierarchical pruning while maintaining high performance, making it more practical for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: High cost and time-consuming nature of OCT data acquisition has led to scarcity of large-scale datasets, hindering development of deep learning models for 3D fingerprint recognition.

Method: Three-stage framework: 2D style transfer, 3D structure expansion network, and OCT realism refiner using 3D GAN to generate synthetic OCT fingerprints from 2D images.
...
Conclusion: Print2Volume effectively overcomes data scarcity in OCT fingerprint recognition, enabling significant performance improvements through synthetic data generation.
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of surgical recordings is time-consuming, and computer vision/machine learning approaches for automated analysis require substantial sample data which is scarce in the medical field, particularly for gynecologic laparoscopy procedures.

Method: Creation of the GLENDA dataset through collaboration with leading medical experts, containing region-based annotations of endometriosis (dislocation of uterine-like tissue) in gynecologic laparoscopy images.

Result: The first publicly available dataset of its kind specifically focused on gynecologic laparoscopy with annotated endometriosis cases, enabling development of automated analysis tools.

Conclusion: GLENDA addresses the critical need for annotated medical datasets in gynecologic laparoscopy, providing essential training data for computer vision and machine learning approaches to improve surgical video analysis and post-surgical activities.
...
<details>
  <summary>Details</summary>
Motivation: Recorded surgical videos provide detailed information but automatic content indexing remains challenging due to the specialized nature of medical video content, particularly for surgical instrument identification and segmentation.

Method: The authors use a region-based fully convolutional network for instance-aware surgical instrument segmentation (binary classification) and instrument recognition (multi-class classification) in laparoscopic gynecology videos.

Result: The method achieves high accuracy for localizing and segmenting instrument regions even with a moderately low number of training examples, but determining specific instrument types remains very challenging due to the high similarity between surgical instruments.

Conclusion: While surgical instrument segmentation from background is achievable with good accuracy, multi-class instrument recognition presents significant challenges that need to be addressed in future work, likely requiring more sophisticated approaches to handle instrument similarity.
...
<details>
  <summary>Details</summary>
Motivation: Leverage large amounts of unlabeled remote sensing data through self-supervised learning to improve representation learning for satellite imagery, addressing the limitations of current methods.

Method: Develop SatDINO based on DINO framework, incorporating ground sample distance (GSD) encoding and adaptive view sampling for tailored satellite imagery representation learning.

Result: SatDINO outperforms state-of-the-art MAE-based methods and achieves competitive results across multiple benchmarks and testing setups.

Conclusion: Contrastive self-supervised learning (DINO) is highly effective for remote sensing, with SatDINO providing superior performance and novel enhancements that can be independently applied.
...
<details>
  <summary>Details</summary>
Motivation: Current WSI metadata lacks standardization, requiring manual inspection which is impractical for large collections containing millions of images.

Method: Proposes a general framework with 2D index maps and three-layer tissue organization (source, tissue type, pathological alterations) using common syntax and semantics for interoperability.

Result: Enables automated content analysis and selection for AI training/validation, demonstrated in clinical pathology with specific examples in WSI catalogs, ML, and graph-based representations.

Conclusion: The standardized tissue mapping framework provides fine-grained WSI content information, overcoming manual inspection limitations and enabling scalable AI development across various medical domains.
...
Method: Proposes ICPL - unsupervised Incremental Learning using Confidence-based Pseudo-labels. Integrates pseudo-labels into various CIL methods with confidence-based selection to replace human annotations.

Result: Achieves competitive results compared to supervised methods. Outperforms state-of-the-art class-iNCD methods by more than 5% in final accuracy. Validated on CIFAR100, ImageNet100, and fine-grained datasets with measured computational complexity.

Conclusion: ICPL provides an effective solution for unsupervised incremental learning that works well in resource-constrained environments and demonstrates real-world practicality, bridging the gap between supervised CIL and unlabeled data scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Synthetic medical data has domain gaps that limit its generalizability to real clinical settings, particularly in X-ray imaging where differences in attenuation behavior, noise characteristics, and soft tissue representation exist between synthetic and real images.

Method: Proposes MedShift, a class-conditional generative model based on Flow Matching and Schrodinger Bridges that learns a shared domain-agnostic latent space and supports unpaired image translation across multiple domains without domain-specific training.

Result: MedShift demonstrates strong performance despite smaller model size compared to diffusion-based approaches, offering flexibility to prioritize either perceptual fidelity or structural consistency during inference.

Conclusion: MedShift provides a scalable and generalizable solution for domain adaptation in medical imaging, with the introduced X-DigiSkull dataset serving as a benchmark for future domain translation models in this field.
...
Method: Deep learning approach using 3-m resolution PlanetScope imagery with simulated tree crowns using Gaussian kernels of scalable size. Training based on billions of points automatically extracted from airborne lidar data.
...
Conclusion: The method provides a scalable framework for global, high-resolution tree monitoring that can identify trees both inside and outside forests, and is adaptable to future satellite missions.
...
Method: Hierarchical organization of Gaussians by scale within anchor-based structure, with coarser-level representing low-resolution structure and finer-level handling details. Hybrid deformation and spawning strategy for motion modeling, plus bidirectional adaptive masking to remove static regions and prioritize informative viewpoints.
...
Conclusion: M provides an efficient and scalable framework for dynamic scene representation with 3D Gaussian Splatting, achieving better performance with reduced computational requirements.
...
TL;DR: A lightweight 125M-parameter image captioning model achieves performance comparable to large multimodal generalists, with a novel Sharp-Eyed Refinement framework that addresses visual blindness through improved visual grounding.
...
Method: Developed a specialist model 56x smaller than LLaMA-7B, evaluated on single-sentence and detailed captioning tasks. Created Sharp-Eyed Refinement framework with DeepLens component that extracts detailed visual representations by focusing on informative regions identified during initial glance.

Result: The lightweight model achieves performance comparable to large multimodal generalists. The framework effectively addresses visual blindness issues caused by ineffective attention mechanisms and limited visual representations.

Conclusion: The specialist model shows strong potential for on-device applications, and the Sharp-Eyed Refinement framework successfully enhances caption quality through improved visual grounding, outperforming prior small captioning models and large generalists.
...
<details>
  <summary>Details</summary>
Motivation: Foundation models show promise for AI-based dementia diagnosis but their integration into federated learning systems remains underexplored, particularly for decentralized clinical settings where data privacy is crucial.
...
<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods focus on either feature truncation or scoring functions individually, and evaluate on specific OOD datasets, missing the potential benefits of combining multiple techniques for better robustness.

Method: Theoretically and empirically demonstrates that state-of-the-art feature truncation and scoring functions can be effectively combined. Proposes MME score that unifies multiple OOD detectors into a single scoring function through ensemble aggregation.

Result: MME significantly outperforms recent state-of-the-art methods across large-scale and small-scale benchmarks covering near-OOD and far-OOD scenarios. Achieves average FPR95 of 27.57% on ImageNet-1K benchmark, improving by 6% over best baseline using BiT model.

Conclusion: Combining multiple OOD detection techniques through ensemble methods creates a more robust and effective solution that handles various types of OOD samples better than individual approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional data transformation methods for adversarial attacks apply augmentation to entire images, which can introduce background interference and cause false detections in object detection models, reducing attack effectiveness.

Method: Proposes a localized augmentation approach that selectively applies data transformations only to the target regions containing adversarial patches, avoiding unnecessary augmentation of background and non-target areas.

Result: Experiments on HRSC2016 dataset show the method effectively increases adversarial patch attack success rates and enhances transferability compared to full-image augmentation approaches.

Conclusion: Localized augmentation focusing only on target regions reduces background interference, allows loss function to better concentrate on adversarial patch effects, and improves both attack success and transferability for ship detection systems.
...
Method: Introduce ELV-Halluc benchmark, conduct experiments to confirm SAH existence and patterns, test positional encoding strategies, and use DPO with 8K adversarial data pairs to improve semantic distinction.

Result: Confirmed SAH existence showing it increases with semantic complexity and rapid semantic changes. Achieved 27.7% SAH reduction and improvements on both ELV-Halluc and Video-MME benchmarks.

Conclusion: SAH is a distinct hallucination type in long videos that requires specialized mitigation. Positional encoding and DPO strategies effectively reduce SAH by improving semantic aggregation and event boundary recognition.
...
<details>
  <summary>Details</summary>
Motivation: Vision transformer foundation models struggle with fine features in micrographs and large image sizes common in materials/biological analysis due to their patch-based approach.

Method: Train a convolutional neural network to upsample low-resolution foundation model features using the input image as reference, then apply this upsampler to featurize and segment various microscopy images.

Result: Upsampled features enable separation of hard-to-segment phases like hairline cracks, and interactive segmentation produces high-quality results much faster with far fewer labels than traditional CNNs.

Conclusion: The approach provides efficient, high-quality segmentation for microscopy images by leveraging enhanced foundation model features without requiring extensive training or fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in drone scenarios with wide field of view and complex compositional semantics, where existing VLMs lack fine-grained semantics and hierarchical methods depend on precise entity partitioning.

Method: Proposes HCCM with two components: (1) Region-Global Image-Text Contrastive Learning (RG-ITC) for hierarchical local-to-global semantics without precise partitioning, (2) Region-Global Image-Text Matching (RG-ITM) for local semantic consistency evaluation. Also includes Momentum Contrast and Distillation (MCD) for robustness against incomplete/ambiguous text descriptions.

Result: Achieves state-of-the-art Recall@1 of 28.8% (image retrieval) and 14.7% (text retrieval) on GeoText-1652. Demonstrates strong zero-shot generalization with 39.93% mean recall on unseen ERA dataset, outperforming fine-tuned baselines.

Conclusion: HCCM effectively addresses drone vision-language challenges through hierarchical cross-granularity learning, showing superior performance and generalization capabilities without requiring precise scene partitioning or rigid constraints.
...
<details>
  <summary>Details</summary>
Motivation: Gaussian splatting requires dense scene observations and fails to reconstruct occluded/unobserved areas. Conventional regression-based methods produce blurry, implausible results and cannot capture multiple plausible explanations for hidden surfaces.

Method: Proposes a generative formulation using a latent diffusion model conditioned on single input images. Uses a Variational AutoReconstructor to learn a latent space from 2D images in self-supervised manner, then trains diffusion model over this latent space.

Result: Generates faithful reconstructions and diverse samples that complete occluded surfaces, enabling high-quality 360-degree renderings from single images.

Conclusion: The method successfully addresses the limitations of conventional approaches by using generative modeling to handle ambiguity in occluded areas and produce complete 3D scene reconstructions from sparse inputs.
...
<details>
  <summary>Details</summary>
Motivation: Pairwise comparison is more reliable than absolute rating but requires O(n^2) annotations, which is too expensive for large datasets. Recent work reduced this to O(n log n), but further efficiency improvements are needed.

Method: EZ-Sort uses CLIP model for zero-shot hierarchical pre-ordering, initializes bucket-aware Elo scores, and runs uncertainty-guided human-in-the-loop MergeSort to replace obvious comparisons with automated ones.

Result: On face-age estimation (FGNET), historical image chronology (DHCI), and retinal image quality assessment (EyePACS), EZ-Sort reduced human annotation cost by 90.5% vs exhaustive and 19.8% vs prior work (n=100), while maintaining or improving inter-rater reliability.

Conclusion: Combining CLIP-based priors with uncertainty-aware sampling provides an efficient and scalable solution for pairwise ranking tasks, significantly reducing human annotation burden.
...
<details>
  <summary>Details</summary>
Motivation: Modeling human-object interactions from egocentric perspective is important due to wearable device adoption, but largely unexplored. The paper investigates how much interaction information can be recovered from only head and wrist tracking.

Method: Uses Diffusion Transformer architecture with unique three-variate diffusion process that jointly models human motion, object trajectory, and contact sequence. Operates in head-centric canonical space and employs conveyor-based inference for processing sequences of any length.

Result: ECHO outperforms existing methods that lack the same flexibility, setting state-of-the-art in egocentric HOI reconstruction through extensive evaluation.

Conclusion: The proposed ECHO framework successfully demonstrates that comprehensive human-object interaction information can be recovered from minimal egocentric observations (head and wrists only), offering a flexible and robust solution for wearable device applications.
...
<details>
  <summary>Details</summary>
Motivation: To assess how current general-purpose vision-language models transfer their abilities to urban domain understanding, which requires fine-grained spatial reasoning about objects, layouts, and depth cues.

Method: Comparative study of three VLMs (BLIP-2, InstructBLIP, LLaVA-1.5) using zero-shot evaluation and fine-tuning with a synthetic VQA dataset constructed from street-view images with segmentation, depth, and object detection predictions, paired with LLM-generated Chain-of-Thought answers.

Result: VLMs perform reasonably well in zero-shot settings, but fine-tuning with the synthetic CoT-supervised dataset substantially boosts performance, especially for challenging question types like negation and counterfactuals.

Conclusion: Urban spatial reasoning presents a new challenge for VLMs, and synthetic dataset construction with chain-of-thought supervision provides an effective approach for adapting general-purpose models to specialized domains.
...
Method: Temporal Flow Matching (TFM) - a generative trajectory method that learns underlying temporal distributions, can predict last context images as a fallback, and supports 3D volumes, multiple prior scans, and irregular sampling.
...
<details>
  <summary>Details</summary>
Motivation: Current Leibovich score has limited patient-level resolution and excludes imaging information, creating need for better personalized recurrence risk prediction in clear cell renal cell carcinoma.

Method: Modular deep learning framework with pretrained encoders and Cox-based survival modeling, tested across unimodal, late fusion, and intermediate fusion setups using CT and whole-slide images.

Result: WSI-based models consistently outperformed CT-only models, intermediate fusion further improved performance, best model approached adjusted Leibovich score, radiology added value primarily through fusion.

Conclusion: Foundation model-based multimodal integration is feasible for personalized ccRCC risk prediction, future work should explore better fusion strategies, larger datasets, and improved CT encoders.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of existing deep learning CGH methods: black-box end-to-end networks ignoring physics, CNN's limited receptive fields, and ASM-based methods constrained to finite near-fields.

Method: Proposes Deep Unfolding Network (DUN) with two modules: adaptive bandwidth-preserving model (ABPM) for wider working distances, and phase-domain complex-valued denoiser (PCD) with deformable self-attention for global feature capture.

Result: Achieves PSNR over 35 dB, demonstrates state-of-the-art performance on both simulated and real data, with wider working distances compared to ASM-based methods.

Conclusion: The proposed DUN framework provides more flexibility, interpretability, and better performance by combining physical modeling with deep learning, overcoming key limitations in current CGH reconstruction methods.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for human-in-the-loop approaches in clinical PET/CT imaging to efficiently refine automated lesion segmentation, overcoming challenges of tracer heterogeneity and multi-center variability.

Method: Extended nnU-Net pipeline with promptable capabilities by encoding user-provided foreground/background clicks as additional input channels using Euclidean Distance Transform (EDT), with online simulation of user interactions and custom point sampling strategy.

Result: Ensemble of EDT-based models achieved strongest cross-validation performance, reducing both false positives and false negatives compared to baseline models, with consistent outperformance over Gaussian kernel encodings.

Conclusion: Promptable models show significant potential for enabling efficient, user-guided segmentation workflows in multi-tracer, multi-center PET/CT imaging, with EDT encoding proving superior for spatial prompt representation.
...
Method: Proposes a geometric mapping based on camera parameters adapted to the scene, with a probabilistic projection mechanism using confidence scores to refine alignment and filter irrelevant elements. Also improves temporal processing by selectively accumulating reliable information over time using confidence scores.

Result: Demonstrates improved performance over state-of-the-art approaches on nuScenes and Argoverse2 datasets, with particularly pronounced improvements on nuScenes and in challenging long perception ranges, indicating better generalization.

Conclusion: The proposed probabilistic projection mechanism with confidence scoring effectively addresses hallucination issues in HD map construction and provides more accurate geometric mapping from image space to BEV space.
...
<details>
  <summary>Details</summary>
Motivation: Word-level OCR has shifted the bottleneck from character segmentation to word segmentation, limiting context for language models and introducing segmentation errors.

Method: Transition from word-level to line-level OCR by processing entire lines instead of individual words, bypassing word detection errors and providing larger sentence context for better language model utilization.
...
Conclusion: Line-level OCR represents a natural progression that improves both accuracy and efficiency, with potential to better leverage advances in large language models for document image processing.
...
<details>
  <summary>Details</summary>
Motivation: To address the computational limitations of existing diffusion-based data augmentation methods that require enterprise-grade GPUs and thousands of synthetic images, making synthetic data generation more accessible.
...
Result: Achieves up to 21.3% improvement in mAP@.50:.95 compared to ODGEN baseline, using only 500 synthetic images (vs 5000) and consumer-grade GPUs like RTX 4090.

Conclusion: Quality and efficiency-focused approach with FLORA surpasses state-of-the-art performance with far greater efficiency, making synthetic data creation more practical and accessible for real-world object detection scenarios.
...
Method: Parallel entropy monitoring of CNN activations, specifically testing on VGG-16 to detect entropy shifts caused by adversarial perturbations.
...
Conclusion: CNNs inherently encode distribution shifts in activation patterns, enabling practical real-time adversarial detection through activation entropy monitoring without compromising original model performance.
...
<details>
  <summary>Details</summary>
Motivation: LVLMs struggle with reading values from digital measurement devices in real-world conditions with clutter, occlusions, extreme viewpoints, and motion blur, which are common in AR and head-mounted camera applications.

Method: Developed CAD2DMD-SET tool using 3D CAD models, advanced rendering, and high-fidelity image composition to generate diverse synthetic DMD datasets. Also created DMDBench validation set of 1,000 annotated real-world images.

Result: Fine-tuning LVLMs with CAD2DMD-SET data yielded substantial improvements - InternVL showed 200% score increase using ANLS metric without degrading performance on other tasks.

Conclusion: CAD2DMD-SET significantly improves LVLM robustness and performance in challenging DMD reading scenarios, and will be released as open-source for community use.
...
<details>
  <summary>Details</summary>
Motivation: Current sound source localization methods perform poorly with negative audio (silence, noise, offscreen sounds) and are only evaluated on positive cases with single visible sound sources.
...
Conclusion: The approach addresses limitations of current methods by handling negative audio cases and providing better evaluation metrics and datasets for comprehensive assessment.
...
TL;DR: CLIP-DCA improves domain generalization for CLIP by enhancing domain awareness instead of forcing domain invariance, showing significant gains on challenging out-of-distribution datasets.


<details>
  <summary>Details</summary>
Motivation: Current domain generalization evaluation for foundation models like CLIP is inadequate because web-scale pretraining data may already cover benchmark datasets, making it hard to test truly unseen scenarios. CLIP's performance deteriorates significantly on more out-of-distribution data.
...
Result: CLIP-DCA shows significant improvements compared to existing methods, particularly on datasets that are more out-of-distribution, demonstrating better domain generalization performance.

Conclusion: Enhancing domain awareness is a prerequisite for effective domain-invariant classification in foundation models, contrary to traditional domain invariance approaches that discard beneficial domain-aware representations.
...
<details>
  <summary>Details</summary>
Motivation: Humans excel at generalizing to novel concepts in open-world settings, but most video learning focuses on typical closed-set data. This paper explores how atypical unusual videos can benefit open-world learning.

Method: Collected a new video dataset of atypical data (sci-fi, animation, etc.) and fed them into model training for representation learning. Evaluated on three open-world tasks: OOD detection, novel category discovery, and zero-shot action recognition.

Result: Straightforward learning with atypical data consistently improved performance across all three tasks. Categorical diversity boosted OOD detection, semantic diversity improved novel category discovery with smaller datasets, and helped zero-shot action recognition generalize to unseen classes.
...
TL;DR: Proposes unsupervised video continual learning (uVCL) framework that learns from unlabeled video sequences without task boundaries, using Kernel Density Estimation and novelty detection for dynamic memory expansion.


<details>
  <summary>Details</summary>
Motivation: Addresses the gap in unsupervised continual learning for videos, as prior work focused on supervised settings requiring labels and task boundaries, which are costly and impractical for real-world video applications.

Method: Uses Kernel Density Estimation of deep embedded video features from unsupervised video transformers, with novelty detection to dynamically expand memory clusters and transfer learning from previous tasks.

Result: Substantially enhances performance when learning multiple tasks successively, validated on UCF101, HMDB51, and Something-to-Something V2 datasets without labels or class boundaries.

Conclusion: Provides a non-parametric solution for unsupervised video continual learning that effectively captures new knowledge across tasks while overcoming computational challenges of video processing.
...
TL;DR: A multi-stage cascaded nnU-Net approach with metric-aware ensembling achieves state-of-the-art PDAC segmentation from MRI, addressing data scarcity and poor contrast challenges.
...
Method: Built on nnU-Net framework with deep multi-stage cascaded pre-training: starting from general anatomical foundation model, sequential fine-tuning on CT pancreatic lesion datasets, then target MRI modalities. Used five-fold cross-validation to evaluate data augmentation schemes and training schedules, followed by metric-aware ensembling of specialist models.

Result: Achieved state-of-the-art boundary precision with MASD of 5.46 mm and HD95 of 17.33 mm for Task 1. Top cross-validation Tumor Dice scores: 0.661 for Task 1 (T1-weighted) and 0.523 for Task 2 (T2-weighted). Found trade-off where aggressive augmentation gives best volumetric accuracy while default augmentations yield superior boundary precision.

Conclusion: The work presents a robust methodology for developing specialized high-performance models in limited data scenarios, demonstrating that custom heterogeneous ensembles of specialist models with metric-aware strategy are highly effective for complex medical imaging tasks like PDAC segmentation.
...
TL;DR: GPT-5 outperforms previous LLMs in radiation oncology benchmarks, achieving 92.8% accuracy on multiple-choice exams and generating clinically useful treatment plans with rare hallucinations, though expert oversight remains necessary.


<details>
  <summary>Details</summary>
Motivation: To evaluate GPT-5's performance in clinical decision support for radiation oncology, specifically assessing its accuracy, comprehensiveness, and potential for hallucinations compared to previous models like GPT-4 and GPT-3.5.

Method: Used two benchmarks: (1) ACR Radiation Oncology In-Training Examination with 300 multiple-choice items, and (2) 60 authentic radiation oncology vignettes rated by four board-certified radiation oncologists for correctness, comprehensiveness, and hallucinations using Fleiss' kappa for inter-rater reliability.

Result: GPT-5 achieved 92.8% accuracy on TXIT benchmark (vs GPT-4's 78.8% and GPT-3.5's 62.1%), with high ratings for correctness (3.24/4) and comprehensiveness (3.59/4) in vignette evaluations. Hallucinations were rare, but inter-rater agreement was low (kappa 0.083).

Conclusion: GPT-5 shows significant improvement over previous models in radiation oncology decision support but still requires rigorous expert oversight due to substantive errors in complex scenarios and variability in clinical judgment assessments.
...
TL;DR: TMUAD introduces a Three-Memory framework using text, object-level, and patch-level memory banks to unify structural and logical anomaly detection, achieving state-of-the-art performance across industrial and medical datasets.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection is challenging due to limited normal data, and existing methods lack effective handling of logical anomalies that require understanding object relationships rather than just structural features.

Method: Proposes a Three-Memory framework: 1) class-level text memory bank for logical anomalies using logic-aware text extractor, 2) object-level image memory bank preserving complete object contours, 3) patch-level memory bank for structural anomalies. These banks retrieve similar normal images and fuse multi-level anomaly scores.

Result: Achieves state-of-the-art performance across seven publicly available datasets involving both industrial and medical domains.

Conclusion: The unified approach using complementary memory banks effectively handles both structural and logical anomalies, demonstrating superior performance across diverse application domains.
...
TL;DR: VoCap is a flexible video model that handles promptable video object segmentation, referring expression segmentation, and object captioning using various input modalities (text, box, mask). It achieves SOTA results on multiple video understanding tasks.
...
Method: Propose VoCap model that consumes video and multimodal prompts, produces spatio-temporal masklets with object-centric captions. Annotate SAV dataset with pseudo captions using VLM preprocessing, creating SAV-Caption dataset. Train on mixed image/video datasets.

Result: State-of-the-art results on referring expression video object segmentation, competitive on semi-supervised video object segmentation, and establishes benchmark for video object captioning.

Conclusion: VoCap provides a unified solution for multiple video understanding tasks with strong performance, and the SAV-Caption dataset enables scalable training for video object captioning.
...
TL;DR: This paper reveals that verb classification in situation recognition is inherently multi-label due to semantic ambiguity, proposes a single positive multi-label learning approach, and introduces GE-VerbMLP with graph networks and adversarial training, achieving over 3% MAP improvement.
...
Method: Proposes to reformulate verb classification as single positive multi-label learning (SPMLL) and develops Graph Enhanced Verb Multilayer Perceptron (GE-VerbMLP) that combines graph neural networks to capture label correlations and adversarial training to optimize decision boundaries.
...
Conclusion: Verb classification in situation recognition is inherently multi-label, and the proposed SPMLL framework with GE-VerbMLP effectively addresses this challenge, demonstrating significant performance improvements.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous driving benchmarks focus on spatial and visual QA tasks but lack comprehensive testing of traffic rule understanding, signage recognition, and edge cases that human drivers must master to pass driving tests.

Method: Created DriveQA benchmark with extensive text and vision-based scenarios covering traffic regulations. Conducted experiments with state-of-the-art LLMs and MLLMs, evaluated performance across categories, and tested fine-tuning effects and environmental factor sensitivity.

Result: LLMs/MLLMs perform well on basic traffic rules but struggle with numerical reasoning, complex right-of-way scenarios, traffic sign variations, and spatial layouts. Fine-tuning improves accuracy, particularly in regulatory sign recognition and intersection decisions. Pretraining enhances downstream task performance on real-world datasets.

Conclusion: DriveQA provides a valuable benchmark for evaluating driving knowledge in AI models. Fine-tuning on comprehensive driving knowledge data significantly improves model performance and generalization capabilities for real-world autonomous driving applications.
