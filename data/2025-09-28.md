...
Motivation: Hallucinations in VLMs undermine reliability, and current detection methods using VLMs are computationally intensive and increase latency. There's a need for efficient, lightweight detection approaches.

Method: Train traditional ML models using signals based on VLM's next-token probabilities (NTPs), which quantify model uncertainty. Augment with linguistic NTPs from generated text and integrate VLM hallucination prediction scores.
...
Conclusion: NTP-based lightweight methods provide simple, efficient solutions for hallucination detection that can enhance VLM reliability without the computational overhead of using VLMs for detection.
...
<details>
  <summary>Details</summary>
Motivation: Offline handwritten signature verification is challenging in writer-independent settings, and existing methods typically require real-world signature datasets for training, which can be limited or unavailable.

Method: Uses Riemannian Gaussian Mixture on SPD matrices to generate synthetic writers and their properties from a small set of genuine samples, then applies metric learning on synthetic SPD point pairs.

Result: Achieves low error rates on both Western and Asian signature datasets under intra- and cross-dataset evaluation protocols.

Conclusion: Demonstrates the potential of generating synthetic data in Riemannian spaces for effective writer-independent signature verification systems.
...
Method: Developed a highly efficient diffusion transformer with a powerful VAE to reduce image tokens, pretrained on billions of text-image pairs, and incorporated multi-modal post-training with a fine-tuned VLM model. Used adversarial distillation, distribution matching, quantization, and speculative decoding for inference acceleration.

Result: Achieves state-of-the-art results on both T2I and multimodal image editing, with inference time of up to 1.8 seconds for generating 2K images. Demonstrates exceptional multimodal capabilities in complex tasks including precise image editing and in-context reasoning.

Conclusion: Seedream 4.0 pushes the boundary of generative AI by extending traditional T2I systems into more interactive and multidimensional creative tools, making it accessible for both creativity and professional applications.
...
<details>
  <summary>Details</summary>
Motivation: Breast cancer is a major global health concern, and early detection is crucial for improving treatment outcomes. While deep learning methods show promise, they struggle with accuracy due to limited labeled datasets for training.

Method: A Contrastive Learning framework using ResNet-50 trained in a semi-supervised approach with similarity index on large unlabeled mammogram data, enhanced with various augmentations and transformations, then fine-tuned on a small labeled dataset.

Result: Achieved 96.7% accuracy in detecting breast cancer on benchmark datasets INbreast and MIAS, outperforming existing state-of-the-art methods.

Conclusion: The proposed Contrastive Learning framework effectively addresses the challenge of limited labeled data in breast cancer detection and demonstrates superior performance compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: To leverage Foundation Models' zero-shot generalization capabilities for automated quality inspection in manufacturing, replacing tedious labeling tasks and enabling cross-product model reuse.

Method: Tested multiple recent Foundation Models on both custom real-world industrial image data and public benchmark datasets using text prompts to describe anomalies.
...
Conclusion: Current Foundation Models show significant limitations when applied to real-world industrial quality inspection scenarios, despite their strong performance on academic benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models are customized for specific tasks, which is inefficient for applications with multiple modular tasks requiring different latent domains.

Method: Encoder-decoder framework that learns transformation-aware, generalizable representations; uses lightweight CNN-based backbone instead of larger transformers.

Result: Multiple vision modules (demosaicing, denoising, depth estimation, semantic segmentation) can be performed efficiently in the shared Neural Space.

Conclusion: The universal Neural Space architecture reduces redundancy, improves generalization across domain shifts, and establishes efficient multi-task vision pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Edge camera systems need regular model updates but have limited computational power, requiring efficient data selection strategies to reduce transmission costs while maintaining model quality.

Method: A high-confidence stream-based strategy combined with a diversity-based approach for selecting training images, focusing on minimizing dataset queries while maintaining training iterations.

Result: The proposed approach produces high-quality models with minimal dataset queries compared to other strategies, achieving similar training load with better efficiency.

Conclusion: Combining high-confidence stream-based selection with diversity-based approaches effectively balances model quality and transmission costs for edge camera systems.
...
<details>
  <summary>Details</summary>
Motivation: Traditional virtual try-on systems require precise binary masks that are difficult to create, model-dependent, and sometimes impossible for complex styling scenarios like changing sleeve positions. The motivation is to simplify user experience and enable more complex styling controls.

Method: Leverages Vision Language Models (VLMs) and image segmentation models to automatically generate binary masks based on user-provided images and free-text style instructions. This enables instruction-following virtual try-on without manual mask creation.

Result: InstructVTON achieves state-of-the-art results with styling control, simplifies the end-user experience by removing mask-drawing requirements, and enables multiple rounds of image generation for complex try-on scenarios that traditional masking-based approaches cannot handle.

Conclusion: The system demonstrates interoperability with existing virtual try-on models and provides a more flexible, user-friendly approach to virtual try-on through natural language instructions and automated mask generation.
...
<details>
  <summary>Details</summary>
Motivation: Altered fingerprint recognition is crucial for security applications like border control and forensics, where adversaries deliberately modify ridge patterns to evade detection, requiring robust recognition systems.

Method: Uses VGG16 backbone for feature extraction and cosine similarity to compare embeddings, evaluated on SOCOFing Real-Altered subset with three difficulty levels (Easy, Medium, Hard).
...
Conclusion: DeepAFRNet addresses limitations of prior work by using real altered samples and per-level metrics, demonstrating readiness for real-world deployments where both security and recognition resilience are critical.
...
<details>
  <summary>Details</summary>
Motivation: To enhance bimanual robotic manipulation by incorporating semantic understanding from pre-trained vision models into 3D voxel-based policies.

Method: Extract attention maps from DINOv2 Vision Transformer, interpret them as pixel-level saliency scores, lift them into 3D voxel grid, and integrate into behavior cloning policy.
...
Conclusion: Attention-guided featurization from pre-trained vision models significantly enhances robotic manipulation performance in voxel-based policies.
...
<details>
  <summary>Details</summary>
Motivation: Blueberry detection in natural environments is challenging due to variable lighting, occlusions, and motion blur. Deep learning detectors need large-scale diverse datasets and proper accuracy/speed/memory trade-offs for practical deployment.

Method: Comparative analysis of 36 model variants from YOLO (v8-v12) and RT-DETR (v1-v2) families on a new blueberry dataset. Models were fine-tuned using Unbiased Mean Teacher-based semi-supervised learning on additional unlabeled images.

Result: YOLOv12m achieved 93.3% mAP@50, RT-DETRv2-X achieved 93.6% mAP@50. After SSL fine-tuning, RT-DETRv2-X reached 94.8% mAP@50. Mid-sized models offered the best accuracy-speed balance.

Conclusion: RT-DETRv2-X performed best overall after semi-supervised learning. More SSL research is needed to better leverage cross-domain unlabeled data. The dataset and software are publicly available for further research.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning for mammogram interpretation is limited by low-resolution datasets and small sample sizes, requiring data-centric approaches to improve performance without additional labels or architectural changes.

Method: Introduces ROI augmentation where full images are probabilistically replaced with random ROI crops from a precomputed bounding-box bank during training, with optional jitter for variability. Evaluated with patient-level cross-validation.

Result: Modest average ROC-AUC gains (best with p_roi=0.10, alpha=0.10), varying performance across folds, flat to slightly lower PR-AUC, while maintaining training-time efficiency and unchanged inference costs.
...
<details>
  <summary>Details</summary>
Motivation: Mirror reflections are common in everyday environments and can provide stereo information within a single capture, as both real and reflected virtual views are visible simultaneously, enabling multi-view stereo from a single image.

Method: The method treats mirror reflections as auxiliary views, designs a transformation to construct physically valid virtual cameras for direct pixel-domain generation of virtual views, and uses a symmetric-aware loss to refine pose estimation by exploiting geometric symmetry introduced by mirrors.

Result: Extensive experiments on real-world and synthetic data demonstrate the method's effectiveness for generalizable and robust 3D reconstruction, with the framework naturally extending to dynamic scenes for efficient per-frame geometry recovery.

Conclusion: The proposed approach enables multi-view stereo from single images with mirrors, simplifying the imaging process while being compatible with powerful feed-forward reconstruction models, with a provided synthetic dataset for quantitative evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Current methods using overhead imagery miss important facade and access cues that determine habitability, while street-view imagery captures these details but is sparse and difficult to align with parcels for occupancy assessment.

Method: A language-guided framework that links panoramic video to parcels, rectifies views to facades, and analyzes interpretable attributes like entry blockage, temporary coverings, and localized debris using two decision strategies: a transparent one-stage rule and a two-stage design separating perception from conservative reasoning.

Result: Evaluated across two post-Hurricane Helene surveys, the two-stage approach achieved precision of 0.927, recall of 0.781, and F-1 score of 0.848, outperforming the one-stage baseline (precision 0.943, recall 0.728, F-1 0.822).

Conclusion: The pipeline provides auditable, scalable occupancy assessments with intermediate attributes and spatial diagnostics that reveal residual errors, enabling targeted quality control and integration into geospatial and emergency-management workflows.
...
Method: Study 1: Direct labeling of animations by participants. Study 2: Measuring representational geometry of 27 social interactions through human similarity judgments and comparing with model predictions based on visual features, labels, and semantic embeddings from descriptions.
...
Method: Proposes EGS framework with: (1) E(2)-Steerable CNN encoder for stable feature extraction under rotation and viewpoint shifts, and (2) graph structure with virtual super-node that connects to all local nodes to aggregate global semantics and redistribute to local regions, enforcing global-local consistency.
...
Conclusion: EGS effectively addresses cross-view geo-localization challenges by combining rotation-invariant feature extraction with global-local consistency modeling, demonstrating superior cross-domain generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Infrared small targets lack distinctive features and blend into noisy backgrounds, creating a conflict between capturing spatial details for small targets and extracting semantic context for larger ones, which existing methods with fixed gradient operators fail to address.
...
Result: The method provides precise infrared small target detection and localization by effectively combining structural semantics and edge refinement, addressing the feature misalignment problem in detecting targets of varying sizes.

Conclusion: The proposed dual-path approach offers a promising solution for infrared small target detection by decoupling edge enhancement and semantic modeling, enabling robust performance in cluttered and noisy backgrounds through unified structural and edge processing.
...
Method: The paper proposes SHOT dataset with 1,979 basketball video clips from 5 camera views and 6 individual attributes, and introduces GIFT framework that extracts individual features and models group dynamics to forecast intention emergence.
...
Conclusion: The work successfully addresses the gap in collective intention recognition by introducing group intention forecasting, providing a comprehensive dataset and framework that enable future research in this emerging field.
...
<details>
  <summary>Details</summary>
Motivation: Maritime object detection faces challenges due to scarce annotated data and poor generalization across different maritime attributes, with models often underperforming in underrepresented scenarios like open-sea environments.

Method: Proposes Neptune-X framework with two key components: 1) X-to-Maritime generative model with Bidirectional Object-Water Attention module for realistic scene synthesis, and 2) Attribute-correlated Active Sampling for dynamic selection of task-relevant synthetic samples.
...
Conclusion: Neptune-X effectively addresses maritime object detection limitations through synthetic data generation and intelligent sample selection, demonstrating substantial improvements in detection performance across diverse maritime scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Crater-Based Navigation has been primarily studied for short-duration powered descent and landing missions, but lunar mapping missions involve sparse, oblique imagery under varying illumination over year-long campaigns, posing greater challenges for pose estimation.
...
Conclusion: This work provides the first comprehensive assessment of Crater-Based Navigation in a true lunar mapping setting and informs operational conditions for future missions.
...
Method: Systematic investigation of representational alignment across vision-only and language-only models, including analysis of layer-wise alignment, robustness tests, forced-choice "Pick-a-Pic" tasks, and examination of exemplar averaging effects.

Result: Alignment peaks in mid-to-late layers, is robust to appearance changes but collapses with semantic alterations, mirrors human preferences in image-caption matching, and is amplified by embedding averaging across exemplars.
...
Method: Convert 2D objects to 3D using existing 3D generation models, perform interactive editing in 3D space, then re-render to 2D from specified views. Combine geometric control with style and content control through diffusion adapters.

Result: The framework produces geometrically controlled and style-consistent edited images without requiring training.

Conclusion: FreeInsert successfully addresses geometric control and style consistency challenges in personalized image composition through a 3D-based training-free approach.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models for human photo synthesis face degraded scenes, insufficient control, and suboptimal perceptual identity, requiring better solutions for realistic human photo generation.

Method: Uses triple-flow fused PerGeneration approach to combine counter-directional latent spaces, leverages face swapping and pretrained diffusion models for zero-shot enhancement, and introduces ResInversion for noise rectification to reduce inversion time.

Result: Achieves state-of-the-art results in scene diversity, identity fidelity, and training-free controls, with ResInversion reducing inversion time by 129 times compared to null-text inversion.

Conclusion: CustomEnhancer provides an effective framework for enhancing identity customization models with improved performance and efficiency, enabling precise controlled personalization without controller retraining.
...
<details>
  <summary>Details</summary>
Motivation: With increasing use of NN-based computer vision applications, there's a need for a consolidated platform to implement and evaluate compression methods optimized for downstream vision tasks across various models and datasets.

Method: The platform provides a comprehensive evaluation framework where new coding tools compete to efficiently compress vision network inputs while retaining task accuracy in 'remote' and 'split' inference scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Address practical challenge where domain shifts occur between labeled and unlabeled training data in addition to shifts between training and testing sets, which existing SSDG methods handle poorly due to inaccurate pseudolabels.

Method: Dual-supervised asymmetric co-training (DAC) framework with two sub-models providing cross pseudo supervision, plus feature-level supervision and asymmetric auxiliary self-supervised tasks to handle domain shifts and prevent model collapse.
...
Conclusion: DAC framework effectively addresses the challenging CD-SSDG scenario by integrating feature-level supervision and asymmetric auxiliary tasks to handle domain shifts and improve generalization performance.
...
<details>
  <summary>Details</summary>
Motivation: To extend the successful DEIM framework with DINOv3 features and create a unified design that achieves superior performance-cost trade-off across diverse deployment scenarios from GPU to mobile devices.

Method: For larger models (X, L, M, S): DINOv3-pretrained/distilled backbones with Spatial Tuning Adapter (STA) to convert single-scale to multi-scale features. For ultra-lightweight models (Nano, Pico, Femto, Atto): HGNetv2 with depth/width pruning, simplified decoder, and upgraded Dense O2O.
...
Conclusion: DEIMv2 establishes new state-of-the-art results across all model scales with superior performance-cost trade-off, making it suitable for diverse deployment scenarios from GPU to mobile devices.
...
Conclusion: DAC-LoRA presents an effective method that can be easily integrated into standard PEFT pipelines to significantly enhance robustness of Vision-Language Models against adversarial attacks.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated domain generalization methods based on prompt learning have limited diversity and ignore information from unknown domains, posing challenges for downstream-task adaptation in federated learning settings with domain shift among clients.

Method: The proposed FedDSPG introduces domain-specific soft prompts (DSPs) for each domain, integrates content and domain knowledge into a generative model among clients during training, and uses the generator to obtain DSPs for unseen target domains during inference.
...
Conclusion: FedDSPG provides an effective generative approach for federated domain generalization that successfully handles unknown domains through domain-specific soft prompt generation, demonstrating superior performance compared to existing methods.
...
Method: Leverages contrastive language-image pretraining with vision encoders (ResNet-50, Vision Transformer, Swin Transformer) and BERT-based text encoder, projecting representations into shared embedding space using learnable projection heads (linear or non-linear) with soft CLIP loss.
...
Conclusion: LumbarCLIP offers a promising foundation for automated musculoskeletal diagnosis and clinical decision support through effective multimodal alignment of medical images and text.
...
<details>
  <summary>Details</summary>
Motivation: While vulnerabilities have been identified in earlier frame sampling strategies for VideoLLMs, the safety of the most recent prompt-guided sampling approach remains unexplored, creating a critical research gap.
...
Conclusion: The high success rate of PoisonVID highlights the importance of developing more robust and secure advanced sampling strategies for future VideoLLMs.
...
Conclusion: GoR provides a robust solution for real-world deployment of compressed AI models, with EKD-GoR potentially outperforming full-precision models under optimal conditions.
...
<details>
  <summary>Details</summary>
Motivation: To assess if large-scale web-sourced training data with labeling errors can perform as well as smaller expert-curated datasets for continental-scale plant identification, addressing the challenge of limited expert-verified plant images.

Method: Used two training strategies: large noisy web-collected dataset vs smaller expert-verified dataset, with testing on a third independent dataset from Pl@ntNet mobile application to ensure fair comparison.

Result: The challenge provided evaluation of different approaches from participating research groups, comparing performance between web-sourced and expert-curated training data strategies.

Conclusion: The study represents a milestone in automated plant identification at continental scale, exploring the trade-offs between dataset size and quality in deep learning-based plant recognition systems.
...
<details>
  <summary>Details</summary>
Motivation: Plants have high biodiversity with new cultivars bred each year, making it impossible to build species-dependent counting models for all plants. Current class-agnostic counting and open-world detection models perform poorly on plants due to their dynamic, non-rigid structure that changes with time and space.

Method: TasselNetV4 combines TasselNet's local counting approach with the extract-and-match paradigm from class-agnostic counting. It uses a plain vision transformer with novel multi-branch box-aware local counters to enhance cross-scale robustness.

Result: Extensive experiments on two challenging datasets (PAC-105 and PAC-Somalia) show TasselNetV4 achieves superior counting performance and high efficiency compared to state-of-the-art class-agnostic counting models.

Conclusion: TasselNetV4 emerges as a vision foundation model for cross-scene, cross-scale, and cross-species plant counting, effectively addressing the limitations of species-specific approaches.
...
Method: Proposes a semi-supervised model with a fully differentiable biomarker topology engine that enables joint learning with bidirectional influence between layers and lesions, using disentangled representations separating spatial and style factors.

Result: Outperforms current state-of-the-art in both lesion and layer segmentation on public and internal OCT datasets, and demonstrates ability to generalize layer segmentation to pathological cases using partially annotated data.

Conclusion: The approach demonstrates the potential of using anatomical constraints in semi-supervised learning for accurate, robust, and trustworthy retinal biomarker segmentation.
...
<details>
  <summary>Details</summary>
Motivation: To create realistic biodiversity monitoring conditions by testing plant identification at very large scale with real-world data from participatory sensing, and specifically address the challenge of open-set recognition where systems must handle unknown species.

Method: Used a dataset of over 110,000 images covering 1000 plant species from West Europe, collected through participatory sensing since 2011. Evaluated systems on open-set recognition requiring automatic rejection of false positives from unknown categories.

Result: The challenge successfully tested plant identification methods at unprecedented scale with real-world data, evaluating approaches for handling unknown species in open-set recognition scenarios.

Conclusion: The LifeCLEF 2016 challenge advanced plant identification research by providing large-scale realistic evaluation conditions and specifically addressing the critical open-set recognition problem for biodiversity monitoring applications.
...
Method: Uses pre-trained visual language model to generate captions, then summarizes and reorders them to remove irrelevant information, and generates contextual examples to enhance LLM understanding without expensive training.

Result: Achieved 38.8% accuracy on OK-VQA and 34.6% on A-OKVQA using a 6.7B parameter LLM, demonstrating excellent performance on challenging KB-VQA datasets.

Conclusion: SCRA-VQA effectively enhances LLMs' reasoning ability and task adaptability in KB-VQA through caption summarization and reranking, achieving state-of-the-art results without costly end-to-end training.
...
<details>
  <summary>Details</summary>
Motivation: To systematically analyze the correlation between effectiveness of fidelity/adversarial objectives as optimization goals versus their capability as image quality assessment metrics, which remains underexplored.

Method: Conducted systematic analysis of perceptual optimization and assessment, examining fidelity metrics, adversarial training, discriminator reuse for IQA models, and different discriminator architectures (patch-level, convolutional, vanilla, Transformer-based).

Result: Revealed asymmetry between perceptual optimization and assessment - fidelity metrics effective for IQA aren't necessarily good for optimization, especially under adversarial training. Discriminator designs (patch-level and convolutional) provide better detail reconstruction than vanilla or Transformer alternatives.

Conclusion: Discriminator design plays decisive role in optimization quality, and there's limited transferability between optimization objectives and IQA metrics, suggesting need for more principled approaches to perceptual optimization.
...
Method: Integrates Object Interaction Self-Attention to capture complex object interactions within images, and GAN-Based Debiasing to generate unbiased data distributions for learning robust features.

Result: Extensive experiments on VQA-CP v1 and VQA-CP v2 datasets demonstrate excellent performance compared to existing methods, particularly in handling biased and imbalanced data distributions.

Conclusion: Addressing both object interactions and dataset biases is crucial for advancing VQA tasks, and IOG-VQA effectively combines visual and textual information to overcome inherent dataset biases.
...
<details>
  <summary>Details</summary>
Motivation: Traditional robust principal component analysis (RPCA) methods fail to capture rich variability in real video data due to their sparsity assumption, and video sequences often contain structured noise and background artifacts that obscure dynamic content.
...
Conclusion: FerretNet provides efficient and robust synthetic image detection using minimal parameters and focusing on fundamental generation artifacts.
...
<details>
  <summary>Details</summary>
Motivation: Extend interpretable concept bottleneck models from static images to video data to handle temporal dependencies essential for capturing actions and events in videos.

Method: Introduces MoTIF, a transformer-inspired architecture that adapts concept bottleneck framework for video classification, handling sequences of arbitrary length and enabling analysis of concept motifs across time.

Result: Demonstrates that concept-based modeling can be effectively transferred to video data, maintaining competitive performance while enabling better understanding of concept contributions in temporal contexts.

Conclusion: The MoTIF framework successfully extends interpretable concept modeling to video classification, providing three complementary perspectives for analyzing concepts across time while maintaining performance.
...
<details>
  <summary>Details</summary>
Motivation: Safety-critical surveillance tasks like drone-based wildfire monitoring require 3D object localization, but existing methods (dense depth estimation, 3D scene reconstruction) fail with distant objects or limited computational resources.

Method: Particle filters for both single and multiple target scenarios, using camera poses and image segments, validated through 3D simulation and drone-based image segmentation sequences with GNSS-based camera pose estimates.

Result: Particle filters successfully solved practical localization tasks where other methods failed, demonstrating effectiveness for drone-based wildfire monitoring when paired with existing image segmentation models.

Conclusion: Particle filters provide a flexible, detection-method-independent solution for 3D object localization in resource-constrained scenarios, enabling practical drone-based monitoring applications like wildfire detection.
...
<details>
  <summary>Details</summary>
Motivation: Vision Mamba has global receptive field and low computational complexity but tends to overlook critical local features like textures and edges, which are essential for accurate segmentation in remote sensing contexts.

Method: SwinMamba integrates localized Mamba-style scanning within shifted windows with global receptive field. The first two stages perform local scanning for fine-grained details, while subsequent stages leverage global scanning for broader contextual information. Overlapping shifted windows enhance inter-region information exchange.
...
Conclusion: SwinMamba shows effectiveness and potential as a superior solution for semantic segmentation of remotely sensed imagery by balancing local and global feature perception.
...
<details>
  <summary>Details</summary>
Motivation: Whole slide images in computational pathology have extremely long and variable sequence lengths (200-200K), leading to high data heterogeneity and redundancy, while conventional methods struggle with training efficiency and optimization under limited supervision.

Method: Proposes a pack-based MIL framework that packs multiple variable-length feature sequences into fixed-length ones, includes a residual branch that composes discarded features into hyperslides with tailored labels, and uses an attention-driven downsampler to reduce feature redundancy.
...
Conclusion: Addressing data challenges in computational pathology through the proposed framework shows significant potential in the era of foundation models, offering efficient training while preserving data heterogeneity.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for generating physically plausible human motion use simulator-based motion projection layers that are computationally expensive due to their sequential nature and inability to be parallelized, limiting their efficiency.

Method: The paper proposes SimDiff, which interprets simulator-based motion projection as guidance in diffusion processes and integrates environment parameters (gravity, wind) directly into the denoising process, avoiding repeated simulator calls during inference.
...
Conclusion: SimDiff offers an efficient alternative to simulator-based approaches by embedding physical constraints directly into the diffusion model, enabling fast generation of physically plausible motions with fine-grained environmental control.
...
<details>
  <summary>Details</summary>
Motivation: To understand why certain vision architectures are inherently more robust to Gaussian noise and convert empirical insights into actionable design rules for building more robust models.

Method: Extensive evaluation of 1,174 pretrained vision models to identify robust design patterns, followed by theoretical analysis to explain the observed correlations through causal mechanisms including kernel analysis, downsampling effects, pooling comparisons, and Lipschitz bounds.

Result: Identified four consistent design patterns: larger stem kernels, smaller input resolutions, average pooling, and supervised ViTs over CLIP ViTs, yielding up to 506 rank improvements and 21.6% accuracy gains. Provided theoretical proofs showing how these design choices attenuate noise and reduce sensitivity.
...
<details>
  <summary>Details</summary>
Motivation: Scene graphs provide structured relational representations crucial for decoding complex, dynamic surgical environments, enabling intelligent systems to improve surgical safety, efficiency, and training.
...
Result: Revealed rapid growth with a critical 'data divide', methodological advancement from foundational graph neural networks to specialized foundation models that outperform generalist large vision-language models in surgical contexts.

Conclusion: Surgical scene graphs are maturing into an essential semantic bridge enabling a new generation of intelligent systems, though challenges in data annotation and real-time implementation persist but are being actively addressed.
...
Method: Three-component approach: (1) preprocessing with Laplacian edge detection and K-means clustering for segmentation, (2) synthetic data augmentation via StyleGAN2, (3) UFlow-based neural network for multi-scale feature extraction and anomaly map generation using unsupervised anomaly detection.

Result: 93.8% accuracy on defective samples, 89.3% accuracy on good samples, image-level AUROC of 0.957, pixel-level AUROC of 0.961 on 366 real sensor images, with 0.5 seconds processing time per image.

Conclusion: The system enables automated quality control with high accuracy for detecting both known and novel defect types, providing significant cost savings through efficient on-device implementation.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of extracting insights from lengthy (30-40 minute) multimodal financial advisory podcast videos on social media, which have broad reach but are difficult to process efficiently.

Method: Uses BLIP for visual descriptions, OCR for text patterns, Whisper-based transcription with speaker diarization, modified DPO-based loss with fact-checking, and ranker-based retrieval for aligning keyframes with summarized content.

Result: FASTER demonstrates strong performance, robustness, and generalizability compared to LLMs and VLMs in cross-domain experiments, and introduces the Fin-APT dataset of 470 financial advisory videos.
...
<details>
  <summary>Details</summary>
Motivation: Existing SSL-integrated deep clustering methods require pretraining, clustering learning, or trained models as prerequisites, limiting flexible out-of-box application of SSL learners in image clustering tasks.

Method: Randomly sample pseudo-labeled data, use instance-level classifier to learn with semantically aligned labels, track class transitions to extract high-level similarities, assign cluster-level labels, and trigger SSL learner on unlabeled data.

Result: Superior performance across benchmarks against latest deep image clustering approaches, with only 1.33% accuracy gap on CIFAR-10 compared to SSL methods using ground-truth. Can also boost existing SSL-embedded methods.

Conclusion: ASD enables effective cold-start SSL for image clustering without prerequisites, achieving competitive performance and enhancing existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers produce high-norm artifacts that degrade representation quality, and when these features are distilled to students, artifacts dominate the objective, causing students to overfit to artifacts and underweight informative signals.

Method: SiNGER uses singular nullspace-guided energy reallocation with a LoRA-based adapter to refine teacher features, leveraging nullspace-guided perturbation to preserve information while suppressing artifacts during distillation.
...
Method: Builds on SEnSeI-v2 with improved spectral descriptor, lightweight architecture, and robust padding-band handling. Accepts arbitrary band combinations and wavelengths, feeding into a compact, quantized U-Net-based segmentation model deployed via CPU-FPGA hybrid pipeline using Apache TVM.

Result: Accurate cloud segmentation demonstrated on Sentinel-2 and Landsat 8 datasets across diverse input configurations.
...
TL;DR: SNCE is a single neuron-based concept erasure method that precisely prevents harmful content generation in text-to-image models by manipulating only one neuron responsible for harmful concepts, achieving surgical precision with minimal image quality degradation.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models pose safety risks by generating harmful content, and existing concept erasure methods struggle with precise removal of target concepts while maintaining image quality.

Method: Train a Sparse Autoencoder to map text embeddings into sparse latent space, identify harmful concept-specific neurons using modulated frequency scoring of activation patterns, and suppress activations of those neurons.

Result: Achieves state-of-the-art results in target concept erasure while preserving generation capabilities for non-target concepts, and exhibits strong robustness against adversarial attacks.

Conclusion: SNCE provides an effective and precise approach for concept erasure in text-to-image models with minimal impact on overall image quality and generation capabilities.
...
TL;DR: Proposes KD-SS, a lightweight sub-sampling algorithm for plant point clouds that preserves full resolution and works across different plant species and sensor modalities, enabling segmentation without down-sampling.


<details>
  <summary>Details</summary>
Motivation: Existing plant point cloud segmentation methods are species/sensor-specific and require extensive pre-processing and down-sampling, which loses resolution and limits applicability.
...
Conclusion: KD-SS provides a lightweight, resolution-retaining alternative to intensive pre-processing and down-sampling methods for plant organ segmentation, regardless of species or sensor modality.
...
TL;DR: Mambo is a new FG-BG decomposition framework for few-shot OOD detection that addresses over-reliance on local class similarity and fixed background patch extraction strategies by learning background prompts and using patch self-calibrated tuning.
...
Method: Proposes to learn a background prompt to obtain local background similarity containing both background and image semantic information, then refine it using local class similarity. Uses patch self-calibrated tuning to flexibly select background patches for different samples.

Result: Extensive experiments on real-world datasets demonstrate that Mambo achieves the best performance compared to SOTA methods in terms of OOD detection and near OOD detection settings.

Conclusion: Mambo effectively addresses the limitations of previous methods and achieves superior performance in FS-OOD detection tasks.
...
TL;DR: IPS and WDES are novel stratification methods for image segmentation datasets that create more representative splits than random sampling, reducing evaluation bias and improving model generalization.
...
Method: Two approaches: Iterative Pixel Stratification (IPS) - a label-aware sampling method, and Wasserstein-Driven Evolutionary Stratification (WDES) - a genetic algorithm that minimizes Wasserstein distance to optimize label distribution similarity across splits.

Result: WDES consistently produces more representative splits with lower performance variance across diverse segmentation tasks (street scenes, medical imaging, satellite imagery), proving particularly valuable for small, imbalanced, and low-diversity datasets.

Conclusion: WDES is globally optimal and significantly improves dataset splitting for segmentation tasks, especially in challenging scenarios where conventional methods are most prone to bias.
...
TL;DR: EnGraf-Net uses semantic hierarchy (taxonomy) as supervised signals in an end-to-end deep neural network for fine-grained classification, achieving competitive performance without requiring cropping or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Existing fine-grained classification models rely on part annotations or automatic cropping, which suffer from incomplete representation of local features needed to distinguish similar objects. Humans recognize objects through semantic associations, suggesting that leveraging semantic hierarchies could improve classification.

Method: Proposed EnGraf-Net, an end-to-end deep neural network that uses semantic associations structured as a hierarchy (taxonomy) as supervised signals, eliminating the need for cropping techniques or manual annotations.

Result: Extensive experiments on CIFAR-100, CUB-200-2011, and FGVC-Aircraft datasets show EnGraf-Net achieves competitive performance with state-of-the-art approaches without requiring cropping or annotations.

Conclusion: Semantic hierarchy can be effectively used as supervised signals for fine-grained classification, providing competitive performance while eliminating the need for manual annotations or cropping techniques that may lead to incomplete feature representations.
...
TL;DR: Vision Transformers (ViTs) show varying vulnerability to adversarial patch attacks, with success rates ranging from 40.04% to 99.97% across different models, confirming transferability of CNN attack techniques to ViTs.
...
Method: Used Creases Transformation (CT) technique to design realistic adversarial patches that add subtle geometric distortions, and evaluated transferability of CNN attack techniques on four fine-tuned ViT models for binary person classification.

Result: Significant vulnerability variations observed: attack success rates ranged from 40.04% (google/vit-base-patch16-224-in21k) to 99.97% (facebook/dino-vitb16), with other models achieving 66.40% and 65.17% success rates.

Conclusion: Adversarial patches are transferable from CNNs to ViTs, and pre-training dataset scale and methodology strongly influence model resilience to adversarial attacks.
...
<details>
  <summary>Details</summary>
Motivation: To achieve precise and controllable video concept transfer by decomposing videos into key components and leveraging progressive generation guidance, addressing limitations in existing video editing methods.

Method: Proposes spatial decomposition into foreground, background, and motion flow; uses dual-to-single-stream DiT architecture; self-supervised pretraining with random masking; Chain-of-Prompt mechanism for timestep decomposition; and creates OpenAnimal dataset for benchmarking.
...
Conclusion: UniTransfer demonstrates effective video concept transfer through spatial and temporal decomposition, with the proposed methods enabling fine-grained control and superior performance compared to existing approaches.
...
TL;DR: VTTS enhances multimodal LLMs' reasoning through iterative perception during inference, mimicking human hierarchical attention by progressively refining focus on high-confidence regions guided by updated predictions.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM methods are limited by static perception stages, lacking the dynamic iterative reasoning capability that humans employ for complex multimodal understanding.

Method: VTTS uses Iterative Perception (ITP) mechanism with reinforcement learning and spatio-temporal supervision to progressively refine visual focus based on textual predictions, supported by the VTTS-80K dataset.

Result: Videochat-R1.5 model achieved average 5%+ improvements over Qwen2.5VL baselines across 15+ benchmarks covering video conversation, reasoning, and spatio-temporal perception.

Conclusion: VTTS enables MLLMs to enhance performance by increasing perceptual compute through iterative perception, demonstrating strong generalization across diverse multimodal tasks.
...
TL;DR: Mammo-CLIP Dissect is a concept-based explainability framework that uses a mammography-specific vision-language model to analyze what deep learning models learn in mammography, revealing differences between general vs domain-specific training and trade-offs in concept learning during fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Understanding what DL models learn is essential for safe AI deployment in clinical settings, particularly to move beyond pixel-based explanations and capture textual concepts that better reflect clinical reasoning.

Method: Leverages Mammo-CLIP as a "dissector" to label neurons with human-interpretable textual concepts and quantify their alignment to domain knowledge, investigating concept learning differences across training data and fine-tuning regimes.

Result: Models trained on mammography data capture more clinically relevant concepts and align better with radiologists' workflows. Fine-tuning enhances capture of certain concepts (benign calcifications) but reduces coverage of others (density-related features), showing a trade-off between specialization and generalization.

Conclusion: Mammo-CLIP Dissect provides insights into how CNNs capture mammography-specific knowledge and reveals how domain-specific training and task adaptation shape concept learning, with code and concept set publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLMs suffer from process inconsistency where intermediate reasoning drifts from video dynamics even when final answers are correct, undermining interpretability and robustness.

Method: Reinforcement learning framework with Dynamic Time Warping (DTW)-based process reward that aligns reasoning traces with temporally grounded references, enabling efficient process supervision without auxiliary reward models.

Result: Achieves 87.2% on MOSS-Video benchmark, improves performance on MVBench and MMVU, and produces more consistent reasoning traces across different architectures including Qwen2.5-VL and Phi-2.

Conclusion: The DTW-based process reward framework effectively addresses process inconsistency in video reasoning and demonstrates broad applicability across different model architectures.
...
TL;DR: A novel video generation approach that integrates camera and object motions by converting them into pixel motion maps, using stable diffusion and image-to-video networks to create consistent videos following specified camera trajectories.
...
Method: Proposes integrating camera and object motions by converting them into pixel motion maps, using stable diffusion to learn reference motion maps relative to camera trajectory, and feeding these along with semantic object prior into image-to-video network.

Result: Extensive experiments show the model significantly outperforms state-of-the-art methods by a large margin.

Conclusion: The proposed approach effectively addresses the challenge of generating consistent videos with both camera and object motions by integrating them into unified pixel motion representation.
...
TL;DR: This paper analyzes when discriminators struggle most against image generators, finding that both very simple and highly complex datasets reduce synthetic image detectability, while intermediate-complexity datasets are most favorable for detection.


<details>
  <summary>Details</summary>
Motivation: The rapid progress of image generative AI has blurred the boundary between synthetic and real images, creating an arms race between generators and discriminators. The paper aims to understand the conditions where discriminators are most disadvantaged.

Method: The researchers analyze two key factors: data dimensionality and data complexity. They use Kolmogorov complexity as a measure of intrinsic dataset structure to study how complexity affects detectability of synthetic images.
...
Conclusion: The detectability of synthetic images depends on dataset complexity, with intermediate-complexity datasets being optimal for discriminators, as generators fail to fully capture the distribution and their errors remain visible.
...
TL;DR: WAVECLIP is a unified model for adaptive resolution inference in CLIP using wavelet-based tokenization, enabling efficient multi-resolution processing with dynamic compute-accuracy trade-offs.


<details>
  <summary>Details</summary>
Motivation: To enable adaptive resolution inference in CLIP models, allowing dynamic compute-accuracy trade-offs using a single deployed model rather than multiple specialized models.

Method: Replaces standard patch embeddings with multi-level wavelet decomposition, processes images coarse to fine, uses key-value caching and causal cross-level attention to reuse computation, and employs confidence-based gating for adaptive early exits.

Result: Achieves competitive accuracy with significant computational savings in zero-shot classification, requiring only lightweight distillation from a frozen CLIP teacher.

Conclusion: WAVECLIP provides an efficient approach for adaptive resolution inference that enables users to dynamically balance computation and accuracy using a single model.
...
Result: Quantization consistently improves calibration for underconfident pre-trained models but degrades it for overconfident ones. However, OOD detection can still improve even for poorly calibrated models. Specific QAT methods yield simultaneous gains in zero-shot accuracy, calibration, and OOD robustness.

Conclusion: Quantization can be utilized beyond conventional efficiency-performance trade-offs to deploy efficient, reliable, and robust vision-language models, offering critical insights for multi-objective optimization in model deployment.
...
Method: Created TABLET dataset with 4 million examples from 2 million unique tables (88% preserving original visualizations), including paired image-HTML representations, comprehensive metadata, and provenance information linking to source datasets.
...
TL;DR: A conformal prediction-based method for controlling explanation fidelity in feature attribution, identifying sufficient salient features to preserve model predictions without requiring ground-truth explanations.


<details>
  <summary>Details</summary>
Motivation: Feature attribution methods vary in robustness and may not faithfully reflect the underlying model's reasoning, requiring better fidelity control in explanations.

Method: Proposes four conformity functions to quantify explanation fidelity, identifies sufficient salient features using conformal prediction without needing ground-truth explanations for calibration.
...
Conclusion: The conformal prediction approach successfully enables direct control over explanation fidelity and identifies efficient explanation regions, with super-pixel measures proving superior to pixel-level analysis.
...
TL;DR: Sigma is a unified skeleton-based sign language understanding framework that addresses semantic grounding, local-global balance, and cross-modal learning challenges through sign-aware early fusion, hierarchical alignment, and unified pre-training, achieving SOTA results on multiple SLU tasks.


<details>
  <summary>Details</summary>
Motivation: Current skeleton-based SLU methods face three key limitations: weak semantic grounding (struggle to relate skeletal motions to linguistic meaning), imbalance between local details and global context, and inefficient cross-modal learning for semantically aligned representations.

Method: Proposes Sigma framework with: 1) sign-aware early fusion for deep visual-textual interaction, 2) hierarchical alignment learning to capture both fine-grained details and high-level semantics across modalities, and 3) unified pre-training combining contrastive learning, text matching and language modeling.
...
TL;DR: A comprehensive study evaluating how well automated metrics for text-image generation align with human judgments, revealing that no single metric performs consistently across different compositional tasks.


<details>
  <summary>Details</summary>
Motivation: Current text-image generation evaluation relies heavily on automated metrics that are often adopted by convention rather than being validated against human judgment, making it critical to understand their alignment with human preferences.

Method: Conducted a broad study examining widely used metrics across diverse compositional challenges, comparing different metric families (VQA-based, embedding-based, image-only) and their correlation with human judgments.

Result: No single metric performs consistently across tasks - performance varies with compositional problem type. VQA-based metrics are not uniformly superior, while certain embedding-based metrics perform better in specific cases. Image-only metrics contribute little to compositional evaluation.

Conclusion: Careful and transparent metric selection is crucial for trustworthy evaluation and their use as reward models in generation, as metric performance depends on the specific compositional task.
...
TL;DR: SlideMamba integrates Mamba architecture with Graph Neural Networks for Whole Slide Image analysis, using entropy-based adaptive fusion to balance local and global information, achieving superior performance in predicting gene fusion/mutation status.
...
Conclusion: The integrated architecture with entropy-based adaptive fusion strategy demonstrates strong performance and promising potential for spatially-resolved predictive modeling tasks in computational pathology.
...
TL;DR: Hunyuan3D-Omni is a unified framework for fine-grained, controllable 3D asset generation that accepts multiple input modalities (images, point clouds, voxels, bounding boxes, skeletal pose) in a single cross-modal architecture, enabling precise control over geometry, topology, and pose.
...
Method: Built on Hunyuan3D 2.1, the model unifies all conditioning signals in a single cross-modal architecture and uses progressive, difficulty-aware sampling that biases toward harder signals (e.g., skeletal pose) while downweighting easier ones (e.g., point clouds).
...
Conclusion: Hunyuan3D-Omni addresses the controllability gap in 3D asset generation by providing fine-grained cross-modal controls through a unified framework that handles multiple input modalities effectively.
...
TL;DR: A framework that uses vision-language models to automatically generate semantic attention maps via natural language prompts, aligning CNN attention through an auxiliary loss to improve model reliability and cognitive plausibility without manual annotations.
...
Method: Leverage vision-language models to automatically generate semantic attention maps using natural language prompts, and introduce an auxiliary loss that aligns CNN attention with these language-guided maps.

Result: Achieves state-of-the-art performance on ColorMNIST and remains competitive with annotation-heavy baselines on DecoyMNIST, demonstrating improved generalization, reduced shortcut reliance, and attention that better reflects human intuition.
...
<details>
  <summary>Details</summary>
Motivation: MRI complexity and heterogeneity challenge automated analysis, and existing foundation models are limited in MRI applications due to data scarcity and narrow anatomical focus.
...
TL;DR: SQ-InstructBLIP improves vision-language reasoning by generating image-aware sub-questions and answers through a Questioner-Answerer-Reasoner framework, outperforming previous methods on VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with multi-step reasoning, and using black-box LLMs for iterative sub-question generation has limitations including lack of fine-grained visual content access and reproducibility issues.

Method: Proposed SQ-InstructBLIP with three components sharing the same architecture: Questioner generates image-aware sub-questions, Answerer generates sub-answers, and Reasoner performs final reasoning using the generated sub-question information.

Result: Experiments show SQ-InstructBLIP performs more accurate reasoning than previous works on VQA tasks by using generated sub-questions as additional information.

Conclusion: The proposed method effectively addresses multi-step reasoning challenges in vision-language understanding by generating image-aware sub-questions and answers, demonstrating improved performance over existing approaches.
...
TL;DR: The paper defines hallucination in text-to-image models as bias-driven deviations and proposes a taxonomy with three categories: attribute, relation, and object hallucinations.


<details>
  <summary>Details</summary>
Motivation: Hallucination has been studied in language and vision-language models but not clearly framed for text-to-image generative models. Existing evaluations focus on alignment but overlook what models generate beyond the prompt.
...
Conclusion: The proposed taxonomy provides a foundation for richer assessment of text-to-image models by systematically categorizing hallucination types.
...
TL;DR: Proposed a Person Independence Universal Micro-action Recognition Framework with Distributionally Robust Optimization to learn person-agnostic representations, addressing inter-person variability in micro-action recognition.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail in real-world scenarios due to inter-person variability causing the same action to manifest differently, hindering robust generalization.

Method: Two plug-and-play components: Temporal-Frequency Alignment Module (temporal branch with Wasserstein-regularized alignment and frequency branch with variance-guided perturbations) and Group-Invariant Regularized Loss (pseudo-group partitioning with boundary case up-weighting and subgroup variance regularization).
...
Conclusion: The framework successfully addresses inter-person variability in micro-action recognition through person-agnostic representations and robust optimization principles.
...
TL;DR: The paper proposes a method to improve semantic matching by leveraging 3D geometric foundation models (VGGT) to address geometric ambiguity and nearest-neighbor limitations in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing semantic matching methods suffer from geometric ambiguity due to reliance on 2D foundation model features that fail to disambiguate symmetric structures, and nearest-neighbor matching that ignores cross-image invisibility and manifold preservation.

Method: The approach adapts VGGT for semantic matching by: (i) reusing early feature stages, fine-tuning later ones, and adding a semantic head for bidirectional correspondences; (ii) implementing cycle-consistent training, synthetic data augmentation, and progressive training with aliasing artifact mitigation.
...
Conclusion: The proposed method successfully addresses key limitations in semantic matching by leveraging 3D geometric foundation models and adapting them effectively for cross-instance semantic matching under data scarcity.
...
TL;DR: MedVSR is a specialized video super-resolution framework for medical videos that addresses challenges like camera shake, noise, and frame transitions through Cross State-Space Propagation for alignment and Inner State-Space Reconstruction for artifact reduction.


<details>
  <summary>Details</summary>
Motivation: High-resolution medical videos are crucial for accurate diagnosis but hard to acquire due to hardware limitations and physiological constraints. Current VSR models struggle with medical video challenges like camera shake, noise, abrupt frame transitions, and often introduce artifacts that can mislead doctors.

Method: Proposes MedVSR with two key components: 1) Cross State-Space Propagation (CSSP) that projects distant frames as control matrices to selectively propagate consistent features for alignment, and 2) Inner State-Space Reconstruction (ISSR) that enhances tissue structures through joint long-range spatial feature learning and large-kernel short-range information aggregation.
...
Conclusion: MedVSR provides an effective solution for medical video super-resolution, addressing the unique challenges of medical videos and achieving superior performance compared to existing methods.
...
TL;DR: This paper addresses limitations in multimodal reasoning models by proposing Variance-Aware Sampling (VAS) to stabilize RL fine-tuning, releasing large-scale CoT data and RL QA pairs, and open-sourcing multimodal reasoning models.


<details>
  <summary>Details</summary>
Motivation: Large multimodal reasoning models face two major constraints: lack of open, large-scale, high-quality long chain-of-thought data, and instability of reinforcement learning algorithms in post-training due to gradient vanishing when reward variance is low.

Method: Proposes Variance-Aware Sampling (VAS) guided by Variance Promotion Score (VPS) that combines outcome variance and trajectory diversity to promote reward variance and stabilize policy optimization. Also releases curated resources including ~1.6M long CoT data and ~15k RL QA pairs.

Result: Experiments across mathematical reasoning benchmarks demonstrate effectiveness of both curated data and VAS. Comprehensive ablation studies provide insight into component contributions. Theoretically establishes that reward variance lower-bounds expected policy gradient magnitude.

Conclusion: The work provides practical solutions to RL fine-tuning instability through VAS, contributes valuable data resources to the community, and establishes standardized baselines for multimodal reasoning research.
...
TL;DR: A new foundation model using Prithvi-EO Vision Transformer was pre-trained on Sentinel-3 OLCI data and fine-tuned for marine monitoring tasks including chlorophyll concentration quantification and ocean primary production estimation.


<details>
  <summary>Details</summary>
Motivation: To leverage AI foundation models for ocean science applications where labeled data is sparse and expensive to collect, enabling better use of small amounts of high-quality labeled data.

Method: Used Prithvi-EO Vision Transformer architecture pre-trained on Sentinel-3 OLCI data, then fine-tuned on downstream tasks including chlorophyll concentration quantification and ocean primary production estimation.

Result: The model demonstrated utility for marine monitoring, effectively using small labeled datasets and capturing detailed spatial patterns of ocean color while matching point observations.
...
TL;DR: SHINE is a training-free framework for high-fidelity image composition that addresses complex lighting challenges and diverse high-resolution inputs using pretrained diffusion models without latent inversion or attention surgery.


<details>
  <summary>Details</summary>
Motivation: Existing image composition models struggle with complex lighting conditions (accurate shadows, water reflections) and diverse high-resolution inputs. While modern diffusion models encode essential physical and resolution priors, they lack a framework to utilize them effectively without problematic techniques like latent inversion or brittle attention surgery.
...
Conclusion: SHINE provides an effective training-free framework for seamless, high-fidelity image insertion that outperforms existing methods, particularly in handling complex lighting conditions and high-resolution inputs.
...
TL;DR: QuantVGGT is a novel quantization framework for Visual Geometry Grounded Transformers (VGGTs) that addresses heavy-tailed activation distributions and multi-view calibration instability through dual-smoothed quantization and noise-filtered diverse sampling, achieving 3.7× memory reduction and 2.5× acceleration while maintaining 98% accuracy.


<details>
  <summary>Details</summary>
Motivation: Large-scale transformers in 3D reconstruction models like VGGTs have prohibitive computational and memory costs that hinder real-world deployment. PTQ faces unique obstacles with billion-scale VGGTs due to heavy-tailed activation distributions from data-independent tokens and unstable calibration from multi-view 3D data.

Method: QuantVGGT uses Dual-Smoothed Fine-Grained Quantization (pre-global Hadamard rotation and post-local channel smoothing) to handle heavy-tailed distributions, and Noise-Filtered Diverse Sampling (outlier filtering via deep-layer statistics and frame-aware diverse calibration clusters) to ensure stable quantization ranges.

Result: QuantVGGT achieves state-of-the-art results across benchmarks and bit-widths, with 4-bit quantization delivering 3.7× memory reduction and 2.5× acceleration in real-hardware inference while maintaining reconstruction accuracy above 98% of full-precision counterpart.

Conclusion: QuantVGGT demonstrates vast advantages and practicality for resource-constrained scenarios, providing an effective solution for compressing billion-scale VGGTs while preserving performance.
...
TL;DR: NewtonGen integrates data-driven video synthesis with learnable physical principles through Neural Newtonian Dynamics to enable physically consistent text-to-video generation with precise parameter control.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video models produce unrealistic motions and lack controllability because they learn motion distributions solely from appearance without understanding underlying dynamics.
...
Conclusion: Integrating learnable physical principles with data-driven synthesis addresses fundamental limitations in current text-to-video generation for improved physical consistency and controllability.
...
TL;DR: SD3.5-Flash is an efficient few-step distillation framework that enables high-quality image generation on consumer devices through optimized distribution matching, timestep sharing, and comprehensive pipeline optimizations.


<details>
  <summary>Details</summary>
Motivation: To democratize access to high-quality image generation by making it computationally feasible on accessible consumer devices like mobile phones and desktop computers, overcoming the prohibitive computational requirements of existing rectified flow models.

Method: Uses a reformulated distribution matching objective for few-step generation, introduces timestep sharing to reduce gradient noise and split-timestep fine-tuning to improve prompt alignment, combined with pipeline optimizations including text encoder restructuring and specialized quantization.
