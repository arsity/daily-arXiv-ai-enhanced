...
Method: Proposes LUMA with dual-path anchoring: (1) lightweight MoCLIP model for temporal semantic supervision via contrastive learning, (2) frequency domain alignment using low-frequency DCT components, adaptively fused through temporal modulation mechanism.

Result: Achieves state-of-the-art performance with FID scores of 0.035 on HumanML3D and 0.123 on KIT-ML, and accelerates convergence by 1.4× compared to baseline.
...
<details>
  <summary>Details</summary>
Motivation: To test if basic visual understanding is truly solved in VLMs by challenging them with densely populated scenes that require detailed analysis, unlike existing benchmarks that focus on global image understanding.

Method: Created a dataset of 2,720 question-answer pairs using high-resolution scans of public-domain paintings with multiple figures and detailed backdrops, manually annotated across six task categories to probe thorough scene understanding.

Result: Even the best model (o3) among 37 tested achieved only 19.6% accuracy on the hardest test split and 69.5% overall, revealing significant gaps in counting, OCR, and logical reasoning capabilities.

Conclusion: Current VLMs significantly overestimate their visual understanding capabilities, especially in densely populated scenes, exposing critical gaps that need addressing for true visual intelligence.
...
Method: Uses a pretrained 3D VAE to encode videos, fuses with target HR prompts via trainable spatio-temporal layers with AdaLN, and applies FiLM in decoder with fine-tuned output layer for accurate physiological modulation.
...
<details>
  <summary>Details</summary>
Motivation: To address the core challenge of spatial reasoning in VLMs - perspective taking, which requires multiple cognitive capabilities like object recognition across views, relative position grounding, and mental transformation simulation.

Method: Created a benchmark with fine-grained diagnostic categories targeting different spatial transformations, progressively structured from single-object tasks to demanding multi-object perspective-taking scenarios. Evaluated 37 state-of-the-art VLMs.

Result: Revealed systematic weaknesses including strong egocentric bias, poor rotational understanding, and inconsistencies under symmetrical/syntactic reformulations. Human subjects achieved 91.2% accuracy, with human response time strongly correlating with VLM accuracy.

Conclusion: SpinBench provides critical insights into spatial reasoning gaps in VLMs and highlights their limitations in reasoning about physical space, while capturing spatial reasoning challenges shared across humans and VLMs.
...
<details>
  <summary>Details</summary>
Motivation: Standard architectures like ConvLSTM fail to model long-range dependencies in land subsidence forecasting, and prior work is limited by uni-modal data paradigms that don't effectively capture complex non-linear dynamics.

Method: Proposes Multi-Modal Spatio-Temporal Transformer (MM-STT) with joint spatio-temporal attention mechanism that processes all multi-modal features (dynamic displacement data and static physical priors) in a unified manner.

Result: On EGMS dataset, MM-STT establishes new state-of-the-art, reducing long-range forecast RMSE by an order of magnitude compared to all baselines including STGCN and STAEformer.
...
<details>
  <summary>Details</summary>
Motivation: State-of-the-art VLMs struggle with 3D understanding from 2D inputs, while expert pure vision models achieve high accuracy in depth estimation but require task-specific architectures and losses. The research aims to bridge this gap.

Method: Uses text-based supervised fine-tuning with sparse labels for per-pixel metric depth estimation. Addresses pixel reference and cross-dataset camera ambiguity through visual prompting and intrinsic-conditioned augmentation. No dense prediction heads or complex regression/regularization losses needed.

Result: DepthLM surpasses accuracy of most advanced VLMs by over 2x with smaller models, making VLMs comparable with pure vision models for the first time. Trained VLMs naturally avoid over-smoothing and have fewer flying points at boundaries than pure vision models.

Conclusion: Simple text-based fine-tuning is sufficient for VLMs to unlock strong 3D understanding. The approach enables a single VLM to cover various 3D tasks beyond metric depth estimation.
...
<details>
  <summary>Details</summary>
Motivation: High-resolution mapping of Pan-Arctic sea ice with reliable uncertainty is essential for operational sea ice charting, but faces challenges including subtle ice signature features, model uncertainty, and data heterogeneity.

Method: Designed a high-resolution Transformer model with global and local modules for better feature extraction, extended it with Bayesian treatment of parameters for uncertainty quantification, and fused Sentinel-1, RCM, and AMSR2 data at decision-level to address data heterogeneity.

Result: Tested on Pan-Arctic datasets from September 2021, achieving both high-resolution sea ice concentration maps and robust uncertainty maps compared to other uncertainty quantification approaches.
...
<details>
  <summary>Details</summary>
Motivation: To overcome barriers in infrastructure-based sensing for work zone safety, including perspective distortion, complex geometry, occlusions, and cost limitations.

Method: Integrated roadside camera and LiDAR sensors in cosimulation environment with Kalman Filter-based late fusion strategy for trajectory consistency and accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs fail at AI-generated image detection because they reason about fakes without truly seeing them - their vision encoders miss subtle forgery traces and finetuning causes knowledge forgetting.

Method: Proposed Forensic-Chat framework that trains MLLMs to perceive artifacts first (artifact-aware visual perception) before reasoning, and created ExplainFake-Bench benchmark for evaluation.

Result: Extensive experiments show superiority in generalization and genuinely reliable explainability for fake image detection.

Conclusion: The 'seeing before reasoning' paradigm with Forensic-Chat enables MLLMs to become effective, explainable detectors of AI-generated images by grounding reasoning in actual visual observations.
...
<details>
  <summary>Details</summary>
Motivation: Malicious actors are using real-time deepfake technology for phishing attacks during video meetings, exploiting the inability of deepfakes to mimic authentic gaze patterns and nonverbal communication.

Method: Built a model using explainable features derived from research on gaze patterns during dyadic conversations, utilizing point-of-gaze tracking to detect inconsistencies in deepfake behavior.

Result: Achieved 82% accuracy on a novel dataset created specifically for this research.

Conclusion: This is the first method to successfully utilize point-of-gaze tracking for deepfake detection, providing a novel biometric-based defense against real-time deepfake phishing attacks.
...
<details>
  <summary>Details</summary>
Motivation: Need for reliable object localization in Mars Sample Return campaign where robotic arms must pick up objects with low clearance under severely constrained hardware limitations.

Method: Uses custom renderer with new template matching metric tailored to edge domain, working with low-fidelity textureless 3D models as inputs.

Result: Method consistently outperforms state-of-the-art in compute and memory-constrained scenarios, showing superior robustness and accuracy in synthetic datasets, physical testbeds, and actual Mars imagery.
...
<details>
  <summary>Details</summary>
Motivation: Existing behavioral control methods for VLMs have limitations - system prompting can be overridden by users, and activation-based steering requires invasive model access, preventing use with API-based services and closed-source models.

Method: Generate universal visual inputs (VISOR++ images) that induce target activation patterns across multiple VLMs, enabling behavioral control through optimized images alone without runtime model access.

Result: VISOR++ images achieved performance parity with steering vectors for refusal, sycophancy and survival instinct tasks on open-access models, showed promise for unseen models (both open and closed-access), and preserved 99.9% performance on 14,000 MMLU evaluation tasks.

Conclusion: VISOR++ provides an effective, deployment-agnostic method for behavioral control in VLMs through visual inputs alone, overcoming limitations of existing approaches while maintaining model performance on unrelated tasks.
...
Method: Vision-Zero uses a strategic self-play framework where VLMs engage in "Who Is the Spy"-style games across multiple roles, generating training data autonomously without human annotation. It introduces Iterative Self-Play Policy Optimization (Iterative-SPO) that alternates between Self-Play and reinforcement learning with verifiable rewards.
...
Conclusion: The framework demonstrates strong generalization across diverse domains using arbitrary images, mitigates performance plateaus in self-play training, and enables sustainable long-term improvements for VLMs.
...
Method: Proposes a hybrid approach that combines mathematical/clustering segmentation models with insights from U-Net, leveraging strengths of both methods to reduce dependency on large-scale training data.
...
Conclusion: The hybrid model provides a robust solution for choroidal nevus segmentation, forming a foundation for decision support systems to enhance diagnostic speed and accuracy in clinical practice.
...
Method: Systematically evaluated state-of-the-art MLLMs and introduced FishNet++, a large-scale multimodal benchmark with 35,133 textual descriptions, 706,426 key-point annotations, and 119,399 bounding boxes for comprehensive evaluation and development.
...
Conclusion: FishNet++ provides a comprehensive suite of annotations to facilitate development and evaluation of specialized vision-language models capable of advancing aquatic science by addressing the domain knowledge gap in marine biology.
...
<details>
  <summary>Details</summary>
Motivation: Existing graph convolution methods for ViGs lack a versatile aggregation method that effectively captures complex node-neighbor relationships without architecture-specific refinements.
...
Conclusion: The proposed method achieves strong performance while maintaining efficiency, delivering competitive accuracy with comparable FLOPs to prior vision GNN architectures.
...
<details>
  <summary>Details</summary>
Motivation: Address limited annotated medical data by studying generalized few-shot learning for chest X-ray pathology classification, where new classes are learned while leveraging knowledge of known classes.
...
Result: Increasing classes per episode and training examples improves performance; transfer learning consistently beats ProtoNet; higher resolution improves accuracy but increases computation; efficient models achieve comparable performance with fewer resources.

Conclusion: Transfer learning is effective for few-shot medical image classification, MetaChest enables comprehensive evaluation, and practical trade-offs exist between accuracy and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing medical segmentation models are fragmented - trained on single knowledge sources and specific to individual tasks/modalities/organs, which contrasts with clinical practice where experts seamlessly integrate diverse knowledge sources.

Method: Uses dual-prompt representation (1-D sparse prompts for 'what' and 2-D dense prompts for 'where') dynamically routed through Mixture-of-Experts decoder to integrate semantic priors, in-context knowledge from few-shot examples, and interactive feedback.
...
Conclusion: K-Prism successfully mirrors clinical flexibility by unifying three knowledge paradigms, enabling flexible switching between segmentation approaches without architectural modifications and demonstrating superior performance across diverse medical imaging tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods either produce uniform resolution outputs that are computationally expensive for high-resolution training or require dense observations and lengthy optimization. There's a need for spatially varying detail levels that can reconstruct fine details in critical regions on demand.

Method: GaussianLens is a feed-forward densification framework that fuses multi-modal information from initial low-resolution 3DGS and multi-view images. It uses a pixel-guided densification mechanism to capture details under large resolution increases.

Result: The method demonstrates superior performance in local fine detail reconstruction and strong scalability to images up to 1024×1024 resolution, avoiding the high cost of uniformly high-resolution reconstructions.

Conclusion: The proposed localized high-resolution reconstruction approach effectively bridges the gap between computational cost and user needs for fine details in specific regions, enabling efficient on-demand detail enhancement in 3D scene reconstruction.
...
Method: Extended the preliminary LMOD benchmark with 50% dataset expansion (especially color fundus photography), broadened task coverage (binary/multi-class diagnosis, severity classification, demographic prediction), and systematically evaluated 24 state-of-the-art MLLMs.

Result: Top-performing models achieved ~58% accuracy in disease screening under zero-shot settings, but performance remained suboptimal for challenging tasks like disease staging and severity classification.

Conclusion: The dataset, curation pipeline, and leaderboard will be publicly released to advance ophthalmic AI applications and help reduce the global burden of vision-threatening diseases, though current MLLMs show both promise and limitations.
...
<details>
  <summary>Details</summary>
Motivation: Existing cross-view object geo-localization methods are constrained by predefined anchors, limiting their flexibility and performance. The uncertainty of object position in cross-view scenarios also poses challenges for accurate localization.

Method: AFGeo uses an anchor-free formulation that predicts four directional offsets (left, right, top, bottom) to ground-truth boxes from each pixel. It incorporates Gaussian Position Encoding to model click point uncertainty and a Cross-view Object Association Module to relate objects across viewpoints.

Result: The model achieves state-of-the-art performance on benchmark datasets while being lightweight and computationally efficient with minimal parameter overhead.

Conclusion: Anchor-free formulation combined with Gaussian Position Encoding and cross-view association provides an effective solution for cross-view object geo-localization, overcoming limitations of anchor-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Cross-modal retrieval models like CLIP show degraded performance when retrieving keys composed of fused image-text modality, and existing approaches require careful dataset curation and fail to generalize to unseen modality combinations.

Method: Proposes Generalized Contrastive Learning (GCL) - a loss formulation that enforces contrastive learning across all modalities within mini-batches using existing image-caption paired datasets to learn a unified representation space.

Result: Demonstrates consistent performance improvements on off-the-shelf multimodal retrieval models (VISTA, CLIP, TinyCLIP) using M-BEIR, MMEB, and CoVR benchmarks.

Conclusion: GCL effectively addresses multimodal retrieval challenges without the burdensome need for new dataset curation, showing improved performance across various benchmarks and models.
...
<details>
  <summary>Details</summary>
Motivation: Traditional CNNs require large datasets which are expensive to collect. Synthetic images from video games offer a low-cost alternative to real data collection.
...
Result: Synthetic images proved reliable for training, contributing to all networks' performance with highest mAP reaching 99%.

Conclusion: Synthetic images from video games provide a reliable, low-cost data source for training neural networks, effectively replacing expensive real data collection.
...
<details>
  <summary>Details</summary>
Motivation: Existing remote sensing image captioning studies focus on image-level descriptions, lacking object-level fine-grained interpretation, which prevents full utilization of the rich semantic and structural information in remote sensing images.

Method: Proposed Geo-DLC task for object-level captioning, constructed DE-Dataset with 25 categories and 261,806 annotated instances, developed DE-Benchmark evaluation suite using LLM-assisted QA, and designed DescribeEarth MLLM with scale-adaptive focal strategy and domain-guided fusion module leveraging remote sensing vision-language features.
...
Conclusion: The proposed Geo-DLC task and DescribeEarth model effectively address the limitation of image-level captioning by providing object-level fine-grained descriptions, enabling better utilization of remote sensing image information for applications like environmental monitoring and urban planning.
...
Result: YOLOv9 with ConSinGAN achieved highest accuracy of 91.3% and detection time of 146 ms, outperforming other YOLO versions.

Conclusion: The proposed YOLOv9 model with data augmentation successfully establishes a practical automated optical inspection system that can be applied to various industrial components.
...
<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detection methods overfit to specific forgery traits, while source attribution is limited by scarce well-categorized datasets. There's a need for robust attribution that works with limited samples in real-world scenarios.

Method: Introduces OmniDFA framework with open-set, few-shot source identification paradigm. Constructs OmniFake dataset with 1.17M images from 45 generative models. Uses few-shot learning to identify unseen generators with limited samples.
...
Conclusion: The proposed framework provides a practical solution for real-world AI-generated image analysis, combining detection and attribution capabilities with strong generalization performance.
...
Method: AIMCoT introduces three components: Context-enhanced Attention-map Generation (CAG) for reliable attention maps, Active Visual Probing (AVP) for goal-oriented region selection using information theory, and Dynamic Attention-shifting Trigger (DAT) for optimal visual information insertion timing.
...
<details>
  <summary>Details</summary>
Motivation: To understand why and how diffusion models memorize training data, addressing privacy and copyright concerns that arise from memorization despite their success in image generation.

Method: Analyzed latent space dynamics, denoising processes, and intermediate latents to identify mechanisms driving memorization, including examining training loss, classifier-free guidance effects, and trajectory convergence.

Result: Found that memorization is driven by overestimation of training samples during early denoising, where classifier-free guidance amplifies predictions, memorized prompts inject training images into noise predictions, and deviations from theoretical denoising schedule correlate with memorization severity.

Conclusion: Early overestimation is the central underlying mechanism of memorization in diffusion models, explaining how initial randomness is suppressed and replaced by memorized content.
...
<details>
  <summary>Details</summary>
Motivation: Current medical vision-language models fail to account for many-to-many modality mappings common in medical decision-making, where multiple imaging and text modalities need to be integrated.

Method: Uses probabilistic contrastive learning with InfoNCE loss and Hellinger distance to align four modalities (chest X-rays, ECGs, echocardiograms, clinical text) into unified probabilistic embedding space, plus probabilistic synthetic sampling loss for intra-modality binding.
...
Conclusion: Probabilistic modeling effectively handles many-to-many medical modality mappings and enables robust multimodal integration for medical prognostication.
...
Method: Embeds prompts and candidate images in CLIP space, applies sparse autoencoder to uncover semantic deviations into interpretable factors, selects negative samples based on reconstruction difficulty, semantic deviation from positive, and mutual diversity, and uses Plackett-Luce objective with importance sampling for training efficiency.
...
Conclusion: Semantic-aware, multi-negative sampling is effective for preference-based learning in multimodal DPO, addressing limitations of existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Prior VPR methods neglect the dynamic interplay between spatial context and visual similarity during training, focusing instead on descriptor fine-tuning or fixed sampling strategies.

Method: SAGE uses a lightweight Soft Probing module to learn residual weights for patch descriptors, reconstructs an online geo-visual graph fusing geographic proximity and visual similarity, and employs greedy weighted clique expansion for hard sample mining with a frozen DINOv2 backbone and parameter-efficient fine-tuning.
...
Conclusion: SAGE demonstrates that jointly improving local feature aggregation, organizing samples during training, and hard sample mining through spatial-visual adaptive graph exploration significantly enhances VPR performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal models struggle with precise attribute control and identity preservation in face editing, especially when conditional landmarks deviate significantly from the source due to large expression/pose changes or inaccurate landmark estimates.

Method: Uses landmark tokenizer to quantize raw coordinates into discrete facial tokens, location-mapping positional encoding for unified processing of facial and image tokens, and landmark predictor leveraging vision-language models with structured chain-of-thought for accurate target landmark inference.

Result: Outperforms state-of-the-art methods by 7.8% in identity preservation and 4.6% in semantic consistency. Created HFL-150K dataset with 150K real face pairs for training.

Conclusion: LaTo enables flexible geometry-appearance interactions with high efficiency and strong identity preservation, addressing limitations of rigid geometric constraints in existing face editing methods.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of Video Object Segmentation (VOS) in complex real-world scenarios, particularly handling long-term object disappearances/reappearances and small/inconspicuous objects as presented in the MOSEv1 and LVOS datasets.

Method: Freeze SAM2 feature extractor during training while fine-tuning other components. Use confidence-guided fusion segmentation with pixel-check strategy during inference to progressively refine predictions by combining multiple models.

Result: Achieved J&F score of 86.37% on test set, ranking 1st in the MOSEv1 Challenge at LSVOS 2025.

Conclusion: The approach effectively addresses VOS challenges in complex scenarios through robust feature preservation and multi-model fusion strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing HMR methods either produce single deterministic outputs or probabilistic methods that trade off accuracy for diversity. The inherent ambiguity in recovering 3D pose from 2D observations requires modeling the full distribution of plausible solutions.

Method: Uses SO(3) diffusion model to generate pose parameter distributions as 3D rotations, employs transformer for hierarchical joint structure learning, and combines it with a small MLP-based denoising model for per-joint distribution conditioning.

Result: The model effectively predicts accurate pose probability distributions and demonstrates improved performance in handling the ambiguity of 3D pose recovery from 2D images.

Conclusion: The proposed SO(3) diffusion approach successfully models well-aligned distributions to 2D observations, providing both accuracy and diversity in human mesh recovery without the traditional trade-offs.
...
<details>
  <summary>Details</summary>
Motivation: Existing drag-based image editing methods primarily operate on 2D pixel plane with limited 3D cues, leading to imprecise and inconsistent edits in geometry-intensive scenarios such as rotations and perspective transformations.
...
Conclusion: GeoDrag enables coherent, high-fidelity, and structure-consistent editing in a single forward pass, effectively addressing limitations of previous 2D-only drag-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing volume-based methods rely heavily on multi-view pixel back-projection ray intersections, causing reconstruction quality to depend on input view density with poor performance in overlapping regions and unobserved areas.

Method: Proposes IPDRecon with three core components: Pixel-level Confidence Encoder (PCE), Affine Compensation Module (ACM), and Image-Plane Spatial Decoder (IPSD) to decode 3D structural information from 2D images through physical imaging processes.

Result: Achieves superior reconstruction stability on ScanNetV2, maintaining nearly identical quality when view count reduces by 40%, with coefficient of variation of 0.24%, performance retention rate of 99.7%, and maximum performance drop of 0.42%.
...
<details>
  <summary>Details</summary>
Motivation: To establish baselines for financial short-form video captioning and understand how different modality combinations affect performance in joint reasoning tasks.

Method: Tested 7 modality combinations (T, A, V, TA, TV, AV, TAV) on 624 annotated YouTube short-form videos across 5 topics using multimodal LLMs.

Result: Video alone performed strongly on 4 of 5 topics. Selective pairs like TV or AV often surpassed TAV, suggesting too many modalities may introduce noise.

Conclusion: Established first baselines for financial short-form video captioning, highlighting video's importance and showing that optimal performance may require selective modality combinations rather than using all available modalities.
...
Conclusion: Reasoning-enhanced training significantly improves diagnostic accuracy, consistency, and interpretability, highlighting its importance for high-stakes medical AI. The unified framework validates the approach for handling ultrasound's complexities.
...
Method: Reformulates VITON as a linear inverse problem using trajectory-aligned solvers with residual prior-based initialization and artifact-free measurement-guided sampling that combines data consistency, frequency-level correction, and periodic standard denoising.

Result: Experiments on VITON-HD, DressCode, and SHHQ-1.0 show ART-VITON effectively preserves identity and background, eliminates boundary artifacts, and improves visual fidelity and robustness over state-of-the-art baselines.
...
Method: Train T2I models to prefer matched prompts over mismatched prompts (constructed by perturbing original captions with LLMs), extending DPO and KTO to create TDPO and TKTO.

Result: Quantitative and qualitative evaluations show TPO methods outperform original counterparts with better human preference scores and improved text-to-image alignment.

Conclusion: TPO provides a "free-lunch" alignment framework that eliminates dependency on costly human annotations while improving text-to-image alignment performance.
...
<details>
  <summary>Details</summary>
Motivation: To assess and diagnose the capacity of multimodal large language models for humor understanding, which has practical applications in enhancing human-machine interaction engagement.

Method: Created v-HUB benchmark with curated minimally verbal short videos paired with rich annotations (captions, descriptions, explanations), and evaluated diverse MLLMs including specialized Video-LLMs and versatile OmniLLMs on tasks like caption matching and humor explanation.

Result: MLLMs struggle to comprehend humor from visual cues alone, with significant performance drops in caption matching when moving from text-based to video-based evaluation. Audio integration improves humor understanding.

Conclusion: Current MLLMs face difficulties in visual humor comprehension, and incorporating audio helps, suggesting the promise of richer multimodal integration for complex video understanding tasks.
...
Result: PCPO stabilizes training, significantly accelerates convergence, improves image quality by mitigating model collapse, and substantially outperforms existing policy gradient baselines including state-of-the-art DanceGRPO.

Conclusion: PCPO effectively addresses the fundamental issue of disproportionate credit assignment in T2I model alignment, providing a more stable and effective training framework that delivers superior performance across all metrics.
...
<details>
  <summary>Details</summary>
Motivation: Previous inversion methods for text-guided image editing struggle to adhere closely to target text prompts because inverted noise maps that faithfully reconstruct source images limit editing flexibility.

Method: Proposes Editable Noise Map Inversion (ENM Inversion) that searches for optimal noise maps and includes editable noise refinement to align with desired edits by minimizing differences between reconstructed and edited noise maps.

Result: Extensive experiments show ENM Inversion outperforms existing approaches across various image editing tasks in both preservation and edit fidelity with target prompts. The method also successfully applies to video editing with temporal consistency.

Conclusion: ENM Inversion effectively addresses the trade-off between content preservation and editability in text-guided image editing, providing superior performance and extending to video editing applications.
...
Method: EvoQuality adapts self-consistency to IQA by generating pseudo-labels through pairwise majority voting on the VLM's outputs, then uses these rankings to create a fidelity reward for iterative optimization via group relative policy optimization (GRPO).

Result: EvoQuality boosts the base VLM's zero-shot performance by 31.8% on PLCC across IQA benchmarks and achieves competitive or superior performance to state-of-the-art supervised models on 5 out of 7 benchmarks.

Conclusion: The framework demonstrates that VLMs can autonomously refine perceptual capabilities through self-supervised learning, achieving strong performance without human annotations.
...
<details>
  <summary>Details</summary>
Motivation: ECGs are low-cost and accessible, while echocardiograms require significant hospital resources but are crucial for cardiac assessment. The goal is to use ECGs as a more accessible method to predict cardiac function measurements.
...
Conclusion: EchoingECG successfully enables ECG-based prediction of cardiac function measurements with improved performance and uncertainty awareness, making cardiac assessment more accessible.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks use multiple-choice questions based on image annotations, which don't adequately assess VLMs' embodied reasoning abilities through precise visual grounding.

Method: Created PIO benchmark with hierarchical evaluation across three stages (S1-S3) using data from indoor, kitchen, driving, and robotic manipulation scenarios. Tested over ten state-of-the-art VLMs.

Result: GPT-4o underperforms in precise visual grounding compared to some open-source models. MoLMO performs well in S1-S2 but struggles with S3's visual trace planning requirements.

Conclusion: PIO benchmark reveals limitations in current VLMs' embodied reasoning capabilities, particularly in tasks requiring precise visual grounding and trace planning.
...
<details>
  <summary>Details</summary>
Motivation: Parameter-Efficient Fine-Tuning (PEFT) of foundation models for agricultural computer vision is challenging due to limited training data and complex field conditions, requiring adaptation methods that work under extreme data constraints.

Method: DSGA module with dynamic similarity graph construction using learnable polynomial decay-initialized weight ranking and adaptive local feature aggregation, integrated with Low-Rank Adaptation (LoRA) to create a complementary optimization framework for capturing both local and global dependencies.

Result: Achieved 17.31% improvement in Structure-measure and 62.36% gain in adaptive F-measure compared to baseline SAM fine-tuning, with progressive performance gains as shot count increased (2, 4, 8, 10 shots). Accurate pod-counting with adjusted R-squared of 0.8987 for images with 10-120 pods.

Conclusion: The proposed DSGA with LoRA framework provides effective parameter-efficient fine-tuning for agricultural computer vision tasks, demonstrating practical utility for automated monitoring applications with only 4.00M trainable parameters (4.26% of original SAM).
...
TL;DR: GeoLifeCLEF 2023 was an open machine learning challenge that used deep learning and remote sensing data to predict plant species distribution across Europe through multi-label classification.


<details>
  <summary>Details</summary>
Motivation: To advance species distribution modeling using deep learning and remote sensing data, and understand spatio-temporal distribution of species for ecological and conservation purposes.

Method: Organized a competition using 5 million plant observations with single positive labels, combined with high-resolution remote sensing imagery, land cover, elevation data, and coarse-resolution climate, soil, and human footprint variables. Used multi-label classification to predict species composition in standardized survey plots.

Result: The competition highlighted biases in methods trained on single positive labels when evaluated with multi-label metrics, and identified effective learning strategies that combine both single and multi-label data during training.

Conclusion: The challenge successfully advanced species distribution modeling, revealed important methodological insights about label biases, and demonstrated effective hybrid training approaches for ecological prediction tasks.
...
Conclusion: VELA provides an effective solution for automatic evaluation of long and detailed image captions generated by multimodal LLMs, addressing limitations of existing approaches.
...
TL;DR: A training-free reward-guided image editing framework that formulates editing as a trajectory optimal control problem using diffusion models, achieving superior balance between reward maximization and source image fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing reward-guided approaches focus on image generation but are largely unexplored for image editing tasks that require preserving source image semantics while enhancing target rewards.

Method: Formulates editing as trajectory optimal control where diffusion model reverse process is treated as controllable trajectory from source image, with adjoint states iteratively updated to steer the editing process.

Result: Significantly outperforms existing inversion-based training-free guidance baselines across distinct editing tasks, achieving superior balance between reward maximization and source image fidelity without reward hacking.

Conclusion: The proposed framework enables effective reward-guided image editing without training, maintaining semantic content while enhancing target objectives through optimal control formulation.
...
TL;DR: Multimodal reasoning in VLMs enhances logical inference but can impair perceptual grounding through visual forgetting. The paper proposes VAPO to anchor reasoning to visual input, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: To address the dual nature of multimodal reasoning where enhanced logical inference comes at the cost of perceptual grounding and visual forgetting.

Method: Proposes Vision-Anchored Policy Optimization (VAPO), a method that explicitly steers reasoning toward visually grounded trajectories.

Result: VAPO-Thinker-7B significantly strengthens visual reliance and achieves state-of-the-art results on multiple benchmarks.

Conclusion: VAPO effectively addresses visual forgetting in multimodal reasoning while maintaining strong performance across diverse visual tasks.
...
TL;DR: MuSLR is the first benchmark for multimodal symbolic logical reasoning, testing VLMs' ability to apply formal logic to multimodal inputs. Current VLMs struggle significantly, with GPT-4.1 achieving only 46.8%. LogiCAM framework improves performance by 14.13% by applying formal logical rules.
...
Result: 7 state-of-the-art VLMs all struggle with multimodal symbolic reasoning, with best model GPT-4.1 achieving only 46.8%. LogiCAM boosts GPT-4.1's Chain-of-Thought performance by 14.13%, with larger gains on complex logics like first-order logic. Error analysis shows 70% of failures stem from logical misalignment between modalities.

Conclusion: Current VLMs have significant limitations in multimodal symbolic logical reasoning. The LogiCAM framework demonstrates that applying formal logical rules can substantially improve performance, and the identified logical misalignment issues provide key insights for future model development.
...
<details>
  <summary>Details</summary>
Motivation: Current industrial anomaly detection relies on foundation models but primarily focuses on textual prompt tuning, leaving visual processing fragmented across different models. The authors aim to create a unified visual framework that works across diverse foundation models.

Method: Proposes Patch-Exclusive Anomaly Detection (PatchEAD) framework with visual prompting techniques including an alignment module and foreground masking. It's training-free and compatible with various foundation models.

Result: Superior few-shot and batch zero-shot performance compared to prior work, despite not using textual features. The study also examines how backbone structure and pretrained characteristics affect patch-similarity robustness.

Conclusion: A well-unified patch-only framework enables quick, calibration-light deployment without needing carefully engineered textual prompts, providing actionable guidance for selecting foundation models for real-world visual inspection.
...
Method: Hardware-agnostic approach using multiple cameras for 360° coverage, integrating low-light image enhancement module, automatic geometric transformation computation between LiDAR and cameras without specialized targets, and color correction for uniform camera feeds.

Result: Real-time performance achieved with reliable colorization even under very low illumination, successfully recovering scene details that would otherwise remain undetectable using Velodyne Puck Hi-Res LiDAR and four-camera configuration.

Conclusion: The proposed system provides robust, real-time colorized point cloud generation with complete 360-degree coverage and excellent low-light performance, eliminating the need for specialized calibration targets.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on slide-level prompts and fail to capture subtype-specific phenotypic variations of histological entities (nuclei, glands) that are critical for cancer diagnosis.

Method: Uses LLMs to generate entity-level and slide-level prompts, entity-guided cross-attention for entity-level features, cross-scale entity graph learning to enrich representations, and combines entity-level and slide-level predictions.
...
TL;DR: DeepSketcher introduces a comprehensive suite for "thinking with images" paradigm in VLMs, featuring a 31k CoT dataset and a model that generates visual thoughts directly in embedding space without external tools.


<details>
  <summary>Details</summary>
Motivation: To advance the emerging "thinking with images" paradigm in VLMs by addressing limitations in data construction accuracy, structural design, and application scenarios for more faithful multimodal reasoning.
...
Conclusion: DeepSketcher enables tool-free and more flexible "thinking with images" through direct visual embedding manipulation, advancing multimodal reasoning capabilities.
...
TL;DR: mpLLM is a hierarchical mixture-of-experts architecture for visual question answering on multi-parametric 3D brain MRI, using modality-level and token-level experts to efficiently fuse multiple 3D modalities without pretraining.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of limited image-text paired supervision in medical imaging and enable effective visual question answering for multi-parametric 3D brain MRI data.

Method: Uses prompt-conditioned hierarchical mixture-of-experts (MoE) architecture with modality-level and token-level projection experts, plus synthetic VQA protocol generating medically relevant questions from segmentation annotations.
...
Conclusion: The study provides the first clinically validated VQA dataset for 3D brain mpMRI, a novel multimodal LLM handling multiple 3D modalities, and demonstrates strong medical utility through empirical results.
...
TL;DR: This paper introduces the first systematic study of Multimodal Multi-Turn (MMT) dialogue safety, creates the MMDS dataset with 4,484 annotated samples, develops an automated red-teaming framework using MCTS, and presents LLaVAShield for joint risk detection in user inputs and assistant responses.
...
Method: Systematic definition of MMT dialogue safety, creation of MMDS dataset with fine-grained annotations, development of automated multimodal multi-turn red-teaming framework based on Monte Carlo Tree Search (MCTS), and creation of LLaVAShield for joint risk detection.
...
Conclusion: The work addresses critical safety gaps in multimodal multi-turn dialogues through systematic formulation, dataset creation, automated testing framework, and effective moderation tool that will be publicly released to support future research.
...
TL;DR: VLM-FO1 reframes object perception from coordinate generation to feature retrieval, using a Hybrid Fine-grained Region Encoder and token-based referencing to achieve state-of-the-art fine-grained perception without compromising general visual understanding.
...
Method: Proposes VLM-FO1 framework with Hybrid Fine-grained Region Encoder (dual vision encoder) to generate region tokens rich in semantic and spatial detail, and token-based referencing system for LLM to reason about specific visual regions. Uses two-stage training strategy.

Result: Achieves state-of-the-art performance across diverse benchmarks, demonstrating exceptional capabilities in object grounding, region generation understanding, and visual region reasoning.

Conclusion: VLM-FO1 establishes an effective paradigm for building perception-aware VLMs that bridges the gap between high-level reasoning and fine-grained visual grounding.
...
TL;DR: Deep neural networks are vulnerable to adversarial examples. This study examines how training data characteristics affect adversarial robustness across 36 vision models, finding that robustness follows logarithmic scaling laws with data volume and model size, but data quality and training objectives are more crucial than raw scale.


<details>
  <summary>Details</summary>
Motivation: To understand how training data characteristics (volume, quality, curation) affect adversarial robustness across different vision models and training paradigms, challenging the assumption that scale alone drives robustness.
...
Result: Robustness follows logarithmic scaling: 10x data increase reduces attack success rate by ~3.2%, while 10x model size increase reduces ASR by ~13.4%. Self-supervised models on curated datasets (e.g., DINOv2) outperform models on larger but less curated datasets. Adversarial fine-tuning improves generalization for structural but not color variations.

Conclusion: While scaling improves robustness, data quality, architecture, and training objectives play a more decisive role than raw scale in achieving broad-spectrum adversarial resilience. Human evaluation shows persistent gaps between human and machine vision.
...
<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods treat modality and class as independent factors, leading to fragmented solutions, excessive memory overhead, and issues with shared decoding paths that struggle with large domain variations, causing distorted normality boundaries and high false alarm rates.

Method: Uses Mixture-of-Experts-driven feature decompression with a 'general to specific' paradigm. Multi-modal inputs are compressed into general features by an encoder with feature compression, then decompressed via sparsely-gated cross MoE that dynamically selects expert pathways based on modality and class. Includes grouped dynamic filtering and MoE-in-MoE structure for efficiency.
...
Conclusion: UniMMAD provides a unified framework that effectively addresses the limitations of existing fragmented anomaly detection approaches through adaptive and disentangled reconstruction, achieving superior performance across diverse domains and modalities.
...
TL;DR: The paper proposes CO3, a plug-and-play corrective sampling strategy that improves multi-concept prompt fidelity in text-to-image diffusion models by steering away from regions where joint prompt behavior overlaps too strongly with single concepts.


<details>
  <summary>Details</summary>
Motivation: Multi-concept prompts like "a cat and a dog" often fail in diffusion models, yielding images where one concept is missing, faint, or awkwardly colliding with another due to the model drifting into mixed modes that over-emphasize strongly learned single concepts.

Method: Introduces corrective sampling that steers away from regions where joint prompt behavior overlaps too strongly with any single concept, aiming for "pure" joint modes where all concepts coexist with balanced visual presence. Also adapts sampling to remain in favorable weight regimes for existing guidance schemes.
...
Conclusion: Lightweight corrective guidance can substantially mitigate brittle semantic alignment behavior in modern diffusion systems without requiring model tuning, complementing standard classifier-free guidance.
...
TL;DR: SS-ACL is an annotation-free framework for vision-grounded medical report generation that uses self-supervised anatomical consistency learning to align reports with anatomical regions through textual prompts, achieving superior performance without expert annotations.


<details>
  <summary>Details</summary>
Motivation: Existing methods require extensive expert annotations for detection modules, leading to high labeling costs and limited generalizability due to pathology distribution bias across datasets.

Method: Proposes Self-Supervised Anatomical Consistency Learning (SS-ACL) that constructs a hierarchical anatomical graph based on human anatomy structure, enforces intra-sample spatial alignment through recursive region reconstruction, and uses region-level contrastive learning for inter-sample semantic alignment.
...
Conclusion: SS-ACL provides an effective annotation-free approach for generating clinically accurate and visually grounded medical reports, demonstrating strong performance across multiple metrics and downstream tasks.
...
TL;DR: A flexible tracking framework using pose estimation for automated salmon welfare monitoring that outperforms state-of-the-art pedestrian trackers in underwater aquaculture environments.
...
Method: Uses pose estimation network to extract bounding boxes around salmon and their body parts, employs specialized modules to handle underwater challenges, and leverages high-detail body part tracks for welfare indicator calculation.

Result: Outperforms BoostTrack (state-of-the-art pedestrian tracker) on two salmon tracking challenges: ID transfers in crowded scenes and ID switches during turning. Also demonstrates effectiveness for tail beat wavelength analysis.

Conclusion: The proposed framework provides an effective solution for automated salmon welfare monitoring in aquaculture, addressing specific underwater challenges and enabling multi-indicator welfare assessment through body part tracking.
...
TL;DR: PinPoint3D is an interactive framework for fine-grained 3D part segmentation that uses user point clicks to generate precise part-level masks, addressing limitations in existing methods through a novel data synthesis pipeline and achieving significant performance improvements.
...
Method: Developed PinPoint3D framework using user point clicks for interactive segmentation, and created a new 3D data synthesis pipeline to generate large-scale scene-level datasets with dense part annotations to overcome data scarcity.
...
Conclusion: PinPoint3D represents a significant advancement towards more nuanced and precise machine perception and interaction in complex 3D environments, enabling fine-grained part segmentation with high efficiency.
...
TL;DR: RH-Partial2Global improves VICL by addressing limitations in current methods through jackknife conformal prediction and covering design-based sampling for better in-context example selection.


<details>
  <summary>Details</summary>
Motivation: Current VICL methods rely on the similarity-priority assumption without sufficient justification and use random sampling that leads to incomplete coverage and redundant comparisons, adversely affecting global ranking.

Method: RH-Partial2Global uses jackknife conformal prediction to construct reliable alternative sets and covering design-based sampling for comprehensive pairwise preference coverage.
...
Conclusion: The proposed method provides reliable and holistic selection of in-context examples in VICL, addressing key limitations of existing approaches.
...
TL;DR: VRWKV-Editor is a novel video editing model that uses linear spatio-temporal aggregation with RWKV transformer to achieve linear complexity, providing 3.7x speedup and 60% lower memory usage while maintaining competitive performance.
...
Result: Achieves up to 3.7x speedup and 60% lower memory usage compared to state-of-the-art diffusion-based methods, while maintaining competitive frame consistency and text alignment performance.

Conclusion: The proposed method effectively addresses computational complexity limitations in video editing, with performance gap becoming more significant for longer videos compared to self-attention architectures.
...
<details>
  <summary>Details</summary>
Motivation: Pixel-level object recognition from egocentric images is important for assistive technologies and activity monitoring, but progress is hindered by scarce annotated datasets and costly manual labeling.

Method: Proposes NS-iHOS task and WISH model - an end-to-end approach that learns human-object interaction detection from natural language narrations, distilling knowledge to enable in-hand object segmentation without using narrations at test time.

Result: WISH surpasses all baselines on EPIC-Kitchens and Ego4D datasets, recovering more than 50% of the performance of fully supervised methods without fine-grained pixel-wise annotations.

Conclusion: Natural language narrations provide effective weak supervision for in-hand object segmentation, enabling significant performance gains without costly manual annotations.
...
Method: Decomposes IQA into four subtasks: distortion detection, distortion analysis, tool selection, and tool execution. Uses a planner to formulate strategies, an executor to collect perceptual evidence via tools, and a summarizer to integrate evidence for scores and explanations.
...
Conclusion: AgenticIQA provides an effective framework for dynamic, query-aware image quality assessment that integrates scoring and interpretation while maintaining high accuracy and human-aligned explanations.
...
TL;DR: PFDepth is the first pinhole-fisheye framework for heterogeneous multi-view depth estimation that leverages complementary characteristics of both camera types through unified architecture and novel 3D Gaussian representation.


<details>
  <summary>Details</summary>
Motivation: To exploit complementary characteristics of pinhole and fisheye imagery (undistorted vs. distorted, small vs. large FOV, far vs. near field) for joint optimization in depth estimation.

Method: Unified architecture processing arbitrary combinations of pinhole/fisheye cameras; lifts 2D features into canonical 3D volumetric space; uses Heterogeneous Spatial Fusion for distortion-aware volumetric feature fusion; reformulates voxel fusion into 3D Gaussian representation with learnable latent Gaussian spheres.

Result: Sets state-of-the-art performance on KITTI-360 and RealHet datasets over current mainstream depth networks.

Conclusion: This is the first systematic study of heterogeneous pinhole-fisheye depth estimation, offering both technical novelty and valuable empirical insights.
...
TL;DR: A fourth-order nonlinear PDE model combining diffusion and wave properties is proposed for multiplicative noise suppression, outperforming second-order models by reducing blocky artifacts while preserving fine details.


<details>
  <summary>Details</summary>
Motivation: Second-order PDE models for multiplicative noise suppression often introduce blocky artifacts during early denoising stages, requiring a more effective approach that maintains image quality.
...
Result: The model achieved better PSNR and MSSIM scores for images with ground truth, lower SI for SAR images, and preserved structure/color consistency in color images compared to existing second-order anisotropic diffusion approaches.

Conclusion: The proposed fourth-order PDE model effectively suppresses multiplicative noise while avoiding blocky artifacts and preserving fine details, demonstrating superior performance across grayscale, SAR, and color images.
...
TL;DR: SETR is a two-stage zero-shot composed image retrieval method that uses intersection-driven coarse retrieval and LLM-based fine-grained re-ranking to overcome CLIP's limitations in handling irrelevant background details and fine-grained semantics.
...
Method: Two-stage approach: (1) Coarse retrieval with intersection-driven strategy that retains only overlapping semantics between reference image and relative text, (2) Fine-grained re-ranking using adapted multimodal LLM with Low-Rank Adaptation for binary semantic relevance judgments.
...
TL;DR: GeoLink is a multimodal framework that integrates OpenStreetMap (OSM) data with remote sensing foundation models to enhance geospatial intelligence through cross-modal spatial correlations during both pretraining and downstream tasks.


<details>
  <summary>Details</summary>
Motivation: There is a modality gap between remote sensing and OSM data in terms of data structure, content, and spatial granularity, making effective synergy challenging. Most existing RS foundation models focus only on imagery, limiting their ability to leverage rich geographic context.
...
Conclusion: The study demonstrates the potential of multimodal synergy in advancing high-level geospatial artificial intelligence and highlights the importance of spatial correlation for effective integration of geospatial data modalities.
...
TL;DR: PatchVSR enables patch-wise video super-resolution using pre-trained video diffusion models, overcoming limitations of full-size VSR through dual-stream adaptation and location-aware patch synthesis.


<details>
  <summary>Details</summary>
Motivation: To overcome the intensive computation of full-attention in existing video super-resolution methods and enable flexible output resolutions by leveraging pre-trained video diffusion models for patch-level generation.

Method: Proposes PatchVSR with dual-stream adapter: patch branch maintains content fidelity from input patches, global branch provides context from resized full video, and location information injection for better patch contextualization. Includes multi-patch joint modulation for visual consistency.

Result: Achieves high-fidelity, high-resolution patch-level detail synthesis and enables competitive 4K VSR using a 512x512 base model with high efficiency.

Conclusion: Patch-based paradigm provides flexibility and efficiency for video super-resolution, allowing high-resolution output generation from lower-resolution base models while maintaining visual consistency across patches.
...
TL;DR: Causally-Guided Gaussian Perturbations (CGP) improves OOD generalization by injecting spatially varying noise guided by causal masks from Vision Transformers, with stronger perturbations on background regions to encourage reliance on causally relevant features.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of OOD generalization in deep learning, especially for biomedical images where distribution shifts are subtle and pervasive, and overcome limitations of existing methods that may overlook causal mechanisms.

Method: Propose CGP framework that applies spatially varying Gaussian noise to input images using soft causal masks from Vision Transformers - stronger perturbations on background, weaker on foreground to reduce reliance on spurious correlations.

Result: Experimental results on WILDS benchmark Camelyon17 show consistent performance gains over state-of-the-art OOD baselines.

Conclusion: Causal perturbation is a promising tool for reliable and interpretable OOD generalization in deep learning models.
...
TL;DR: SeMoBridge addresses CLIP's intra-modal misalignment in few-shot classification by mapping images into text modality while preserving semantics, outperforming existing methods especially in low-data scenarios.


<details>
  <summary>Details</summary>
Motivation: CLIP's performance in few-shot classification is limited by intra-modal misalignment caused by modality gap and exclusive inter-modal training, making direct image-to-image comparisons unreliable.

Method: Introduces Semantic Modality Bridge that maps images into text modality while preserving semantic content. It's closed-form and can be optionally trained with multi-modal supervision combining image and text-alignment losses.

Result: SeMoBridge-T (trained version) requires only a fraction of training time while overall outperforming other methods, particularly in low-data scenarios (1, 2, and 4 shots).

Conclusion: SeMoBridge effectively addresses CLIP's intra-modal misalignment issue for few-shot classification through lightweight semantic modality bridging, achieving superior performance with minimal training overhead.
...
TL;DR: Extends HAMMER model to detect global scene inconsistencies like foreground-background mismatch using a lightweight segmentation-guided scoring pipeline without retraining.


<details>
  <summary>Details</summary>
Motivation: HAMMER fails when main subjects are contextually misplaced into implausible backgrounds due to label-space bias, local attention focus, and spurious text-foreground alignment.

Method: Proposes segmentation-guided scoring (SGS) pipeline that uses person/face segmentation masks to separate foreground/background, extracts embeddings with vision-language model, and computes region-aware coherence scores fused with HAMMER's predictions.
...
Conclusion: Demonstrates importance of region-aware reasoning in multimodal disinformation detection and provides publicly available scripts for segmentation and scoring.
...
Method: Used OpenAI's gpt-image-1 with carefully designed prompts to generate human-centric news-style images where authentic figures are placed into absurd/impossible backdrops. Captions were produced under three conditions (literal, text attribute, text split), creating three manipulation categories: FG-BG, FG-BG+TA, and FG-BG+TS. Implemented quality control pipelines for face visibility, perceptual hash deduplication, OCR-based text scrubbing, and realistic headline length.

Result: Created DGM4+ dataset extension with 5,000 high-quality samples that introduces foreground-background mismatches and their hybrids with text manipulations, complementing existing datasets to test detectors on both local and global reasoning.

Conclusion: The DGM4+ dataset strengthens evaluation of multimodal models like HAMMER that struggle with FG-BG inconsistencies, providing a comprehensive benchmark for detecting both local and global multimodal manipulations.
...
TL;DR: A geometric framework using principal fiber bundle sections to mod out symmetries in computer vision, avoiding data augmentation by measuring dissimilarities between object orbits under symmetry groups.


<details>
  <summary>Details</summary>
Motivation: Most computer vision and medical datasets have inherent symmetries (rotation, scaling, etc.) that should be incorporated in classification tasks without relying on data augmentation for more sustainable algorithms.

Method: Uses the concept of sections of principal fiber bundles to quotient out symmetries, enabling simple metrics to measure dissimilarities between object orbits. Presents a 2-parameter family of canonical curve parameterizations.

Result: Demonstrated the methodology on object contour datasets for translation, rotation, scaling, and reparameterization symmetries. Provides optimized sections to maximize class separation.

Conclusion: The geometric framework offers a sustainable alternative to data augmentation for handling symmetries, with wide applicability potential across various domains. Code and tutorials are publicly available.
...
TL;DR: Proposed 3D pseudo-ground-truth labels using foundation models Grounded-SAM and Metric3Dv2 with temporal information for densification, achieving significant performance improvements in semantic occupancy prediction without complex rendering strategies.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised methods for semantic occupancy prediction use computationally expensive techniques like novel view synthesis, cross-view rendering, and depth estimation, which incur high computational costs and memory usage during training.

Method: Generate 3D pseudo-ground-truth labels using foundation models Grounded-SAM and Metric3Dv2, leverage temporal information for label densification, and propose EasyOcc model that learns solely from these labels without complex rendering strategies.

Result: mIoU increased by 45% from 9.73 to 14.09 when implemented in OccNeRF; EasyOcc achieved 13.86 mIoU; state-of-the-art performance of 7.71 mIoU on full scene without camera mask, outperforming previous best by 31%.

Conclusion: Foundation models, temporal context, and choice of loss computation space are critical for self-supervised learning in comprehensive scene understanding, with proposed method being easily integrable into existing models.
...
TL;DR: A real-time deep learning framework predicts penalty kick direction using both visual and pose data, achieving 89% accuracy before ball contact.


<details>
  <summary>Details</summary>
Motivation: Penalty kicks are crucial in championships, and goalkeepers need to anticipate kicker's intent from subtle biomechanical cues within very short time windows.
...
TL;DR: Reason-3D is a text-to-scene generation model that uses large reasoning models to create 3D environments from text prompts, addressing limitations in complex geometry handling and instruction adherence through object retrieval and spatial reasoning.
...
Method: Integrates object retrieval using captions covering physical, functional, and contextual attributes, places objects based on implicit and explicit layout constraints, and refines positions with collision-aware spatial reasoning using large reasoning models.
...
Conclusion: Demonstrates advanced spatial reasoning abilities of modern large reasoning models and contributes to text-to-scene generation field, with codebase released to advance research in object retrieval and placement with LRMs.
...
TL;DR: EVODiff introduces an information-theoretic approach to diffusion model inference, optimizing conditional entropy during denoising to significantly improve efficiency and quality over existing gradient-based solvers.
...
Result: EVODiff significantly outperforms state-of-the-art solvers: reduces reconstruction error by 45.5% (FID from 5.10 to 2.78) at 10 NFE on CIFAR-10, cuts NFE cost by 25% (20 to 15 NFE) on ImageNet-256, and improves text-to-image generation with reduced artifacts.

Conclusion: The information-theoretic perspective provides fundamental insights for diffusion model inference, and EVODiff demonstrates superior performance by systematically optimizing conditional entropy during denoising.
...
TL;DR: EchoGen is a novel feed-forward framework that enables subject-driven generation using Visual Auto-Regressive (VAR) models, achieving high fidelity and quality with significantly faster inference than diffusion-based methods.


<details>
  <summary>Details</summary>
Motivation: To resolve the trade-off between computationally expensive fine-tuning methods and slow diffusion-based approaches for subject-driven generation, by leveraging VAR models' fast sampling speeds and strong generative quality.

Method: Uses a dual-path injection strategy with semantic encoder for abstract identity (via decoupled cross-attention) and content encoder for visual details (via multi-modal attention) to disentangle and preserve subject characteristics.

Result: Achieves subject fidelity and image quality comparable to state-of-the-art diffusion methods with significantly lower sampling latency.

Conclusion: EchoGen successfully bridges VAR models with subject-driven generation, offering an efficient feed-forward alternative to existing methods while maintaining high quality and fidelity.
...
TL;DR: EntroPE introduces entropy-guided dynamic patching for time series forecasting, preserving temporal coherence while maintaining computational efficiency of patch-based transformers.
...
Method: EntroPE consists of two modules: Entropy-based Dynamic Patcher (EDP) that uses conditional entropy to detect transition points and determine patch boundaries, and Adaptive Patch Encoder (APE) that employs pooling and cross-attention to capture intra-patch dependencies and produce fixed-size latent representations, processed by a global transformer.

Result: Experiments across long-term forecasting benchmarks show that EntroPE improves both accuracy and efficiency compared to existing approaches.
...
TL;DR: Automated pipeline for text-guided edge-case synthesis using LLM and Text-to-Image models to generate challenging training data, improving object detection robustness.


<details>
  <summary>Details</summary>
Motivation: Manual curation of challenging edge cases for training data is a major bottleneck in improving deep neural network performance and mitigating dataset bias.

Method: Fine-tune Large Language Model via preference learning to rephrase image captions into diverse textual prompts that steer Text-to-Image model toward generating difficult visual scenarios.

Result: Achieves superior robustness on FishEye8K object detection benchmark, surpassing both naive augmentation and manually engineered prompts.

Conclusion: Establishes scalable framework that shifts data curation from manual effort to automated, targeted synthesis for developing more reliable AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack comprehensive evaluation for human-centric scene understanding due to absence of benchmarks that consider both granular human-oriented perception and higher-dimensional causal reasoning, compounded by the physical complexity of human body annotation challenges.

Method: Created Human-MME benchmark with: 1) Diverse human scenes across 4 primary visual domains, 15 secondary domains, and 43 sub-fields; 2) Progressive evaluation from granular perception to higher-dimensional reasoning across 8 dimensions with 19,945 image-question pairs; 3) Automated annotation pipeline and human-annotation platform for high-quality data.

Result: Extensive experiments on 17 state-of-the-art MLLMs exposed limitations in human-centric understanding. The benchmark enables evaluation of multi-person and multi-image mutual understanding through choice, short-answer, grounding, ranking, judgment questions, and complex combinations.

Conclusion: Human-MME provides a holistic evaluation framework that guides future MLLM research toward better human-centric image understanding, with all data and code publicly available.
...
<details>
  <summary>Details</summary>
Motivation: While pedestrian detection is critical for autonomous driving safety, fairness concerns related to pose variations and occlusions remain underexplored despite being equally important for reliable performance.

Method: Systematic evaluation of five pedestrian-specific detectors and three general-purpose models on the ECP-DP dataset, using Equal Opportunity Difference metric and Z-test for statistical significance across various pedestrian poses and joint occlusions.

Result: Detection models show biases against pedestrians with parallel legs, straight elbows, and lateral views. Lower body joint occlusions have more negative impact than upper body/head occlusions. Cascade R-CNN achieves lowest miss-rate and smallest bias.

Conclusion: This is the first comprehensive pose- and occlusion-aware fairness evaluation in pedestrian detection for autonomous driving, revealing significant biases that need addressing for equitable and reliable detection systems.
...
Method: Dual-model architecture combining CNN for cell type classification and Vision Transformer (ViT) for multi-attribute classification, tested on PBC and WBCAtt datasets with 8 cell types and 11 morphological attributes.

Result: Achieved new benchmark of 94.62% accuracy for multi-attribute classification, significantly enhancing model interpretability and offering substantial time and cost efficiency over human annotation.
...
Method: Extended a SOTA visual-based 360-degree video saliency detection approach by incorporating text descriptions, using a vision-language model for representation, similarity estimation module, and viewport spatio-temporal cross-attention mechanism.

Result: Quantitative and qualitative evaluations on the TSV360 dataset showed TSalV360 is competitive with SOTA visual-based approaches and capable of performing customized text-driven saliency detection effectively.

Conclusion: TSalV360 successfully enables text-driven saliency detection in 360-degree videos, providing users with the ability to customize which objects/events should be salient based on text descriptions.
...
TL;DR: GSDD proposes a sparse 2D Gaussian representation for dataset distillation that replaces dense pixel-level representations with fewer Gaussian primitives to improve efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Address computational and storage burdens of modern model training by creating compact datasets, overcoming redundancy and scalability issues in conventional dense pixel-level approaches.

Method: Uses 2D Gaussian primitives to encode critical discriminative information in distilled images, with CUDA-based splatting operators for parallel inference and training to ensure efficiency and scalability.
...
Conclusion: GSDD provides an effective, scalable, and broadly applicable sparse representation method for dataset distillation that outperforms conventional approaches.
...
TL;DR: This paper presents an experimental study on generating plausible textual explanations for video summarization outcomes by extending an existing framework with a Large Multimodal Model and evaluating plausibility through semantic overlap between visual explanations and video summaries.
...
Method: Extended an existing multigranular explanation framework by integrating LLaVA-OneVision to generate textual descriptions of visual explanations, then evaluated plausibility by quantifying semantic overlap between these descriptions and video summaries using SBERT and SimCSE sentence embeddings.
...
TL;DR: MGCE is a framework for Generalized Category Discovery that addresses limitations in existing methods by exploiting multi-granularity conceptual information and automatically estimating category numbers in unlabeled data.
...
Method: MGCE uses Dynamic Conceptual Contrastive Learning (DCCL) for concept mining and dual-level representation learning, and Multi-Granularity Experts Collaborative Learning (MECL) with multiple experts at different granularities and concept alignment for cross-expert collaboration.

Result: MGCE achieves state-of-the-art results on nine fine-grained visual recognition benchmarks, with 3.6% average improvement over parametric approaches that require known category numbers, particularly excelling in novel-class accuracy.

Conclusion: MGCE effectively addresses key GCD challenges by adaptively mining multi-granularity concepts and automatically estimating category numbers, making it suitable for practical open-world applications.
...
TL;DR: IMG is a re-generation-based multimodal alignment framework that uses MLLMs to identify misalignments and an Implicit Aligner to manipulate diffusion features, achieving better alignment without extra data or editing operations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multimodal alignment face limitations: finetuning requires scarce high-quality preference data, while editing-based methods may degrade overall image quality. A scalable, quality-preserving solution is needed.

Method: 1) Use MLLM to identify misalignments between generated images and prompts; 2) Implicit Aligner manipulates diffusion conditioning features for re-generation; 3) Train with Iteratively Updated Preference Objective to formulate alignment goals.

Result: IMG outperforms existing alignment methods on SDXL, SDXL-DPO, and FLUX in both qualitative and quantitative evaluations. It also works as a plug-and-play adapter to enhance prior finetuning-based methods.

Conclusion: IMG provides an effective, data-free solution for multimodal alignment that preserves image quality and can flexibly integrate with existing methods, addressing key limitations of current approaches.
...
TL;DR: Donut-MINT uses mechanistic interpretability to compress large Vision-Language Models for document understanding, achieving reduced inference time and memory usage while maintaining strong DocVQA performance.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models like Donut are too costly for real-time or resource-constrained applications, creating a need for efficient compression methods.

Method: Knowledge distillation with mechanistic interpretability to analyze internal computations, identifying essential subcomponents to retain while approximating, skipping, or reparametrizing others.
...
TL;DR: Farsighted-LAM improves latent action models with geometry-aware spatial encoding and multi-scale temporal modeling, addressing poor spatial understanding and limited temporal perception. SSM-VLA integrates this with visual Chain-of-Thought reasoning for enhanced decision consistency.
...
Method: Proposed Farsighted-LAM with geometry-aware spatial encoding and multi-scale temporal modeling to capture structural priors and dynamic motion patterns. Built SSM-VLA framework integrating structured perception with visual Chain-of-Thought module for explicit environmental reasoning.
...
TL;DR: The paper introduces PRPO, a reinforcement learning method that improves deepfake detection by aligning LLM reasoning with visual evidence at paragraph level, achieving high detection accuracy and reasoning scores.


<details>
  <summary>Details</summary>
Motivation: Deepfake detection is constrained by dataset scarcity and poor performance of multimodal LLMs, which often produce misaligned or hallucinatory explanations despite strong reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Standard affine transformation worsens results in low-bit PTQ by applying uniform parameters to all outputs, leading to accuracy degradation.

Method: CAT employs cluster-specific affine parameters to refine low-bit quantized outputs with minimal additional parameters, integrated into a novel PTQ framework.

Result: Achieves up to 53.18% Top-1 accuracy on W2A2 ResNet-18 and improves existing PTQ baselines by over 3% as a plug-in.

Conclusion: CAT effectively reduces quantization error in low-bit PTQ without requiring model fine-tuning, outperforming prior methods across diverse architectures.
...
TL;DR: ProfVLM is a compact vision-language model that reformulates skill proficiency estimation as generative reasoning, jointly predicting skill levels and generating expert feedback from multi-view videos while using significantly fewer parameters and training time than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing skill proficiency estimation methods rely on black-box video classifiers that ignore multi-view context and lack explainability, creating a need for more transparent and context-aware approaches.

Method: Uses an AttentiveGatedProjector to dynamically fuse multi-view features from a frozen TimeSformer backbone into a language model tuned for feedback generation, trained on EgoExo4D with expert commentaries.

Result: Surpasses state-of-the-art methods while using up to 20x fewer parameters and reducing training time by up to 60%, achieving superior accuracy across diverse activities with natural language critiques aligned with performance.
...
TL;DR: Point2RBox-v3 improves weakly-supervised oriented object detection using point annotations through progressive label assignment and a prior-guided dynamic mask loss to address inefficient pseudo label utilization and poor quality.


<details>
  <summary>Details</summary>
Motivation: To reduce the cost of manual labeling for oriented object detection by using point annotations, while addressing deficiencies in existing methods: inefficient pseudo label utilization and poor quality.

Method: Uses Progressive Label Assignment (PLA) to dynamically estimate instance sizes at different training stages, and Prior-Guided Dynamic Mask Loss (PGDM-Loss) that combines advantages of watershed algorithm and SAM model for both sparse and dense scenes.

Result: Achieves competitive performance: 66.09%/56.86%/41.28%/46.40%/19.60%/45.96% on DOTA-v1.0/DOTA-v1.5/DOTA-v2.0/DIOR/STAR/RSAR datasets, especially strong in scenarios with large object size variations or sparse occurrences.

Conclusion: Point2RBox-v3 is the first model using dynamic pseudo labels for label assignment and creatively combines watershed algorithm with SAM model, achieving excellent performance across both sparse and dense scenes.
...
TL;DR: Flower is a solver for inverse problems that uses pre-trained flow models to produce consistent reconstructions through an iterative three-step process, unifying plug-and-play methods with generative inverse solvers.


<details>
  <summary>Details</summary>
Motivation: To develop a solver that leverages pre-trained flow models for inverse problems, providing reconstructions consistent with measurements while unifying different methodological perspectives.

Method: Iterative three-step procedure: (i) flow-consistent destination estimation using velocity network, (ii) refinement via projection onto feasible set defined by forward operator, (iii) time-progression step re-projecting along flow trajectory.

Result: Flower achieves state-of-the-art reconstruction quality across various inverse problems while using nearly identical hyperparameters.

Conclusion: Flower provides a theoretical framework that approximates Bayesian posterior sampling and demonstrates practical effectiveness with consistent hyperparameter settings across different problems.
...
TL;DR: The paper introduces VFF, a continuous 3D Video Fourier Field representation for video super-resolution that enables flexible spatio-temporal sampling without explicit frame warping, achieving state-of-the-art results with improved sharpness and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Traditional video super-resolution methods decouple spatial and temporal components and rely on brittle explicit frame warping for motion compensation, which can lead to artifacts and inconsistencies.

Method: Encode video as a continuous 3D Video Fourier Field using Fourier-like sinusoidal basis with coefficients predicted by a neural encoder, allowing analytical Gaussian point spread function for aliasing-free reconstruction at arbitrary scales.

Result: Substantially improves both spatial and temporal super-resolution, sets new state-of-the-art across multiple benchmarks, delivers sharper and more temporally consistent reconstructions while being computationally more efficient than existing baselines.

Conclusion: The joint modeling approach through continuous spatio-temporal representation offers superior video super-resolution performance compared to decoupled methods, providing flexible sampling and high-quality reconstruction across various upscaling factors.
...
TL;DR: SQUARE is a training-free zero-shot composed image retrieval framework that uses MLLMs in two stages: semantic query augmentation for better intent capture and efficient batch reranking for improved ranking accuracy.
...
Conclusion: SQUARE provides a simple yet effective training-free approach for zero-shot CIR that better captures user intent through MLLM-enhanced semantic guidance and efficient reranking.
...
TL;DR: The paper introduces \mname, a reward model trained on a large-scale human preference dataset for instruction-guided image editing, which achieves state-of-the-art human correlation and enables scaling of high-quality training data.
...
Result: \mname achieves state-of-the-art human correlation on benchmarks (GenAI-Bench, AURORA-Bench, ImagenHub, \benchname) and successfully selects high-quality data from ShareGPT-4o-Image, leading to improved model training.

Conclusion: \mname effectively serves as a reward model to scale high-quality training data for image editing and will be released to help the community.
...
TL;DR: The paper introduces Task-oriented Temporal Grounding (ToTG) to localize key time intervals in long videos based on task descriptions, presents a benchmark (ToTG Bench), and proposes TimeScope framework with progressive reasoning and a curated dataset (ToTG Pile) that outperforms existing methods.
...
Conclusion: TimeScope effectively addresses the challenging ToTG problem through progressive temporal grounding, demonstrating superior performance over current approaches.
...
TL;DR: ScalingAR is the first test-time scaling framework for next-token prediction autoregressive image generation that uses token entropy to improve generation quality without early decoding or external rewards.
...
TL;DR: PANDA is a generalist video anomaly detection system using MLLMs that automatically handles any scene and anomaly types without training data or manual adjustments through agentic AI engineering capabilities.


<details>
  <summary>Details</summary>
Motivation: Current VAD methods require domain-specific training data and manual adjustments for new scenarios, suffering from high labor costs and limited generalization. The goal is to achieve generalist VAD that works automatically without training or human involvement.
...
Conclusion: PANDA demonstrates generalizable and robust anomaly detection capability, validating its effectiveness as a generalist VAD system that operates automatically across diverse scenarios.
...
Conclusion: The research enhances video generation systems by enabling effective retrieval and transfer of motion priors, facilitating synthesis of realistic motion dynamics.
...
TL;DR: Proposes difficulty-aware evaluation for SISR models using high-frequency and rotation-invariant edge indices to better differentiate models that produce visually different results despite similar average scores.
...
Method: Two image-difficulty measures: high-frequency index and rotation-invariant edge index to predict test images where models yield significantly different visual results, plus an evaluation method that reflects these visual differences in objective measures.
...
Conclusion: Difficulty-aware performance evaluation provides more meaningful assessment of SISR models by accounting for image difficulty variations and visual quality differences that average scores miss.
...
TL;DR: PRISM is a progressive three-stage framework for image deraining that combines multi-scale feature aggregation with hybrid attention and state-space modeling to achieve fine-grained recovery and global consistency.
...
Method: Progressive three-stage framework: Coarse Extraction Network (CENet) and Frequency Fusion Network (SFNet) using Hybrid Attention UNet (HA-UNet) for multi-scale feature aggregation, plus Hybrid Domain Mamba (HDMamba) for joint spatial and wavelet domain modeling, and Refine Network (RNet) for fine-grained structure recovery.
...
Conclusion: PRISM provides improved image quality for deraining tasks through its progressive multi-stage approach that effectively balances local detail recovery with global consistency.
...
Method: QuaRTZ uses 8-bit min-max quantization for outlier handling and compresses to 4 bits via leading-zero suppression to retain least significant bits (LSBs), balancing outlier preservation and LSB precision.
...
Conclusion: QuaRTZ successfully enables 4-bit post-training quantization for diffusion models by preserving both outliers and small activations critical for texture fidelity, demonstrating superior performance without requiring additional FP16 components.
...
TL;DR: An Automated Vehicle Inspection (AVI) platform using 11 synchronized cameras and specialized deep learning models for real-time variant-aware quality control and defect detection in automotive production lines.


<details>
  <summary>Details</summary>
Motivation: Ensuring every vehicle leaving production meets correct variant specifications and is free from visible defects is increasingly complex, requiring automated multi-view inspection systems.

Method: Eleven synchronized cameras capture 360° vehicle views, with specialized modules: YOLOv8 for part detection, EfficientNet for ICE/EV classification, Gemini-1.5 Flash for mascot OCR, and YOLOv8-Seg for scratch-and-dent segmentation. A view-aware fusion layer standardizes evidence, and a VIN-conditioned rule engine compares detected features against expected manifest.

Result: AVI achieves 93% verification accuracy, 86% defect-detection recall, and sustains 3.3 vehicles/min, surpassing single-view or no segmentation baselines by large margins.

Conclusion: This is the first publicly reported system that unifies multi-camera feature validation with defect detection in a deployable automotive industry setting, delivering interpretable pass/fail reports in approximately 300ms.
...
TL;DR: Stylos is a single-forward 3D Gaussian framework for 3D style transfer that works on unposed content from single images or multi-view collections, using a reference style image to generate stylized 3D scenes without per-scene optimization or precomputed poses.


<details>
  <summary>Details</summary>
Motivation: To enable geometry-aware, view-consistent 3D style transfer that generalizes to unseen categories, scenes, and styles without requiring per-scene optimization or precomputed poses, addressing limitations of existing methods.

Method: Uses Transformer backbone with dual pathways: geometry predictions with self-attention for geometric fidelity, and style injection via global cross-attention for visual consistency. Incorporates voxel-based 3D style loss to align aggregated scene features with style statistics.

Result: Demonstrates high-quality zero-shot stylization across multiple datasets, achieving view-consistent stylization while preserving geometry, with scalability from single view to large-scale multi-view settings.

Conclusion: Stylos effectively enables 3D style transfer through global style-content coupling, the proposed 3D style loss, and scalable framework design, showing strong generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Indoor scene classification is challenging due to complex object relationships and spatial layouts, with important applications in robotics and sensitive content analysis like child sexual abuse imagery classification.

Method: Convert images into Scene Graphs and use Graph Attention Networks for inference, directly modeling interactions between scene components through structured graph representations.
...
Conclusion: Structured scene representations using Scene Graphs and Graph Attention Networks provide a robust paradigm for indoor scene classification and CSAI classification, offering explainability and privacy benefits.
...
TL;DR: A lightweight CBAM-CNN model with only 2.13M parameters achieves 95.58% accuracy for betel leaf disease classification, outperforming traditional pre-trained CNNs through attention mechanisms and balanced dataset training.


<details>
  <summary>Details</summary>
Motivation: Betel leaf diseases threaten crop security and cause financial losses, requiring timely detection. AI can help boost production by predicting diseases early.

Method: Proposed a lightweight CNN with Convolutional Block Attention Module (CBAM) that adaptively focuses on important spatial and channel features, trained on 10,185 images across three disease classes.
...
Conclusion: The lightweight CBAM-CNN model effectively detects betel leaf diseases with high accuracy and interpretability, providing a practical solution for agricultural disease management.
...
TL;DR: The paper presents CoGuide, a diffusion-based method that reconstructs floorplans from user movement trajectories using a contrastive embedding space to overcome challenges with non-differentiable path-planning functions.


<details>
  <summary>Details</summary>
Motivation: Direct inversion from movement trajectories to floorplans is ill-posed since multiple layouts can explain the same movements. Traditional approaches face instability due to the non-invertible and non-differentiable nature of path-planning functions.

Method: Uses diffusion-based posterior sampling with a contrastive embedding space that brings compatible floorplan-trajectory pairs close together while pushing mismatched pairs apart. This creates a smoother surrogate likelihood score to guide the denoising process.

Result: CoGuide produces more consistent floorplans from trajectories and demonstrates greater robustness compared to differentiable-planner baselines and guided-diffusion methods across extensive experiments.

Conclusion: The contrastive embedding approach provides a valid approximation of the true likelihood score, enabling effective posterior sampling for floorplan reconstruction from movement data despite the challenges posed by non-differentiable forward operators.
...
TL;DR: A systematic post-training pipeline for small language models that combines curriculum-based supervised fine-tuning and offline on-policy knowledge distillation to achieve state-of-the-art performance on edge devices.


<details>
  <summary>Details</summary>
Motivation: Large language models are too computationally expensive for edge environments, creating a need for efficient small models that can maintain high performance on resource-constrained devices.

Method: Developed a post-training pipeline with curriculum-based supervised fine-tuning (SFT) and offline on-policy knowledge distillation to enhance small model accuracy.

Result: The resulting instruction-tuned model achieves state-of-the-art performance among billion-parameter models, demonstrating strong generalization under strict hardware constraints while maintaining competitive accuracy across various tasks.

Conclusion: This work provides a practical and efficient solution for developing high-performance language models on Ascend edge devices, bridging the gap between model size and performance requirements.
...
TL;DR: DEPTHOR++ is a robust depth completion framework that enhances noisy dToF signals using RGB guidance, addressing real-world calibration errors and sensor anomalies through synthetic training, anomaly detection, and specialized network design.


<details>
  <summary>Details</summary>
Motivation: Existing depth enhancement methods assume ideal dToF inputs and perfect alignment, overlooking real-world calibration errors and anomalies, which limits practical applicability in high-precision tasks like 3D reconstruction and SLAM.

Method: Three key components: 1) Synthetic dataset-based simulation for robust training, 2) Learnable-parameter-free anomaly detection to remove erroneous measurements, 3) Specialized depth completion network integrating RGB images and monocular depth priors for challenging regions.

Result: Achieved state-of-the-art performance with 22% RMSE and 11% Rel improvement on ZJU-L5, 37% improvement in mirror regions on Mirror3D-NYU, and 22% average gain over RealSense L515 measurements on Hammer dataset.

Conclusion: The proposed framework significantly enhances depth completion robustness, enabling low-cost sensors to outperform higher-end devices and demonstrating strong generalizability across diverse real-world scenarios.
...
TL;DR: Ferret-UI Lite is a compact 3B parameter GUI agent that achieves competitive performance on GUI grounding and navigation tasks across mobile, web, and desktop platforms using optimized training techniques.
...
Method: Used techniques optimized for small models: curated diverse GUI data from real and synthetic sources, chain-of-thought reasoning, visual tool-use, and reinforcement learning with designed rewards.
...
TL;DR: Stable Cinemetrics (SCINE) is a structured evaluation framework for professional video generation that introduces four hierarchical taxonomies (Setup, Event, Lighting, Camera) with 76 fine-grained controls, enabling comprehensive assessment of video generation models through automated pipelines and expert human evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models and benchmarks fail to capture the complexity and requirements of professional video generation, lacking structured evaluation methods aligned with filmmaking practices.

Method: Developed four disentangled hierarchical taxonomies (Setup, Event, Lighting, Camera) with 76 control nodes, created a benchmark with professional prompts, built automated pipelines for prompt categorization and question generation, conducted large-scale human study with 80+ film professionals evaluating 20K videos from 10+ models.

Result: Current models show significant gaps, particularly in Events and Camera-related controls. Trained an automatic evaluator (vision-language model) that outperforms existing zero-shot baselines and aligns with expert annotations.

Conclusion: SCINE is the first approach to situate professional video generation within video generative models, providing structured evaluation pipelines and detailed analyses to guide future research in cinematic control capabilities.
...
TL;DR: Using machine learning to automate proofreading in EM connectomics, reducing manual annotation costs by 80% while achieving 90% of guided proofreading value, and automatically merging 200k fragments to increase connectivity completion.


<details>
  <summary>Details</summary>
Motivation: Manual proofreading is the bottleneck in scaling EM connectomics and comparative connectomics, requiring extensive human effort for connectome reconstruction.
...
Result: Achieved 80% cost reduction while obtaining 90% of guided proofreading value; automatically merged 200k fragments (equivalent to 4 proofreader years) and increased connectivity completion by 1.3%.

Conclusion: Machine learning can significantly reduce manual proofreading effort in connectomics while maintaining high quality, enabling scaling of EM connectome reconstructions.
...
TL;DR: DiffCamera is a diffusion transformer model that enables flexible image refocusing by allowing users to specify arbitrary focus points and blur levels, overcoming the limitation of fixed depth-of-field in photography.


<details>
  <summary>Details</summary>
Motivation: Fixed depth-of-field effects in photography are problematic when subjects are out of focus, and existing methods don't allow flexible refocusing after image capture.

Method: Uses a diffusion transformer framework trained on simulated image pairs with varying focus planes and bokeh levels, enhanced by a stacking constraint that enforces physically grounded refocusing behavior based on photographic principles.
...
Conclusion: The proposed method successfully enables flexible post-capture refocusing, advancing photography and generative AI applications with precise depth-of-field manipulation.
...
TL;DR: SAGANet introduces video object segmentation-aware audio generation, enabling precise control over sound synthesis using visual segmentation masks, video, and text cues for professional Foley workflows.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal audio generation models lack precise user control and object-level prioritization, limiting their applicability in professional Foley workflows where specific objects need focused sound generation.

Method: SAGANet is a multimodal generative model that leverages visual segmentation masks along with video and textual cues for controllable audio generation, providing fine-grained and visually localized control.
...
Conclusion: SAGANet successfully addresses the gap in precise object-level audio control and introduces a new benchmark for segmentation-aware Foley synthesis with the Segmented Music Solos dataset.
...
Conclusion: Dimensionality reduction is essential for improving feature quality and classification performance, not just a pre-processing step.
...
<details>
  <summary>Details</summary>
Motivation: Panoramic depth estimation faces challenges due to limited panoramic data availability and spherical distortions, leading to poor zero-shot generalization and inefficient processing methods.

Method: Created ~607K panoramic RGB-depth pairs through data curation from perspective images, and developed SphereViT which explicitly uses spherical coordinates to maintain geometric consistency in panoramic features.

Result: Achieved state-of-the-art performance with 38% average improvement on AbsRel over strongest zero-shot baselines, outperforming prior in-domain methods and demonstrating superior efficiency as an end-to-end solution.

Conclusion: DA² provides an accurate, efficient, and zero-shot generalizable solution for panoramic depth estimation, with both code and curated data being released to the community.
...
TL;DR: HART is a unified framework for sparse-view human reconstruction that outputs watertight clothed meshes, aligned SMPL-X body meshes, and Gaussian-splat representations from uncalibrated RGB images.
...
Result: State-of-the-art performance: 18-23% Chamfer Distance improvement for mesh reconstruction, 6-27% PA-V2V drop for SMPL-X estimation, 15-27% LPIPS decrease for novel-view synthesis across multiple datasets.

Conclusion: Feed-forward transformers can serve as scalable models for robust human reconstruction in real-world settings, achieving strong performance despite training on only 2.3K synthetic scans.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D shape completion methods rely on pre-aligned scans, which leak pose and scale cues. This causes networks to memorize absolute positions rather than learning intrinsic geometry, leading to performance collapse when alignment is absent in real data.

Method: Proposed a SIM(3)-equivariant shape completion network with modular layers that successively canonicalize features, reason over similarity-invariant geometry, and restore the original frame. Uses a de-biased evaluation protocol to remove hidden cues.
...
Conclusion: Full SIM(3) equivariance is an effective route to truly generalizable shape completion, enabling robust performance across different domains without relying on pose and scale cues.
...
<details>
  <summary>Details</summary>
Motivation: Current SLAM benchmarks don't adequately address the specific challenges of egocentric data from wearable devices, such as diverse motions, dynamic visual content, and long sessions with time-varying sensor calibration, and lack sufficiently accurate ground truth.
...
Conclusion: The introduced dataset and benchmark enable evaluation of SLAM systems under realistic egocentric conditions, including extreme scenarios like walking at night or vehicle travel, with tracks of varying difficulty levels for comprehensive analysis.
...
TL;DR: Query-Kontext bridges VLMs and diffusion models using multimodal "kontext" tokens to separate generative reasoning from visual synthesis, achieving state-of-the-art performance in text-to-image tasks.


<details>
  <summary>Details</summary>
Motivation: Current unified multimodal models entangle multimodal generative reasoning with high-fidelity synthesis, limiting their effectiveness. The authors aim to decouple these capabilities by delegating reasoning to VLMs and synthesis to diffusion models.

Method: Three-stage training: 1) Connect VLM to lightweight diffusion head via multimodal kontext tokens, 2) Scale to pre-trained diffusion model for enhanced realism, 3) Add low-level image encoder for improved fidelity and instruction tuning. Uses comprehensive data pipeline with real, synthetic, and open-source datasets.

Result: Matches strong unified baselines and outperforms task-specific state-of-the-art methods in several cases across image generation, editing, customized generation, and multi-subject composition.

Conclusion: The proposed Query-Kontext approach effectively separates multimodal generative reasoning from visual synthesis, enabling superior performance by leveraging the strengths of both VLMs and diffusion models.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for improving spatial relationship following in T2I generation became incompatible with modern architectures as they evolved for better image quality, creating a need for training-free position control solutions.
...
Result: Stitch significantly improves spatial accuracy, achieving 218% improvement on FLUX for GenEval's Position task and 206% on PosEval benchmark, with state-of-the-art results using Qwen-Image (54% improvement over previous models).

Conclusion: The method demonstrates that targeted attention heads can effectively capture object isolation information, enabling training-free integration of position control into leading T2I models while maintaining visual quality.
...
Result: Achieves 2× improvement in global pose estimation over baselines, operates at 20 FPS with 6 GB GPU memory for thousands of images.
