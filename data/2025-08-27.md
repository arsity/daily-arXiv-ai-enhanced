...
Motivation: Address the challenge of expensive and scarce annotated sonar data for underwater 3D object detection, exploring whether reliable detection can be achieved without real-world training data.

Method: Developed two approaches: 1) Physics-based sonar simulation pipeline generating synthetic training data for neural networks, and 2) Model-based template matching system leveraging geometric priors of target objects.

Result: Neural networks trained on synthetic data achieved 98% mAP on simulated scenes but dropped to 40% mAP on real sonar data due to domain shift. Template matching maintained 83% mAP on real data without any training.

Conclusion: Template matching outperforms data-hungry deep learning in underwater domains, demonstrating remarkable robustness and opening new possibilities for autonomous underwater applications in data-scarce environments.
...
<details>
  <summary>Details</summary>
Motivation: Current brain tumor detection methods suffer from limited generalization, computational inefficiency, lack of interpretability, and transparency issues, making them untrustworthy for clinical applications.

Method: Feature-level fusion of MobileNetV2 and DenseNet201 streams trained on augmented dataset of 6,020 MRI scans (glioma, meningioma, pituitary tumors, normal) using 5-fold cross-validation with GradCAM for visual explanations.

Result: 99.75% training accuracy, 98.35% testing accuracy, F1 score of 0.9835 (95% CI: 0.9743-0.9920), +3.67% accuracy increase over baselines, 39.3% training time reduction vs VGG19, with clear tumor localization via GradCAM.

Conclusion: MobileDenseAttn is an efficient, high-performance, interpretable model with strong potential for real-world clinical brain tumor detection applications.
...
<details>
  <summary>Details</summary>
Motivation: To identify systematic deficiencies in Vision Language Models' multimodal grounding capabilities, particularly their ability to recall factual knowledge when presented with visual vs. textual references to entities.

Method: Conducted controlled studies comparing VLM performance on factual recall with textual vs. visual references, analyzed internal state patterns, and developed probes to detect linking failures without retraining.

Result: VLMs' factual recall ability halves when forced to rely on image representations; internal state probes achieve 92%+ accuracy in flagging unreliable responses; selective prediction coverage increases by 7.87% with error reduction of 0.9%.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of severe radiance ambiguity caused by mismatched features in sparse image inputs for 3D reconstruction, which leads to poor reconstruction quality.

Method: Enriches neural implicit representations by adding patch-based semantic logits optimized with signed distance field and radiance field. Introduces novel regularization using geometric primitive masks to mitigate shape ambiguity.

Result: Achieves 44% reduction in average chamfer distance for SparseNeuS and 20% for VolRecon on DTU dataset. When used as plugin for dense reconstruction baselines (NeuS and Neuralangelo), reduces average error by 69% and 68% respectively.

Conclusion: The proposed semantic-aware approach significantly improves 3D reconstruction quality from sparse images by effectively addressing radiance and shape ambiguity through semantic enrichment and geometric regularization.
...
<details>
  <summary>Details</summary>
Motivation: Illegal landfills pose hazardous threats worldwide but are difficult to detect manually. There's a lack of good quality public datasets for landfill detection due to security concerns, making automated detection crucial for saving time and resources.

Method: Used lightweight deep learning models (Mobilenetv2, Googlenet, Densenet, MobileVit) to avoid overfitting, then combined the best performers into an ensemble model using fusion techniques for binary classification.

Result: Achieved 92.33% accuracy, 92.67% precision, 92.33% sensitivity, 92.41% F1 score, and 92.71% specificity in detecting illegal landfills from aerial imagery.

Conclusion: Lightweight models combined through ensemble techniques provide effective landfill detection while avoiding overfitting, demonstrating the potential for scalable automated monitoring of illegal waste sites.
...
Conclusion: SSU effectively addresses structural dependency and semantic misalignment challenges in multimodal sentiment analysis through systematic integration of modality-specific structures and cross-modal semantic grounding, demonstrating superior performance and interpretability.
...
<details>
  <summary>Details</summary>
Motivation: To enable fast and high-quality 3D face avatar generation from single images regardless of input pose, overcoming limitations of slow optimization-based methods and existing feed-forward approaches.

Method: Uses a 3DGS face template from multi-view training data, encodes input images into pose-invariant latent embeddings, and decodes to predict residuals to template parameters via feed-forward inference.

Result: Outperforms existing feed-forward methods in reconstruction quality, runs 1000x faster than optimization methods, and supports real-time identity interpolation and attribute editing.

Conclusion: FastAvatar combines excellent reconstruction quality with speed, expanding 3DGS applications for photorealistic avatars in consumer and interactive systems.
...
<details>
  <summary>Details</summary>
Motivation: Biometrics improve humanitarian operations efficiency but pose privacy risks for vulnerable populations in emergency contexts.

Method: Developed mobile biometric system with PolyProtect template protection on neural network face embeddings, evaluated verification/identification accuracy, irreversibility, and unlinkability using EdgeFace feature extractor on real-world humanitarian dataset.

Result: Promising experimental results showing PolyProtect's effectiveness for both face and fingerprint biometrics in identification scenarios, with first-ever evaluation for fingerprints.
...
<details>
  <summary>Details</summary>
Motivation: Current vision foundation models lack explicit reasoning capabilities for relational tasks in fine-grained activity recognition, egocentric video understanding, and medical image analysis where spatial, temporal, and semantic dependencies are crucial.
...
Result: The hybrid approach improves fine-grained semantic fidelity, out-of-distribution robustness, interpretability, computational efficiency, and achieves favorable memory/hardware efficiency compared to FM-only baselines.

Conclusion: Next-generation foundation models should incorporate explicit relational interfaces, with future research focusing on learned dynamic graph construction, multi-level relational reasoning, cross-modal fusion, and evaluation protocols for structured vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Automatic License Plate Recognition struggles with illegible license plates, and existing super-resolution methods don't solve the core recognition problem. There's a need for selective image pre-processing to optimize performance and efficiency.

Method: Created a novel dataset with fine-grained annotations including occlusion levels, four legibility categories, and character labels. Proposed a classification task using ViT, ResNet, and YOLO networks to categorize LP images as good enough, needing super-resolution, or unrecoverable.

Result: All three baseline models (ViT, ResNet, YOLO) achieved F1 scores below 80%, demonstrating the difficulty of the task. The analysis of SR and LP recognition methods confirmed the need for further research.

Conclusion: The introduced LPLC dataset provides a valuable benchmark for legibility classification research. The poor performance of baseline models highlights the complexity of the problem and underscores the need for continued development in this area.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of general-purpose VLMs in medical tasks, including limited specialized diagnostic accuracy and high inference costs for clinical deployment.

Method: Hierarchical framework with: 1) Specialist - lightweight domain-trained image classifier for fast diagnostic predictions, 2) Generalist - compressed conversational VLM for natural language explanations guided by Specialist's predictions, 3) Knowledge graph-based retrieval module for factual grounding.

Result: 18% improvement in diagnostic accuracy over strongest baseline, 20% reduction in VRAM requirement, 5% reduction in latency on multimodal dermatology dataset.

Conclusion: Specialist-Generalist system provides practical paradigm for building lightweight, trustworthy, and clinically viable AI systems in medical domains.
...
<details>
  <summary>Details</summary>
Motivation: Real-world face images often suffer from degradations like noise, blur, and compression artifacts that reduce quality and hinder downstream applications, necessitating effective quality assessment methods.

Method: Organized a challenge where participants created efficient FIQA models constrained by computational limits (0.5 GFLOPs, 5M parameters) to predict MOS scores on in-the-wild face images with various resolutions and realistic degradations.

Result: The challenge attracted 127 participants who submitted 1519 final entries, with comprehensive evaluations performed using correlation metrics on a dataset of real-world face images.

Conclusion: The challenge successfully advanced practical FIQA approaches by fostering development of lightweight, efficient models for assessing face image quality under realistic degradation conditions.
...
<details>
  <summary>Details</summary>
Motivation: Detecting anomalies in surveillance footage is challenging due to their unpredictable and context-dependent nature, requiring methods that can generalize to unseen behaviors.

Method: Hybrid architecture using TimeSformer for spatiotemporal features, DPC for temporal forecasting, and CLIP for semantic context with text prompts. Joint training with InfoNCE and CPC losses, plus context-gating mechanism for scene-aware decision making.

Result: Framework successfully integrates predictive modeling with vision-language understanding to detect anomalies in complex environments without prior exposure to anomaly examples.

Conclusion: This approach bridges temporal reasoning and semantic context for zero-shot anomaly detection, providing a generalizable solution for surveillance applications with code made publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the bottleneck of expensive manual annotation for 3D scene flow datasets and the performance gap between self-supervised and supervised methods, especially in challenging scenarios like long-range and adverse weather conditions.
...
Result: On MAN TruckScenes dataset, DoGFlow outperforms existing self-supervised methods and achieves over 90% of fully supervised performance with only 10% of ground truth data, significantly improving label efficiency.

Conclusion: DoGFlow provides an effective self-supervised solution for 3D scene flow estimation that eliminates dependency on manual annotations while maintaining high performance, making it scalable for autonomous navigation systems.
...
TL;DR: SatSkylines is a 3D building generation method that uses satellite imagery and coarse geometric priors to create detailed building structures, addressing limitations of existing approaches.
...
Result: The approach effectively generates detailed building models from satellite imagery and coarse priors, demonstrating strong generalization ability through extensive evaluations.

Conclusion: SatSkylines provides an effective solution for 3D building generation that bridges the gap between satellite imagery and detailed geometric modeling, offering both accuracy and computational efficiency.
...
Result: Adapter-only transfer is significantly more robust and effective in low-data scenarios and for MSTP selection, while full-network fine-tuning produces superior STP detection with sufficient data.

Conclusion: The work establishes feasibility of STP detection, provides baseline performance metrics, and offers insights into efficient model adaptation for future AI-driven navigation aids and level-design tools.
...
<details>
  <summary>Details</summary>
Motivation: Current audio-driven animation methods work well for speech/singing but fail in complex film/TV productions requiring nuanced interactions, realistic body movements, and dynamic camera work.

Method: Proposed Wan-S2V model built upon Wan framework, designed specifically for cinematic contexts with enhanced expressiveness and fidelity.

Result: Extensive experiments show Wan-S2V significantly outperforms cutting-edge models like Hunyuan-Avatar and Omnihuman. Also demonstrates versatility in long-form video generation and precise lip-sync editing.

Conclusion: Wan-S2V successfully addresses the challenge of achieving film-level character animation, offering superior performance and broader applications compared to existing solutions.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal fusion methods for cancer survival analysis face limitations: fixed fusion schemes cause over-reliance on predefined combinations, and MoE-based methods limit information interaction between decoupled features.

Method: DeReF framework with three components: modality decoupling module with regional cross-attention, random feature reorganization strategy, and dynamic MoE fusion modules to enhance feature diversity and interaction.
...
Conclusion: The proposed DeReF framework successfully addresses limitations of existing fusion methods by improving feature combination diversity and information interaction, demonstrating superior performance in cancer survival analysis.
...
TL;DR: ROSE is a video object removal framework that specifically addresses object side effects like shadows, reflections, light, translucency and mirror effects using synthetic data generation and diffusion transformers.


<details>
  <summary>Details</summary>
Motivation: Existing video object removal methods struggle with eliminating object side effects (shadows, reflections, etc.) due to scarcity of paired video data for supervision.

Method: Uses 3D rendering engine for synthetic data generation, constructs automatic pipeline for diverse paired dataset, implements diffusion transformer-based video inpainting with reference-based erasing and additional supervision for side effect prediction.
...
Conclusion: The framework successfully addresses object side effect removal through synthetic data generation and specialized model architecture, presenting a comprehensive benchmark (ROSE-Bench) for evaluation.
...
Method: Two-pronged approach: 1) Data: Constructed HMD-270K dataset using Motion-Detail Fusion and Fine-Grained Examination pipeline; 2) Optimization: Developed Caption Set Equivalence Reward (CSER) with Group Relative Policy Optimization for unit-to-set matching and bidirectional validation.
...
Conclusion: The proposed HMD-270K dataset and OwlCap model with motion-detail balance effectively address the motion-detail imbalance problem in video captioning, and both will be publicly released to advance video captioning research.
...
<details>
  <summary>Details</summary>
Motivation: Automated detection of Oracle Bone Inscriptions (OBIs) from rubbing images is challenging due to degradation factors like noise and cracks that limit conventional detection networks' effectiveness in digital archaeology.

Method: Proposes clustering-based feature space representation learning using Oracle Bones Character font library as prior knowledge. Incorporates specialized loss function from clustering results to optimize feature representation, integrated into total network loss.

Result: Experiments on two OBIs detection datasets using Faster R-CNN, DETR, and Sparse R-CNN frameworks all demonstrated significant performance improvements.

Conclusion: The method effectively addresses degradation challenges in OBIs detection by leveraging prior knowledge through clustering-based representation learning, showing robust performance across multiple detection frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Existing spatial domain SNR priors for underwater image enhancement cannot effectively separate cross-channel interference and provide limited noise suppression and structure amplification capabilities.

Method: Proposes Fourier Attention SNR-prior Transformer (FAST) that uses frequency domain SNR priors, decomposing features into amplitude and phase spectra. Includes Frequency Adaptive Transformer (FAT) bottleneck with gated attention mechanism to merge low- and high-frequency branches in a U-shaped architecture.

Result: Trained on 4,800 paired images from UIEB, EUVP, and LSUI datasets, SFormer achieves 3.1 dB gain in PSNR and 0.08 improvement in SSIM compared to recent methods, successfully restoring colors, textures, and contrast.

Conclusion: Frequency domain SNR priors combined with Fourier attention and adaptive frequency processing significantly outperform spatial domain approaches for underwater image enhancement.
...
Method: Hierarchical network design with low-level stages using CNNs for single-frame detail preservation and high-level stages using Mamba architecture for spatio-temporal relationships. Introduces Spatio-temporal Cross Scan (STCS) module for long-range context integration across frames and positions.

Result: The proposed approach aims to overcome EF calculation biases caused by ultrasound image noise and other factors by balancing single-frame and multi-frame processing.

Conclusion: The hierarchical spatio-temporal segmentation network synergizes local detail modeling with global dynamic perception to improve EF estimation accuracy in echocardiography videos.
...
<details>
  <summary>Details</summary>
Motivation: Current UDA methods relying on fine-tuning feature extractors are inefficient, less interpretable, and poorly scalable to modern architectures. Domain shifts primarily manifest as boundary misalignment rather than feature degradation.

Method: Feature-space Planes Searcher (FPS) framework that optimizes decision boundaries by leveraging domain-invariant geometric patterns (intra-class clustering and inter-class separation) in pre-trained models while keeping the feature encoder frozen.

Result: Achieves competitive or superior performance to state-of-the-art methods on public benchmarks, scales efficiently with multimodal large models, and shows versatility across diverse domains including protein structure prediction, remote sensing classification, and earthquake detection.

Conclusion: FPS provides a simple, effective, and generalizable paradigm for transfer learning, particularly in domain adaptation tasks, with substantially reduced memory and computational costs through offline feature extraction.
...
Method: Customized InceptionV3 extracts spatial features, LSTM models temporal dependencies, and ensemble genetic algorithm with ADFSA selects compact feature sets balancing accuracy, redundancy, and complexity.

Result: Achieves 99.65% recognition accuracy on UCF-YouTube dataset, reduces features to as few as 7, and enhances inference time for real-time performance.

Conclusion: The lightweight and scalable framework enables practical HAR deployment on edge devices like Raspberry Pi for applications in public safety, assistive technology, and autonomous monitoring systems.
...
Method: Proposes ColorGS framework with Colored Gaussian Primitives (dynamic anchors with learnable color parameters) and Enhanced Deformation Model (time-aware basis functions + time-independent deformations) for adaptive texture encoding and precise deformation capture.

Result: Achieves PSNR of 39.85 (1.5 higher than prior 3DGS methods) and SSIM of 97.25% on DaVinci robotic surgery videos and benchmark datasets, while maintaining real-time rendering efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Neural networks trained on imbalanced datasets tend to favor majority classes, leading to poor recognition performance for minority classes. Standard flooding regularization doesn't address class imbalance specifically.

Method: Extension of flooding regularization applied at class level with class-specific flooding levels based on class frequencies. Suppresses overfitting in majority classes while allowing sufficient learning for minority classes.

Result: Validated on imbalanced image classification. Improves minority class performance and achieves better overall generalization compared to conventional flooding regularizations.

Conclusion: Class-wise flooding regularization effectively addresses class imbalance by tailoring regularization strength to each class's frequency, leading to improved minority class recognition and better overall model generalization.
...
<details>
  <summary>Details</summary>
Motivation: Neural networks often overfit to specific samples/domains, leading to poor generalization. While Curriculum Learning (CL) addresses this through difficulty-based sample selection and SAM improves robustness via flat minima, combining them is challenging because flat regions make traditional difficulty measures ineffective.
...
Result: Evaluated on image classification, fine-grained recognition, and domain generalization tasks. Outperforms existing curriculum-based and flatness-aware training strategies, preserving strengths of both CL and SAM.

Conclusion: ADM enables effective combination of Curriculum Learning and SAM by providing informative difficulty measures in flat regions, leading to improved generalization performance across multiple tasks.
...
<details>
  <summary>Details</summary>
Motivation: Underwater object detection faces unique challenges including degraded image quality and imbalanced class distribution, with unclear underlying causes for performance disparities across marine species.

Method: Manipulated DUO dataset to separate object detection into localization and classification tasks, used YOLO11 and TIDE for localization analysis, and conducted classification experiments with balanced data to investigate scallop class under-performance.

Result: Foreground-background discrimination identified as most problematic stage regardless of data quantity; persistent precision gaps found even with balanced data, indicating intrinsic feature-based challenges beyond data scarcity.

Conclusion: Recommend imbalanced distributions for precision prioritization and balanced distributions for recall prioritization; improving under-performing classes requires algorithmic advances, especially in localization modules.
...
<details>
  <summary>Details</summary>
Motivation: Traditional CAD generative methods don't align with industrial workflows that start with 2D drawings. Automatic generation from 2D vector drawings remains underexplored despite being critical for engineering design.
...
Result: Developed CAD-VGDrawing dataset for training and evaluation. The framework effectively transforms 2D engineering drawings into parametric CAD models while preserving geometric precision.

Conclusion: Drawing2CAD successfully bridges the gap between traditional 2D drawing workflows and modern CAD generation, providing an effective solution for converting engineering drawings to parametric models with preserved design intent.
...
<details>
  <summary>Details</summary>
Motivation: Existing AVSR systems struggle to estimate audio reliability and adapt modality weighting in noisy environments, leading to poor performance when audio quality deteriorates.

Method: Router-gated cross-modal feature fusion that adaptively reweights audio and visual features based on acoustic corruption scores, using gated cross-attention in decoder layers to reinforce visual cues when audio is unreliable.

Result: Achieves 16.51-42.67% relative reduction in word error rate compared to AV-HuBERT on LRS3 dataset, with ablation studies confirming both router and gating mechanisms contribute to improved robustness.

Conclusion: The proposed framework effectively enables models to pivot toward visual modality when audio quality is poor, demonstrating significant improvements in noisy environment speech recognition.
...
<details>
  <summary>Details</summary>
Motivation: Existing HOI benchmarks use exact class matching that penalizes VLMs and methods for plausible interpretations not explicitly annotated, creating unfair evaluation.

Method: Introduces new benchmark with multiple-answer multiple-choice format where questions include ground-truth positives and curated negatives to reduce ambiguity.

Result: Enables direct comparison between general-purpose VLMs and specialized HOI methods, providing fair evaluation that doesn't penalize valid predictions.

Conclusion: The proposed benchmark offers the first evaluation protocol suitable for both VLMs and HOI-specific methods, providing better insights into HOI understanding progress.
...
Method: Integrates Multimodal Chain-of-Thought reasoning and Retrieval-Augmented Generation to create semantically plausible visual options, plus a discrimination module to identify suitable content.

Result: Experimental results show superiority in content discrimination, question generation, and visual option generation across various subjects and educational levels.

Conclusion: CmOS effectively addresses the gap in visual option generation for MCQs and provides a scalable solution for high-quality educational content creation.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in deploying real-time physiological monitoring systems on resource-constrained platforms, including scalability, interoperability, and performance issues in smart environments with low-power computing devices.
...
Result: System achieves continuous, reliable operation at 30 frames per second with adaptive feedback through collaborative user interface. Demonstrates robustness under real-time constraints while minimizing computational overhead.

Conclusion: Provides practical solutions for optimizing real-time biosignal monitoring performance in healthcare and human-computer interaction applications, addressing key deployment challenges on low-power devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing online mapping approaches require expensive ground-truth HD maps for training, which are costly and geographically limited, hindering reliable generalization.

Method: Generates pseudo-labels by reconstructing road surface from multi-camera imagery using Gaussian splatting and pre-trained 2D segmentation. Introduces mask-aware assignment algorithm and loss function to handle partially masked pseudo-labels.
...
Conclusion: PseudoMapTrainer provides a novel approach to overcome the dependency on expensive ground-truth maps, making online mapping more scalable and accessible through pseudo-label generation from unlabeled data.
...
<details>
  <summary>Details</summary>
Motivation: Effective waste sorting is critical for sustainable recycling, but AI research lags behind commercial systems due to limited datasets and reliance on legacy object detectors. The paper aims to advance AI-driven waste detection by addressing domain-specific limitations.

Method: Benchmarked state-of-the-art OVOD models with LLM-optimized prompts, fine-tuned transformer-based detectors, and proposed a soft pseudo-labeling strategy that fuses ensemble predictions using spatial and consensus-aware weighting for semi-supervised training.

Result: Achieved new baseline of 51.6 mAP with fine-tuned detectors. The ensemble-based pseudo-labeling on unlabeled ZeroWaste-s subset achieved performance gains surpassing fully supervised training, demonstrating effective scalable annotation pipelines.

Conclusion: The work contributes rigorous baselines, robust ensemble-based pseudo-labeling pipeline, high-quality annotations for ZeroWaste-s, and systematic OVOD evaluation under real-world waste sorting conditions, advancing AI-driven waste detection research.
...
Method: Construct a graph with impression tags as nodes and co-occurrence relationships as edges, then apply spectral embedding to obtain impression vectors for each tag.

Result: The method assigns similar vectors to impression tags that frequently co-occur, unlike BERT and CLIP which yield very different vectors. Outperforms both in qualitative and quantitative evaluations for impression-guided font generation.

Conclusion: The proposed spectral embedding approach effectively captures font-impression relationships and is particularly useful for impression-based font generation and retrieval applications.
...
<details>
  <summary>Details</summary>
Motivation: National Forest Inventories require labor-intensive on-site campaigns, and current remote sensing approaches rely on Random Forest classifiers with hand-designed features. There's a need for more efficient methods that can update inventories more frequently using limited annotated data.

Method: Extracted time-series data from Sentinel-1, Sentinel-2, ERA5 satellites and SRTM data using Google Earth Engine. Fine-tuned a publicly available remote sensing time series foundation model (pre-trained on unannotated global data) for tree species classification with smaller labeled datasets.

Result: Fine-tuning the pre-trained foundation model outperformed current state-of-the-art NFI classification in the Netherlands by up to 10% across all datasets, demonstrating that classic hand-defined harmonic features are insufficient for this task.

Conclusion: Deep AI features from pre-trained foundation models significantly improve classification accuracy for data-limited applications like NFI classification, offering a powerful complement to traditional forest inventory processes using openly available satellite data.
...
<details>
  <summary>Details</summary>
Motivation: Address the MIDOG 2025 Challenge Track 2 focusing on binary classification of mitotic figures, tackling challenges of severe class imbalance, high morphological variability, and domain heterogeneity across different tumor types, species, and scanners.

Method: Leverages ConvNeXt V2 base model with 60% center cropping preprocessing and 5-fold cross-validation ensemble strategy, using mixed precision training for computational efficiency.

Result: Achieved robust performance on the diverse MIDOG 2025 dataset, demonstrating effectiveness for mitotic figure subtyping.

Conclusion: The solution shows modern convolutional architectures are effective for mitotic figure classification while maintaining computational efficiency through strategic preprocessing and training optimizations.
...
<details>
  <summary>Details</summary>
Motivation: Existing micro-expression analysis methods rely on window-level classification with fixed window sizes and hard decisions, limiting their ability to capture complex temporal dynamics. Current video-level regression approaches still depend on manually predefined window-based interval decoding.
...
Conclusion: The method effectively addresses limitations of existing approaches by enabling precise spotting of onset, apex, and offset phases while making efficient use of limited data through parameter sharing and complementary information exploitation.
...
Method: Uses geometric modeling of errors along with detection and scoring mechanisms through image-processing techniques on three hospital datasets with participants at various skill levels.

Result: The geometric metrics effectively replicate expert raters' scoring for the errors considered, enhancing efficiency and reliability of assessment.

Conclusion: The proposed quantitative framework advances microsurgical training protocols by providing objective, reliable assessment of surgical proficiency.
...
<details>
  <summary>Details</summary>
Motivation: Pixel-level synthesis methods for video stabilization struggle with robust generalization due to diverse motion profiles and visual content in videos. Fixed parameters make it difficult to handle this variability effectively.

Method: Proposes rapid adaptation of models to each input video during inference using low-level visual cues. Includes jerk localization module and targeted adaptation strategy focusing on high-jerk segments to maximize stability with fewer adaptation steps.

Result: Achieves significant performance gains even with single adaptation pass. Consistently improves various full-frame synthesis models in both qualitative and quantitative terms across diverse real-world datasets.

Conclusion: The method enables modern stabilizers to overcome state-of-the-art approaches while maintaining full-frame output and providing user control mechanisms similar to classical approaches, demonstrating strong versatility.
...
<details>
  <summary>Details</summary>
Motivation: Ensure fairness across demographic groups in medical diagnosis under distribution shifts caused by variations in imaging equipment and clinical practice, addressing residual cross-modal misalignment in existing debiasing approaches.

Method: Parallel dual-branch architecture separates sensitive and target attributes, uses orthogonal text anchors via linear projections, cross-attention mechanisms, hypernetwork for instance-aware visual prompts, and prototype-based regularization for feature separation and alignment.
...
Conclusion: DualFairVL effectively addresses cross-modal misalignment and fairness gaps in medical imaging, providing a parameter-efficient solution for equitable healthcare diagnosis under distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: Existing DETR-based HOI detection models use randomly initialized queries with vague representations, limiting effectiveness. Humans in HOI are fixed while objects/interactions vary, requiring better query enhancement.
...
Conclusion: DQEN effectively enhances query representations in HOI detection through object-aware and semantic fusion approaches, demonstrating improved performance over previous methods with randomly initialized queries.
...
<details>
  <summary>Details</summary>
Motivation: End-to-end autonomous driving approaches are difficult to interpret due to deep neural networks with non-linear decision boundaries, making it challenging to understand AI-driven decisions in complex urban scenarios, which is crucial for trustworthy AI deployment.
...
Conclusion: The method successfully enhances interpretability while optimizing control commands in autonomous driving, demonstrating that interpretable AI can achieve superior performance and safety compared to black-box approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional captioning and retrieval tasks focus on surface-level recognition of people/objects/scenes but overlook contextual and semantic dimensions that define real-world events.

Method: Built on OpenEvents V1 dataset with two tracks: Event-Enriched Image Retrieval & Captioning, and Event-Based Image Retrieval. 45 teams from 6 countries participated, evaluated through Public and Private Test phases.
...
Conclusion: EVENTA provides benchmark for event-level understanding with applications in journalism, media analysis, cultural archiving, and accessibility, advancing beyond traditional image analysis approaches.
...
<details>
  <summary>Details</summary>
Motivation: To analyze differences in space utilization and emergent behavioral patterns between group and single pedestrians in real-world trajectory data.

Method: Segment trajectories into time bins, apply Transformer-based pair classification model to identify groups, and establish spatial (convex hull area, enclosing circle radius, heatmap densities) and behavioral (velocity change, angle deviation, clearance radius, trajectory straightness) metrics.

Result: Developed classification pipeline and dataset structuring for scalable analysis across different sequence lengths (60, 100, 200 frames), with typology of encounter types established.

Conclusion: Establishes groundwork for future quantitative analysis of pedestrian interactions, with implications for pedestrian simulation and space design validation in crowd dynamics research.
...
<details>
  <summary>Details</summary>
Motivation: Monitoring coral reefs at large spatial scales is essential for ecosystem health assessment but challenging due to limited resolution of drone imagery and high cost of pixel-level annotations, limiting scalability of deep learning methods.

Method: Multi-scale weakly supervised semantic segmentation combining classification-based supervision, spatial interpolation, and self-distillation techniques to transfer fine-scale information from underwater to aerial imagery.
...
<details>
  <summary>Details</summary>
Motivation: Traditional GIS map-making requires domain expertise and is time-consuming, while current generative AI models struggle with accurate spatial composition and semantic layout control in map generation.

Method: The authors integrate vector data to guide map generation with textual prompts for style specification, creating a model that generates accurate maps in controlled styles, and develop a web application for improved usability.

Result: A user study with professional cartographers showed the potential of the developed application and GenAI models in helping both non-experts and professionals create maps more efficiently.

Conclusion: The approach demonstrates the viability of AI-assisted map-making, with outlined technical improvements and a redefined role for cartographers in advancing this new paradigm.
...
<details>
  <summary>Details</summary>
Motivation: To create a vision model that achieves state-of-the-art performance with efficient inference for fixed context-length tasks, while maintaining training stability without relying on common techniques like data augmentation or positional embeddings.

Method: Proposes a vision architecture combining token mixing, sequence-pooling operations, and convolutional tokenizers. The model is designed to be more efficient than Scaled Dot Product Attention transformers when context length is smaller than embedding dimension.

Result: Achieved 46.29% top-1 accuracy (vs 36.50% baseline) and 76.31% top-5 accuracy (vs 66.33% baseline) on CIFAR100 benchmark. The model is 60% the size of SDPA transformers and shows high training stability without needing mixup, positional embeddings, or learning rate scheduling.

Conclusion: The proposed architecture demonstrates superior performance and efficiency for fixed context-length vision tasks, offering a stable training alternative that eliminates the need for several common training techniques while maintaining competitive results.
...
TL;DR: This paper addresses privacy concerns in NeRF-based visual localization by proposing a privacy assessment protocol and developing ppNeSF, a NeRF variant trained with self-supervised segmentation labels instead of RGB images to protect sensitive scene details while maintaining localization accuracy.


<details>
  <summary>Details</summary>
Motivation: NeRF-based visual localization methods encode fine scene details in their geometry representations, creating privacy vulnerabilities when deployed in cloud-based services where sensitive information could potentially be recovered.

Method: The authors propose a privacy assessment protocol for NeRF representations and develop ppNeSF (Privacy-Preserving Neural Segmentation Field), a NeRF variant trained with self-supervised segmentation supervision instead of RGB images to obscure identifiable details while preserving 3D discriminativeness.

Result: The method shows that standard NeRFs trained with photometric losses store fine-grained details making them vulnerable to privacy attacks. ppNeSF achieves state-of-the-art visual localization results while protecting privacy through coarse segmentation labels.

Conclusion: The paper successfully addresses privacy concerns in NeRF-based visual localization by demonstrating privacy vulnerabilities in standard approaches and proposing an effective privacy-preserving alternative that maintains high localization accuracy.
...
TL;DR: RAG-based Document VQA with text and visual retrieval variants significantly outperforms concatenate-all-pages baseline, achieving up to +22.5 ANLS improvement without requiring OCR text extraction.


<details>
  <summary>Details</summary>
Motivation: Current Document VQA systems struggle with multi-page documents due to memory constraints from concatenating all pages or using large vision-language models. RAG offers a memory-efficient alternative by retrieving relevant segments first.

Method: Systematically evaluated RAG integration into Document VQA using two retrieval variants: text-based retrieval with OCR tokens and purely visual retrieval without OCR. Tested across multiple models and benchmarks including MP-DocVQA, DUDE, and InfographicVQA.

Result: Text-centric RAG improved baseline by up to +22.5 ANLS, visual variant achieved +5.0 ANLS improvement without text extraction. Retrieval and reranking components drove most gains, while layout-guided chunking strategy failed to help.
...
TL;DR: GRAS is a new benchmark for evaluating demographic biases (gender, race, age, skin tone) in Vision Language Models, revealing concerning bias levels with a novel interpretable scoring metric.


<details>
  <summary>Details</summary>
Motivation: As Vision Language Models become integral to real-world applications, understanding their demographic biases is critical for ensuring fair and equitable AI systems.

Method: Introduces GRAS benchmark with diverse demographic coverage and proposes GRAS Bias Score metric. Benchmarks five state-of-the-art VLMs using visual question answering with multiple question formulations.

Result: Reveals concerning bias levels - the least biased model scored only 2 out of 100 on GRAS Bias Score. Shows VQA bias evaluation requires multiple question formulations.

Conclusion: VLMs exhibit significant demographic biases requiring systematic evaluation. GRAS provides comprehensive benchmarking and interpretable scoring to address this critical issue.
...
TL;DR: RoofSeg is a novel end-to-end transformer-based network for roof plane segmentation from LiDAR point clouds that addresses edge accuracy issues through geometric constraints and adaptive weighting.
...
Method: Developed RoofSeg - an edge-aware transformer-based network using encoder-decoder framework with learnable plane queries. Includes Edge-Aware Mask Module (EAMM) for edge region improvement, adaptive weighting strategy in mask loss, and new plane geometric loss for training constraints.

Result: The proposed method achieves improved roof plane segmentation accuracy, particularly in edge regions, through geometric prior incorporation and end-to-end processing.

Conclusion: RoofSeg provides a comprehensive solution to the three main problems in current roof plane segmentation methods, offering better accuracy through transformer architecture, geometric constraints, and edge-aware processing.
...
<details>
  <summary>Details</summary>
Motivation: Microplastic pollution is widespread and harmful to humans, causing liver infections, intestinal injuries, and gut flora imbalance. There's a need for effective detection methods to identify microplastic presence in biological samples.

Method: MDN combines fluorescence microscopy with Nile Red dye staining and convolutional neural networks for segmentation. The approach involves dataset preparation, fluorescence imaging, and deep learning-based localization and counting of microplastic fragments in blood samples.
...
Conclusion: The MDN model demonstrates high effectiveness in microplastic detection and opens avenues for applying this approach to human blood samples for preliminary data collection on microplastic contamination.
...
TL;DR: ProPy adapts CLIP for partially relevant video retrieval using a prompt pyramid and interaction mechanism to handle multi-granularity events, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing PRVR methods use unimodal features and underutilize powerful pretrained vision-language models like CLIP for segment-level video retrieval.
...
Result: Achieves state-of-the-art performance on three public datasets, significantly outperforming previous models.

Conclusion: ProPy successfully bridges the gap by adapting CLIP for PRVR with systematic architectural innovations, demonstrating the value of leveraging pretrained vision-language models for segment-level video retrieval tasks.
...
TL;DR: Self-supervised pre-training on large geometric artery datasets improves wall shear stress assessment in coronary arteries with limited clinical data.


<details>
  <summary>Details</summary>
Motivation: Big data shows promise for patient care, but collecting large medical datasets for training machine learning models is challenging, especially for hemodynamic biomarkers like wall shear stress in coronary arteries.

Method: Used self-supervised pre-training on 8,449 geometric artery models by computing heat kernel signatures via Laplacian eigenvectors, then applied learned representations to assess wall shear stress in a small clinical dataset of 49 patients.

Result: Geometric representations learned from the large dataset significantly boosted segmentation of coronary arteries into low, mid, and high wall shear stress regions, even with limited training data.

Conclusion: Self-supervised pre-training on geometric artery models effectively addresses data scarcity and improves hemodynamic biomarker assessment in coronary arteries with small clinical datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing surface defect detection methods fail to meet industrial demands for high performance, efficiency, and adaptability across diverse data annotation scenarios encountered in real-world manufacturing processes.
...
Conclusion: SuperSimpleNet represents a promising step forward in addressing real-world manufacturing challenges and bridging the gap between academic research and industrial applications with its unified approach to diverse supervision paradigms.
...
<details>
  <summary>Details</summary>
Motivation: To improve single-pixel imaging reconstruction performance by learning optimized binary illumination patterns specifically tailored for applications like fluorescence microscopy.

Method: Bilevel optimization using Straight-Through Estimator to handle non-differentiable binary patterns, combined with Total Deep Variation regulariser in the bilevel formulation.

Result: Learned patterns demonstrate superior reconstruction performance compared to baseline methods on CytoImageNet microscopy dataset, particularly in highly undersampled scenarios.

Conclusion: The proposed bilevel optimization approach successfully learns task-specific binary illumination patterns that significantly enhance single-pixel imaging performance for microscopy applications.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras cannot generate events in static/low-motion scenes under fixed illumination, making them unsuitable for many computer vision tasks without complex hardware solutions.

Method: Uses a simple rotating unbalanced mass to induce periodic vibrational motion, combined with a motion-compensation pipeline that removes the injected motion to yield clean events.
...
Conclusion: The proposed lightweight vibration approach effectively addresses event camera limitations in static scenes, enabling persistent event generation without complex hardware.
...
TL;DR: Lightweight UNet++ with topology-aware loss achieves state-of-the-art text line segmentation using only 3 annotated pages per manuscript, significantly outperforming complex models on historical documents.
...
Result: 200% increase in Recognition Accuracy and 75% increase in Line Intersection over Union on U-DIADS-TL dataset. Achieves F-Measure score on par with or exceeding DIVA-HisDB competition winner using only 3 annotated pages.
...
TL;DR: The paper addresses monocular 3D visual grounding by solving the problem where text embeddings are sensitive to numerical value magnitudes but ignore measurement units, proposing two methods (3DTE and TGE) to enhance 3D perception and achieve state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current monocular 3D visual grounding methods show that text embeddings are sensitive to numerical value magnitudes but largely ignore measurement units, leading to performance degradation despite equivalent physical lengths, indicating weak 3D comprehension in pre-trained language models.

Method: Proposes two methods: 1) 3D-text Enhancement (3DTE) - pre-processing method that enhances comprehension of unit mapping relationships by augmenting distance descriptor diversity in text queries; 2) Text-Guided Geometry Enhancement (TGE) module - projects basic text features into geometrically consistent space to enhance 3D-text information and guide geometry feature attention.

Result: Achieves substantial improvements over previous methods with 11.94% accuracy gain in the "Far" scenario on Mono3DRefer dataset, setting new state-of-the-art results.

Conclusion: The proposed 3DTE and TGE methods effectively enhance 3D perception in text embeddings and geometry features, significantly improving monocular 3D visual grounding performance by addressing unit sensitivity issues in language models.
...
TL;DR: WEF-PE is a novel positional encoding method using Weierstrass elliptic functions that preserves 2D spatial structure in Vision Transformers, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional positional embeddings in Vision Transformers disrupt 2D spatial structure and lack geometric constraints, failing to establish proper correspondence between Euclidean distances and sequential indices, limiting spatial prior utilization.

Method: Proposes Weierstrass Elliptic Function Positional Encoding (WEF-PE) that uses complex domain representation with doubly periodic elliptic functions to naturally encode 2D coordinates, leveraging non-linear geometric properties and algebraic addition formulas for relative positional information.
...
Conclusion: WEF-PE provides a mathematically principled solution that better preserves 2D spatial relationships in Vision Transformers, demonstrating superior performance and more coherent semantic focus compared to conventional positional encoding methods.
...
Result: The report presents results from each challenge, highlights top-performing solutions, and provides insights into community progress in football video analysis tasks.
...
TL;DR: A novel mesh generation framework that separates vertex and face generation, reducing token redundancy by 77% and achieving 8x faster generation speed while improving mesh quality.


<details>
  <summary>Details</summary>
Motivation: Existing mesh generation methods tokenize meshes into sequences that redundantly reuse vertices multiple times (since each vertex is shared by multiple faces), leading to excessively long token sequences and inefficient generation processes.

Method: 1) Use autoregressive model only for vertex generation (reducing tokens to ~23% of existing methods) 2) Employ bidirectional transformer to complete mesh in single step by capturing vertex relationships and building adjacency matrix 3) Add fidelity enhancer to refine vertex positions 4) Implement post-processing to remove undesirable edge connections

Result: Achieves more than 8x faster mesh generation speed compared to state-of-the-art approaches while producing higher quality meshes.

Conclusion: The proposed framework successfully addresses vertex redundancy in mesh generation through separate vertex and face processing, significantly improving both efficiency and quality of artistic mesh generation.
...
TL;DR: All-in-One Slider is a lightweight module that enables fine-grained control over multiple image attributes through sparse semantic decomposition of text embeddings, eliminating the need for training separate sliders for each attribute.


<details>
  <summary>Details</summary>
Motivation: Existing T2I diffusion models struggle with progressive attribute manipulation, especially for detailed content like human faces. Current methods require training individual sliders for each attribute, leading to parameter redundancy and limited scalability.

Method: Decomposes text embedding space into sparse, semantically meaningful attribute directions. Once trained, functions as a general-purpose slider supporting zero-shot manipulation of unseen attributes and attribute composition.
...
Conclusion: The All-in-One Slider provides interpretable, fine-grained control over various attributes while supporting zero-shot manipulation and attribute composition, significantly improving flexibility and scalability compared to One-for-One approaches.
...
TL;DR: A method for generating large-scale 3D driving scenes with accurate geometry and controllability, combining proxy geometry generation with score distillation from 2D image priors.


<details>
  <summary>Details</summary>
Motivation: Existing neural reconstruction methods are limited to static environments and constrained by captured data, while diffusion models lack geometry grounding and causality. This work bridges the gap between controllable generation and physically accurate 3D scenes.

Method: Combines proxy geometry and environment representation generation with score distillation from learned 2D image priors. Allows prompt-guided geometry and high-fidelity texture/structure conditioned on map layouts.
...
Conclusion: The approach enables high controllability for generating large-scale 3D driving scenes with accurate geometry, addressing limitations of both reconstruction methods and diffusion models.
...
TL;DR: OmniHuman-1.5 is a video avatar framework that generates semantically coherent character animations using multimodal LLMs and specialized DiT architecture, achieving superior performance in lip-sync, video quality, and semantic consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video avatar models focus on physical likeness but lack deeper semantic understanding of emotion, intent, and context, producing motions that only synchronize with low-level audio cues rather than capturing authentic character essence.

Method: Leverages Multimodal Large Language Models to synthesize structured textual representations for high-level semantic guidance, and introduces a specialized Multimodal DiT architecture with Pseudo Last Frame design to effectively fuse multimodal inputs and mitigate inter-modality conflicts.

Result: Achieves leading performance across comprehensive metrics including lip-sync accuracy, video quality, motion naturalness, and semantic consistency with textual prompts. Shows remarkable extensibility to complex scenarios involving multi-person and non-human subjects.

Conclusion: The proposed framework successfully bridges the gap between physical plausibility and semantic coherence, enabling generation of character animations that are contextually and emotionally resonant through accurate interpretation of joint semantics from audio, images, and text inputs.
...
TL;DR: VFTrack is a real-time particle tracking framework that automatically detects and tracks individual carbon nanotube particles in SEM image sequences, enabling kinematic analysis of CNT micropillar growth with optimal feature detection and matching.
...
Method: Integrates handcrafted or deep feature detectors and matchers within a particle tracking framework, systematically evaluating combinations using 13,540 manually annotated trajectories to identify optimal detector-matcher pair (ALIKED detector with LightGlue matcher).

Result: Achieves F1-score of 0.78 and α-score of 0.89, decomposes motion vectors into axial growth, lateral drift, and oscillations, enabling calculation of heterogeneous regional growth rates and reconstruction of evolving CNT pillar morphologies.

Conclusion: Enables automated nano-material characterization and bridges the gap between physics-based models and experimental observation, facilitating real-time optimization of CNT synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Current video segmentation landscape is fragmented with task-specific models, and many real-world applications require unprompted segmentation that detects and tracks all objects without external cues.

Method: Recasts streaming video segmentation as sequential mask prediction using state-space models, maintaining fixed-size spatial state and enabling parallel training across frames for efficiency.
...
Conclusion: AUSM successfully unifies prompted and unprompted video segmentation in a single scalable architecture with improved performance and training efficiency.
...
TL;DR: Style4D-Bench is the first benchmark for 4D stylization, featuring evaluation protocols, a baseline method (Style4D), and curated dynamic scenes. Style4D uses 4D Gaussian Splatting with Style Gaussian Representation and geometry-preserved style transfer to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To standardize evaluation and facilitate progress in 4D stylization by creating the first comprehensive benchmark suite for this emerging research area.
...
Conclusion: Style4D-Bench serves as a valuable resource for benchmarking and advancing research in stylized rendering of dynamic 3D scenes, with Style4D establishing a strong baseline for 4D stylization.
...
TL;DR: Articulate3D is a training-free method that uses language instructions to pose 3D assets through a two-step process: generating target images with modified structure-pose decoupling, then optimizing mesh alignment using keypoint correspondences instead of differentiable rendering.
...
Method: Two-step approach: 1) Modify image generator with self-attention rewiring (RSActrl) to create target images conditioned on input image and text, decoupling source structure from pose. 2) Align mesh to target images through multi-view pose optimization using keypoint correspondences rather than differentiable rendering.

Result: Method successfully manipulates poses across diverse 3D objects and free-form text prompts while maintaining original mesh identity. Quantitative evaluations and user study show superiority, with method preferred over 85% of existing approaches.

Conclusion: Articulate3D provides an effective training-free solution for language-controlled 3D pose manipulation, demonstrating that keypoint-based optimization outperforms differentiable rendering for articulation tasks.
...
TL;DR: VoxHammer is a training-free 3D editing method that performs precise local editing in 3D latent space while preserving unedited regions through inversion trajectory prediction and feature replacement.


<details>
  <summary>Details</summary>
Motivation: 3D local editing is crucial for game industry and robot interaction, but existing methods struggle with preserving unedited regions and maintaining overall coherence when editing multi-view images before reconstruction.

Method: VoxHammer predicts inversion trajectory of 3D models to obtain inverted latents and key-value tokens, then replaces denoising features of preserved regions with these cached features during editing to ensure consistency.

Result: VoxHammer significantly outperforms existing methods in 3D consistency of preserved regions and overall quality, as demonstrated on the Edit3D-Bench dataset with human-annotated samples.

Conclusion: The method enables high-quality 3D editing with precise preservation of unedited areas and coherent integration of edited parts, promising for synthesizing paired data for in-context 3D generation.
...
<details>
  <summary>Details</summary>
Motivation: Electromagnetic signals differ significantly from text and images with high heterogeneity, strong background noise, and complex time-frequency structures, making existing general models ineffective. Current methods lack cross-task generalization and suffer from scarce high-quality datasets.

Method: Built the first unified and largest standardized electromagnetic signal dataset. Developed a length-adaptive multi-signal packing method and hardware-aware training strategy that leverage physical properties of electromagnetic signals for efficient representation learning from heterogeneous multi-source signals.

Result: EMind achieves strong performance and broad generalization across many downstream tasks, demonstrating effective transition from task-specific models to a unified framework for electromagnetic intelligence.

Conclusion: The proposed EMind foundation model successfully bridges large-scale pretraining with the unique nature of electromagnetic signals, providing a unified framework that overcomes previous limitations in electromagnetic signal processing and enables cross-task generalization.
...
TL;DR: Federated learning framework for ischemic stroke lesion segmentation using DWI sequences, achieving better performance than centralized approaches while preserving data privacy across multiple healthcare centers.
...
Method: Developed a collaborative federated learning framework (FedAvg) that shares knowledge from deep center-independent representations across 14 healthcare centers with 2031 studies, without sharing raw patient data.
...
Conclusion: The federated learning approach provides an effective solution for stroke lesion segmentation that generalizes well across different healthcare centers while addressing data privacy concerns and limited labeled data availability.
...
<details>
  <summary>Details</summary>
Motivation: Machine unlearning for removing sensitive data from pre-trained models hasn't been explored in medical image classification, despite the critical need for privacy in healthcare applications.
...
TL;DR: Comparative study of ResNet50, Inception v3 and VGG19 for psoriasis skin lesion classification, with Inception v3 showing best performance (97.5% accuracy) for diagnostic support.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare the performance of different CNN architectures for automated classification of psoriasis skin lesions to support medical diagnosis.
...
Result: Inception v3 achieved the best performance with 97.5% accuracy and F1-Score (± 0.2), outperforming ResNet50 and VGG19 models.
...
TL;DR: A novel weakly-supervised method for macular edema segmentation that leverages retinal layer correlations and test-time adaptation to bridge performance gaps with fully-supervised approaches.
...
Method: Enhances an off-the-shelf adversarial framework with layer-structure-guided post-processing and test-time adaptation (TTA) strategy, reframing edema prediction as confirming intersection points between edema contours and retinal layers.

Result: Extensive experiments on two public datasets show improved accuracy and robustness in edema segmentation, narrowing the performance gap between weakly-supervised and fully-supervised models.

Conclusion: Incorporating retinal layer information and TTA strategy significantly enhances weakly-supervised edema segmentation performance while maintaining cost-effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: MRI acquisition is time-consuming and costly, leading to interest in accelerated MRI methods. While most research focuses on perfect reconstruction, many applications only need accurate segmentation from accelerated data, but existing methods lack comprehensive comparison and unified evaluation standards.

Method: The study compares 7 segmentation approaches for undersampled MRI data, focusing on one-stage (combined reconstruction+segmentation) vs two-stage (reconstruction followed by segmentation) methods. Tests were conducted on two MRI datasets with multi-coil k-space data and human-annotated segmentation ground-truth.
...
Conclusion: For segmenting accelerated MRI data, straightforward two-stage approaches that maintain data-consistency are more effective than sophisticated one-stage methods, providing practical guidance for clinical applications.
...
TL;DR: RDDM is an end-to-end diffusion model that restores photo-realistic images directly from sensor RAW data, bypassing traditional ISP pipelines and achieving superior fidelity compared to sRGB-domain methods.


<details>
  <summary>Details</summary>
Motivation: Existing sRGB-domain diffusion models face a dilemma between high fidelity and realistic generation due to processing lossy sRGB inputs and ignoring the availability of sensor RAW images in edge devices, leading to suboptimal performance.
...
Conclusion: RDDM effectively addresses the limitations of sRGB-domain approaches by working directly with RAW sensor data, providing an end-to-end solution that outperforms conventional two-stage ISP + IR pipelines.
