...
Motivation: Current hallucination detection methods using VLMs are computationally intensive and increase model latency. There's a need for efficient on-the-fly detection of hallucinations in VLM-generated text.

Method: Train traditional ML models using NTP-based features from VLMs, which quantify model uncertainty. Also explore augmenting with linguistic NTPs and integrating VLM hallucination prediction scores.

Result: NTP-based features are effective predictors of hallucinations, enabling simple ML models to achieve performance comparable to strong VLMs. Augmentation with linguistic NTPs and VLM scores further improves detection performance.

Conclusion: NTP-based lightweight methods provide efficient hallucination detection that enhances VLM reliability, paving the way for simple solutions without heavy computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Offline signature verification is challenging in writer-independent settings where models must generalize to unseen individuals. Current methods depend on real-world datasets, which limits scalability and generalization.

Method: Uses Riemannian Gaussian Mixture on SPD matrices to generate synthetic writers from a small set of genuine samples. Riemannian Gaussian sampling creates positive/negative synthetic SPD populations, followed by metric learning with similar/dissimilar pairs.

Result: Experiments on Western and Asian signature datasets show low error rates under intra- and cross-dataset evaluation protocols, demonstrating effective generalization.

Conclusion: The approach highlights the potential of generating synthetic data in Riemannian spaces for writer-independent signature verification systems, reducing dependency on real-world training data.
...
<details>
  <summary>Details</summary>
Motivation: To extend traditional text-to-image systems into more interactive and multidimensional creative tools by unifying multiple image generation tasks within a single efficient framework.

Method: Developed a highly efficient diffusion transformer with powerful VAE to reduce image tokens, pretrained on billions of text-image pairs with comprehensive data collection across vertical scenarios, and incorporated multi-modal post-training with VLM for joint T2I and image editing training.

Result: Achieves state-of-the-art results on both T2I and multimodal image editing, with inference time of 1.8 seconds for 2K images, exceptional multimodal capabilities in complex tasks, and support for multi-image reference and multiple output generation.

Conclusion: Seedream 4.0 pushes the boundary of generative AI for both creativity and professional applications by enabling efficient high-resolution image generation and complex multimodal tasks within a unified framework.
...
<details>
  <summary>Details</summary>
Motivation: Breast cancer is a major global health concern where early detection is crucial. Deep learning methods for computer-aided detection face challenges with limited labeled datasets, which this study aims to address.

Method: Used Contrastive Learning framework with Resnet-50 in semi-supervised approach, employing similarity index on large unlabeled mammogram data with various augmentations and transformations, then fine-tuned on small labeled dataset.

Result: Achieved 96.7% accuracy in breast cancer detection on benchmark datasets INbreast and MIAS, outperforming existing state-of-the-art methods.

Conclusion: The proposed Contrastive Learning framework effectively addresses the limited labeled data problem in breast cancer detection and demonstrates superior performance compared to current methods.
...
Result: All tested Foundation Models failed on real-world industrial image data, while performing well on public benchmark datasets, revealing a significant performance gap.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models in imaging and vision are inefficient for applications requiring multiple modular tasks, as each task requires mapping to different latent domains, leading to redundancy and poor generalization across domain shifts.

Method: A lightweight CNN-based encoder-decoder framework that pre-computes transformation-aware, generalizable representations in a shared Neural Space, enabling multiple downstream AI modules to operate on the same feature space.

Result: The proposed architecture demonstrates efficient performance across various vision tasks including demosaicing, denoising, depth estimation, and semantic segmentation within the shared Neural Space.

Conclusion: The Neural Space approach establishes an efficient foundation for multi-task vision pipelines, offering better generalization, reduced redundancy, and wider hardware compatibility compared to larger transformer backbones.
...
<details>
  <summary>Details</summary>
Motivation: Edge camera systems need regular model updates but face computational constraints. Current practice uses complex teacher models on central servers to annotate data for training smaller edge models, requiring efficient image selection to balance transmission costs and model quality.

Method: The approach combines high-confidence stream-based strategy with diversity-based selection to identify the most valuable training images, minimizing dataset queries while maintaining training iterations.

Result: The proposed method produces high-quality models with minimal dataset queries compared to standard approaches, achieving similar training load efficiency.

Conclusion: A hybrid strategy of high-confidence stream selection and diversity-based filtering is effective for optimizing edge model training by reducing transmission costs while maintaining or improving model quality.
...
<details>
  <summary>Details</summary>
Motivation: Traditional virtual try-on systems require precise binary masks that are difficult to create, model-dependent, and insufficient for complex styling instructions like "sleeves rolled up" scenarios where masks cannot capture the desired transformation.

Method: The system uses Vision Language Models (VLMs) and image segmentation models to automatically generate binary masks based on user-provided images and free-text style instructions, enabling complex styling control without manual mask drawing.

Result: InstructVTON achieves state-of-the-art results by being interoperable with existing virtual try-on models, simplifying the user experience and enabling try-on scenarios that were previously impossible with masking-based approaches alone.

Conclusion: The system demonstrates that automated mask generation through VLMs can overcome limitations of traditional masking-based virtual try-on, providing fine-grained styling control through natural language instructions while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Altered fingerprint recognition is crucial for security applications like border control and forensics, as adversaries can modify ridge patterns to evade detection, requiring robust recognition systems.
...
Result: Achieves accuracies of 96.7%, 98.76%, and 99.54% for Easy, Medium, and Hard levels respectively with strict thresholds. Threshold sensitivity study shows accuracy drops significantly when threshold is relaxed from 0.92 to 0.72.

Conclusion: DeepAFRNet addresses limitations of prior work by using real altered samples and demonstrates readiness for real-world deployments where security and recognition resilience are critical.
...
<details>
  <summary>Details</summary>
Motivation: To enhance bimanual robotic manipulation by incorporating semantic cues from pre-trained vision transformers into 3D voxel representations.
...
Conclusion: Attention-guided featurization from pre-trained vision transformers significantly enhances robotic manipulation performance in voxel-based policies.
...
Method: Evaluated 36 model variants on a curated dataset of 661 canopy images with 85,879 labeled blueberries. Applied Unbiased Mean Teacher-based semi-supervised learning on 1,035 unlabeled images for fine-tuning.

Result: YOLOv12m achieved 93.3% mAP@50, RT-DETRv2-X achieved 93.6% mAP@50. After SSL fine-tuning, RT-DETR-v2-X reached 94.8% mAP@50. Mid-sized models offered best accuracy-speed balance.

Conclusion: RT-DETR-v2-X performed best overall. Semi-supervised learning improved detection accuracy, but more research is needed to better leverage cross-domain unlabeled data. Dataset and software made publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Breast cancer screening with mammography is crucial for early detection, but deep learning models face limitations due to small datasets and low resolution. The authors aim to improve performance through simple data-centric augmentation without requiring additional labels or architectural changes.

Method: The method involves probabilistically replacing full mammogram images with random ROI crops during training, sampled from a precomputed bounding-box bank. The approach includes optional jitter for variability and is evaluated using patient-level cross-validation with ROC-AUC, PR-AUC, and efficiency metrics.

Result: ROI augmentation (with optimal parameters p_roi = 0.10, alpha = 0.10) yielded modest ROC-AUC gains on Mini-DDSM, though performance varied across folds. PR-AUC remained flat to slightly lower. The method maintained inference-time efficiency since augmentation is training-only.

Conclusion: Simple ROI augmentation strategies can enhance mammography classification in data-constrained settings without requiring additional labels or architectural modifications, demonstrating the value of data-centric approaches.
...
<details>
  <summary>Details</summary>
Motivation: Mirror reflections provide stereo information in a single capture, enabling multi-view stereo from a single image without complex setups.

Method: Design transformation to construct physically valid virtual cameras from mirror reflections, use symmetric-aware loss for pose refinement, and extend to dynamic scenes.

Result: Enables robust 3D reconstruction compatible with feed-forward models, validated on synthetic dataset and real-world experiments.

Conclusion: Mirror reflections can effectively serve as virtual views for single-image 3D reconstruction, simplifying imaging while maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To understand what semantic representations humans employ to recognize social interactions from simple moving shapes, beyond just visual features.
...
<details>
  <summary>Details</summary>
Motivation: Address two key challenges in cross-view geo-localization: (1) achieving robustness under severe appearance variations from different UAV orientations and fields of view, and (2) establishing reliable correspondences that capture both global scene-level semantics and fine-grained local details.

Method: Propose EGS framework with E(2)-Steerable CNN encoder for stable feature extraction under rotation/viewpoint shifts, and a graph with virtual super-node that connects to all local nodes to aggregate global semantics and redistribute to local regions for global-local consistency.

Result: Extensive experiments on University-1652 and SUES-200 benchmarks show EGS consistently achieves substantial performance gains and establishes new state-of-the-art in cross-domain CVGL.

Conclusion: EGS effectively enhances cross-domain generalization in cross-view geo-localization by addressing viewpoint variation challenges through steerable features and global-local consistency mechanisms.
...
Method: Proposes a Dual-Path Edge Network with: 1) Bidirectional Interaction Module using Local and Global Self-Attention for multi-scale feature dependencies, and 2) Multi-Edge Refiner with cascaded Taylor finite difference operators and attention-driven gating for precise edge localization.
...
Conclusion: The proposed framework offers a promising solution that successfully addresses the fundamental challenge of feature misalignment in infrared small target detection through complementary edge enhancement and semantic modeling paths.
...
<details>
  <summary>Details</summary>
Motivation: Traditional intention recognition focuses on individual intentions but overlooks the complexities of collective intentions in group settings, creating a need to understand shared goals emerging through multiple individuals' actions.

Method: The paper introduces SHOT dataset with 1,979 basketball video clips from 5 camera views, annotated with 6 individual attributes, and proposes GIFT framework that extracts fine-grained individual features and models evolving group dynamics.
...
Conclusion: The work successfully addresses the gap in collective intention recognition by introducing GIF task, providing a comprehensive dataset, and demonstrating an effective framework for forecasting group intention emergence.
...
<details>
  <summary>Details</summary>
Motivation: Maritime object detection faces two key challenges: scarcity of annotated data and poor generalization across diverse maritime attributes. Models trained on existing datasets often underperform in underrepresented scenarios like open-sea environments.
...
Conclusion: The proposed Neptune-X framework effectively addresses maritime data scarcity and generalization issues through synthetic data generation and intelligent sample selection, demonstrating substantial improvements in detection performance across diverse maritime scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing Crater-Based Navigation methods are designed for short-duration landing missions with nadir views and good lighting, but lunar mapping missions involve sparse, oblique imagery under varying illumination over year-long campaigns, requiring a more robust solution.
...
Conclusion: This represents the first comprehensive assessment of CBN in true lunar mapping settings and informs operational conditions for future missions.
...
Method: Systematic investigation using forced-choice "Pick-a-Pic" tasks, analyzing layer-wise alignment patterns, testing robustness to semantic vs. appearance changes, and examining exemplar averaging effects.
...
Conclusion: Unimodal networks converge on a shared semantic code that aligns with human judgments and benefits from exemplar aggregation.
...
Method: Convert 2D objects to 3D using existing 3D generation models, perform interactive editing in 3D space, then re-render to 2D with specified views. Combine geometric control with style/content control via diffusion adapters.

Result: The framework produces geometrically controlled and style-consistent edited images without requiring training.

Conclusion: FreeInsert successfully addresses limitations of current methods by introducing 3D geometric control and ensuring style consistency through a training-free approach using diffusion models.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models for human photo synthesis face degraded scenes, insufficient control, and suboptimal perceptual identity. The authors aim to overcome these limitations by enhancing existing identity customization models.

Method: The framework uses face swapping techniques and pretrained diffusion models to obtain additional representations in a zero-shot manner. It employs a triple-flow fused PerGeneration approach that combines two compatible counter-directional latent spaces to manipulate personalized models. Also introduces ResInversion for noise rectification via pre-diffusion, reducing inversion time by 129x compared to NTI.

Result: Experiments show CustomEnhancer achieves state-of-the-art results in scene diversity, identity fidelity, and training-free controls. The ResInversion method demonstrates significant efficiency improvements over null-text inversion.

Conclusion: CustomEnhancer provides a comprehensive solution for personalized human photo synthesis with enhanced control, fidelity, and efficiency, eliminating the need for controller retraining for each model.
...
<details>
  <summary>Details</summary>
Motivation: With increasing use of neural network-based computer vision applications, there's a need for a consolidated platform to implement and evaluate compression methods optimized for downstream vision tasks given the variety of vision tasks, models, and datasets.

Method: The platform provides a comprehensive evaluation framework where coding tools compete to compress input data while retaining task accuracy in "remote" and "split" inferencing scenarios, incorporating standard codecs under development.

Result: The study showcases various use cases examining compression gain on several datasets in terms of bit-rate versus task accuracy. The platform has been adopted by MPEG for FCM standard development.

Conclusion: CompressAI-Vision serves as a common ground for evaluating vision-optimized compression methods and is publicly available as open-source software, contributing to the standardization efforts in feature coding for machines.
...
<details>
  <summary>Details</summary>
Motivation: Conventional SSDG methods assume labeled and unlabeled data are available for each source domain, but real-world scenarios often have domain shifts between labeled and unlabeled training data, which existing methods handle poorly due to inaccurate pseudolabels.

Method: DAC framework builds on co-training with two sub-models providing cross pseudo supervision, adding feature-level supervision and asymmetric auxiliary tasks to address inaccurate pseudo supervision and enhance domain-invariant feature learning.

Result: Extensive experiments on Fundus, Polyp, and SCGM medical image datasets demonstrate robust generalizability of the DAC framework in handling domain shifts.

Conclusion: The proposed DAC framework effectively addresses the challenging CD-SSDG scenario by integrating feature-level supervision and asymmetric auxiliary tasks to improve generalization performance under domain shifts.
...
<details>
  <summary>Details</summary>
Motivation: To extend the successful DEIM framework by incorporating DINOv3 features to create a more scalable and efficient real-time object detection system that can cover diverse deployment scenarios from GPU to mobile devices.
...
Result: DEIMv2-X achieves 57.8 AP with 50.3M parameters (surpassing 56.5 AP models requiring >60M parameters). DEIMv2-S is first sub-10M model to exceed 50 AP (50.9 AP with 9.71M parameters). DEIMv2-Pico achieves 38.5 AP with only 1.5M parameters, matching YOLOv10-Nano with 50% fewer parameters.

Conclusion: DEIMv2 establishes new state-of-the-art results with superior performance-cost trade-off across diverse scenarios, demonstrating significant improvements in parameter efficiency while maintaining high detection accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) are critical for safety-sensitive applications but remain vulnerable to adversarial attacks. CLIP, as the backbone for many VLMs, is a high-value target whose vulnerabilities can cascade across the multimodal AI ecosystem.

Method: Dynamic Adversarial Curriculum (DAC-LoRA) framework that integrates adversarial training into PEFT using an intelligent curriculum of progressively challenging attacks, guided by First-Order Stationary Condition (FOSC) and TRADES-inspired loss.

Result: Achieves substantial improvements in adversarial robustness without significantly compromising clean accuracy. The framework is lightweight and broadly applicable.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated domain generalization methods based on prompt learning have limited diversity and ignore information from unknown domains, posing challenges for downstream-task adaptation in federated learning settings with domain shifts among clients.
...
Conclusion: The proposed generative approach for handling federated domain generalization tasks effectively addresses limitations of existing prompt learning methods by generating diverse domain-specific soft prompts that can adapt to unknown domains.
...
Method: Uses contrastive language-image pretraining with vision encoders (ResNet-50, ViT, Swin Transformer) and BERT text encoder. Features are projected into shared embedding space via learnable projection heads and trained with soft CLIP loss.

Result: Achieves 95.00% accuracy and 94.75% F1-score on test set despite class imbalance. Linear projection heads outperform non-linear variants in cross-modal alignment.

Conclusion: LumbarCLIP provides a promising foundation for automated musculoskeletal diagnosis and clinical decision support through effective multimodal learning.
...
<details>
  <summary>Details</summary>
Motivation: While vulnerabilities have been identified in earlier frame sampling strategies for VideoLLMs, the safety of the latest prompt-guided sampling approach remains unexplored, creating a critical security gap.

Method: PoisonVID uses a closed-loop optimization strategy that iteratively optimizes universal perturbations to suppress harmful frame relevance scores, guided by a depiction set constructed from paraphrased harmful descriptions using GPT-4o-mini and a shadow VideoLLM.

Result: Comprehensive evaluation across three prompt-guided sampling strategies and three advanced VideoLLMs shows PoisonVID achieves 82% to 99% attack success rate.

Conclusion: The high success rate of PoisonVID highlights the vulnerability of current prompt-guided sampling strategies and emphasizes the importance of developing more secure future sampling approaches for VideoLLMs.
...
Method: Proposes Game of Regularizer (GoR) with dynamic loss weighting using two trainable parameters, and introduces QAT-EKD-GoR ensemble distillation framework using multiple heterogeneous teacher models.
...
<details>
  <summary>Details</summary>
Motivation: To assess if large-scale web-sourced plant image datasets with labeling errors can be as effective as smaller, curated expert datasets for continental-scale automated plant identification systems.

Method: Used two training datasets: one large noisy dataset collected from web sources and one smaller expert-verified dataset. Tested on a third independent dataset from the Pl@ntNet mobile application to ensure fair comparison.

Result: The challenge provided comparative evaluation of different training strategies for plant identification at continental scale with 10,000 species and 1.1M images.

Conclusion: The study demonstrated the feasibility of using web-sourced data for large-scale plant identification systems, highlighting the trade-offs between dataset size and quality in automated species recognition.
...
<details>
  <summary>Details</summary>
Motivation: Existing semi-supervised methods for retinal OCT segmentation often produce anatomically implausible results, fail to model layer-lesion interactions effectively, and lack guarantees on topological correctness, which limits their clinical utility.

Method: Proposes a fully differentiable biomarker topology engine that enables joint learning with bidirectional influence between layers and lesions. Uses disentangled representation learning to separate spatial and style factors, and enforces lesion locations relative to segmented layers.

Result: Outperforms current state-of-the-art methods on public and internal OCT datasets for both lesion and layer segmentation, demonstrating ability to generalize layer segmentation to pathological cases using partially annotated data.

Conclusion: The approach shows potential for using anatomical constraints in semi-supervised learning to achieve accurate, robust, and trustworthy retinal biomarker segmentation, addressing key limitations of existing methods.
...
Method: Used a dataset built through participatory sensing involving tens of thousands of contributors since 2011, evaluating systems on open-set recognition where models must identify known species while rejecting unknown ones.

Result: The challenge successfully tested various identification approaches on a massive dataset, providing insights into system performance when dealing with unknown categories in real-world biodiversity monitoring.

Conclusion: The LifeCLEF 2016 edition advanced plant identification research by introducing open-set recognition challenges, demonstrating the importance of robust systems that can handle unknown species in practical biodiversity applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use LLMs with noisy image captions that contain irrelevant information, and LLMs lack proper understanding of VQA tasks, limiting their reasoning capabilities.

Method: Uses pre-trained visual language model to generate captions, then summarizes and reorders them to exclude unrelated information, creating contextual examples to help LLMs better understand images and questions.

Result: Achieved 38.8% accuracy on OK-VQA and 34.6% on A-OKVQA using a 6.7B parameter LLM, demonstrating excellent performance without expensive end-to-end training.

Conclusion: SCRA-VQA effectively enhances LLM reasoning and task adaptability in KB-VQA by optimizing caption quality and structure, providing a cost-effective alternative to full model training.
...
<details>
  <summary>Details</summary>
Motivation: To systematically analyze the correlation between effectiveness of fidelity/adversarial objectives as optimization goals versus their capability as IQA metrics, which remains underexplored despite their central role in perceptual optimization.

Method: Conducted systematic analysis comparing fidelity metrics' performance in IQA versus their effectiveness in perceptual optimization, particularly examining discriminator design impact including patch-level, convolutional, and Transformer-based architectures.

Result: Found that fidelity metrics excelling in IQA are not necessarily effective for perceptual optimization, with this misalignment more distinct under adversarial training. Discriminator design plays decisive role - patch-level and convolutional architectures provide better detail reconstruction than vanilla or Transformer-based alternatives.

Conclusion: The findings advance understanding of loss function design and its connection to IQA transferability, providing insights for more principled approaches to perceptual optimization by revealing the optimization-assessment asymmetry.
...
Method: The model uses Object Interaction Self-Attention to capture complex object interactions within images and a GAN-Based Debiasing framework to generate unbiased data distributions for learning robust features.
...
Conclusion: The study highlights the importance of addressing both object interactions and dataset biases to advance VQA tasks, demonstrating that IOG-VQA effectively combines visual and textual information to mitigate inherent biases.
...
<details>
  <summary>Details</summary>
Motivation: Video sequences often contain structured noise and background artifacts that obscure dynamic content, and traditional robust principal component analysis (RPCA) methods fail to capture the rich variability in real video data due to their sparsity assumption limitations.
...
Method: Leverages local pixel dependencies based on Markov Random Fields to reconstruct images and expose texture/edge inconsistencies. Proposes FerretNet, a 1.1M parameter network trained on ProGAN data.
...
Conclusion: FerretNet provides efficient and robust synthetic image detection by focusing on fundamental generation artifacts, demonstrating strong generalization across diverse generative models.
...
Conclusion: MoTIF successfully extends interpretable concept bottleneck models to video classification, providing three complementary interpretability perspectives (global, local, temporal) for analyzing concept contributions in video understanding.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D object localization methods like dense depth estimation and 3D scene reconstruction are infeasible for distant objects or computationally limited tasks such as drone-based wildfire monitoring.

Method: Proposes using particle filters for single and multiple target 3D localization, tested with 3D simulation and drone-based image segmentation sequences using GNSS camera pose estimates.

Result: Particle filters successfully solved practical localization tasks using camera poses and image segments where other methods failed, demonstrating flexibility across detection methods.

Conclusion: The method enables effective drone-based wildfire monitoring when paired with existing image segmentation models, providing a viable solution for resource-constrained 3D object localization.
...
<details>
  <summary>Details</summary>
Motivation: Vision Mamba has global receptive field and low computational complexity but overlooks critical local features like textures and edges that are essential for accurate remote sensing segmentation

Method: Integrates localized Mamba-style scanning within shifted windows with global receptive field. First two stages perform local scanning for fine-grained details, last two stages use global scanning for contextual information. Overlapping shifted windows enhance inter-region information exchange

Result: Extensive experiments on LoveDA and ISPRS Potsdam datasets show SwinMamba outperforms state-of-the-art methods

Conclusion: SwinMamba demonstrates effectiveness and potential as a superior solution for semantic segmentation of remotely sensed imagery by balancing local and global feature perception
...
Method: Proposes a pack-based multiple instance learning (MIL) framework that: 1) packs sampled variable-length feature sequences into fixed-length ones for batched training; 2) uses residual branches to compose discarded features into hyperslides with tailored labels; 3) employs attention-driven downsampling to compress features and reduce redundancy.

Result: Achieves up to 8% accuracy improvement while using only 12% of the training time in PANDA(UNI) benchmark. Extensive experiments show significant potential for addressing data challenges in computational pathology.

Conclusion: Focusing on data challenges in computational pathology through the proposed pack-based MIL framework demonstrates substantial improvements in both accuracy and efficiency, highlighting the importance of addressing data heterogeneity and redundancy in the era of foundation models.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for generating physically plausible human motion use simulator-based motion projection layers which are computationally expensive due to their sequential nature that prevents parallelization.

Method: The paper proposes SimDiff, which interprets simulator-based motion projection as guidance within the diffusion process and conditions the denoising on environment parameters (e.g., gravity, wind) to generate physically plausible motions without repeated simulator calls.
...
Conclusion: The proposed SimDiff approach successfully integrates physical constraints into the diffusion process, enabling efficient and controllable generation of physically plausible human motions while generalizing to novel environmental conditions.
...
Method: Extensive evaluation of 1,174 pretrained vision models combined with theoretical analysis including proofs about low-pass stem kernels, anti-aliased downsampling, pooling mechanisms, and Lipschitz bounds for CLIP ViTs.
...
<details>
  <summary>Details</summary>
Motivation: Scene graphs provide structured relational representations crucial for decoding complex surgical environments, making them essential for advancing surgical AI systems.

Method: PRISMA-ScR-guided scoping review that systematically analyzes SG applications, methodological advancements, and future directions in surgical research.

Result: The field has advanced from foundational graph neural networks to specialized foundation models that outperform generalist large vision-language models. SGs now support both analytical tasks (workflow recognition, safety monitoring) and generative tasks (surgical simulation).

Conclusion: Surgical scene graphs are maturing into an essential semantic bridge that enables intelligent systems to improve surgical safety, efficiency, and training, despite ongoing challenges in data annotation and real-time implementation.
...
Conclusion: The system enables automated quality control with significant cost savings and can detect both known and novel defect types without requiring extensive labeled datasets.
...
<details>
  <summary>Details</summary>
Motivation: The dynamic propagation of social media has increased access to financial advisory content through podcasts, but extracting insights from lengthy (30-40 minute) multimodal segments remains challenging.
...
Result: Comprehensive cross-domain experiments confirm FASTER's strong performance, robustness, and generalizability compared to LLMs and VLMs. The framework also introduces Fin-APT, a dataset of 470 financial advisory videos.
...
Method: ASD randomly samples pseudo-labeled data, uses instance-level classifier to learn semantically aligned labels, tracks class transitions to extract high-level similarities, assigns cluster-level labels to pseudo-labeled data, and triggers SSL learner training on unlabeled data.

Result: ASD shows superior performance across benchmarks against latest deep image clustering approaches, with only 1.33% accuracy gap on CIFAR-10 compared to SSL methods using ground-truth. It also boosts existing SSL-embedded methods.

Conclusion: ASD enables effective cold-start SSL application for image clustering without prerequisites, achieving competitive performance and enhancing existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers produce high-norm artifacts that degrade representation quality. When these features are transferred via knowledge distillation, artifacts dominate the objective, causing students to overfit to artifacts and underweight informative signals, diminishing gains from larger models.

Method: Singular Nullspace-Guided Energy Reallocation (SiNGER) uses principled teacher feature refinement with nullspace-guided perturbation to preserve information while suppressing artifacts. The perturbation is implemented efficiently with a LoRA-based adapter requiring minimal structural modification.
...
TL;DR: Fast-SEnSeI is a lightweight, sensor-independent encoder module for flexible on-board cloud segmentation across multispectral sensors with varying band configurations.


<details>
  <summary>Details</summary>
Motivation: Most cloud segmentation models are tightly coupled to specific sensor configurations and rely on ground-based processing, limiting their flexibility and on-board deployment capabilities.

Method: Builds upon SEnSeI-v2 with improved spectral descriptor, lightweight architecture, and robust padding-band handling. Accepts arbitrary spectral band combinations and wavelengths, producing fixed-size feature maps for a compact, quantized U-Net-based segmentation model. Uses CPU-FPGA hybrid pipeline with Apache TVM for embedded CPUs and FPGA deployment.
...
Conclusion: Fast-SEnSeI enables efficient, flexible cloud segmentation suitable for space-qualified hardware deployment, overcoming sensor-specific limitations of existing approaches.
...
TL;DR: SNCE is a novel method that precisely removes harmful concepts from text-to-image models by manipulating only a single neuron, achieving surgical precision with minimal image quality degradation.
...
Result: SNCE achieves state-of-the-art concept erasure performance while preserving generation capabilities for non-target concepts and exhibits strong robustness against adversarial attacks.

Conclusion: The proposed SNCE method provides an effective and precise solution for safe text-to-image generation by enabling surgical concept removal through single neuron manipulation.
...
TL;DR: KD-SS is a lightweight sub-sampling algorithm for plant point clouds that preserves full resolution, enabling segmentation without down-sampling across different plant species and sensor modalities.


<details>
  <summary>Details</summary>
Motivation: Existing plant organ segmentation methods are species-specific, require extensive pre-processing, and force down-sampling due to hardware constraints, limiting their general applicability.

Method: Proposed KD-SS algorithm for biological point cloud sub-sampling that is sensor-agnostic and species-agnostic, allowing segmentation of full-resolution point clouds without down-sampling.

Result: KD-SS combined with state-of-the-art segmentation models achieves satisfying results across various modalities (photogrammetry, laser triangulation, LiDAR) and plant species.

Conclusion: KD-SS serves as a lightweight, resolution-retaining alternative to intensive pre-processing and down-sampling methods for plant organ segmentation, regardless of species or sensor modality.
...
<details>
  <summary>Details</summary>
Motivation: Existing FG-BG decomposition methods suffer from low robustness due to over-reliance on local class similarity and fixed background patch extraction strategies, leading to poor performance in FS-OOD detection.

Method: Proposes learning background prompts to obtain local background similarity containing both background and semantic information, then refining it with local class similarity. Uses patch self-calibrated tuning to flexibly select background patches based on sample diversity.
...
TL;DR: The paper introduces two novel dataset splitting methods for image segmentation tasks: Iterative Pixel Stratification (IPS) and Wasserstein-Driven Evolutionary Stratification (WDES), which address the limitations of random splitting that leads to biased evaluations and poor model generalization.


<details>
  <summary>Details</summary>
Motivation: Random dataset splitting in image segmentation often creates unrepresentative test sets due to multi-label structure and class imbalance, resulting in biased model evaluations and poor generalization. Existing stratification methods from classification don't effectively extend to segmentation tasks.

Method: Two methods are proposed: 1) Iterative Pixel Stratification (IPS) - a label-aware sampling method for segmentation, and 2) Wasserstein-Driven Evolutionary Stratification (WDES) - a genetic algorithm that minimizes Wasserstein distance to optimize label distribution similarity across dataset splits. WDES is proven to be globally optimal given sufficient generations.

Result: Evaluation using new statistical heterogeneity metrics shows WDES consistently produces more representative splits than random sampling. Applied across diverse segmentation tasks (street scenes, medical imaging, satellite imagery), WDES leads to lower performance variance and improved model evaluation. WDES is particularly valuable for small, imbalanced, and low-diversity datasets.

Conclusion: WDES effectively addresses the limitations of conventional splitting strategies in segmentation tasks, especially for challenging datasets with class imbalance and limited diversity, providing more reliable model evaluation and better generalization.
...
TL;DR: EnGraf-Net is a fine-grained classification model that uses semantic associations structured as a hierarchy (taxonomy) as supervised signals in an end-to-end deep neural network, achieving competitive performance without requiring cropping techniques or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Existing fine-grained classification models rely on part annotations or automatic cropping methods, which suffer from incomplete representation of local features. The authors propose that semantic associations (hierarchical taxonomy) can better capture the relationships needed to distinguish highly similar classes.

Method: The EnGraf-Net model leverages semantic associations structured as a hierarchy (taxonomy) as supervised signals within an end-to-end deep neural network. It does not require part annotations, bounding boxes, or cropping techniques.
...
Conclusion: Semantic associations structured as a hierarchy provide effective supervised signals for fine-grained classification, enabling competitive performance without the need for manual annotations or complex cropping techniques.
...
TL;DR: Vision Transformers (ViTs) are vulnerable to adversarial patch attacks despite their reputation for robustness, with attack success rates varying significantly across different ViT models (40.04% to 99.97%) when transferring CNN-based attack techniques.


<details>
  <summary>Details</summary>
Motivation: To investigate the security vulnerabilities of Vision Transformers (ViTs) against evasion attacks, particularly adversarial patches, and examine the transferability of CNN-based attack techniques to ViT architectures.

Method: Used Creases Transformation (CT) technique to design realistic adversarial patches that add subtle geometric distortions similar to natural clothing wrinkles. Evaluated four fine-tuned ViT models on binary person classification tasks using adversarial patches transferred from CNN attack methods.

Result: Significant vulnerability variations observed: google/vit-base-patch16-224-in21k (40.04% success), facebook/dino-vitb16 (99.97% success), google/vit-base-patch16-224 (66.40% success), facebook/dinov3-vitb16 (65.17% success).

Conclusion: Adversarial patches can be effectively transferred from CNNs to ViTs, with pre-training dataset scale and methodology strongly influencing model resilience to adversarial attacks, confirming cross-architectural transferability of evasion attacks.
...
TL;DR: UniTransfer is a novel architecture for video concept transfer that uses spatial and temporal decomposition with a progressive paradigm, achieving precise control through foreground/background/motion decoupling and Chain-of-Prompt diffusion guidance.


<details>
  <summary>Details</summary>
Motivation: To achieve precise and controllable video concept transfer by decomposing videos into spatial components (foreground, background, motion) and temporal stages, enabling fine-grained control over video generation and editing.

Method: Proposes spatial decomposition into foreground subject, background, and motion flow; dual-to-single-stream DiT-based architecture; self-supervised pretraining with random masking; Chain-of-Prompt mechanism for timestep decomposition using LLM guidance; and creates OpenAnimal dataset for benchmarking.
...
Conclusion: UniTransfer effectively achieves precise video concept transfer through spatial and temporal decomposition, demonstrating superior performance over existing methods and providing a robust framework for controllable video generation.
...
TL;DR: VTTS enhances multimodal LLMs' reasoning through iterative perception during inference, mimicking human hierarchical attention by progressively refining focus on high-confidence regions with reinforcement learning supervision.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM methods are limited by static perception stages and fail to achieve human-level reasoning through dynamic, iterative visual analysis.

Method: VTTS uses Iterative Perception (ITP) mechanism with reinforcement learning and spatio-temporal supervision, supported by VTTS-80K dataset for iterative perception training.
...
Conclusion: VTTS successfully enables MLLMs to enhance performance through increased perceptual compute, demonstrating effective generalization across diverse multimodal reasoning tasks.
...
Result: Mammography-trained models capture more clinically relevant concepts and align better with radiologists' workflows. Fine-tuning enhances certain concept categories (benign calcifications) but reduces others (density-related features), showing a specialization-generalization trade-off.
...
TL;DR: MOSS-ChatV is a reinforcement learning framework with DTW-based process reward that addresses process inconsistency in video reasoning for MLLMs, achieving state-of-the-art performance on video benchmarks.
...
Method: Introduces MOSS-ChatV, a reinforcement learning framework with Dynamic Time Warping (DTW)-based process reward that aligns reasoning traces with temporally grounded references without auxiliary reward models. Also creates MOSS-Video benchmark for training and evaluation.
...
Conclusion: The DTW-based process reward framework effectively addresses process inconsistency in video reasoning, demonstrating broad applicability across architectures and producing more interpretable and stable reasoning traces.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with consistency and generalizability when generating videos with camera trajectories, especially when both camera and object motions are present. Current approaches often learn motions separately, causing confusion about relative motion between camera and objects.

Method: Proposes integrating camera and object motions by converting them into pixel motion. Uses a stable diffusion network to learn reference motion maps relative to camera trajectory, then combines these maps with semantic object priors in an image-to-video network to generate videos that accurately follow camera trajectories while maintaining consistent object motions.

Result: Extensive experiments show the model significantly outperforms state-of-the-art methods by a large margin.

Conclusion: The proposed approach effectively addresses the challenge of generating consistent videos guided by camera trajectories by unifying camera and object motions through pixel motion conversion, demonstrating superior performance compared to existing methods.
...
TL;DR: Discriminators are most disadvantaged when datasets have either very low or very high complexity, while intermediate complexity creates the best conditions for detecting synthetic images.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of image generative AI has blurred the line between real and synthetic images, creating an arms race between generators and discriminators. This research aims to understand when discriminators are at the greatest disadvantage in this competition.
...
Result: Both very simple and highly complex datasets reduce the detectability of synthetic images. Generators can learn simple datasets almost perfectly, while extreme diversity in complex datasets masks imperfections. Intermediate-complexity datasets provide the most favorable conditions for detection.

Conclusion: The effectiveness of discriminators depends on dataset complexity, with optimal detection occurring at intermediate complexity levels where generators fail to fully capture the distribution and their errors remain visible.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient multi-resolution processing within a single CLIP model, allowing adaptive inference that can start with low resolution and refine only when needed, reducing computational costs while maintaining competitive accuracy.

Method: Replaces standard patch embeddings with multi-level wavelet decomposition, uses key-value caching and causal cross-level attention to reuse computation, and implements confidence-based gating for adaptive early exits with lightweight distillation from a frozen CLIP teacher.

Result: Achieves competitive accuracy with significant computational savings, enabling users to dynamically choose compute-accuracy trade-offs using a single deployed model.

Conclusion: WAVECLIP provides an effective approach for adaptive resolution inference in CLIP, demonstrating that wavelet-based tokenization with early exits can deliver computational efficiency without sacrificing performance.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate the impact of quantization on CLIP models beyond accuracy, focusing on reliability metrics like calibration and OOD detection, which are crucial for efficient and reliable deployment.
...
Result: Quantization improves calibration for underconfident models but may degrade it for overconfident ones; OOD detection can still improve despite poor calibration; specific QAT methods yield simultaneous gains in accuracy, calibration, and robustness.

Conclusion: Quantization offers benefits beyond efficiency, enabling improved reliability and robustness in CLIP deployment, challenging the traditional efficiency-performance trade-off.
...
Method: Created TABLET dataset with 4M examples from 2M unique tables (88% original visualizations), including paired image-HTML representations, metadata, and provenance information linking to source datasets.
...
TL;DR: A conformal prediction-based approach for controlling explanation fidelity in feature attribution methods, identifying sufficient salient features to preserve model predictions without requiring ground-truth explanations.


<details>
  <summary>Details</summary>
Motivation: Current feature attribution methods for image predictions vary in robustness and may not faithfully reflect the underlying model's reasoning, requiring better fidelity control.

Method: Proposes four conformity functions to quantify explanation fidelity, identifies sufficient salient features to preserve predictions regardless of excluded features, and evaluates using five explainers across six image datasets.

Result: FastSHAP consistently outperforms competing methods in fidelity and informational efficiency (explanation region size), with super-pixel based conformity measures proving more effective than pixel-wise ones.

Conclusion: The conformal prediction approach enables direct control over explanation fidelity and provides more reliable feature attribution without needing ground-truth explanations for calibration.
...
TL;DR: Sigma is a unified skeleton-based sign language understanding framework that addresses semantic grounding, local-global balance, and cross-modal learning challenges through sign-aware fusion, hierarchical alignment, and multi-task pre-training.
...
Result: Achieves state-of-the-art results on isolated sign language recognition, continuous sign language recognition, and gloss-free sign language translation across multiple benchmarks spanning different sign and spoken languages.

Conclusion: Demonstrates the effectiveness of semantically informative pre-training and proves skeletal data as a viable stand-alone solution for SLU, showing impact through superior performance across diverse tasks and languages.
...
TL;DR: This paper presents a comprehensive study evaluating how well automated metrics for text-image generation align with human judgments, revealing that no single metric performs consistently across different compositional tasks.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of text-image generation relies heavily on automated metrics that are often adopted by convention rather than validated against human judgment, making it critical to understand their alignment with human preferences.

Method: The authors conducted a broad study examining widely used metrics for compositional text-image evaluation, analyzing their behavior across diverse compositional challenges and comparing different metric families' alignment with human judgments.
...
Conclusion: The findings underscore the importance of careful and transparent metric selection for trustworthy evaluation and their use as reward models in generation, as metric performance is highly task-dependent.
...
Conclusion: The integrated Mamba-GNN architecture with adaptive fusion strategy shows strong performance and promising potential for spatially-resolved predictive modeling tasks in computational pathology.
...
Conclusion: The unified multi-modal framework provides fine-grained control over 3D asset generation, addressing limitations of existing methods and enabling more practical adoption in real-world applications.
...
TL;DR: A framework using vision-language models to automatically generate semantic attention maps via natural language prompts, which aligns CNN attention through an auxiliary loss to improve model reliability without manual annotations.


<details>
  <summary>Details</summary>
Motivation: CNNs often exploit superficial correlations rather than making predictions for the right reasons. Existing methods require labor-intensive expert annotations, limiting scalability.

Method: Leverage vision-language models to automatically generate semantic attention maps using natural language prompts. Introduce an auxiliary loss that aligns CNN attention with these language-guided maps.

Result: Achieves state-of-the-art performance on ColorMNIST and remains competitive with annotation-heavy baselines on DecoyMNIST. Shows improved generalization, reduced shortcut reliance, and attention maps that better reflect human intuition.
...
TL;DR: Decipher-MR is a 3D MRI-specific vision-language foundation model trained on 200,000 MRI series from 22,000+ studies, enabling robust medical imaging analysis across diverse clinical tasks with minimal computational overhead.
...
TL;DR: SQ-InstructBLIP improves vision-language reasoning by generating image-aware sub-questions and answers through a three-component architecture (Questioner, Answerer, Reasoner) to overcome limitations of LLMs in multi-step visual reasoning.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with multi-step reasoning tasks, and LLM-based approaches have limitations: they cannot access fine-grained visual content and use black-box models that are inaccessible and difficult to reproduce.

Method: Proposes SQ-InstructBLIP with three components sharing the same architecture: Questioner generates sub-questions, Answerer generates sub-answers, and Reasoner performs final reasoning using the sub-question information for the main VQA task.
...
Conclusion: The proposed method effectively improves inference performance in vision-language understanding by iteratively generating image-aware informative sub-questions and answers, addressing the limitations of existing LLM-based approaches.
...
TL;DR: The paper proposes a new framework for defining and categorizing hallucination in text-to-image models as bias-driven deviations, introducing three types: attribute, relation, and object hallucinations.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of text-to-image models focus only on alignment with prompts but overlook what models generate beyond prompts, failing to address bias-driven hallucinations that reveal underlying model biases.

Method: The authors define hallucination in text-to-image models as bias-driven deviations and propose a taxonomy with three categories: attribute hallucinations (incorrect attributes), relation hallucinations (incorrect relationships), and object hallucinations (unprompted objects).

Result: This framing establishes an upper bound for evaluation and helps surface hidden biases in text-to-image models.

Conclusion: The proposed taxonomy provides a foundation for richer assessment of text-to-image models by systematically addressing bias-driven hallucinations beyond simple prompt alignment.
...
TL;DR: A Person Independence Universal Micro-action Recognition Framework that addresses inter-person variability through distributionally robust optimization, featuring temporal-frequency alignment and group-invariant regularization to achieve robust generalization.


<details>
  <summary>Details</summary>
Motivation: Existing micro-action recognition methods fail in real-world scenarios due to inter-person variability causing the same action to manifest differently, which hinders robust generalization across different individuals.

Method: Proposes a framework with two plug-and-play components: 1) Temporal-Frequency Alignment Module with dual-branch design (temporal branch uses Wasserstein-regularized alignment, frequency branch uses variance-guided perturbations) and consistency-driven fusion; 2) Group-Invariant Regularized Loss that partitions samples into pseudo-groups, up-weights boundary cases, and regularizes subgroup variance.

Result: Experiments on the large-scale MA-52 dataset demonstrate superior performance over existing methods in both accuracy and robustness, achieving stable generalization under fine-grained conditions.

Conclusion: The proposed framework effectively addresses person-specific variations in micro-action recognition through distributionally robust optimization principles, enabling robust generalization across different individuals in real-world scenarios.
...
TL;DR: This paper proposes a novel approach for semantic matching that addresses geometric ambiguity and nearest-neighbor rule limitations by adapting VGGT's 3D geometric foundation model features through strategic fine-tuning and training strategies.


<details>
  <summary>Details</summary>
Motivation: Existing semantic matching approaches suffer from geometric ambiguity (failing to disambiguate symmetric structures using 2D foundation models) and limitations of the nearest-neighbor rule (ignoring cross-image invisibility and neglecting manifold preservation).

Method: The approach adapts VGGT's 3D geometric foundation model by: (i) retaining early feature stages while fine-tuning later ones and adding a semantic head for bidirectional correspondences; (ii) implementing cycle-consistent training, synthetic data augmentation, and progressive training with aliasing artifact mitigation to handle data scarcity.
...
Conclusion: The proposed method successfully addresses key limitations in semantic matching by leveraging VGGT's geometric foundation while adapting it for cross-instance semantic matching scenarios under data scarcity constraints.
...
TL;DR: MedVSR is a specialized video super-resolution framework for medical videos that addresses alignment challenges and artifact issues through Cross State-Space Propagation and Inner State-Space Reconstruction modules.
...
Method: Proposes MedVSR with two key components: Cross State-Space Propagation (CSSP) for precise alignment by projecting distant frames as control matrices, and Inner State-Space Reconstruction (ISSR) for enhancing tissue structures through joint long-range spatial learning and large-kernel short-range information aggregation.
...
Conclusion: MedVSR provides an effective solution for medical video super-resolution, addressing the unique challenges of medical videos and producing high-quality results suitable for clinical diagnosis.
...
Result: Experiments across mathematical reasoning benchmarks demonstrate effectiveness of both curated data and VAS. The method establishes that reward variance lower-bounds expected policy gradient magnitude.

Conclusion: The work provides comprehensive resources including data, code, and models to advance multimodal reasoning research, with VAS serving as a practical mechanism to stabilize RL optimization through variance promotion.
...
TL;DR: A new foundation model using Prithvi-EO Vision Transformer architecture pre-trained on Sentinel-3 OLCI data shows utility for marine monitoring tasks, particularly for chlorophyll concentration quantification and ocean primary production estimation.
...
Method: Used Prithvi-EO Vision Transformer architecture pre-trained to reconstruct Sentinel-3 Ocean and Land Colour Instrument data, then fine-tuned on downstream tasks including chlorophyll concentration quantification and ocean primary production estimation.
...
TL;DR: SHINE is a training-free framework for high-fidelity image composition that addresses complex lighting challenges and high-resolution inputs using diffusion models without latent inversion or attention surgery.


<details>
  <summary>Details</summary>
Motivation: Existing image composition models struggle with complex lighting conditions (shadows, reflections) and diverse high-resolution inputs, while current diffusion models lack proper frameworks to utilize their physical and resolution priors effectively.

Method: SHINE introduces manifold-steered anchor loss using pretrained customization adapters (e.g., IP-Adapter) to guide latents for faithful subject representation while preserving background integrity, along with degradation-suppression guidance and adaptive background blending.
...
Conclusion: SHINE provides an effective training-free solution for high-quality image composition that handles complex lighting and high-resolution scenarios, with the introduced ComplexCompo benchmark addressing the lack of rigorous evaluation standards.
...
TL;DR: QuantVGGT is a novel quantization framework for Visual Geometry Grounded Transformers that addresses heavy-tailed activation distributions and multi-view data challenges, achieving 3.7× memory reduction and 2.5× acceleration while maintaining 98% accuracy.


<details>
  <summary>Details</summary>
Motivation: Billion-scale VGGTs face prohibitive computational and memory costs that hinder real-world deployment. PTQ struggles with heavy-tailed activation distributions from special tokens and unstable calibration from multi-view 3D data.

Method: Proposes Dual-Smoothed Fine-Grained Quantization (pre-global Hadamard rotation + post-local channel smoothing) and Noise-Filtered Diverse Sampling (outlier filtering + frame-aware diverse calibration clusters).

Result: Achieves state-of-the-art results across benchmarks and bit-widths, with 4-bit quantization delivering 3.7× memory reduction and 2.5× acceleration while maintaining >98% reconstruction accuracy.

Conclusion: QuantVGGT demonstrates vast advantages and practicality for resource-constrained scenarios, enabling efficient deployment of billion-scale 3D reconstruction models.
...
Method: Proposes Neural Newtonian Dynamics (NND) that models and predicts Newtonian motions, injecting latent dynamical constraints into video generation by jointly leveraging data priors and dynamical guidance.

Result: Enables physically consistent video synthesis with precise parameter control.
...
<details>
  <summary>Details</summary>
Motivation: To democratize access to high-quality image generation by making it computationally feasible on accessible consumer devices like mobile phones and desktop computers.
...
Conclusion: SD3.5-Flash makes advanced generative AI truly accessible for practical deployment across the full spectrum of devices.
