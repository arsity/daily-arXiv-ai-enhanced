...
Motivation: Enable privacy control in shared-space mixed reality meetings by allowing users to selectively remove personal or sensitive items from their environment so they're not visible to other participants.
...
Result: System achieves frame rates exceeding 20 fps at 720p resolution, demonstrating real-time performance for practical privacy-preserving MR applications.

Conclusion: The portable, robust system proves the feasibility of real-time diminished reality without requiring fixed secondary viewpoints or prior 3D environment scanning, enabling effective privacy control in MR meetings.
...
<details>
  <summary>Details</summary>
Motivation: Progress in surgical vision-language pre-training is constrained by limited dataset scale, procedural diversity, semantic quality, and hierarchical structure of existing datasets.

Method: Created SurgLaVi dataset using fully automated pipeline that generates fine-grained transcriptions, segments videos into procedural units, and applies dual-modality filtering for quality. Developed SurgCLIP, a CLIP-style video-text contrastive framework with dual encoders.

Result: SurgCLIP achieves consistent improvements across phase, step, action, and tool recognition, surpassing prior state-of-the-art methods by large margins. Released SurgLaVi-beta with 113k clip-caption pairs from public data.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning models for 3D brain MRI are task-specific with limited labeled data and poor generalization across tasks/populations. Few 3D foundation models exist and they have limitations in resolution, scope, or accessibility.

Method: Developed a general SimCLR-based self-supervised learning foundation model pre-trained on 18,759 patients (44,958 scans) from 11 public datasets spanning diverse neurological diseases. Compared against Masked Autoencoders (MAE) and supervised baselines.

Result: The fine-tuned SimCLR model outperformed all other models across four diverse downstream prediction tasks in both in-distribution and out-of-distribution settings. Achieved superior performance even when fine-tuned with only 20% of labeled training samples for Alzheimer's disease prediction.

Conclusion: The work provides a broadly applicable and accessible foundation model for clinical brain MRI analysis, demonstrating the effectiveness of SimCLR-based SSL for 3D neuroimaging with strong generalization capabilities across diverse neurological conditions.
...
<details>
  <summary>Details</summary>
Motivation: RGB-to-HSI reconstruction is ill-posed and can become physically inconsistent when camera spectral sensitivity and scene illumination are misspecified, requiring a more robust approach.

Method: Formulates RGB-to-HSI as physics-grounded inverse problem with nuclear norm regularization in learnable transform domain, explicitly estimates CSS and illumination, introduces data-adaptive low-rank subspace SVT to avoid full SVD costs, and develops USCTNet deep unfolding solver with parameter estimation module.
...
Conclusion: The proposed USCTNet framework effectively addresses physical consistency issues in RGB-to-HSI reconstruction through explicit parameter estimation and efficient optimization techniques, achieving superior performance compared to existing methods.
...
Method: Evaluated LLaMA 3.2 Instruct on BraTS 2020 dataset for glioma classification and segmentation using three methods (center point, bounding box, polygon extraction), comparing performance before/after fine-tuning against custom 3D CNNs.

Result: CNNs achieved 80% accuracy in classification with balanced metrics, while LLMs reached 76% accuracy but suffered from poor specificity (18%). Fine-tuning improved specificity to 55% but reduced overall accuracy to 72%. For segmentation, LLMs consistently clustered predictions near image center without distinguishing tumor characteristics.
...
<details>
  <summary>Details</summary>
Motivation: Traditional part segmentation methods rely on appearance-based semantic cues, which don't align well with object articulation and kinematic structure. There's a need for methods that produce structural components consistent across views and time for animation tasks.

Method: Uses dual-branch diffusion model with spatial color encoding to map part masks to RGB-like images, allowing shared VAE between branches. Includes Bidirectional Diffusion Fusion module and contrastive part consistency loss for cross-branch consistency. Trained on KinematicParts20K dataset of 20K rigged objects.

Result: SP4D generalizes well to diverse scenarios including real-world videos, novel generated objects, and rare articulated poses. Produces kinematic-aware outputs suitable for downstream animation and motion tasks.

Conclusion: The framework successfully generates kinematic parts aligned with object articulation, enabling easy extraction of 3D skeletal structures and skinning weights with minimal manual intervention, advancing part-based video generation for animation applications.
...
Method: Uses pose information for rough hand/body localization, then segments these parts through video to maintain shape details. Segmentations focus RGB processing on relevant body parts, effectively combining both modalities.

Result: Outperforms state-of-the-art methods on complex ChaLearn249 IsoGD dataset. Ablation studies show strong benefits from focusing on signer's body and hands.

Conclusion: SegSLR successfully combines RGB and pose information through video segmentation, maintaining crucial details and improving ISLR performance compared to existing approaches.
...
TL;DR: SCOPE framework integrates speech recognition with vision foundation models for real-time surgical instrument segmentation and tracking without manual cues or labeled data.


<details>
  <summary>Details</summary>
Motivation: Current surgical segmentation methods require domain-specific labeled data and manual inputs, limiting adaptability in dynamic operating environments where hands-free interaction is needed.

Method: Speech-guided collaborative perception framework combining LLM reasoning with open-set VFMs, using clinician speech feedback to guide segmentation and instrument tracking in surgical videos.
...
Conclusion: The human-AI collaboration paradigm enables adaptable, hands-free surgical tools for dynamic operating rooms, overcoming limitations of traditional supervised approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional computer vision systems fail with real-world camera effects due to lack of training data. Existing data generation methods suffer from high costs, sim-to-real gaps, or inaccurate camera effect modeling.

Method: Two-stage pipeline: 1) Reconstructs dynamic scenes from multi-view videos using 4D Gaussian Splatting, 2) Applies physically-based ray tracing to generate videos with controllable, accurate camera effects.
...
Conclusion: 4D-GRT provides an effective solution for generating training data with physically accurate camera effects, overcoming limitations of previous approaches and enabling better performance of computer vision systems in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Automate core video editing tasks rather than just retrieval or UI, addressing the need for efficient content creation across various applications.

Method: Multi-agent approach with Editor agent using video editing tools and Critic agent providing natural language feedback, using learning-based communication between specialized agents.
...
Conclusion: The proposed multi-agent framework effectively automates video editing through sequential decision making and specialized agent collaboration with natural language interaction.
...
<details>
  <summary>Details</summary>
Motivation: Gadolinium-based contrast agents (GBCAs) used in glioma imaging pose safety, cost, and accessibility concerns. Predicting enhancement from non-contrast MRI offers a safer alternative for assessing tumor aggressiveness and treatment planning.

Method: Analyzed 1,446 glioma cases from 4 TCIA datasets. Extracted 108 radiomics features from non-contrast T1WI, combined with 48 dimensionality reduction methods and 25 classifiers (1,200 total pipelines). Used rotational validation training on 3 datasets and testing on the fourth.

Result: Cross-validation accuracy: 0.91-0.96; External testing: 0.87-0.98 (avg 0.93). F1, precision, recall stable (0.87-0.96), ROC-AUC varied more (0.50-0.82). MI with ETr pipeline consistently performed best, balancing accuracy and stability.

Conclusion: Stability-aware model selection enables reliable prediction of contrast enhancement from non-contrast MRI, reducing GBCA reliance and improving cross-center generalizability. Provides scalable template for reproducible ML in neuro-oncology.
...
<details>
  <summary>Details</summary>
Motivation: Dense small objects in UAV imagery are often missed due to long-range viewpoints, occlusion, and clutter, requiring improved detection recall for applications like far-field counting and monitoring.

Method: Overlapping tiling recovers low-confidence candidates, followed by Spatial Gate (DBSCAN on box centroids) and Semantic Gate (DBSCAN on ResNet-18 embeddings) to validate group evidence. Validated groups receive confidence reweighting before class-aware NMS fusion.

Result: Experiments on VisDrone show recall increase from 0.685 to 0.778 (+0.093) with precision adjustment from 0.801 to 0.595, yielding F1=0.669. Post-processing latency averages 0.095s per image.

Conclusion: The framework provides recall-first, precision-trade-off behavior beneficial for recall-sensitive applications, requires no retraining, and integrates with modern detectors. Future work focuses on reducing semantic gating cost and adding temporal cues.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D scene datasets suffer from limited scale/diversity, sanitized layouts lacking small items, and object collisions, hindering Embodied AI advancement.

Method: Integrates three scene sources (real-world scans, procedural generation, designer-created scenes) with comprehensive processing including real-to-sim replication, interactive object incorporation, and collision resolution via physical simulations.

Result: Created dataset with 40K scenes, 1.96M objects across 15 scene types and 288 classes, featuring realistic layouts averaging 41.5 objects per region with preserved small items.

Conclusion: InternScenes enables scaling model training for complex scene tasks, demonstrates value through layout generation and navigation benchmarks, and will be open-sourced to benefit the community.
...
<details>
  <summary>Details</summary>
Motivation: To optimize the trade-off between basis selection and polynomial degree for accurate mathematical handwriting modeling with low computational cost.

Method: Examines condition numbers for polynomial evaluation in Legendre, Legendre-Sobolev, Chebyshev, and Chebyshev-Sobolev bases, and bounds inner product norms for symbol variations.

Result: Provides insights into how different basis choices affect computational efficiency and accuracy in mathematical handwriting representation.

Conclusion: The study offers guidance on selecting optimal basis functions and polynomial degrees for efficient and accurate mathematical handwriting modeling systems.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in glioma progression prediction due to sparse, irregular longitudinal MRI data in clinical practice, where incomplete follow-up sequences create data imbalances and make reliable modeling difficult.

Method: Multitask diffusion framework that simultaneously generates future FLAIR sequences at any time point and estimates spatial probabilistic tumor evolution maps using signed distance fields (SDFs). Integrates pretrained deformation module for temporal dynamics, targeted augmentation pipeline for data scarcity, and radiotherapy-weighted focal loss using radiation dose maps.

Result: The method produces flexible time-dependent probability maps from just two earlier follow-up scans, enabling clinicians to interrogate tumor progression risks at any future temporal milestone. Achieved promising results on both public and private datasets.

Conclusion: The proposed framework effectively addresses clinical data scarcity issues in glioma progression prediction, providing flexible time-agnostic predictions with uncertainty quantification and improved stability through targeted data augmentation and radiotherapy-aware training.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR segmentation methods suffer from high computational complexity and require large training datasets, limiting their generalization in data-scarce applications like autonomous driving.

Method: Uses point-plane projections to learn features from 2D representations of point clouds, and introduces geometry-aware data augmentation that aligns with LiDAR sensor properties to address class imbalance.

Result: Achieves significant improvements in limited-data scenarios and competitive results on SemanticKITTI and PandaSet datasets.

Conclusion: The approach effectively extracts complementary information from LiDAR data alone through 2D projections, providing better performance with less training data while maintaining competitive results on standard benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Open-vocabulary semantic segmentation is crucial for large-scale urban point clouds in applications like digital twins and smart cities, but remains unexplored due to lack of aligned multi-view imagery and poor generalization of existing 3D segmentation methods across diverse urban environments.
...
Method: Uses dual-camera setup (frontal and side views) with computer vision and machine learning. Includes Face Module for identity verification, head pose estimation, gaze tracking, and mouth movement analysis. Hand Module uses fine-tuned YOLOv11 for prohibited item detection. Features are aggregated into an LSTM network for temporal pattern analysis and real-time cheating probability scoring.

Result: Achieves 90.7% accuracy in suspicious activity classification, mAP@.5 of 0.57 for prohibited item detection, and processes video streams at 2.4 FPS without GPU. Demonstrates effectiveness and resource efficiency.

Conclusion: AutoOEP provides an effective, automated proctoring solution that significantly reduces human intervention needs and enhances online assessment integrity.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations in compressive spectral imaging where high compression ratios cause ill-posed reconstruction problems, and existing methods lack interpretability or rely on handcrafted priors.

Method: Proposes dual-camera CASSI with total variation subgradient theory, dynamic regularization using RGB/panchromatic reference images, and adaptive reference generation mechanism for subgradient guidance.

Result: Effectively preserves spatial-spectral structural consistency and demonstrates robust performance across diverse reconstruction scenarios with improved interpretability.

Conclusion: Establishes an interpretable mathematical foundation for computational spectral imaging that balances performance with physical interpretability through TV subgradient optimization.
...
<details>
  <summary>Details</summary>
Motivation: Large-scale foundation models for Earth Observation are computationally expensive, limiting accessibility and reuse for downstream tasks. There's a need for more compact and efficient alternatives.

Method: Proposed MoE-MAE combines sparse expert routing with geo-temporal conditioning (latitude/longitude, seasonal/daily cyclic encodings). Pretrained on BigEarthNet-Landsat dataset and evaluated using linear probes on frozen encoder embeddings.
...
Conclusion: Compact, metadata-aware MoE-MAEs are an efficient and scalable approach for future Earth Observation foundation models, demonstrating that small models can achieve performance comparable to much larger ones.
...
Result: Achieved mean SNR of 26.78, SSIM of 0.81, and VIF of 0.76 for source dataset; improved to SNR 29.31, SSIM 0.87, and VIF 0.81 for target dataset.
...
<details>
  <summary>Details</summary>
Motivation: To understand the key factors that influence the performance of state-of-the-art gaze-based authentication systems, particularly in VR/AR environments, to guide better system design and implementation.

Method: Conducted experiments on a large-scale dataset of 8,849 subjects using Meta Quest Pro equivalent hardware with 72Hz video oculography. Employed state-of-the-art neural network architecture to study effects of eye tracking signal quality, calibration aspects, and gaze filtering.
...
Conclusion: Careful consideration of calibration parameters and signal quality is crucial for optimal gaze-based authentication performance, while simple filtering approaches may not be beneficial and should be used cautiously.
...
<details>
  <summary>Details</summary>
Motivation: Skin tone recognition and generation are crucial for model fairness, healthcare, and generative AI, but remain challenging due to lack of comprehensive datasets and robust methodologies. Existing large multimodal models and image generation models struggle with accurate skin tone handling.

Method: Introduces TrueSkin dataset with 7,299 images categorized into 6 classes under diverse conditions. Benchmarks existing recognition and generation approaches, trains recognition models on TrueSkin, and fine-tunes image generation models with the dataset.

Result: Reveals substantial biases: LMMs misclassify intermediate skin tones as lighter ones, generative models struggle with accurate skin tone production due to prompt biases. Training on TrueSkin improves classification accuracy by over 20% and significantly enhances skin tone fidelity in generation models.

Conclusion: TrueSkin serves as both benchmark and valuable training resource to enhance fairness and accuracy in skin tone recognition and generation tasks, addressing critical gaps in current AI systems.
...
Conclusion: The proposed RL-based transfer learning framework provides an effective automated solution for model selection in animal detection applications with limited data.
...
<details>
  <summary>Details</summary>
Motivation: To develop accurate automatic fungal species recognition tools that can handle highly imbalanced class distributions and reliably identify rare species with few training samples, supporting mycologists and citizen scientists in biodiversity monitoring.
...
Conclusion: The method demonstrates strong potential for accurately identifying both common and rare fungal species, effectively supporting the objectives of FungiCLEF 2025 for large-scale biodiversity monitoring.
...
Method: Performs global clustering across all bags to establish cluster centers, then local clustering within each bag. Computes attention scores within clusters and applies sparse regularization to cluster weights to selectively retain relevant clusters.

Result: Achieves state-of-the-art performance on two public histopathology benchmarks (CAMELYON16, TCGA-NSCLC). Theoretical analysis shows csMIL requires O(s log K) bags to recover s relevant clusters.
...
Conclusion: The proposed framework successfully unlocks skeleton data's potential for zero-shot video anomaly detection through action typicality and uniqueness learning, demonstrating strong generalization capabilities across diverse unseen surveillance scenes.
...
<details>
  <summary>Details</summary>
Motivation: Current manual analysis of kidney organoid microscopy data is limited to coarse classifications, missing valuable pixel-level and longitudinal information needed for detailed PKD research.
...
Result: The platform quantifies key metrics including cyst formation rate, growth velocity, and morphological changes, and generates comprehensive reports for kidney disease research.

Conclusion: Organoid Tracker provides an extensible, open-source framework that accelerates research in kidney development, PKD modeling, and therapeutic discovery without requiring programming expertise.
...
<details>
  <summary>Details</summary>
Motivation: To develop an effective vision language model system for autonomous driving tasks using the DriveLM-nuScenes dataset and compete in the CVPR 2024 Autonomous Grand Challenge
...
Result: Achieved top score of 0.7799 on the validation set leaderboard, ranking 1st place

Conclusion: The comprehensive methodology combining vision language models with depth integration and reasoning techniques proved highly effective for autonomous driving language tasks
...
<details>
  <summary>Details</summary>
Motivation: To develop a reliable traversability costmap prediction system for planetary rovers that can handle challenging terrains by fusing multiple sensor modalities without requiring manual labeling.

Method: Fuses camera and LiDAR data to produce bird's-eye-view terrain costmaps using DINOv3 image encoder, FiLM-based sensor fusion, and self-supervised training with IMU-derived labels. Uses Huber and smoothness loss optimization.
...
Conclusion: The framework demonstrates strong robustness with geometry being the primary factor in traversability cost. Contributions include reproducible simulation, self-supervised labeling pipeline, and multi-modal BEV prediction model. Future work focuses on domain generalization and dataset expansion.
...
<details>
  <summary>Details</summary>
Motivation: Existing end-to-end learning approaches lack effective synergy between perception and control, with transformer-based self-attention producing unstable spatial attention that undermines policy reliability.

Method: Proposes CAA-Policy with Control-Aided Attention mechanism trained self-supervised using backpropagated gradients from control outputs, integrates short-horizon waypoint prediction, and uses a separately trained motion prediction module.
...
Conclusion: The proposed approach enables more robust and generalizable policies by focusing attention on visual features that induce high variance in action outputs rather than merely minimizing training loss.
...
<details>
  <summary>Details</summary>
Motivation: Traditional perspective video models struggle with panoramic content due to fundamental projection differences. Existing solutions are complex and inefficient.

Method: Treats panoramic video generation as an adaptation problem using LoRA to model transformation between perspective and panoramic projections. Fine-tunes pretrained video diffusion model with minimal training data.

Result: Achieves high-quality panoramic generation with proper projection geometry, surpassing previous state-of-the-art in visual quality, left-right consistency, and motion diversity.

Conclusion: LoRA provides an efficient and effective solution for adapting perspective video models to panoramic generation, demonstrating strong performance with minimal training requirements.
...
TL;DR: SMILE is a multi-task learning method that combines hyperspectral super-resolution with unmixing, providing theoretical guarantees for task affinity and convergence while improving unmixing performance.
...
Result: Experiments on synthetic and real datasets substantiate the usefulness of the proposed method.

Conclusion: SMILE provides progressive theoretical support and a new framework for hyperspectral unmixing under super-resolution guidance, addressing key challenges in multi-task learning integration.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to adequately model temporal dependency in multitemporal hyperspectral unmixing, limiting their ability to capture dynamical material evolution over time.

Method: Develops a mathematical model incorporating copula theory, constructs a copula-guided framework with two key modules (copula function estimation and temporal dependency guidance), and provides theoretical validation.

Result: Experimental results on both synthetic and real-world datasets demonstrate the utility and effectiveness of the proposed Cog-TD method.

Conclusion: The paper successfully redefines MTHU with temporal dependency, provides a novel copula-guided framework with theoretical support, and shows improved performance in capturing dynamical material evolution.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D benchmarks focus on urban/indoor scenes and lack disaster-affected areas, while 2D imagery suffers from depth limitations, occlusions, and limited spatial context for damage assessment.

Method: Collected using low-cost UAVs over hurricane-damaged regions, reconstructed via Structure-from-Motion and Multi-View Stereo techniques, with semantic annotations produced through manual 2D labeling projected into 3D space.

Result: The dataset captures 3D large-scale outdoor environments with fine-grained structural damage in real-world disaster contexts, providing a valuable resource for evaluating 3D segmentation models.

Conclusion: 3DAeroRelief serves as a valuable resource for advancing robust 3D vision systems in real-world applications for post-disaster scenarios, highlighting both challenges and opportunities of 3D scene understanding in disaster response.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal signals in real-world scenarios are susceptible to missing due to sensor failures and adverse weather conditions, which deteriorates model performance. Existing generative approaches inadequately handle heterogeneity of multimodal remote sensing data and suffer from heavy dependence on dominant modalities.

Method: Proposes GEMMNet with three key components: 1) Hybrid Feature Extractor (HyFEx) for modality-specific representations, 2) Hybrid Fusion with Multiscale Awareness (HyFMA) for capturing modality-synergistic semantic context across scales, and 3) Complementary Loss (CoLoss) to alleviate bias by encouraging consistency across modalities and tasks.
...
TL;DR: A pipeline to extract and reconstruct dynamic 3D smoke from single real-world videos, enabling interactive simulation for smoke design and editing with improved quality over existing methods.


<details>
  <summary>Details</summary>
Motivation: Current fluid reconstruction methods rely on controlled lab environments, leaving real-world videos largely unexplored despite recent advances in 3D vision for fluid dynamics.

Method: Targeted techniques including smoke extraction with background removal, initialization of smoke particles and camera poses, and inferring multi-view videos from single input.

Result: Outperforms previous methods with +2.22 average PSNR on wild videos and enables realistic fluid dynamics editing through simulation of reconstructed smoke assets.

Conclusion: The method successfully bridges the gap between controlled lab environments and real-world video capture, providing high-quality 4D smoke assets for interactive design and editing applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing mask-based pruning methods like MaskGS use global mean regularization that doesn't align with per-pixel reconstruction loss, which is crucial for image quality along individual camera rays.

Method: Proposes SVR-GS with spatially variant regularizer that renders per-pixel spatial masks from each Gaussian's effective contribution along rays, applying sparsity pressure on low-importance Gaussians. Explores three spatial-mask aggregation strategies with CUDA implementation and gradient analysis.

Result: Reduces Gaussians by 1.79× vs MaskGS and 5.63× vs 3DGS, with only 0.50 dB and 0.40 dB PSNR drops respectively on Tanks&Temples, Deep Blending, and Mip-NeRF360 datasets.

Conclusion: SVR-GS creates significantly smaller, faster, and more memory-efficient models suitable for real-time applications in robotics, AR/VR, and mobile perception.
...
<details>
  <summary>Details</summary>
Motivation: Effective reef monitoring requires accurate coral growth quantification through volumetric and surface area measurements, but coral complex morphology makes this challenging from 2D images.

Method: Uses pre-trained VGGT to extract dense point maps from multi-view images, merges them into unified point cloud with confidence scores, processes through parallel DGCNN decoder heads for joint volume/surface area prediction with Gaussian negative log-likelihood loss for uncertainty estimation.

Result: Achieves competitive accuracy and generalizes well to unseen coral morphologies.

Conclusion: Provides efficient and scalable coral geometry estimation directly from sparse image sets, enabling applications in coral growth analysis and reef monitoring.
...
<details>
  <summary>Details</summary>
Motivation: Existing traffic video understanding approaches struggle with accurately modeling spatiotemporal causality and integrating domain-specific knowledge, limiting effectiveness in complex traffic scenarios.

Method: Built on Qwen2.5-VL backbone, uses LoRA for lightweight fine-tuning with traffic-specific datasets. Introduces knowledge prompting module combining Chain-of-Thought reasoning with Retrieval-Augmented Generation for precise traffic regulation injection.
...
Conclusion: Traffic-MLLM effectively addresses spatiotemporal causality modeling and domain knowledge integration challenges, providing a robust solution for comprehensive traffic scene perception and causal analysis.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction methods using expanded spectral bands suffer from high costs, low accuracy, and poor geometric features. Current NeRF models only work with 3-band RGB data and cannot utilize multi-band information.

Method: Enhanced neural architecture with three modifications: expanded hidden layers for 6-band inputs, redesigned residual functions for better spectral discrepancy calculations, and adapted data compression modules for higher bit-depth multispectral imagery.
...
Conclusion: The proposed Multispectral-NeRF effectively addresses limitations of existing multispectral 3D reconstruction methods by extending NeRF to handle multi-band spectral data with improved accuracy and quality.
...
Method: Proposes Semantic-PHysical Engaged REpresentation (SPHERE) with two modules: 1) Semantic-guided Gaussian Initialization (SGI) that uses dual-branch 3D scene representations to locate focal voxels as anchors for efficient Gaussian initialization, and 2) Physical-aware Harmonics Enhancement (PHE) that incorporates semantic spherical harmonics to model physical-aware contextual details and promote semantic-geometry consistency through focal distribution alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing GAN and VAE-based steganography models suffer from mode collapse, which causes information imbalance between cover and secret images and affects extraction quality.

Method: Proposed StegOT model with multiple channel optimal transport (MCOT) module that transforms multi-peak feature distributions into single-peak distributions to achieve information trade-off.
...
<details>
  <summary>Details</summary>
Motivation: AI skin lesion classification models show bias susceptibility to skin tone representation, particularly with the Fitzpatrick Skin Tone scale which has criticized granularity favoring lighter skin tones.

Method: Trained multiple AI models to classify benign vs. malignant lesions using FST-specific data with different granularity levels (three groups: FST 1/2, 3/4, 5/6 vs. reduced granularity combinations).

Result: Models trained on FST-specific three-group data performed better than general FST-balanced models; reducing granularity (e.g., combining FST 1/2/3/4) had detrimental performance effects.

Conclusion: FST scale granularity significantly impacts model performance, suggesting a need to move away from FST scale to alternative scales that better represent diverse skin tones for fair AI research.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D forest datasets are too small for robust tree segmentation systems, while synthetic data has shown success in other domains like self-driving.

Method: Developed synthetic data generation pipeline integrating game engines with physics-based LiDAR simulation to create large-scale annotated 3D forest dataset, then used synthetic data for pretraining and minimal real data for fine-tuning.

Result: After fine-tuning on just 0.1 hectare of real forest plot, pretrained model achieves segmentations competitive with models trained on full-scale real data, substantially reducing need for labeled real data.

Conclusion: Synthetic data with physics, diversity, and scale can enable more robust 3D forest vision systems, with critical success factors identified for synthetic data usage.
...
<details>
  <summary>Details</summary>
Motivation: Existing concept sliders struggle with no-AIGC images, particularly real-world captured images, creating a need for more robust and versatile image manipulation methods that work across diverse image categories.

Method: Integrates GANs and diffusion models with fine-grained textual and visual guidance in an adversarial manner to refine images, improving upon traditional concept sliders.
...
Conclusion: Beyond Sliders successfully bridges the gap in image manipulation for real-world images, providing superior performance over existing concept slider methods through its innovative adversarial guidance approach.
...
<details>
  <summary>Details</summary>
Motivation: Fine-grained visual classification is highly sensitive to geometric variability (orientations, scales, perspective distortions). Existing equivariant architectures require substantial computational resources and restrict the hypothesis space, while Spatial Transformer Networks offer flexibility but need improved robustness.

Method: Proposes a probabilistic component-wise STN extension that decomposes affine transformations into rotation, scaling, and shearing components. Each component is regressed under geometric constraints using a shared localization encoder, modeled with Gaussian variational posteriors, and uses sampling-based canonicalization during inference. Includes a novel component-wise alignment loss leveraging augmentation parameters.
...
Conclusion: The probabilistic component-wise STN approach provides effective canonicalization for transformer-based vision pipelines, offering improved robustness to geometric variability while maintaining flexibility and being backbone-agnostic.
...
<details>
  <summary>Details</summary>
Motivation: RFID ear tags are unreliable due to loss, damage, and security vulnerabilities. Biometric identification using cattle muzzle patterns offers a robust alternative, but deep learning models face challenges with limited data, collection disruptions, and dynamic herd compositions requiring frequent retraining.

Method: Proposes Cooperative Model-Agnostic Meta-Learning (CCoMAML) with Multi-Head Attention Feature Fusion (MHAFF) as feature extractor. This few-shot learning approach enables efficient learning from limited data samples without requiring model retraining.

Result: Achieved superior performance with 98.46% and 97.91% F1 scores, outperforming current state-of-the-art few-shot learning techniques for cattle identification.

Conclusion: The proposed CCoMAML with MHAFF framework provides an effective solution for real-time cattle identification that is adaptable to new data with minimal samples, addressing the limitations of traditional RFID systems and conventional deep learning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing Bayesian-based FSL methods using KL divergence are vulnerable to adversarial attacks and natural noises, creating a need for more robust few-shot learning approaches that can maintain performance under various perturbations.

Method: The method implements an adversarially and naturally robust Hellinger distance-based feature class aggregation scheme, incorporates attention mechanisms, and introduces a novel Hellinger Similarity contrastive loss function that generalizes cosine similarity contrastive loss for variational few-shot inference.

Result: The approach demonstrates resilience to adversarial perturbations up to ε=0.30 and Gaussian noise up to σ=0.30, achieves gains of 1.20% and 1.40% for 1-shot and 5-shot scenarios on miniImageNet, and superior image reconstruction with FID score of 2.75 compared to traditional VAE (3.43) and WAE (3.38).

Conclusion: ANROT-HELANet establishes new state-of-the-art performance in few-shot learning while maintaining robustness against both adversarial and natural perturbations through its combination of Hellinger distance-based aggregation, attention mechanisms, and novel loss function.
...
Method: Uses CLIP-based framework with two novel components: Context-Aware Semantic Prompt (CASP) for adaptive prompt generation and multi-grained visual-text alignment, and Adaptive Knowledge Fusion and Projection (AKFP) with dual-path learner and Clothing-State-Aware Projection Loss.

Result: Outperforms all state-of-the-art methods across various datasets, demonstrating strong robustness and generalization despite clothing variations and sequential learning challenges.
...
TL;DR: A novel rehearsal-free approach for Class-Incremental Unsupervised Domain Adaptation that uses CLIP to extract domain-invariant class-agnostic attributes represented as key-value pairs, enabling effective domain alignment while reducing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing CI-UDA methods require storing rehearsal samples and perform asymmetric alignment, leading to continuous memory growth and knowledge forgetting. The paper aims to address these limitations by mining domain-invariant knowledge.

Method: Uses CLIP to extract class-agnostic attributes represented as key-value pairs (visual prototype + textual prompt). Maintains two attribute dictionaries (source/target domains) and performs attribute alignment via visual attention consistency and prediction consistency.
...
Conclusion: The proposed attribute-based framework successfully mitigates domain shift while reducing catastrophic knowledge forgetting in a rehearsal-free manner, demonstrating superior performance over existing approaches.
...
TL;DR: Proposes a novel evaluation framework with two metrics to assess synthetic dataset quality through cross-validation and domain transfer learning, enabling standardized and comparable quality assessment.


<details>
  <summary>Details</summary>
Motivation: Current evaluation studies for synthetic datasets lack universally accepted standards, limiting robust quality assessment and optimization guidance for synthetic data utilization.

Method: Integrates generalized cross-validation experiments and domain transfer learning principles, training task-specific models on synthetic and real datasets to form cross-performance matrices and quantify domain transferability.

Result: Experimental validation on Virtual KITTI demonstrates the framework's effectiveness in assessing synthetic data fidelity through simulation quality and transfer quality metrics.

Conclusion: The framework provides a scalable, quantifiable evaluation solution that overcomes traditional limitations and offers a principled approach for guiding synthetic dataset optimization in AI research.
...
<details>
  <summary>Details</summary>
Motivation: Outdoor scenes present challenges with unbounded environments and varying lighting conditions. Existing NeRF and 3DGS approaches suffer from high computational overhead and low-frequency lighting representations that lead to inefficient rendering and suboptimal relighting accuracy.

Method: Two-stage pipeline: 1) Uses monocular normal priors to reconstruct scene geometry with compact 2D Gaussian Splatting (2DGS), 2) Decomposes texture and lighting through a hybrid lighting model combining spherical Gaussian function for high-frequency sunlight and Spherical Harmonic coefficients for low-frequency skylight.
...
Conclusion: ROSGS provides an effective solution for outdoor scene relighting by combining efficient geometric reconstruction with a hybrid lighting model that captures both high-frequency directional sunlight and low-frequency skylight components.
...
TL;DR: APASI is a self-supervised method that mitigates hallucinations in Large Vision-Language Models by having the model self-inject realistic hallucinations to create preference pairs for alignment training, eliminating the need for external human annotations or auxiliary models.


<details>
  <summary>Details</summary>
Motivation: Existing hallucination mitigation methods require external human annotations or auxiliary models for preference data collection, which increases costs and limits sustainable improvement. There's a need for a method that can autonomously reduce hallucinations without external dependencies.

Method: APASI leverages the target LVLM to self-inject hallucinations into generated responses based on three key observations of hallucination patterns, creating preference pairs for alignment training. It uses iterative alignment with curriculum learning to periodically update preference data with increasing challenge levels.
...
Conclusion: APASI demonstrates an effective and generalizable approach for autonomous hallucination mitigation in LVLMs without external dependencies, enabling stable and continuous model enhancement through self-supervised preference alignment.
...
TL;DR: This paper introduces a training-free framework that leverages geometric priors from a Geometric Foundation Model to address viewpoint misalignment challenges in unaligned scene change detection, achieving superior performance without requiring additional training.


<details>
  <summary>Details</summary>
Motivation: Current methods for unaligned scene change detection rely solely on 2D visual cues, which fail under large viewpoint changes and lack explicit geometric reasoning. The limited supervision from small-scale datasets restricts learning generalizable multi-view knowledge.

Method: The proposed framework integrates geometric priors from a Geometric Foundation Model with visual representations from a visual foundation model. It enables reliable identification of visual overlaps, robust correspondence establishment, and explicit occlusion detection without requiring training.

Result: Extensive evaluation on PSCD, ChangeSim, and PASLCD datasets demonstrates that the approach achieves superior and robust performance compared to existing methods.

Conclusion: Leveraging geometric priors from foundation models effectively addresses the core challenges of unaligned scene change detection, providing reliable change detection under viewpoint misalignment without the need for additional training.
...
<details>
  <summary>Details</summary>
Motivation: Floorplan data is readily available and robust to visual changes, but recent methods lack uncertainty modeling in depth predictions and require custom depth networks trained for each environment, limiting generalization.

Method: Introduces a novel probabilistic model that treats depth predictions as explicit probability distributions with uncertainty estimation. Uses off-the-shelf pre-trained monocular depth models instead of per-environment-trained networks.

Result: Achieves 2.7x higher localization recall on long sequences (100 frames) and 16.7x higher on short sequences (15 frames) than state-of-the-art on the challenging LaMAR HGE dataset. Demonstrates significant improvements in accuracy and robustness on large-scale synthetic and real-world datasets.

Conclusion: UnLoc provides an efficient and generalizable solution for sequential camera localization that overcomes key limitations of existing methods through probabilistic uncertainty modeling and the use of pre-trained depth models, enabling better performance in unseen environments.
...
TL;DR: Proposes SIKNet, a learning-aided Kalman filter that uses semantic-independent encoding to improve motion estimation in multi-object tracking, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional Kalman filters based on linear constant-velocity models perform poorly when parameters are mismatched or objects move non-stationarily, requiring better motion estimation for multi-object tracking.

Method: Semantic-Independent KalmanNet (SIKNet) uses a Semantic-Independent Encoder with 1D convolution and fully-connected layers to encode independent semantic information and nonlinear cross-dependencies between heterogeneous elements.

Result: Experimental results on a large-scale semi-simulated dataset show SIKNet outperforms traditional Kalman filters and existing learning-aided filters in robustness and accuracy.

Conclusion: SIKNet provides superior motion estimation for multi-object tracking by effectively handling semantic-independent encoding and nonlinear dependencies, with code publicly available.
...
TL;DR: This doctoral research investigates long-range visual dependency modeling in medical image computing, comparing transformers and MLPs, finding MLPs superior for fine-grained dependency modeling in high-resolution medical images.


<details>
  <summary>Details</summary>
Motivation: Medical image computing requires models that capture both global context and local subtle details, but existing approaches (CNNs and transformers) have limitations - CNNs are local, transformers are computationally expensive for high-resolution features.

Method: The research first explores transformers for pixel-wise and image-wise medical vision tasks, then pioneers MLP-based visual models to capture fine-grained long-range dependencies in medical images through extensive experiments.

Result: MLPs demonstrate feasibility in modeling finer-grained long-range dependency among higher-resolution medical features containing enriched anatomical/pathological details, consistently enhancing performance across various medical vision tasks.

Conclusion: MLPs establish themselves as a superior paradigm over transformers/CNNs for medical image computing, providing a pathway for next-generation medical vision backbones by enabling effective fine-grained long-range dependency modeling.
...
<details>
  <summary>Details</summary>
Motivation: In near-ambient conditions relevant to computer vision, both reflected environmental light and emitted surface light are comparable and time-varying, making separation essential for understanding object properties like emissivity, temperature, reflectance and shape.

Method: Developed a dual-band thermal image formation model using two thermal cameras with different spectral sensitivities, with algorithms to estimate surface emissivity, time-varying temperature, and isolate dynamic background.

Result: Quantitatively evaluated using carefully calibrated emissivities for various materials, showing successful separation of components in complex everyday scenes like glass with hot liquid and people moving in background.

Conclusion: The introduced method successfully addresses the challenge of separating reflected and emitted light components in thermal imaging, enabling more accurate analysis of object properties in computer vision applications under near-ambient conditions.
...
TL;DR: SSL can learn meaningful representations even without strict instance consistency, with moderate view diversity enhancing performance but excessive diversity reducing effectiveness.


<details>
  <summary>Details</summary>
Motivation: Traditional SSL assumes instance consistency where different views of the same image are positive pairs, but this breaks down for non-iconic data where views may contain distinct objects or semantic information.

Method: Extensive ablation studies investigating SSL effectiveness without instance consistency guarantees, using Earth Mover's Distance (EMD) to measure mutual information between views and quantify view diversity.
...
Conclusion: There is an optimal range for view diversity in SSL frameworks, with moderate diversity enhancing performance. EMD serves as an effective estimator for measuring mutual information between views, providing insights for future SSL framework design on diverse data sources.
...
TL;DR: The paper proposes two regularization methods to make CNNs more robust to image corruptions by encouraging shape-based rather than texture-based representations.


<details>
  <summary>Details</summary>
Motivation: CNNs are vulnerable to common image corruptions that humans handle easily because they rely too much on local texture cues rather than global object shapes like humans do.

Method: Two complementary regularization strategies: 1) auxiliary loss enforcing feature consistency between original and low-frequency filtered inputs to reduce texture dependence, and 2) supervised contrastive learning to structure feature space around class-consistent shape representations.

Result: Both methods improved corruption robustness on CIFAR-10-C benchmark without degrading clean accuracy.

Conclusion: Loss-level regularization can effectively steer CNNs toward more shape-aware and resilient representations.
...
TL;DR: GLaVE-Cap is a new video captioning framework that addresses limitations in current local-to-global approaches by introducing TrackFusion for comprehensive local caption generation and CaptionBridge for local-global interaction, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current local-to-global video captioning approaches produce less detailed and contextually inconsistent captions due to lack of fine-grained captioning mechanisms and weak interaction between local and global captions.
...
Result: Achieves state-of-the-art performance on four benchmarks. Ablation studies validate effectiveness of proposed modules and contribution of GLaVE-1.2M dataset to video understanding community.

Conclusion: GLaVE-Cap successfully addresses the limitations of current video captioning approaches through improved local caption generation and enhanced local-global interaction, with comprehensive benchmarks and datasets provided to advance the field.
...
TL;DR: A portable 3D skin reconstruction probe using GelSight tactile imaging achieves micron-level wrinkle measurement accuracy and validates wrinkle depth metrics across body regions, showing significant moisturizer effects.


<details>
  <summary>Details</summary>
Motivation: There is no existing portable, high-resolution device for 3D skin surface reconstruction that has been validated for depth measurement across various body locations, limiting objective dermatological assessment.
...
Result: Achieved mean absolute error of 12.55 microns on wrinkle-like test objects. Validated wrinkle depth metrics across multiple body regions in 15 participants, and demonstrated statistically significant wrinkle height reduction after moisturizer application at three locations.
...
TL;DR: MixANT introduces a mixture of experts approach to dynamically select contextually relevant forget-gate matrices in State Space Models, enabling input-dependent temporal memory for improved human activity anticipation.


<details>
  <summary>Details</summary>
Motivation: Current State Space Models like Mamba have static forget-gate matrices that control temporal memory, limiting their ability to adapt to different input contexts for human activity prediction.

Method: Proposes a mixture of experts architecture that dynamically selects appropriate forget-gate matrices based on input features, enhancing representational capacity while maintaining computational efficiency.
...
TL;DR: Hypernetwork-based method that dynamically generates classification models conditioned on available medical imaging modalities, enabling robust training and inference on incomplete multi-modal data.
...
TL;DR: Proposes weighted rotation blending with quaternion averaging to fix Gaussian splatting artifacts in human avatar animation, replacing complex corrective methods with a simpler vertex-based approach.


<details>
  <summary>Details</summary>
Motivation: Gaussian splatting methods for human avatars suffer from artifacts due to linear blend skinning not handling non-linear rotation properties properly, requiring complex corrective approaches.

Method: Weighted rotation blending approach using quaternion averaging that modifies linear blend skinning technique to handle Gaussian rotation properties correctly.
...
Conclusion: The proposed quaternion-based rotation blending provides a straightforward solution to Gaussian deformation artifacts while maintaining efficiency and compatibility with existing systems.
...
<details>
  <summary>Details</summary>
Motivation: Medical imaging data suffers from domain shifts due to different vendors and acquisition parameters, which impedes learning biologically meaningful representations and discovering disease patterns.

Method: Active learning approach using post-hoc rotation of data latent space to disentangle biological and technical factors, enabling stable tissue-type clustering across different acquisition settings.

Result: Cluster consistency improved by +19.01% (ARI), +16.85% (NMI), and +12.39% (Dice) compared to entangled representations, outperforming four state-of-the-art harmonization methods. Enhanced Cox survival prediction in idiopathic pulmonary fibrosis patients.

Conclusion: The proposed label-free framework effectively facilitates biomarker discovery in multi-center routine imaging data by disentangling biological and technical factors through latent space rotation.
...
TL;DR: A masked autoencoder approach for handling missing MRI sequences in 3D medical imaging, using multi-modal multi-task learning with late-fusion transformer and individual decoders to enable cross-sequence reasoning and robust downstream performance.


<details>
  <summary>Details</summary>
Motivation: Missing input sequences are common in medical imaging data, posing challenges for deep learning models that require complete input data for accurate analysis and diagnosis.

Method: Developed a masked autoencoder paradigm inspired by MultiMAE, treating each MRI sequence as separate modality with late-fusion transformer encoder and individual decoder streams for multi-task reconstruction, enabling cross-sequence reasoning for missing inputs.

Result: Achieved absolute improvement of 10.1 overall Dice score and 0.46 MCC over MAE-ViT baselines in downstream segmentation and classification tasks with missing input sequences, demonstrating superior performance and robustness.

Conclusion: The method provides a flexible and generalizable encoder for brain MRIs that can infer missing sequences from available inputs and be adapted to various downstream applications, with implementation made publicly available.
...
TL;DR: TrajTrack is a lightweight LiDAR-based 3D single object tracking framework that uses historical bounding box trajectories to enhance two-frame trackers, achieving state-of-the-art performance with 4.48% improvement and 56 FPS speed.


<details>
  <summary>Details</summary>
Motivation: Existing 3D SOT methods face a dilemma: two-frame methods are efficient but lack temporal context, while sequence-based methods are robust but computationally expensive. There's a need for a solution that balances efficiency and robustness.

Method: Proposes a trajectory-based paradigm that enhances base two-frame trackers by learning motion continuity from historical bounding box trajectories. Uses explicit motion proposals followed by implicit motion modeling to predict future trajectories and refine initial proposals.

Result: Achieves new state-of-the-art performance on NuScenes benchmark with 4.48% improvement in tracking precision over strong baseline while running at 56 FPS. Demonstrates strong generalizability across different base trackers.

Conclusion: The trajectory-based paradigm effectively resolves the efficiency-robustness trade-off in 3D SOT, providing a lightweight solution that maintains high performance without requiring additional point cloud inputs.
...
<details>
  <summary>Details</summary>
Motivation: Infrared and visible image fusion aims to combine complementary structural and textural information from different spectral domains, but existing methods often lack explicit modeling of inter-modality interactions and semantic preservation in task-critical regions.

Method: Proposes FusionNet with: 1) modality-aware attention mechanism to dynamically adjust feature contributions, 2) pixel-wise alpha blending module for adaptive content-aware fusion, and 3) target-aware loss using weak ROI supervision to preserve semantic consistency.
...
TL;DR: A novel Multiple Instance Learning framework with masked hard instance mining (MHIM-MIL) that addresses bias towards easy instances in computational pathology by focusing on challenging examples through Siamese structure and momentum teacher.


<details>
  <summary>Details</summary>
Motivation: Existing MIL methods in computational pathology focus on identifying salient instances via attention mechanisms, leading to bias towards easy-to-classify instances while neglecting challenging ones, which are crucial for accurate discriminative boundaries.

Method: MHIM-MIL uses a Siamese structure with consistency constraint, class-aware instance probability, and momentum teacher to mask salient instances and implicitly mine hard instances. It employs large-scale random masking for diversity and global recycle network to preserve key features, with exponential moving average for teacher updates.

Result: Experimental results on cancer diagnosis, subtyping, survival analysis tasks across 12 benchmarks demonstrate that MHIM-MIL outperforms latest methods in both performance and efficiency.

Conclusion: The proposed MHIM-MIL framework effectively addresses the bias problem in MIL by focusing on hard instances, achieving superior performance in computational pathology tasks while maintaining efficiency.
...
TL;DR: SFGNet is a novel camouflaged object detection network that incorporates semantic prompts and frequency-domain features to better segment objects that blend into their surroundings, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Most existing camouflaged object detection studies overlook semantic differences among textual prompts of different targets and fine-grained frequency features, leading to suboptimal performance in complex backgrounds and blurred boundaries.

Method: Proposes Semantic and Frequency Guided Network (SFGNet) with Multi-Band Fourier Module (MBFM) to handle complex backgrounds and blurred boundaries, and Interactive Structure Enhancement Block (ISEB) to ensure structural integrity and boundary details.
...
Conclusion: The integration of semantic prompts and frequency-domain features effectively improves camouflaged object detection and boundary perception, with the proposed MBFM and ISEB modules enhancing performance in challenging scenarios.
...
TL;DR: VLMs have latent GUI grounding potential but underperform on explicit coordinate output. Three zero-shot auxiliary reasoning methods (using spatial cues like axes, grids, and labeled intersections) significantly improve GUI grounding performance without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: General vision-language models struggle with GUI grounding tasks despite having latent spatial understanding capabilities, and current fine-tuning approaches require high data and annotation costs.
...
Result: The methods substantially improve GUI grounding performance across four benchmarks and seven open-source/proprietary VLMs, demonstrating the effectiveness of leveraging spatial cues.

Conclusion: Providing explicit spatial cues enables VLMs to better utilize their latent grounding capabilities for GUI tasks without requiring expensive fine-tuning, offering a cost-effective solution for GUI agent development.
...
TL;DR: GPS-SLAM combines Gaussian and SDF representations to achieve real-time 3D reconstruction at 150+ fps, delivering 10x speedup over state-of-the-art methods while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current Gaussian-based SLAM methods are computationally slow (<20 fps) compared to geometry-centric approaches (hundreds of fps), due to heavy computational burden from numerous Gaussians and complex optimization.

Method: Proposes a Gaussian-SDF hybrid representation: SDF for smooth geometry/appearance via RGB-D fusion, plus 3D Gaussians for detail capture. Enables 50% fewer Gaussians and 75% fewer optimization iterations.

Result: Achieves over 150 fps on real-world Azure Kinect sequences - order-of-magnitude speedup over state-of-the-art techniques while maintaining comparable reconstruction quality.

Conclusion: The hybrid representation successfully addresses computational bottlenecks in Gaussian-based SLAM, enabling real-time performance without sacrificing reconstruction quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing cluster-based contrastive learning methods for USVI-ReID focus on commonality within clusters but neglect finer-grained differences among images within each cluster, limiting performance.

Method: HIL framework generates multiple memories per cluster via secondary clustering, uses Multi-Center Contrastive Learning to refine representations, and employs Bidirectional Reverse Selection Transmission for reliable cross-modal matching.

Result: Extensive experiments on SYSU-MM01 and RegDB datasets demonstrate superior performance compared to existing approaches.

Conclusion: The proposed HIL framework effectively addresses fine-grained variations in unsupervised cross-modal person re-identification and outperforms state-of-the-art methods.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for performance prioritization in safety-critical scenarios (e.g., prioritizing 'Human' over 'Dog') and overcome limitations of uniform class distributions by optimizing data distribution to minimize bias and variance.

Method: An iterative, data-centric framework that identifies Pareto optimized class distributions. It determines the optimal number of images per class and can be integrated into existing training pipelines with minimal code changes. Validated using EfficientNet, ResNet and ConvNeXt on CIFAR-10 and iNaturalist21 datasets.

Result: Demonstrated improved and balanced model performance through optimized data distribution. The framework successfully enhances reliability and minimizes bias and variance in the objective function.

Conclusion: BiCDO provides an effective solution for optimizing class distributions in multi-class image classification, enabling targeted performance improvements for specific classes while maintaining overall model balance and reliability.
...
TL;DR: MVQA-68K is a large-scale multi-dimensional video quality assessment dataset with 68K videos annotated across 7 quality dimensions with detailed reasoning, enabling more comprehensive and interpretable VQA that achieves SOTA results.


<details>
  <summary>Details</summary>
Motivation: Traditional VQA methods produce single numerical scores that lack comprehensiveness and interpretability, while the rapid advancement of video generation models like Sora creates need for better quality assessment for pre-training dataset selection.

Method: Created MVQA-68K dataset with over 68,000 videos annotated across 7 quality dimensions (aesthetics, camera movement, dynamic degree, texture detail, composition, visual quality, factual consistency) with chain-of-thought reasoning. Used this dataset to train multimodal LLMs for VQA.

Result: Achieved state-of-the-art results on internal test set and public benchmarks (LSVQ-test, LSVQ-1080p, LIVE-VQC). Incorporating explicit reasoning process during training substantially boosts zero-shot generalization.

Conclusion: MVQA-68K provides a comprehensive and interpretable framework for video quality assessment that significantly enhances MLLM performance and demonstrates the value of detailed reasoning annotations for improving VQA capabilities.
...
TL;DR: HyGDL is a hybrid generative-discriminative framework that addresses shortcut learning in SSL by enforcing content-style disentanglement through vector projection and invariance pre-training.


<details>
  <summary>Details</summary>
Motivation: Self-supervised learning suffers from shortcut learning where models exploit superficial features like texture instead of intrinsic structure, which hinders generalization to unseen domains. Existing methods fail to address the underlying learning mechanism that creates shortcut dependency.

Method: Proposes HyGDL framework with single encoder that analytically defines style as orthogonal component to content via vector projection. Uses invariance pre-training principle: systematically varying style bias while keeping supervision constant to force learning of invariant essence.
...
Conclusion: HyGDL provides a fundamental solution to shortcut learning by altering the underlying learning mechanism through hybrid generative-discriminative approach with analytical style-content separation.
...
TL;DR: This paper introduces a softmax-based frame allocation strategy for video anomaly detection that prioritizes anomaly-dense segments while maintaining full-video coverage, and creates two complementary benchmarks for comprehensive evaluation.
...
Method: Proposes a softmax-based frame allocation strategy that balances sampling across temporal scales by focusing on anomaly-dense segments while ensuring full-video coverage. Constructs two benchmarks: image-based for frame-level reasoning and video-based for temporally localized segments with abnormality scoring.
...
TL;DR: A 3D deepfake framework using 3D Gaussian Splatting for realistic face swapping and reenactment with multi-view consistency and precise control, outperforming 2D methods in 3D rendering quality.


<details>
  <summary>Details</summary>
Motivation: To overcome geometric inconsistencies and limited generalization of conventional 2D deepfake approaches by enabling fully controllable 3D face manipulation with multi-view consistent rendering.
...
Conclusion: Bridges 3D modeling and deepfake synthesis, enabling scene-aware and immersive visual forgeries while revealing potential threats of 3D Gaussian Splatting for manipulation attacks.
...
<details>
  <summary>Details</summary>
Motivation: Random noise initialization in vanilla diffusion models introduces mismatched semantic information in masked regions, leading to biased inpainting results with low consistency and coherence with surrounding areas.
...
Result: Validated on CelebA-HQ, ImageNet, and Places2 datasets, showing effectiveness across all metrics compared to state-of-the-art inpainting methods for both standard and large-mask tasks.

Conclusion: IS-Diff provides a promising training-free approach that produces more harmonious and consistent inpainting results by leveraging distributional harmonious seeds from unmasked regions.
...
TL;DR: A real-world benchmark dataset for all-in-one adverse weather image restoration containing aligned degraded-clean image pairs across rain, snow, and haze conditions.


<details>
  <summary>Details</summary>
Motivation: Existing approaches use synthetic datasets with significant domain gaps in resolution, style, and characteristics, hindering development and fair evaluation of unified models. Lack of large-scale real-world datasets is a critical bottleneck.

Method: Created a real-world benchmark dataset with precisely aligned degraded and clean images captured under various weather conditions (rain, snow, haze), diverse outdoor scenes, and illumination settings.

Result: Provides a foundation for supervised learning and rigorous evaluation. Comprehensive experiments benchmarked task-specific, task-general, and all-in-one restoration methods.

Conclusion: The dataset advances robust and practical all-in-one image restoration in real-world scenarios and has been publicly released.
...
TL;DR: RVMamba and Joint-OCTAMamba frameworks using Mamba state-space models for improved retinal vessel and foveal avascular zone segmentation in OCTA imaging, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current 2D-based methods for retinal vessel segmentation in OCTA imaging provide insufficient accuracy, and existing joint segmentation models show performance imbalance between different tasks like FAZ segmentation.
...
Conclusion: The proposed Mamba-based frameworks effectively address accuracy limitations and performance imbalance in OCTA retinal analysis, providing superior segmentation performance for both retinal vessels and foveal avascular zone.
...
TL;DR: DTGen is a few-shot data augmentation method using diffusion models to generate diverse dirty tableware images for fine-grained recognition, improving classifier performance with limited real data.


<details>
  <summary>Details</summary>
Motivation: Existing tableware cleaning methods suffer from coarse-grained classification and few-shot data scarcity, making them inadequate for industrial applications requiring fine-grained dirty tableware recognition.
...
Conclusion: DTGen validates generative AI's value in few-shot industrial vision and provides a feasible deployment path for automated tableware cleaning and food safety monitoring, with potential for integration into embedded dishwashers.
...
TL;DR: MindVL is a multimodal large language model trained on Ascend NPUs that achieves performance comparable to Qwen2.5-VL using only 1/10 of the training data, featuring native-resolution vision processing and specialized training framework.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient multimodal model that can process images at original variable resolutions without degradation, while being optimized for training on Ascend NPUs with reduced data requirements.

Method: Uses native-resolution Vision Transformers, three-phase training (warm-up, multitask training, supervised instruction tuning), multimodal data packaging, hybrid parallelism, test-time resolution search, and model weight averaging with the Mindspeed-MLLM distributed training framework.
...
Conclusion: MindVL demonstrates that efficient training on specialized hardware with optimized frameworks can achieve state-of-the-art multimodal performance with significantly reduced data requirements, particularly excelling in OCR tasks.
...
TL;DR: A pipeline to extract navigable trails from scanned paper maps for GPS navigation using georeferencing, U-Net segmentation, graph construction, and routing engine refinement.
...
Conclusion: The proposed pipeline successfully enables the use of traditional paper maps in modern GPS-based navigation systems by extracting and converting their trail information.
...
TL;DR: A new industrial metallic dataset (IMD) with 45 true-to-scale industrial components for benchmarking 6D pose estimation in challenging industrial environments with metallic, texture-less, reflective objects.


<details>
  <summary>Details</summary>
Motivation: Existing 6D pose estimation benchmarks use everyday objects with rich textures and low reflectivity, limiting generalization to industrial scenarios where objects are metallic, texture-less, and highly reflective.

Method: Created IMD dataset with 45 industrial components captured using RGB-D camera under natural lighting and varied arrangements. Evaluated state-of-the-art models (XMem, SAM2 for segmentation; BundleTrack, BundleSDF for pose estimation) on three tasks: video object segmentation, 6D pose tracking, and one-shot 6D pose estimation.

Result: The industrial dataset proved more challenging than existing household object datasets, revealing limitations of current models in industrial contexts.

Conclusion: This benchmark provides a baseline for developing and comparing segmentation and pose estimation algorithms that better generalize to industrial robotics scenarios.
...
Method: Proposed Ensemble Distillation that distills knowledge from multiple ensemble models into a single model to reduce computational complexity.
...
Result: Networks with just 8 unique frozen filters achieve over 80% ImageNet accuracy and outperform models with thousands of parameters on smaller datasets.
...
TL;DR: New document layout analysis models integrated into Docling pipeline achieve 20.6-23.9% mAP improvement over previous baseline with comparable runtime, with best model reaching 78% mAP at 28ms/image on A100 GPU.


<details>
  <summary>Details</summary>
Motivation: To develop improved layout analysis models for document conversion tasks by leveraging state-of-the-art object detectors and addressing the need for better performance in document layout detection.

Method: Trained RT-DETR, RT-DETRv2 and DFINE architectures on 150,000 documents (open and proprietary), applied post-processing to raw detections, and evaluated across various benchmarks and hardware environments.

Result: Five new models achieved significant mAP improvements (20.6-23.9%) over previous baseline, with best model "heron-101" reaching 78% mAP at 28ms/image inference time on NVIDIA A100 GPU.

Conclusion: The research establishes best practices for training, evaluating, and deploying document-layout detectors, providing actionable guidance for the document conversion community, with all models and code released under permissive license.
...
Method: Proposes MISRA framework that augments RGB with luminance channels, integrates skip attention to preserve elongated features, and employs Iterative Feedback Module (IFM) for continuity restoration across multiple passes. Also introduces a dedicated microsurgical dataset with fine-grained annotations.
...
Conclusion: MISRA represents a promising step toward reliable scene parsing for computer-assisted and robotic microsurgery, with the framework and dataset providing a benchmark for future research.
...
Result: Outperforms state-of-the-art methods by 5% in APLS on three real-world datasets, with significant performance gains on Didi Chuxing platform trajectory data.

Conclusion: DGMap effectively addresses trajectory density challenges by leveraging global context awareness, improving keypoint detection accuracy and suppressing false connections in both sparse and dense trajectory regions.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of large labeled datasets in clinical environments and limited generalizability of task-specific models have hindered the development of generalizable AI for ultrasound applications.
...
Result: EchoCare outperforms state-of-the-art comparison models across 10 ultrasound benchmarks spanning disease diagnosis, lesion segmentation, organ detection, landmark prediction, quantitative regression, imaging enhancement and report generation with minimal training.

Conclusion: EchoCare provides a fully open and generalizable foundation model that can boost AI development for diverse clinical ultrasound applications through fine-tuning and local adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D face reconstruction methods struggle with diverse facial attributes and conditions, often producing incomplete results. They also require large amounts of expensive 3D facial data for training.
...
Result: Achieves results comparable to state-of-the-art methods on MICC Florence, Facewarehouse and custom datasets, and surpasses SOTA performance in some challenging conditions.

Conclusion: The MSMA framework effectively addresses limitations in capturing detailed multi-scale features under diverse conditions, providing accurate 3D facial parameter estimation from single 2D images without heavy reliance on 3D labeled data.
...
TL;DR: Seg2Track-SAM2 is a novel framework that integrates pre-trained object detectors with SAM2 and a new Seg2Track module for Multi-Object Tracking and Segmentation (MOTS), achieving SOTA performance without fine-tuning while reducing memory usage by 75%.


<details>
  <summary>Details</summary>
Motivation: Current foundation models like SAM2 show strong zero-shot video segmentation but lack sufficient identity management and memory efficiency for direct MOTS application, requiring a specialized framework to address these limitations.

Method: Integrates pre-trained object detectors with SAM2 foundation model and a novel Seg2Track module that handles track initialization, management, and reinforcement. Uses a sliding-window memory strategy and remains detector-agnostic without requiring fine-tuning.

Result: Achieves state-of-the-art performance, ranking 4th overall on KITTI MOTS benchmarks for both car and pedestrian classes, establishes new benchmark in association accuracy (AssA), and reduces memory usage by 75% with negligible performance degradation.

Conclusion: Seg2Track-SAM2 advances MOTS by combining robust zero-shot tracking, enhanced identity preservation, and efficient memory utilization, making it suitable for deployment under resource constraints.
...
TL;DR: SA-UNetv2 improves retinal vessel segmentation by adding cross-scale spatial attention to skip connections and using a weighted BCE+MCC loss to handle class imbalance, achieving state-of-the-art performance with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Retinal vessel segmentation is crucial for early disease diagnosis, but existing SA-UNet underuses attention in skip connections and fails to address severe foreground-background imbalance issues.

Method: Proposed SA-UNetv2 injects cross-scale spatial attention into all skip connections to enhance multi-scale feature fusion and adopts a weighted Binary Cross-Entropy plus Matthews Correlation Coefficient loss to improve robustness to class imbalance.

Result: On DRIVE and STARE datasets, SA-UNetv2 achieves state-of-the-art performance with only 1.2MB memory, 0.26M parameters (less than 50% of SA-UNet), and 1 second CPU inference on 592x592x3 images.
...
TL;DR: FineQuest is a training-free framework for sports VideoQA that uses dual-mode reasoning (reactive/deliberative) and a multimodal sports knowledge graph (SSGraph) to achieve state-of-the-art performance on new sports benchmarks.


<details>
  <summary>Details</summary>
Motivation: VideoQA based on LLMs struggles with the complexity of sports videos due to domain-specific knowledge gaps and the need for specialized reasoning approaches.

Method: Proposes FineQuest framework with: 1) Dual-mode reasoning (reactive for simple queries, deliberative for complex ones) 2) SSGraph - multimodal sports knowledge scene graph covering 9 sports 3) New benchmarks Gym-QA and Diving-QA from FineGym/FineDiving datasets

Result: Achieves state-of-the-art performance on new sports benchmarks (Gym-QA, Diving-QA) and existing SPORTU dataset, while maintaining strong general VideoQA capabilities.

Conclusion: FineQuest demonstrates that training-free frameworks with cognitive-inspired reasoning and domain-specific knowledge graphs can effectively address sports VideoQA challenges without sacrificing general video understanding performance.
...
Method: Leverages neural network training dynamics to assess sample difficulty, aggregates and calibrates model predictions during training to create uncertainty-aware pseudo-labels that capture diagnostic ambiguity.

Result: Superior performance on echocardiography classification benchmark compared to specialized baselines, with improvements in calibration, selective classification, and multi-view fusion.

Conclusion: The proposed uncertainty-aware label augmentation framework enhances model robustness and uncertainty estimation, is architecture-agnostic, and can be applied to any supervised learning pipeline for medical diagnosis.
...
Method: Incorporates focal modulation attention at the encoder-decoder bottleneck and region-aware attention in selective skip connections to enhance feature representation and regional focus while maintaining lightweight architecture.

Result: Outperformed state-of-the-art models on DRIVE, STARE, and CHASE_DB datasets with Dice scores of 84.28%, 88.44%, 85.50% and Jaccard indices of 72.86%, 79.31%, 74.70% respectively, while being extremely lightweight.
...
TL;DR: SpecVLM accelerates vision-language models using speculative decoding with an elastic visual compressor and online-logit distillation, achieving 2.5-2.9x speedups while maintaining lossless decoding.


<details>
  <summary>Details</summary>
Motivation: Direct application of speculative decoding to VLMs faces challenges due to visual token dominance in prefill stage, causing compute and memory inefficiencies, especially with KV cache inflation from high-resolution images and videos.

Method: Combines EAGLE-2-style baseline (EagleVLM) with elastic visual compressor that adaptively selects compression primitives (pruning, pooling, convolution, resampler) and online-logit distillation protocol using teacher logits and penultimate features with cross-entropy + Smooth L1 objective.
...
Conclusion: SpecVLM provides a practical system for accelerating VLMs through adaptive visual compression and efficient online training, demonstrating scalable performance improvements without costly offline distillation.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods don't fully explore the reciprocal promotion between pixel-wise image fusion and cross-modal feature fusion perception tasks from a macroscopic task-level perspective.

Method: Parallel structure with fusion and segmentation sub-networks, heterogeneous feature fusion strategy, multi-stage Transformer decoder, and dynamic factor for adaptive task weighting in multi-task training.

Result: Achieves competitive results compared with state-of-the-art methods in both image fusion quality and semantic segmentation performance.

Conclusion: The proposed MAFS framework successfully demonstrates mutual enhancement between image fusion and semantic segmentation tasks through a unified multi-task approach.
...
Method: Combines sampling-based reachability analysis with conformal inference, introducing novel strategies to reduce conservatism in high-dimensional settings while maintaining rigorous guarantees. The approach is architecture-agnostic and scalable to high-dimensional outputs.
...
Conclusion: The proposed framework provides a practical and scalable solution for probabilistic verification of semantic segmentation networks, offering provable safety guarantees while addressing the limitations of prior methods in high-dimensional settings. A toolbox implementation is made available on Github.
...
TL;DR: This paper bridges generative VLMs and dense vision-language alignment by using synthetic captions from VLMs for improved zero-shot segmentation.


<details>
  <summary>Details</summary>
Motivation: Generative VLMs lack spatially dense alignment between vision and language, while existing dense alignment methods need better semantic understanding.

Method: Densely align images with synthetic descriptions generated by VLMs, leveraging inexpensive and scalable synthetic captions.

Result: Outperforms prior work on zero-shot open-vocabulary segmentation benchmarks while being more data-efficient.

Conclusion: Synthetic captions from VLMs provide excellent high-level semantic understanding for dense alignment methods, bridging two research directions effectively.
...
TL;DR: SDI-GS introduces segmentation-driven initialization to reduce Gaussian count in 3D Gaussian Splatting by 50% while maintaining rendering quality, enabling faster training and lower memory usage for sparse-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with sparse-view settings due to SfM limitations or generate massive Gaussian counts from MVS back-projection, leading to high memory costs and inefficiency.

Method: Leverages region-based segmentation to identify structurally significant regions, enabling selective downsampling of dense point clouds to preserve scene fidelity while reducing Gaussian count.

Result: Reduces Gaussian count by up to 50%, achieves comparable or superior PSNR and SSIM scores with only marginal LPIPS degradation, and enables faster training with lower memory footprint.

Conclusion: SDI-GS advances the practicality of 3DGS for constrained-view scenarios by significantly improving efficiency while maintaining rendering quality through segmentation-driven initialization.
...
Method: SG-VLM is a modular framework that integrates frozen VLMs with scene graph grounding via prompting and visual localization techniques.

Result: Across three benchmarks (NExT-QA, iVQA, ActivityNet-QA) and multiple VLMs, SG-VLM improves causal and temporal reasoning and outperforms prior baselines, though gains over strong VLMs are limited.
...
TL;DR: Dr.V is a hierarchical framework for detecting hallucinations in large video models through fine-grained spatial-temporal grounding at perceptive, temporal, and cognitive levels, comprising a benchmark dataset and satellite agent.


<details>
  <summary>Details</summary>
Motivation: Large video models suffer from hallucinations that produce content conflicting with input videos, requiring systematic detection methods to improve reliability.

Method: Hierarchical framework with Dr.V-Bench dataset (10k instances from 4,974 videos) and Dr.V-Agent that applies spatial-temporal grounding at perceptive and temporal levels followed by cognitive reasoning.
...
Conclusion: Dr.V provides a practical blueprint for robust video understanding in real-world scenarios through systematic hallucination detection.
...
TL;DR: MOT approaches outperform traditional MAT tools for long-term pig tracking, showing potential for improved livestock monitoring accuracy.
...
TL;DR: A novel Weighted Prompt Manipulation (WPM) technique for zero-shot image generation from poems, allowing dynamic adjustment of word importance in diffusion models to create contextually accurate visualizations.


<details>
  <summary>Details</summary>
Motivation: Poetry invites multiple interpretations based on readers' emotions and backgrounds, so there's a need to generate and modify images for poems in a zero-shot setting to accommodate diverse audience requirements.

Method: Introduces WPM technique that systematically modifies attention weights and text embeddings in diffusion models, using LLMs like GPT with poetry datasets to dynamically adjust word importance for enhanced image generation.

Result: The approach enables semantically richer and more contextually accurate visualizations of poems by enhancing or suppressing specific word influences in generated images.

Conclusion: This represents the first integration of weighted prompt manipulation for improving imagery in poetic language, providing a comprehensive methodology for literary domain image generation.
...
Method: Proposes Reverse SAM Parameter Configuration Module to mitigate adverse parameters train-free, and T-Visioner Module with Test-Time Training layers to strengthen advantageous parameters by adapting language task layers to vision tasks.

Result: Achieves state-of-the-art performance on various COD benchmarks, significantly improving SAM's semantic understanding in camouflaged object detection.

Conclusion: SAM-TTT sets a new benchmark in COD by simultaneously suppressing adverse parameters while reinforcing advantageous ones through innovative parameter configuration and test-time training integration.
...
Method: Proposes a depth-aware CycleGAN to bridge domain gap between real bronchoscopic images and airway geometries, plus an airway structure awareness loss to enforce depth consistency within airway lumen while preserving structural integrity.
...
Conclusion: Brea-Depth successfully integrates anatomical priors into foundation model adaptation, enhancing generalization and yielding more accurate bronchoscopic depth estimation with improved structural consistency.
...
TL;DR: Proposes logit-space linear interpolation mixing of in-distribution and out-of-distribution data to improve OOD detection performance, particularly for data near decision boundaries.


<details>
  <summary>Details</summary>
Motivation: Current OOD detection methods like Outlier Exposure struggle to effectively learn class relationships and clearly distinguish between in-distribution and out-of-distribution data, especially near decision boundaries.

Method: Linear interpolation technique in logit space that mixes in-distribution and out-of-distribution data, with consistency enforcement between logit-space and input-space mixing outputs.

Result: Reduces abrupt fluctuations in model outputs near decision boundaries, resulting in smoother and more reliable separation between in-distribution and out-of-distribution data.

Conclusion: Logit-space mixing technique effectively improves OOD detection performance, particularly for fine-grained detection tasks and data close to decision boundaries.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DSSG methods rely mainly on sensor data and assume complete scene reconstructions, limiting their applicability in real-world incremental settings. There's a need to integrate richer semantic information and handle incremental environments.

Method: Heterogeneous graph model that integrates multi-modal information (prior observations, semantic embeddings like CLIP) directly into message-passing process. Uses multiple layers to incorporate global and local scene representations without specialized modules.
...
Conclusion: The proposed approach provides an effective solution for incremental 3D semantic scene graph prediction by leveraging multi-modal information integration, making it suitable for real-world robotics and embodied AI applications.
...
Method: Uses a teacher trained on EEG topographic maps to create frozen Valence/Arousal prototypes. Student model (ResNet-18/50) is trained with Proto-KD (cosine alignment to prototypes) and D-Geo (depression-inspired geometric prior that shapes embedding geometry based on affective findings).

Result: Method improves robustness and generalization across datasets (FERPlus, AffectNet-mini, CK+), with consistent gains attributed to prototypes and D-Geo. 5x5 grid performs better than denser grids for stability.

Conclusion: The framework is simple, deployable, and enhances FER model robustness without architectural complexity, transferring brain-informed priors effectively to image-only models.
...
TL;DR: VMD method uses variation inference and multimodal knowledge distillation to leverage radiologists' domain knowledge for automated diagnosis of carotid plaque vulnerability from 3D MRI images and radiology reports.
...
TL;DR: The paper proposes a novel method to initialize neural networks using graph theory instead of random initialization, achieving state-of-the-art image interpolation with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Conventional DNNs use random initialization and SGD optimization, which risks poor local minima. The authors aim to develop a more reliable initialization method using graph-based approaches.

Method: Initialize a directed graph adjacency matrix from a known interpolator, then learn perturbation matrices from data. Implement restoration via Douglas-Rachford iterations unrolled into an interpretable neural net.

Result: Achieves state-of-the-art image interpolation results while drastically reducing network parameters compared to conventional approaches.

Conclusion: The graph-based initialization and lightweight neural net approach provides superior performance with reduced parameter count, offering a promising alternative to traditional random initialization methods.
...
TL;DR: Sphere-GAN is a novel saliency detection model for 360° videos that uses a Generative Adversarial Network with spherical convolutions to outperform existing state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The rise of immersive applications requires new approaches for processing 360° content. While saliency estimation is well-established for 2D content, there are few effective algorithms for 360° video saliency detection, which is crucial for optimizing transmission and processing.

Method: The paper introduces Sphere-GAN, a Generative Adversarial Network that employs spherical convolutions specifically designed for 360° video content to detect visually salient areas.
...
Conclusion: Sphere-GAN represents an effective solution for 360° video saliency estimation, demonstrating superior performance over existing methods and providing a valuable tool for immersive content processing and optimization.
...
<details>
  <summary>Details</summary>
Motivation: Land cover classification from satellite imagery faces challenges due to complex natural landscapes, visual similarity between classes, and significant class imbalance in datasets.

Method: Dual encoder architecture extracts features from optical and SAR imagery independently, fused using cross-modality attention module (CLAIRE). Uses hybrid RIFT loss (Weighted Focal + Tversky) to handle class imbalance. Includes metric-driven reasoning module with Small Language Model (Phi-3) for explanations.

Result: Achieved mIoU of 56.02% and OA of 84.56% on WHU-OPT-SAR; mIoU 59.89% and OA 73.92% on OpenEarthMap-SAR; mIoU 86.86% and OA 94.58% on cloud-obstructed PIE-RGB-SAR dataset.

Conclusion: The proposed approach effectively addresses class imbalance and improves segmentation performance while providing interpretable explanations through language model reasoning, demonstrating strong generalization and robustness across different datasets and conditions.
...
TL;DR: LiDARCrafter is a unified framework that converts free-form language into editable 4D LiDAR sequences using scene graphs and diffusion models, achieving state-of-the-art performance with object-level editing capabilities.


<details>
  <summary>Details</summary>
Motivation: LiDAR generation remains underexplored despite its importance for accurate 3D perception, with challenges in controllability, temporal stability, and evaluation for 4D LiDAR data.
...
Conclusion: LiDARCrafter offers a foundation for LiDAR-based simulation and data augmentation with strong performance in generating editable 4D LiDAR sequences from language input.
...
TL;DR: VLMs suffer from information loss when projecting visual features into language space, with connectors distorting local geometry by 40-60% and causing performance degradation in retrieval and QA tasks.


<details>
  <summary>Details</summary>
Motivation: To study the under-explored information loss that occurs when vision encoders project visual inputs into language model embedding space through connector components in vision-language models.
...
Result: Connectors substantially distort local geometry (40-60% divergence in k-NN relationships), correlating with retrieval performance degradation. Patch-level reconstruction shows high information loss areas predict where models struggle in visual QA tasks.

Conclusion: The projection step in VLMs causes significant information loss that directly impacts model capabilities, with patch-level analysis providing interpretable insights for model failures in visually grounded tasks.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models pose privacy, fairness, and security risks by generating sensitive/harmful content. There's growing demand to erase specific concepts while preserving overall generative capabilities.

Method: SCORE formulates concept erasure as an adversarial independence problem, minimizing mutual information between target concepts and generated outputs. It integrates adversarial optimization, trajectory consistency, and saliency-driven fine-tuning.

Result: SCORE outperforms state-of-the-art methods (EraseAnything, ANT, MACE, ESD, UCE) by up to 12.5% higher erasure efficacy while maintaining comparable or superior image quality across four benchmarks: object erasure, NSFW removal, celebrity face suppression, and artistic style unlearning.

Conclusion: SCORE sets a new standard for secure and robust concept erasure in diffusion models with theoretical guarantees and superior empirical performance compared to existing approaches.
...
TL;DR: RAM++ is a two-stage framework for all-in-one image restoration that integrates semantic understanding with texture generation to handle extreme degradation scenarios through adaptive masking and feature regularization techniques.


<details>
  <summary>Details</summary>
Motivation: To address limitations of existing degradation-oriented methods in extreme scenarios where degradations are strongly coupled with image structures, and to overcome challenges like unbalanced performance across tasks, overfitting to seen degradations, and weak generalization to unseen ones.

Method: A two-stage framework with three key designs: 1) Adaptive Semantic-Aware Mask (AdaSAM) for pretraining with pixel-level masks on semantically rich regions, 2) Mask Attribute Conductance (MAC) for selective fine-tuning to bridge integrity gaps, and 3) Robust Feature Regularization (RFR) using DINOv2's representations for semantically coherent restoration.
...
Conclusion: The proposed framework successfully integrates high-level semantic understanding with low-level texture generation for content-oriented robust image restoration, demonstrating superior performance across various degradation scenarios.
...
TL;DR: A new open-vocabulary segmentation framework (RSKT-Seg) for remote sensing images that outperforms existing methods by 3.8 mIoU and achieves 2x faster inference through specialized modules for rotation invariance and domain adaptation.


<details>
  <summary>Details</summary>
Motivation: Open-Vocabulary Remote Sensing Image Segmentation (OVRSIS) is underexplored due to lack of standardized benchmarks and domain gap between natural and remote sensing images. Existing OVS models perform poorly when directly applied to remote sensing scenarios.

Method: Proposed RSKT-Seg framework with three key components: 1) Multi-Directional Cost Map Aggregation (RS-CMA) for rotation-invariant visual cues, 2) Efficient Cost Map Fusion (RS-Fusion) transformer for spatial-semantic modeling, and 3) Remote Sensing Knowledge Transfer (RS-Transfer) module for domain adaptation.

Result: RSKT-Seg outperforms strong OVS baselines by +3.8 mIoU and +5.9 mACC on the established OVRSISBench, while achieving 2x faster inference through efficient aggregation.
...
TL;DR: SMARLI is a novel framework for layout-to-image generation that integrates structured masking in attention computation to effectively handle layout conditions while maintaining AR model efficiency.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models struggle with layout-conditioned generation due to sparse layout conditions and feature entanglement risks between different regions and their descriptions.
...
Result: Achieves seamless integration of layout tokens with text and image tokens without compromising generation quality, maintaining superior layout-aware control and AR model efficiency.

Conclusion: SMARLI successfully addresses layout-conditioned generation challenges in AR models through structured masking and optimization techniques, enabling effective layout control while preserving model simplicity and efficiency.
...
TL;DR: A modular computer vision pipeline using state-of-the-art models for automated animal behavior analysis in group housing, achieving 94.2% accuracy on pig behavior recognition with significant improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional manual observation methods for animal behavior analysis are time-consuming, subjective, and not scalable, creating a need for automated solutions in agricultural settings.

Method: Combines zero-shot object detection, motion-aware tracking and segmentation, and vision transformers for feature extraction in a modular pipeline designed to handle animal occlusions and group housing scenarios.
...
TL;DR: AvatarSync is an autoregressive framework that generates realistic talking-head animations from a single image using text/audio input, employing a two-stage approach with phoneme representations to overcome flicker, identity drift, and slow inference issues.


<details>
  <summary>Details</summary>
Motivation: Existing GAN and diffusion-based talking-head animation methods suffer from inter-frame flicker, identity drift, and slow inference speeds, limiting their practical applications. These limitations stem from inherent video generation pipeline issues.

Method: Two-stage autoregressive framework: 1) Facial Keyframe Generation (FKG) with phoneme-level semantic representation using Phoneme-to-Visual Mapping and Text-Frame Causal Attention Mask, 2) Inter-frame interpolation with timestamp-aware adaptive strategy based on selective state space model for bidirectional context reasoning.

Result: Extensive experiments show AvatarSync outperforms existing methods in visual fidelity, temporal consistency, and computational efficiency. The optimized inference pipeline reduces latency while maintaining visual quality.

Conclusion: AvatarSync provides a scalable and controllable solution for talking-head animation that addresses key limitations of previous approaches through its divide-and-conquer design and efficient bidirectional reasoning.
...
TL;DR: A cross-population data augmentation framework that enables fetal pose estimation models to generalize to younger gestational ages using only annotated data from older gestational ages, improving reliability in early GA fetal motion analysis.


<details>
  <summary>Details</summary>
Motivation: Current fetal motion tracking methods fail to generalize to early gestational ages due to significant anatomical changes and lack of annotated early GA data, limiting early clinical detection capabilities.

Method: Developed a fetal-specific augmentation strategy that simulates the distinct intrauterine environment and fetal positioning of early GAs, using only annotated images from older GA cohorts.

Result: Cross-population augmentation yielded reduced variability and significant improvements across both older GA and challenging early GA cases, enabling more reliable pose estimation across gestation.

Conclusion: The framework facilitates early clinical detection and intervention in challenging 4D fetal imaging settings by enabling robust generalization to younger GA clinical cohorts without requiring annotated early GA data.
...
TL;DR: ImplMORe is a deep learning method using implicit surface representations for high-resolution multi-organ reconstruction from 3D medical images, outperforming discrete explicit methods by providing finer surface details at resolutions higher than input images.


<details>
  <summary>Details</summary>
Motivation: Fine-grained surface reconstruction from 3D medical imaging provides advanced diagnostic support and surgical planning, but current methods are limited by resolution constraints and high memory/computing requirements. Implicit representations offer compact solutions but cannot be directly applied to medical images due to architectural and data-related differences.

Method: ImplMORe uses an end-to-end deep learning approach with implicit surface representations, incorporating local features through a 3D CNN encoder and performing multi-scale interpolation to learn features in the continuous domain using occupancy functions.

Result: The method outperforms discrete explicit representation-based surface reconstruction approaches, providing fine-grained surface details of organs at resolutions higher than the input images, as demonstrated on the TotalSegmentator dataset for both single and multiple organ reconstructions.

Conclusion: ImplMORe successfully addresses the resolution limitations in medical image reconstruction by leveraging continuous occupancy functions, enabling higher-resolution organ surface reconstruction than the input image resolution while maintaining computational efficiency.
...
TL;DR: U-Mamba2 is a new neural network combining Mamba2 state space models with U-Net for efficient multi-anatomy CBCT segmentation, achieving top performance in the ToothFairy3 challenge with mean Dice scores of 0.792 and 0.852 in two tasks.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of dental anatomies in CBCT scans is critical for clinical applications like diagnosis and surgical planning, but remains time-consuming and challenging, requiring more efficient and effective solutions.

Method: Integrates Mamba2 state space models into U-Net architecture, adds interactive click prompts with cross-attention blocks, uses self-supervised pre-training, and incorporates dental domain knowledge to improve segmentation efficiency and performance.

Result: Achieved top 3 positions in both ToothFairy3 challenge tasks: Task 1 - mean Dice 0.792, HD95 93.19; Task 2 - mean Dice 0.852, HD95 7.39. Demonstrated both effectiveness and efficiency.

Conclusion: U-Mamba2 provides an efficient and effective solution for multi-anatomy CBCT segmentation by integrating advanced architectures with domain-specific knowledge, achieving state-of-the-art performance in dental imaging challenges.
...
TL;DR: A novel trajectory-controllable unfolding framework for CASSI hyperspectral image reconstruction that ensures smooth optimization paths from noisy estimates to high-quality results using diffusion-inspired trajectories and efficient spatial-spectral Transformer architecture.


<details>
  <summary>Details</summary>
Motivation: Existing deep unfolding networks for CASSI reconstruction suffer from uncontrollable reconstruction trajectories, leading to abrupt quality jumps and non-gradual refinement across stages, which limits reconstruction quality and stability.

Method: Proposes a trajectory-controllable unfolding framework inspired by diffusion trajectories and flow matching, with an efficient spatial-spectral Transformer for hyperspectral reconstruction and a frequency-domain fusion module for feature consistency.

Result: The method achieves better reconstruction quality and efficiency than prior state-of-the-art approaches on both simulation and real data experiments.

Conclusion: The proposed trajectory-controllable unfolding framework with smooth optimization paths and efficient Transformer architecture significantly improves CASSI hyperspectral image reconstruction performance compared to existing methods.
...
TL;DR: TetHeart is an end-to-end framework that reconstructs 4D heart meshes from both complete CMR stacks and sparse intra-procedural slices using deep deformable tetrahedra representation with slice-adaptive feature assembly and weak supervision.


<details>
  <summary>Details</summary>
Motivation: Existing cardiac motion reconstruction methods require complete CMR stacks, limiting their utility in intra-procedural scenarios where only sparse slice observations are available during medical procedures.

Method: Uses deep deformable tetrahedra (explicit-implicit hybrid representation) with attentive slice-adaptive 2D-3D feature assembly, distillation strategy from full to sparse slices, and two-stage weakly supervised motion learning requiring only ED/ES keyframe annotations.

Result: Achieves state-of-the-art accuracy and strong generalization on three large public datasets, with zero-shot evaluation on additional private interventional and public CMR datasets in both pre- and intra-procedural settings.

Conclusion: TetHeart is the first unified framework for 4D multi-structure heart mesh recovery that works with both complete stacks and sparse slices, enabling accurate cardiac motion reconstruction even with extreme sparsity during medical procedures.
...
<details>
  <summary>Details</summary>
Motivation: Existing few-shot segmentation methods require extensive training on large datasets, while SAM2 offers powerful zero-shot segmentation capabilities that could be adapted for few-shot scenarios.
...
Conclusion: FS-SAM2 effectively adapts SAM2 for few-shot segmentation while maintaining its impressive performance and efficiency, supporting any K-shot configuration.
...
<details>
  <summary>Details</summary>
Motivation: Tram-human interaction safety is critical in densely populated areas where collisions can cause injuries or fatalities. The paper aims to improve safety for pedestrians, drivers, cyclists, pets, and tram passengers using digital image processing and AI.

Method: The RailSafeNet framework fuses semantic segmentation (SegFormer B3 model), object detection (YOLOv8), and a rule-based Distance Assessor. It identifies rails, localizes nearby objects, and classifies risk by comparing projected distances with the standard 1435mm rail gauge using only monocular video.

Result: Experiments on RailSem19 dataset show 65% IoU for rail segmentation and 75.6% mAP for object detection at IoU threshold 0.50. The system provides accurate, annotation-light scene understanding for real-time warnings.

Conclusion: RailSafeNet delivers effective real-time track intrusion detection and risk assessment that can warn tram drivers before dangerous situations escalate, improving overall tram safety in urban environments.
...
TL;DR: Reflection-V is a new visual reasoning model that enhances visual reflection capabilities by addressing the problem of diminishing visual attention in current VRMs through vision-centered data construction and visual attention-based RL rewards.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models exhibit limited visual reflection - their attention to visual information decreases rapidly with longer responses, hindering effective "slow thinking" visual reasoning capabilities.

Method: 1) Construct vision-centered reasoning data using an agent that interacts between VLMs and reasoning LLMs for cold-start learning; 2) Employ visual attention-based reward model during reinforcement learning to encourage reasoning based on visual information.

Result: Reflection-V demonstrates significant improvements across multiple visual reasoning benchmarks and maintains stronger, more consistent reliance on visual information during reasoning.

Conclusion: The proposed Reflection-V model effectively enhances visual reflection capabilities in visual reasoning models through systematic data construction and reward design approaches.
...
<details>
  <summary>Details</summary>
Motivation: Automated detection of Major Depressive Disorder (MDD) using sMRI and deep learning can improve diagnostic accuracy and enable early intervention, but existing methods using voxel-level features or predefined brain atlases limit complex pattern capture.

Method: Developed a unified pipeline using Vision Transformers to extract 3D region embeddings from sMRI data, with two region definition strategies: atlas-based (predefined structural/functional atlases) and cube-based (3D patches). Generated cosine similarity graphs to model interregional relationships for GNN classification.
...
Conclusion: The proposed ViT-GNN framework effectively detects MDD from sMRI data, with atlas-based methods demonstrating superior performance, highlighting the importance of domain-specific anatomical priors for MDD detection.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of datasets with hierarchical temporal annotations and extend streaming action perception beyond simple classification to include free-form description generation.

Method: Proposes OpenHOUSE system with specialized streaming module for detecting boundaries between adjacent actions, leveraging LLMs to group atomic actions into higher-level events and enrich existing datasets.

Result: Nearly doubles the performance of direct extensions of existing methods for action boundary detection in streaming video.

Conclusion: OpenHOUSE represents a key step toward integrating powerful generative models for streaming action perception, enabling hierarchical online understanding of video events.
...
<details>
  <summary>Details</summary>
Motivation: Most existing AI foundation models are limited to chest anatomy and fail to generalize across broader clinical tasks in radiology, highlighting the need for more versatile medical vision models.
...
Result: XR-0 achieves state-of-the-art performance on most multi-anatomy tasks and remains competitive on chest-specific benchmarks, demonstrating superior generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate the efficiency and robustness of Low-Rank Adaptation (LoRA) for fine-tuning large vision models in medical imaging applications, specifically for diagnosing Radiation-Induced Lung Injury from CT scans.

Method: Compared LoRA with traditional full fine-tuning and inference-only methods using DinoV2 and SwinV2 models. Used cropped CT images of different sizes (50mm³ and 75mm³) centered at treatment isocenter, and tested different adaptation techniques for 2D models on 3D data.
...
Conclusion: LoRA is an efficient and effective alternative to traditional full fine-tuning for adapting large vision models to medical imaging tasks, offering similar performance with reduced computational requirements.
...
TL;DR: HoloGarment is a novel view synthesis method that generates 360° views of garments from 1-3 images or video, using a hybrid training approach combining real video and synthetic 3D data to bridge domain gaps.


<details>
  <summary>Details</summary>
Motivation: Prior methods rely on synthetic 3D training data of mostly unoccluded static objects, which poorly generalizes to real-world clothing with occlusions, complex poses, and deformations.

Method: Uses implicit training paradigm combining large-scale real video data and small-scale synthetic 3D data to optimize shared garment embedding space. Creates garment "atlas" representation during inference by finetuning on specific videos.
...
Conclusion: HoloGarment effectively bridges the synthetic-real domain gap and enables high-quality 360° novel view synthesis of real-world garments from limited input images or video.
...
TL;DR: Self-supervised learning with domain-adaptive pretraining (DAP) significantly improves action recognition for primate behavior analysis, outperforming state-of-the-art methods by over 6% without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: Computer vision for animal behavior analysis faces high labeling costs as a bottleneck for large-scale datasets. There's a need for data-efficient learning approaches to support ecology, cognition research, and conservation efforts.

Method: Utilized a pretrained V-JEPA model with domain-adaptive pretraining (DAP) - continuing pretraining with in-domain primate behavior data without requiring labeled samples.

Result: Outperformed published state-of-the-art action recognition models by 6.1% accuracy on PanAf dataset and 6.3% mAP on ChimpACT dataset. Most performance gains came from DAP.
...
<details>
  <summary>Details</summary>
Motivation: To systematically organize and compare existing approaches for 3D human pose estimation and mesh recovery from LiDAR data, establish standardized benchmarks, and identify research gaps to advance the field.

Method: Proposes a structured taxonomy to classify methods, performs quantitative comparison of three major datasets, compiles unified evaluation metrics, and establishes benchmark tables for fair comparisons.

Result: Created a comprehensive framework for evaluating LiDAR-based 3D human understanding methods, including standardized metrics and benchmark performance tables across multiple datasets.

Conclusion: The paper provides a foundational resource for the research community, enabling fair comparisons and highlighting critical open challenges and future research directions for advancing LiDAR-based 3D human pose and shape estimation.
...
TL;DR: OmniWorld is a large-scale multi-domain dataset addressing data limitations in 4D world modeling, featuring rich modalities and realistic dynamics to advance 4D reconstruction and video generation.


<details>
  <summary>Details</summary>
Motivation: Current 4D world modeling is constrained by limited data quality - existing datasets lack dynamic complexity, multi-domain diversity, and proper spatio-temporal annotations needed for tasks like 4D reconstruction and future prediction.
...
Result: Benchmarking exposes limitations of current SOTA methods, while fine-tuning on OmniWorld leads to significant performance gains in 4D reconstruction and video generation tasks.

Conclusion: OmniWorld serves as a powerful resource and catalyst for accelerating development of general-purpose 4D world models, advancing machines' holistic understanding of the physical world.
...
Result: LazyDrag outperforms baselines on DragBench in drag accuracy and perceptual quality (validated by VIEScore and human evaluation), enabling complex edits like opening mouths, generating new objects, and context-aware changes with multi-round workflows.

Conclusion: LazyDrag establishes new state-of-the-art performance and paves a new way for editing paradigms by eliminating implicit point matching dependency and unlocking the full generative capability of diffusion models with precise geometric control and text guidance.
...
Result: Significant improvements in accessibility and narrative comprehension for animated content over prior face-detection-based approaches, supported by new CMD-AM dataset of 75 animated movies.

Conclusion: The proposed audio-visual pipeline enables robust animated character recognition and enhances character-centric understanding, benefiting accessibility applications for both visually and hearing impaired audiences.
